   ...
   <div class="ltx_page_content">
    <div class="ltx_document ltx_authors_1line">
     <div class="ltx_abstract">
      ...
      <p class="ltx_p">
       Segmentation of clitics has been shown to improve accuracy on a
...
and Egyptian dialect, improving segmentation F
       <sub class="ltx_sub">
        1
       </sub>
       score on a recently
released Egyptian Arabic corpus to 95.1%, compared to 90.8% for
another segmenter designed specifically for Egyptian Arabic.
      </p>
     </div>
...
translation, parsing, and speech recognition
        [1, 14, 9]
        .
...
to improve performance on Arabic parsing
        [5]
        and Arabic-English
machine translation
        [8]
        . However, the variety of Arabic
...
Maghrebi Arabic, etc.
      ...
      <div class="ltx_para" id="S1.p2">
       <p class="ltx_p">
        We present a single clitic segmentation model that is accurate on both MSA and informal Arabic. The model is an extension of the character-level conditional random field (CRF) model
of
...
that improve segmentation accuracy. Third, we show that dialectal data can be handled in the framework of
        domain adaptation
        . Specifically, we show that even simple feature space augmentation
        [3]
        yields significant improvements in task accuracy.
       </p>
      </div>
      <div class="ltx_para" id="S1.p3">
       <p class="ltx_p">
        We compare our work to the original Green and DeNero model and two
other Arabic segmentation systems: the MADA+TOKAN toolkit v. 3.1
        [6]
        and its Egyptian dialect variant, MADA-ARZ v. 0.4
        [7]
        . We
...
a 95.1% F
        ...
        segmentation score evaluated against a gold standard
...
and can be applied directly to any dialect for which segmented training data is available. The source code is available in the latest public release of the Stanford Word Segmenter (
...
where
        𝐗={x1,…,xN}
        is the observed input sequence
and
        𝐘={y1,…,yN}
        is the sequence of labels
we seek to predict. Green and DeNero use a linear-chain
model with
        𝐗
        as the sequence of input
        characters
        , and
        𝐘*
       ...
       <p class="ltx_p">
        where
        ϕ
        is the feature map defined in Section
        2.1
        . Their model classifies each
        yi
        as one of I (continuation of
...
orthographic rules. In our model, each
        yi
        can take on one of
the six values
        {I,O,B,F,RewAl,RewTa}
        ...
        <ul class="ltx_itemize" id="I2">
         <li class="ltx_item" id="I2.i1" style="list-style-type:none;">
          ...
          <div class="ltx_para" id="I2.i1.p1">
           <p class="ltx_p">
            a five-character window around the current character: for each
            -2≤δ≤2
            and
            1≤i≤N
            , the triple
         ...
         <li class="ltx_item" id="I2.i2" style="list-style-type:none;">
          ...
          <div class="ltx_para" id="I2.i2.p1">
           <p class="ltx_p">
            n
            -grams consisting of the current character and up to three preceding
characters: for each
            2≤n≤4
            and
            n≤i≤N
            , the character-sequence/label-sequence
pair
            (xi-n+1⁢…⁢xi,yi-n+1⁢…⁢yi)
           </p>
          </div>
        ...
        <ul class="ltx_itemize" id="I3">
         <li class="ltx_item" id="I3.i1" style="list-style-type:none;">
          ...
          <div class="ltx_para" id="I3.i1.p1">
           <p class="ltx_p">
            Word length and position within a word: for each
            1≤i≤N
            ,
the pairs
            (ℓ,yi)
            ...
            , and
            b
            are the total length of the word containing
            xi
            ,
the number of characters after
            xi
            in the word, and the number
of characters before
            xi
            in the word, respectively. Some incorrect
segmentations produced by the original system could be ruled out with
the knowledge of these statistics.
           </p>
          </div>
         </li>
...
primarily in Egyptian dialect.
       ...
       <div class="ltx_para" id="S2.SS2.p2">
        <p class="ltx_p">
         The approach to domain adaptation we use is that of
         feature space augmentation
         [3]
         . Each indicator
feature from the model described in Section
         2.1
         is replaced by
         N+1
         features in the augmented model, where
         N
...
(here,
         ...
         N+1
         features consist of the
original feature and
         N
         ‘‘domain-specific’’ features, one for each of the
         N
         domains, each of which is active only when both the original feature
is present and the current text comes from its assigned domain.
        </p>
       </div>
      </div>
     </div>
     <div class="ltx_section" id="S3">
      ...
      <div class="ltx_para" id="S3.p1">
       <p class="ltx_p">
        We train and evaluate on three corpora: parts 1–3 of the newswire Arabic Treebank
(ATB),
        the Broadcast News Arabic Treebank (BN),
        and parts 1–8 of the BOLT Phase 1 Egyptian Arabic Treebank (ARZ).
        These correspond respectively to the domains in section
        2.2
        .
We target the segmentation scheme used by these corpora (leaving morphological affixes and the definite article attached).
For the ATB, we use the same split as
        Chiang et al. (2006)
        . For each
of the other two corpora, we split the data into 80% training, 10%
development, and 10% test in chronological order by document.
        We train the Green and DeNero model
and our improvements using L-BFGS with
        L2
        regularization.
       </p>
      </div>
      <div class="ltx_subsection" id="S3.SS1">
       ...
       <div class="ltx_para" id="S3.SS1.p1">
        <p class="ltx_p">
         We use two evaluation metrics in our experiments. The first is an F
         <sub class="ltx_sub">
          1
         </sub>
         precision-recall measure, ignoring orthographic rewrites. F
         <sub class="ltx_sub">
          1
         </sub>
         scores provide a more informative assessment of performance
than word-level or character-level accuracy scores, as over 80% of tokens in the development sets consist of only one segment, with an average of one segmentation every 4.7 tokens (or one every 20.4 characters).
        </p>
       </div>
       <div class="ltx_para" id="S3.SS1.p2">
        <p class="ltx_p">
         The second metric we use is the TEDEval metric
         [13]
         .
TEDEval was developed to evaluate joint segmentation and parsing
         in Hebrew, which requires a greater variety of orthographic rewrites
...
with an approximate randomization test
         [15, 12]
         , with
         R=
         10,000 samples.
      ...
      <div class="ltx_subsection" id="S3.SS2">
       ...
       <div class="ltx_para" id="S3.SS2.p1">
        <p class="ltx_p">
         Table
         1
         contains results on the development set for
...
further improve segmentation on all datasets. Table
         2
         shows the
...
our segmenter is faster than MADA. Table
         3
         compares
...
features) on the ARZ development set; see Table
        4
        .
...
token, and therefore represent a deficiency in the feature set; and
          </p>
         </div>
        </li>
...
affixes. An example of this is &lt;wamistur&gt; ‘‘and
         Mister
         [English]’’,
        ...
        <ul class="ltx_itemize" id="I6">
         <li class="ltx_item" id="I6.i1" style="list-style-type:none;">
          ...
          <div class="ltx_para" id="I6.i1.p1">
           <p class="ltx_p">
            &lt;wafi.tarIqaT&gt; “and in the way”
segmented as &lt;wa&gt;- + &lt;fi.tarIqaT&gt; (correct analysis is &lt;wa&gt;- + &lt;fi—&gt; + &lt;.tarIqaT&gt;). &lt;f.tr&gt; ‘‘break’’/‘‘breakfast’’ is a common Arabic root, but the presence of &lt;q&gt; should indicate that &lt;f.tr&gt; is not the root in this case.
           </p>
          </div>
         </li>
...
pronoun and the
         nisba
         adjective ending (which turns a noun
