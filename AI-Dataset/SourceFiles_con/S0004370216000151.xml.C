   ...
   <content>
    <section label="1">
     ...
     <section label="1.3">
      ...
      <paragraph>
       Section 8 reports an implementation that we built realising the techniques described in Sections 5 Verifying, 6 Verifying, 7 Verifying. Specifically the section introduces MCMAS-P, a parameterised version of MCMAS, an open-source model checker for the verification of MAS. As we explain, MCMAS-P conducts an iterative check on the existence of certain simulations that guarantee, by the methods of Sections 5 Verifying, 6 Verifying, 7 Verifying, that a cutoff exists. If this can be shown, the checker performs plain model checking on corresponding concrete systems in line with the requirements of the theory developed. We report the experimental results obtained.
      </paragraph>
      <paragraph>
    ...
    <section label="2">
     ...
     <section label="2.2">
      ...
      <list>
       <list-item label="i.">
        An asynchronous action from a template i is uniquely instantiated for each concrete agent performing role i. Thus, whenever an asynchronous action is performed, exactly one agent is active in the global transition.
       </list-item>
       <list-item label="ii.">
        An agent-environment action, modelling agent-environment communication, is instantiated as an asynchronous action, but each instantiation is shared by the concrete environment. Hence, whenever an agent-environment action is performed, exactly one agent and the environment participate in the global transition.
       </list-item>
       <list-item label="iii.">
        A role-synchronous action, describing multi-agent-environment communication, is instantiated only once; the copy is shared by all the concrete agents performing role i and the concrete environment. Therefore, whenever a role-synchronous action is performed, all the agents performing role i and the environment participate in the global transition.
       </list-item>
       <list-item label="iv.">
        Similarly, global-synchronous actions are instantiated only once, but the copy is shared by all the concrete agents and the concrete environment. Consequently, whenever a global-synchronous action is performed, all the agents and the environment participate in the global transition.
       </list-item>
       <list-item label="v.">
      ...
      <paragraph label="Definition 2.8">
       Concrete agentGiven a {a mathematical formula}PIIS=〈T,E,V〉 of k roles and {a mathematical formula}n¯∈Nk, the concrete agent {a mathematical formula}(i,j)=〈Lij,ιij,Actij,Pij,tij〉 is defined as follows.
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}Lij=Li×{j} is set of concrete local states;
       </list-item>
       <list-item label="•">
        {a mathematical formula}ιij∈Lij is the initial concrete state;
       </list-item>
       <list-item label="•">
        {a mathematical formula}Actij is the set of concrete local actions that is defined as the union of the following sets of actions.
       </list-item>
       <list-item label="•">
        {a mathematical formula}Pij:Lij→P(Actij) is defined as {a mathematical formula}Pij(l)={a:aτ∈Pi(lτ)}, where {a mathematical formula}aτ ({a mathematical formula}lτ, respectively) denotes the corresponding template action (state, respectively) from which a (l, respectively) has been instantiated;
       </list-item>
       <list-item label="•">
        {a mathematical formula}tij:Lij×Actij→Lij is given by {a mathematical formula}tij(l,a)=l′ iff {a mathematical formula}ti(lτ,aτ)=lτ′.
       </list-item>
      </list>
      <paragraph>
       So, each local state of a concrete agent is made of the template local states indexed by the name of the agent in question and inherits from its template the actions, the protocols and the transition function. The concrete environment is similarly obtained by instantiating each action shared with the agent templates.
      </paragraph>
      <paragraph label="Definition 2.9">
       Concrete environmentGiven a {a mathematical formula}PIIS=〈T,E,V〉 of k roles and {a mathematical formula}n¯∈Nk, the concrete environment {a mathematical formula}E(n¯)=〈LE(n¯),ιE(n¯),ActE(n¯),PE(n¯),tE(n¯)〉 is defined as follows.
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}LE(n¯)=LE;
       </list-item>
       <list-item label="•">
        {a mathematical formula}ActE(n¯)=⋃(i,j)∈A(n¯)Actij;
       </list-item>
       <list-item label="•">
        {a mathematical formula}PE(n¯):LE(n¯)→P(ActE(n¯)) is defined as {a mathematical formula}PE(n¯)(lE)={a:aτ∈PE(lE)};
       </list-item>
       <list-item label="•">
        {a mathematical formula}tE(n¯):LE(n¯)×ActE(n¯)→LE(n¯) is given by {a mathematical formula}tE(n¯)(lE,a)=lE′ iff {a mathematical formula}tE(lE,aτ)=lE′;
       </list-item>
      </list>
      <paragraph>
       Finally, a parameterised system's instantiation, and the concrete semantics we consider, is the IIS composed of the concrete agents and the concrete environment. The concrete system's valuation function is defined on atomic propositions indexed by the agents' identities so that a proposition holds on a global state iff the proposition holds by the template valuation function on the template state that the agent indexing the proposition is in the global state. This will enable us in Section 3 to specify collective properties that range over all concrete agents irrespectively of the size of the system.
      </paragraph>
      <paragraph label="Definition 2.10">
       Concrete systemGiven a {a mathematical formula}PIIS=〈T,E,V〉 of k roles and {a mathematical formula}n¯∈Nk, the concrete system{a mathematical formula}PIIS(n¯), composed of {a mathematical formula}∏1≤i≤kn¯(i) concrete agents, is a tuple{a mathematical formula} The concrete valuation function {a mathematical formula}V(n¯):G→P(AP) is defined on the set {a mathematical formula}G=L11×…×Lkn¯(k)×LE(n¯) of possible global states and on the set {a mathematical formula}AP=(AP1×{1,…,n¯(1)})∪…∪(APk×{1,…,n¯(k)}) of atomic propositions as follows:{a mathematical formula} where l is the template local state of agent {a mathematical formula}(i,j) in g.
      </paragraph>
      <paragraph>
     ...
     <section label="2.3">
      ...
      <section label="2.3.1">
       ...
       <list>
        ...
        <list-item label="•">
         reached. This is also a multi-role action that is guarded by TR. Following the transition described above, a concrete transition via the {a mathematical formula}(reached,(j,i)) action is enabled. This transition causes the food source j to change its state to F thus modelling that robot i has succeeded in reaching the food source j.
        </list-item>
        <list-item label="•">
         deposit. The above transition enables the multi-role action {a mathematical formula}(deposit,(i,j)) that is guarded by TFS. A transition via this action causes the robot to move to state R.
        </list-item>
        <list-item label="•">
         scan. Finally, scan is a multi-role action that is guarded by TFS. Intuitively, robot i may fail to reach the food source j (i.e., the {a mathematical formula}(reached,(j,i)) action is not performed). In this case the {a mathematical formula}(scan,(i,j)) action is enabled. Upon this transition the robot updates its state to SA.
        </list-item>
       </list>
      </section>
      <section label="2.3.2">
       ...
       <list>
        <list-item label="•">
         {a mathematical formula}p_enter,n_enter. In addition to the agent performing the action entering the tunnel, the environment participates in the global transition. This causes the environment to change its state to RED, thereby disallowing other trains to enter the tunnel.
        </list-item>
        <list-item label="•">
     ...
     <section label="2.4">
      The systems {a mathematical formula}SMR,SGS,SFE
      <paragraph>
      ...
      <paragraph>
       {a mathematical formula}SMR is the class of PIIS generated from agent templates defined only on asynchronous, agent-environment, and multi-role actions.{a mathematical formula}
      </paragraph>
      <paragraph>
       Decentralised systems may be encoded in {a mathematical formula}SMR using the machinery of multi-role actions, whereas centralised systems can be represented in {a mathematical formula}SMR using the communication primitive of agent-environment actions. As a result, the {a mathematical formula}SMR class is particularly suitable for modelling swarm robotics, which are naturally decentralised systems, but interacting with their environment [58].
      </paragraph>
      <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.1">
      {a mathematical formula}ACTL⁎K∖X
      <paragraph>
     ...
     <section label="3.3">
      ...
      <paragraph>
       The above will enable us to reduce the size of the formulae to check when we introduce the parameterised model checking problem in Section 4.
      </paragraph>
     </section>
     <section label="3.4">
      <section-title>
      ...
      <paragraph label="Definition 3.7">
       Stuttering simulationA relation {a mathematical formula}∼ss⊆G(n¯)×G(n¯′) is a stuttering simulation between {a mathematical formula}S(n¯) and {a mathematical formula}S(n¯′) if {a mathematical formula}ι(n¯)∼ssι(n¯′) and whenever {a mathematical formula}g∼ssg′ we have the following.
      </paragraph>
      <list>
       <list-item label="(i)">
        {a mathematical formula}V(n¯)(g)=V(n¯′)(g′);
       </list-item>
       <list-item label="(ii)">
        If {a mathematical formula}g∼ijg1, then {a mathematical formula}g′∼ijg′1 for some {a mathematical formula}g′1 such that {a mathematical formula}g1∼ssg′1;
       </list-item>
       <list-item label="(iii)">
      ...
      <list>
       <list-item label="(i)">
        {a mathematical formula}V(n¯)(g)∩X=V(n¯′)(g′)∩X, where {a mathematical formula}X=⋃1≤i≤k{(p,j):p∈APi and 1≤j≤m¯(i)};
       </list-item>
       <list-item label="(ii)">
        If {a mathematical formula}g∼ijg1, for {a mathematical formula}(i,j)∈A(m¯), then {a mathematical formula}g′∼ijg′1 for some {a mathematical formula}g′1 such that {a mathematical formula}g1∼m¯ssg′1.
       </list-item>
      </list>
      <paragraph>
       We say that a concrete system {a mathematical formula}S(n¯′){a mathematical formula}m¯-stuttering simulates a concrete system {a mathematical formula}S(n¯), denoted {a mathematical formula}S(n¯)≤m¯ssS(n¯′), if there is an {a mathematical formula}m¯-stuttering-simulation relation between {a mathematical formula}S(n¯) and {a mathematical formula}S(n¯′). The following shows that {a mathematical formula}m¯-indexed formulae are preserved under {a mathematical formula}m¯-stuttering simulation.
      </paragraph>
      <paragraph label="Theorem 3.9">
       Let{a mathematical formula}S(n¯)≤m¯ssS(n¯′)for a given{a mathematical formula}m¯∈Nksuch that{a mathematical formula}n¯≥m¯,n¯′≥m¯. If{a mathematical formula}S(n¯′)⊨∀v¯ϕ(v¯), then{a mathematical formula}S(n¯)⊨∀v¯ϕ(v¯), for any{a mathematical formula}m¯-indexed formula{a mathematical formula}∀v¯ϕ(v¯).
      </paragraph>
      <paragraph label="Proof">
       Fix an {a mathematical formula}m¯-indexed formula {a mathematical formula}∀v¯ϕ(v¯). By Lemma 3.6 it suffices to show that {a mathematical formula}S(n¯′)⊨ϕ[trivial] implies {a mathematical formula}S(n¯)⊨ϕ[trivial]. Thus note that {a mathematical formula}ACTL⁎K∖X formulae are known to be preserved under stuttering simulation [13]. Then note that atomic propositions and epistemic modalities in {a mathematical formula}ϕ[trivial] refer only to agents in {a mathematical formula}A(m¯). The latter two observations entail {a mathematical formula}S(n¯)⊨ϕ([trivial]).  □
      </paragraph>
      <paragraph>
       The result above will enable us to access the truth of a specification on an arbitrarily big system by checking it on a smaller system. To do this, however, only conditions (i) and (iii) of Definition 3.8 have to be considered. Indeed, as the following shows, in the context of fully symmetric parameterised systems, the notion of stuttering simulation preserving temporal formulae [65] and the notion of stuttering simulation preserving temporal-epistemic formulae [13] coincide.
      </paragraph>
    ...
    <section label="5">
     ...
     <section label="5.1">
      ...
      <paragraph label="Definition 5.2">
       Environment loopA subsequence {a mathematical formula}giai…gj, {a mathematical formula}j&gt;i&gt;0, of a path {a mathematical formula}g1a1… in {a mathematical formula}S(n¯) is an environment loop if {a mathematical formula}lsE(gi)=lsE(gj).
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Definition 5.3">
       Environment loop conditionA global state {a mathematical formula}π(d) in a path π in {a mathematical formula}S(n¯) has the environment loop condition, denoted {a mathematical formula}ELC(π(d)), if there is a pair of agents {a mathematical formula}(i,r) and {a mathematical formula}(j,q) with {a mathematical formula}(i,r)≠(j,q) such that the following hold:
      </paragraph>
      <list>
       <list-item label="(i)">
        {a mathematical formula}π(e)→Xπ(e+1)→Y⁎π(d), where {a mathematical formula}e&lt;d, {a mathematical formula}X=AEir∪MRir and {a mathematical formula}Y=⋃(x,y)∈A(n¯)Axy;
       </list-item>
       <list-item label="(ii)">
        {a mathematical formula}π(d)→Y′⁎π(e′−1)→X′π(e′), where {a mathematical formula}e′&gt;d, {a mathematical formula}X′=AEjq∪MRjq and {a mathematical formula}Y′=⋃(x,y)∈A(n¯)Axy.
       </list-item>
      </list>
      <paragraph>
       In other words a global state g in a path π has the environment loop condition if: (i) there is an agent who lastly synchronised with the environment earlier in the path; (ii) there is a different agent who firstly synchronised with the environment later in the path (see Fig. 9).
      </paragraph>
      <paragraph label="Example 5.4">
       Consider the following path of the concrete Train-Gate-Controller with two prioritised trains and two normal trains (Fig. 5).{a mathematical formula} The states {a mathematical formula}(A1,W2,PG,L1,L2) and {a mathematical formula}(A1,A2,PG,L1,L2) have the environment loop condition, whereas the other states do not.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Lemma 5.5">
       Consider a PIIS{a mathematical formula}S=〈T,E,V〉∈SMRwith{a mathematical formula}T≤aesEand a path π in{a mathematical formula}S(n¯). If{a mathematical formula}ELC(π(d))for some{a mathematical formula}d&gt;1, then{a mathematical formula}[d]πis an environment loop.
      </paragraph>
      <paragraph label="Proof">
     ...
     <section label="5.2">
      Model checking procedure for {a mathematical formula}SMR systems
      <paragraph>
     ...
     <section label="5.4">
      Soundness of the Check_{a mathematical formula}SMR procedure
      <paragraph>
       We can now assess the soundness of the Check_{a mathematical formula}SMR procedure.
      </paragraph>
      <paragraph label="Theorem 5.8">
       Given{a mathematical formula}S=〈T,E,V〉∈SMRand an{a mathematical formula}m¯-indexed formula{a mathematical formula}∀v¯ϕ(v¯), if{a mathematical formula}T≤aesE, thenCheck_{a mathematical formula}SMR(S,∀v¯ϕ(v¯))returns true iff{a mathematical formula}S⊨∀v¯ϕ(v¯).
      </paragraph>
      <paragraph>
       To prove this result we need to establish some intermediate steps. Firstly observe that by the definition of the PMCP and Theorem 3.9, it suffices to show that if {a mathematical formula}T≤aesE, then: (i) the cutoff system {a mathematical formula}S(c¯){a mathematical formula}m¯-stuttering simulates every bigger system; (ii) every bigger system {a mathematical formula}m¯-stuttering simulates {a mathematical formula}S(c¯). As related states in each of the simulations need only to agree on the atomic propositions indexed by the agents in {a mathematical formula}A(m¯), we only have to simulate transitions taken by the agents in {a mathematical formula}A(m¯). We first show the former.
      </paragraph>
      <section label="5.4.1">
       Part A: the cutoff system {a mathematical formula}m¯-stuttering simulates every bigger system
       <paragraph>
      ...
      <section label="5.4.2">
       ...
       <paragraph label="Corollary 5.13">
        Consider a PIIS{a mathematical formula}S=〈T,E,V〉∈SMRwith{a mathematical formula}T≤aesEand an{a mathematical formula}m¯-indexed formula{a mathematical formula}∀v¯ϕ(v¯). Then,{a mathematical formula}S(c¯)⊨∀v¯ϕ(v¯)iff{a mathematical formula}∀n¯≥c¯.S(n¯)⊨∀v¯ϕ(v¯), where{a mathematical formula}c¯=cutofff_SMR(S,m¯).
       </paragraph>
       <paragraph label="Proof">
        Let {a mathematical formula}n¯≥c¯ be arbitrary. By Theorem 3.9 it suffices to show that {a mathematical formula}S(n¯)≤m¯ssS(c¯) and {a mathematical formula}S(c¯)≤m¯ssS(n¯). The former is given by Lemma 5.11, and the latter is given by Lemma 5.12.  □
       </paragraph>
       <paragraph>
        This concludes the proof of Theorem 5.8. Corollary 5.13 provides a constructive methodology for solving the PMCP by giving the conditions under which the problem can be solved by checking each concrete system up to the cutoff system.
       </paragraph>
      </section>
     </section>
    </section>
    <section label="6">
     Verifying {a mathematical formula}SGS systems
     <paragraph>
      We now investigate the {a mathematical formula}SGS class of systems. The class is generated from agent templates defined on asynchronous, agent-environment and global-synchronous actions. By Theorem 4.3, cutoffs do not generally exist for this class. However, we show that the existence of an agent-environment simulation between the agent and environment templates guarantees a cutoff. We show how to calculate the cutoff under these conditions.
     </paragraph>
     <paragraph>
      We begin with an analysis of the agent-environment simulation for {a mathematical formula}SGS systems. We then define the model checking procedure, and we exemplify it on the train-gate-controller. Finally, we prove its soundness. To simplify our analysis in the following we assume without loss of generality that for each action {a mathematical formula}a∈ActE we have that {a mathematical formula}|{l∈LE:a∈PE(l)}|=1; i.e., an environment's action is enabled at exactly one template state.
     </paragraph>
     <section label="6.1">
      <section-title>
       Agent-environment simulation
     ...
     <section label="6.2">
      Model checking procedure for {a mathematical formula}SGS systems
      <paragraph>
     ...
     <section label="6.4">
      ...
      <section label="6.4.1">
       ...
       <paragraph>
        Fig. 12 shows the {a mathematical formula}m¯-stuttering simulation between {a mathematical formula}S(n¯) and {a mathematical formula}S(c¯).
       </paragraph>
       <paragraph label="Lemma 6.4">
        Consider a PIIS{a mathematical formula}S=〈T,E,V〉∈SGSof k roles with{a mathematical formula}T≤aesEand{a mathematical formula}m¯∈Nk. Then,{a mathematical formula}S(n¯)≤m¯ssS(c¯)for all{a mathematical formula}n¯≥c¯, where{a mathematical formula}c¯=cutoff_SGS(S,m¯).
       </paragraph>
       <paragraph label="Proof">
        ...
        <list>
         {a mathematical formula}π′(x,Act)=π(x,Act) if {a mathematical formula}π(x,Act)∈⋃(i,r)∈A(c¯)Actir;otherwise, {a mathematical formula}π′(x,Act)=ϵ.Partition
        </list>
        <paragraph>
        ...
        <paragraph>
         Consequently, {a mathematical formula}π′ is a valid path, and {a mathematical formula}π(y)∼m¯ssπ′(y) for each {a mathematical formula}y≥1. Therefore, {a mathematical formula}∼m¯ss satisfies simulation requirement (iii). As {a mathematical formula}(g,g′) was arbitrary, {a mathematical formula}S(n¯)≤m¯ssS(c¯) is entailed as required.  □
        </paragraph>
       </paragraph>
       <paragraph>
        We have thus shown that the cutoff system {a mathematical formula}m¯-stuttering simulates every bigger system. We conclude the proof for Theorem 6.3 by showing that every bigger system {a mathematical formula}m¯-stuttering simulates the cutoff system.
       </paragraph>
      </section>
      <section label="6.4.2">
       Part B: every bigger system {a mathematical formula}m¯-stuttering simulates the cutoff system
       ...
       <paragraph label="Lemma 6.5">
        Consider a PIIS{a mathematical formula}S=〈T,E,V〉∈SGSof k roles with{a mathematical formula}T≤aesEand{a mathematical formula}m¯∈Nk. Then,{a mathematical formula}S(c¯)≤m¯ssS(n¯)for all{a mathematical formula}n¯≥c¯, where{a mathematical formula}c¯=cutoff_SGS(S,m¯).
       </paragraph>
       <paragraph label="Proof">
    ...
    <section label="7">
     ...
     <section label="7.3">
      ...
      <section label="7.3.1">
       ...
       <paragraph label="Lemma 7.3">
        Consider a PIIS{a mathematical formula}S=〈T,E,V〉∈SFEof k roles and{a mathematical formula}m¯≥Nk. Then,{a mathematical formula}S(n¯)≤m¯ssS(c¯)for all{a mathematical formula}n¯≥c¯, where{a mathematical formula}c¯=cutoff_SFE(S,m¯).
       </paragraph>
       <paragraph label="Proof">
    ...
    <section label="8">
     ...
     <section label="8.2">
      ...
      <paragraph>
       So, we have that {a mathematical formula}T∼aesE iff {a mathematical formula}Si satisfies the formulae in {a mathematical formula}Δi for all i with {a mathematical formula}1≤i≤k. This has considerable advantages in applications in terms of efficiency as the following demonstrates.
      </paragraph>
     </section>
     <section label="8.3">
      <section-title>
