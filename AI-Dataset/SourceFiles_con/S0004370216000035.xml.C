  ...
  <root>
   ...
   <content>
    ...
    <section label="2">
     ...
     <section label="2.2">
      ...
      <list>
       ...
       <list-item label="•">
        {a mathematical formula}FPNP: class of function problems solvable in deterministic polynomial time by executing a polynomial number of calls to an NP oracle.
       </list-item>
       <list-item label="•">
        {a mathematical formula}FPNP[log]: class of function problems solvable in deterministic polynomial time by executing a logarithmic number of calls to an NP oracle.
       </list-item>
       <list-item label="•">
    ...
    <section label="3">
     ...
     <paragraph>
      Definition 4 requires {a mathematical formula}P(R)=1. If this condition is not met, a single call to {a mathematical formula}P(R) can be issued to test this property. And, if {a mathematical formula}P(R)=0, then there is no set {a mathematical formula}W⊆R such that {a mathematical formula}P(W)=1 due to the monotonicity of P.
     </paragraph>
     <paragraph label="Definition 5">
      Minimal setLet P be a monotone predicate over a set {a mathematical formula}R, and let {a mathematical formula}M⊆R be such that {a mathematical formula}P(M)=1. The set {a mathematical formula}M is minimal iff {a mathematical formula}∀M′⊊M¬P(M′).
     </paragraph>
     <paragraph label="Definition 6">
      MSMP problemGiven a monotone predicate P over a set {a mathematical formula}R, the Minimal Set over a Monotone Predicate (MSMP) problem consists in finding a minimal subset {a mathematical formula}M of {a mathematical formula}R such that {a mathematical formula}P(M) holds.
     </paragraph>
     <section label="3.1">
      <section-title>
       Monotone predicate forms and problem examples
      </section-title>
      <paragraph>
    ...
    <section label="4">
     ...
     <section label="4.1">
      ...
      <paragraph>
       Before we study the problems of monotone predicates, we introduce the problem MaxSet, which we will exploit in proofs throughout the article.
      </paragraph>
      <paragraph label="Definition 14">
       MaxSet problemLet {a mathematical formula}{F1(X),…,Fn(X)} be a set of formulas defined on a set of variables X. The function problem of computing the subset of the formulas {a mathematical formula}Fi that are satisfiable is the MaxSet problem for {a mathematical formula}{F1,…,Fn}.
      </paragraph>
      <paragraph label="Theorem 2">
       We immediately obtain that MaxSet can be solved by a polynomial number of non-adaptive calls to an NP oracle. MaxSetis in{a mathematical formula}FP∥NP.
      </paragraph>
      <paragraph label="Proof">
       The satisfiability of each formula {a mathematical formula}Fi can be tested independently of the other formula. Thus, n non-adaptive calls can be used for solving the MaxSet problem and so MaxSet is in {a mathematical formula}FP∥NP.  □
      </paragraph>
      <paragraph>
       A less obvious but crucial result is that MaxSet can be solved by a logarithmic number of calls to an NP witness-oracle.
      </paragraph>
      <paragraph label="Theorem 3">
       MaxSetis in{a mathematical formula}FPNP[wit,log].
      </paragraph>
      <paragraph label="Proof">
       We reduce MaxSet to partial MaxSAT which, by Proposition 1, is in {a mathematical formula}FPNP[wit,log]. Consider {a mathematical formula}n+1 sets of distinct variables {a mathematical formula}X1,…,Xn, and Z, with {a mathematical formula}|Xi|=|X|, {a mathematical formula}1≤i≤n, and {a mathematical formula}|Z|=n. Define {a mathematical formula}zi↔Fi[X/Xi] and let,{a mathematical formula} It is plain to conclude that the largest number of {a mathematical formula}zi variables that can be set to 1 such that {a mathematical formula}T is satisfied identifies the largest set of satisfiable formulas. Formula {a mathematical formula}T can be encoded into an equisatisfiable CNF formula in polynomial time by introducing additional variables (e.g. [28]). The resulting clauses are marked as hard. Finally, define the set of soft clauses to be the set {a mathematical formula}{zi|1≤i≤n}. Therefore, the largest set of soft clauses that can be simultaneously satisfied such that {a mathematical formula}T is satisfied represents all of the Boolean functions that are satisfiable. Thus, MaxSet is in {a mathematical formula}FPNP[wit,log].  □
      </paragraph>
      <paragraph label="Remark 4">
       An immediate generalization of Proposition 3 is that {a mathematical formula}FP∥NP⊆FPNP[wit,log] as any parallel queries to an NP oracle can be replaced by a logarithmic number of calls to a witness oracle. A similar result has been shown elsewhere in the context of function oracles [29, Thm. 3.13].
      </paragraph>
      <paragraph>
       We continue by deriving a couple of useful lemmas for predicates of form {a mathematical formula}B.
      </paragraph>
      <paragraph label="Lemma 1">
       Let P be a predicate of the form{a mathematical formula}Bover a set{a mathematical formula}Rand let{a mathematical formula}Wbe a subset of{a mathematical formula}R. The following equality holds{a mathematical formula}P(W)=⋀u∈R∖W¬SAT(G∧σ(u)). In another words,{a mathematical formula}P(W)holds iff for all the elements u in the complement of{a mathematical formula}W, the formula{a mathematical formula}G∧σ(u)is unsatisfiable.
      </paragraph>
      <paragraph label="Proof">
       The lemma is proved by a series of equalities stemming from the definition of form {a mathematical formula}B (Definition 8) and the predicate {a mathematical formula}SAT (Section 2.1).{a mathematical formula}  □
      </paragraph>
     ...
     <section label="4.2">
      ...
      <paragraph>
       The first result shows that minimal sets for predicates of form {a mathematical formula}L can be solved by a logarithmic number of calls to an NP witness-oracle.
      </paragraph>
      <paragraph label="Theorem 4">
       Let Γ be anMSMPproblem represented by a monotone predicate P of form{a mathematical formula}Lover a set{a mathematical formula}R. Then Γ is in{a mathematical formula}FPNP[wit,log].
      </paragraph>
      <paragraph label="Proof">
      ...
      <paragraph>
       For predicates of form {a mathematical formula}B, a minimal set can be computed with a polynomial number of non-adaptive calls to an NP oracle, or, with a logarithmic number of calls to an NP witness-oracle.
      </paragraph>
      <paragraph label="Theorem 5">
       Let Γ be anMSMPproblem represented by monotone predicate P of form{a mathematical formula}Bover a set{a mathematical formula}R. Then Γ is in{a mathematical formula}FP∥NPand in{a mathematical formula}FPNP[wit,log].
      </paragraph>
      <paragraph label="Proof">
       Let {a mathematical formula}M⊆R be a minimal set of P and let {a mathematical formula}u∈R. From Proposition 2, formula {a mathematical formula}G∧σ(u) is satisfiable iff {a mathematical formula}u∈M. Hence, computing {a mathematical formula}M translates to deciding the satisfiability of {a mathematical formula}G∧σ(u) for each {a mathematical formula}u∈R. This is an instance of the MaxSet problem and therefore it belongs to {a mathematical formula}FP∥NP due to Proposition 2 and to {a mathematical formula}FPNP[wit,log] due to Proposition 3.  □
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       We begin by a lemma for an arbitrary monotone predicate P. This lemma shows that if there is a unique minimal set, this set can be obtained by a polynomial number of queries to P.
      </paragraph>
      <paragraph label="Lemma 4">
       Let P be a monotone predicate on a set{a mathematical formula}Rsuch that it has a unique minimal set{a mathematical formula}M. For any element{a mathematical formula}u∈Rit holds that{a mathematical formula}u∈Miff{a mathematical formula}P(R∖{u})=0.
      </paragraph>
      <paragraph label="Proof">
    ...
    <section label="5">
     ...
     <paragraph>
      Observe that the obtained results also hold for MSMP problems when there are at most k minimal sets rather than exactly k minimal sets since one can iterate over {a mathematical formula}k′∈1..k to find the right {a mathematical formula}k′.
     </paragraph>
    </section>
    <section label="6">
     <section-title>
