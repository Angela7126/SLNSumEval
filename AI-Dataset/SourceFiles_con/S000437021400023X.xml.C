  ...
  <root>
   ...
   <content>
    ...
    <section label="2">
     ...
     <paragraph label="Definition 4">
      Given a CSP {a mathematical formula}P=〈V,D,C〉, a search state S for P where each variable {a mathematical formula}x∈V has a non-empty domain: {a mathematical formula}|S(x)|&gt;0, and a constraint {a mathematical formula}c∈C, the GAC propagator for c returns a new search state {a mathematical formula}S′ which:
     </paragraph>
     <list>
      <list-item label="1.">
       For all variables not in {a mathematical formula}scope(c): is identical to S.
      </list-item>
      <list-item label="2.">
       For all variables in {a mathematical formula}scope(c): omits all (and only) literals in S that are not supported in c, and is otherwise identical to S.
      </list-item>
     </list>
    </section>
    <section label="3">
     ...
     <section label="3.5">
      ...
      <section label="3.5.2">
       ...
       <list>
        <list-item label="Branch :">
         {a mathematical formula}〈var,val,pos〉 — If the value val is not in the domain of the variable var then jump to position pos. Otherwise, execution continues with the next instruction in the sequence. A jump to −1 ends execution of the virtual machine.
        </list-item>
        <list-item label="Prune :">
         {a mathematical formula}〈var1,val1,var2,val2,…,−1〉 — Prune a set of literals from the variable domains. The operands are a list of variable–value pairs terminated by −1.
        </list-item>
        <list-item label="Return :">
         {a mathematical formula}〈〉 — End execution of the virtual machine.
        </list-item>
       </list>
       <paragraph>
    ...
    <section label="5">
     ...
     <section label="5.2">
      ...
      <paragraph>
       The Reified Sumgeq implementation uses a sum to represent the conjunction. The negation of some b variables is achieved with views [19], therefore no auxiliary variables are introduced. The sum constraint is reified to the {a mathematical formula}moves[t,m] variable, as follows: {a mathematical formula}[(b[t,f1]+⋯+b[t+1,f3])⩾6]⇔moves[t,m].
      </paragraph>
      <paragraph>
       The Min implementation uses a single min constraint. Again views are used for negation and no auxiliary variables are introduced. The constraint is as follows: {a mathematical formula}min(b[t,f1],…,b[t+1,f3])=moves[t,m].
      </paragraph>
      <paragraph>
       The move constraint is also implemented using the Lighttable, Table, MDDC and {a mathematical formula}STR2+ propagators. The table has 64 satisfying tuples. The Regular implementation [17] has 9 states and uses a ternary table constraint (representing the transition table) with 17 satisfying tuples.
      </paragraph>
      <paragraph>
    ...
    <section label="6">
     ...
     <section label="6.2">
      ...
      <paragraph label="Definition 10">
       Given constraint c, a permutation g is variable-stable iff, given two literals {a mathematical formula}〈x,d1〉, {a mathematical formula}〈x,d2〉 from the same variable, then {a mathematical formula}g(〈x,d1〉) and {a mathematical formula}g(〈x,d2〉) are also literals from the same variable.
      </paragraph>
     </section>
     <section label="6.3">
      <section-title>
    ...
    <section label="7">
     ...
     <section label="7.1">
      ...
      <list>
       <list-item>
        — Hash table indexed by canonical image c, containing a pair {a mathematical formula}〈g,id〉 where {a mathematical formula}g∈G is a permutation mapping a node-state S to c, and id is a number that identifies the node where S was seen.
       </list-item>
       <list-item>
        — Set (implemented as a hash table) containing canonical images. When a tree node is deleted because it (and the subtree beneath it) contains no prunings, the canonical image of it is stored in DeletedLookup.
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph>
       Fig. 3 shows an example of a symmetry-reduced propagator tree created by GenTreeSym with the entailment heuristic. Fig. 4 illustrates the difference made by symmetry reduction on the LABS 2 constraint.
      </paragraph>
     </section>
     <section label="7.2">
      <section-title>
      ...
      <section label="7.2.1">
       ...
       <paragraph label="Theorem 3">
        Given a partially symmetric constraint c defined by parameters{a mathematical formula}〈n1,d1,n2,d2〉, the size of a symmetry-reduced tree for c that performs whole-variable branching is as follows, where{a mathematical formula}d=max(d1,d2).{a mathematical formula}
       </paragraph>
       <paragraph>
        To take our example of a totally symmetric constraint with domain size 3, the bound from the previous section is {a mathematical formula}O(n23), and we have improved it to {a mathematical formula}O(n8).
       </paragraph>
      </section>
     </section>
     <section label="7.3">
      ...
      <section label="7.3.1">
       ...
       <list>
        <list-item label="Perm :">
         {a mathematical formula}〈l1,l2,…,ln〉 — Apply the given permutation of the literals. The number of operands is the sum of the sizes of the initial domains.
        </list-item>
        <list-item label="Jump :">
         {a mathematical formula}〈pos〉 — Jump to the position given.
        </list-item>
       </list>
       <paragraph>
     ...
     <section label="7.4">
      ...
      <paragraph>
       For all our experiments we use {a mathematical formula}JumpCutoff=1 to obtain the smallest (in the VM representation) symmetry-reduced trees.
      </paragraph>
     </section>
     <section label="7.5">
      <section-title>
    ...
    <section label="8">
     ...
     <section label="8.2">
      ...
      <paragraph>
       Table 8 shows our results for peg solitaire. We omit run times and just give node rates because all methods explore the same tree. Of the two hand-written propagators (Min and Reified Sumgeq), Min is always superior (Table 3) so we omit Reified Sumgeq from this table. We also omit Lighttable, Table, MDDC, {a mathematical formula}STR2+ and Regular.
      </paragraph>
      <paragraph>
     ...
     <section label="8.3">
      ...
      <paragraph>
       The same pattern can be seen on the {a mathematical formula}n=25 instance (Table 10). For both {a mathematical formula}n=25 and {a mathematical formula}n=30, the fastest configuration is the compiled standard propagator tree, group two. Longer constraints slow the solver down substantially. The other instances {a mathematical formula}n∈{26,27,28,29} also exhibit the same pattern.
      </paragraph>
      <paragraph>
       Table 9, Table 10 also show that propagator trees compare well to the generic GAC propagators as the arity is increased. {a mathematical formula}STR2+ is the fastest of the generic GAC propagators and it is consistently slower than all propagator tree methods.
      </paragraph>
      <paragraph>
     ...
     <section label="8.4">
      ...
      <paragraph>
       In the previous Life experiment we found Sum to be more efficient than any of the generic propagators and the Regular decomposition (as shown in Table 5, Table 6). The symmetry-reduced tree compares well to Sum, being approximately twice as fast for all instances.
      </paragraph>
      <paragraph>
       As Table 7 shows, the overhead of generating the compiled, symmetry-reduced Life propagator is 50.15 s in total, therefore on five instances ({a mathematical formula}n=6, {a mathematical formula}p∈{5,6} and {a mathematical formula}n=7, {a mathematical formula}p∈{3,4,5}) that propagator tree more than pays back its overhead.
      </paragraph>
      <paragraph>
       Immigration
      </paragraph>
      ...
      <paragraph>
       The Immigration constraint has {a mathematical formula}8!×2=80,640 symmetries. It is not possible to generate the standard propagator tree within 12 GB of memory. The symmetry-reduced tree has 34,712 nodes.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       As in previous experiments we have five generic GAC methods: Lighttable, Table, MDDC and {a mathematical formula}STR2+ with a table containing 19,683 satisfying tuples, and the Regular decomposition [17] with 25 states and ternary table constraints (for the transition table) with 67 satisfying tuples.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       The Brian's Brain constraint has {a mathematical formula}8!=40,320 symmetries. It is not possible to generate the standard propagator tree for this constraint within 12 GiB of memory. The symmetry-reduced propagator tree has 135,575 nodes. This can be executed using the VM, but not by code generation (Section 7.3.2) because the compiler exceeds 12 GiB of memory.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       As for Immigration we have five generic GAC methods: Lighttable, Table, MDDC and {a mathematical formula}STR2+ with a table containing 19,683 satisfying tuples, and the Regular decomposition [17] with 11 states and ternary table constraints (the transition table) with 27 satisfying tuples.
      </paragraph>
      <paragraph>
       Table 13 shows our results. In the case of Brian's Brain, the Sum encoding performs particularly badly. For example when {a mathematical formula}n=6, {a mathematical formula}p=6, Sum takes over 600 times more search nodes than the other methods.
      </paragraph>
      <paragraph>
       Once again the symmetry-reduced tree outperforms all types of table constraint and the Regular decomposition. The total overhead of generating the symmetry-reduced tree (from Table 7) is 2749 s. If the tree were generated once for each instance, it would repay its overhead only on the hardest instance {a mathematical formula}n=8, {a mathematical formula}p=6. However in general we amortise the cost of generating the tree over all instances.
      </paragraph>
     </section>
     <section label="8.5">
      <section-title>
      ...
      <paragraph>
       We filtered the benchmark set to remove any benchmarks containing no scopes of the set of 683 relations. We also filtered out benchmarks that take more than 12 GiB memory.{sup:4} 1930 benchmarks remained from 34 series.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       When comparing MDDC to propagator trees, each benchmark is executed three times. First it is executed with all extensional relations implemented by MDDC. Second, each of the 683 relations with a standard propagator tree are implemented by the propagator tree and the other relations by MDDC. Third, each of the relations with a symmetry-reduced propagator tree are implemented by that propagator tree and the others by MDDC. Similarly, to compare to Table each benchmark was executed three times. Each run had a time limit of 30 minutes and they were performed 32 in parallel on an AMD Opteron 6272 at 2.1 GHz.
      </paragraph>
      <paragraph>
   ...
   <appendices>
    <section label="Appendix A">
     <section-title>
      Canonicalisation of sequences of objects
     </section-title>
     <paragraph>
     ...
     <paragraph label="Definition 14">
      Given the following:
      <list>
       a list {a mathematical formula}L=[l1,…,ln];a canonicalising function{a mathematical formula}f(li,Hc) for the {a mathematical formula}li and any group {a mathematical formula}Hc; anda stabilising function {a mathematical formula}s(li,Hs) which returns (for any group {a mathematical formula}Hs) the subgroup of {a mathematical formula}Hs which stabilises {a mathematical formula}li,If
      </list>
      <paragraph>
       L is the empty list return the identity element of G, otherwise,
      </paragraph>
      <list>
       <list-item label="1.">
        Find {a mathematical formula}GCan=f(L[1],G).
       </list-item>
       <list-item label="2.">
        Find {a mathematical formula}GStab=s(L[1]GCan,G).
       </list-item>
       <list-item label="3.">
        Generate the list {a mathematical formula}L′ where {a mathematical formula}∀i∈{2..n}.L′[i−1]=L[i]GCan, which is one element shorter than L.
       </list-item>
       <list-item label="4.">
        Return the permutation {a mathematical formula}GCan.Can(L′,GStab).
       </list-item>
      </list>
     </paragraph>
     <paragraph>
     ...
     <list>
      <list-item label="1.">
       SetStabiliser({a mathematical formula}S,G) : Generates the subset of G which stabilises S.
      </list-item>
      <list-item label="2.">
       MinimalImagePerm({a mathematical formula}S,[Stab,]G) : Generates the element h of G such that {a mathematical formula}∀g∈G.h(S)⩽g(S). The function may optionally be given {a mathematical formula}Stab=SetStabiliser(S,G) to provide a performance improvement. This is the canonicalising function for sets that we use in Algorithm 7.
      </list-item>
     </list>
     <paragraph>
