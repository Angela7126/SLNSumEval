  ...
  <root>
   ...
   <content>
    ...
    <section label="2">
     ...
     <paragraph>
      In this section we give the basic definitions that constitute the framework of binary aggregation with integrity constraints. First, we review the classical setting of binary aggregation, based on work by Wilson [40] and Dokow and Holzman [11], to which we add the notion of integrity constraint as a mean of specifying the set of individual ballots to be considered rational. We then present a list of examples from both practical applications and classical frameworks of aggregation that can be modelled in our framework. Finally, we state the crucial definition of collective rationality and we review the classical axiomatic method for binary aggregation procedures. We conclude by axiomatising the class of quota rules and the majority rule over binary combinatorial domains.
     </paragraph>
     <section label="2.1">
      <section-title>
      ...
      <paragraph label="Definition 1">
       Given a finite set of issues {a mathematical formula}I and a finite set of individuals {a mathematical formula}N, an aggregation procedure is a function {a mathematical formula}F:DN→D, mapping each profile of binary ballots to an element of {a mathematical formula}D.{sup:1}
      </paragraph>
      <paragraph>
       Let {a mathematical formula}F(B)j denote the result of the aggregation on issue j. Note that we do not put any restriction on the domain on which aggregation procedures are defined, i.e., Definition 1 includes a requirement known as universal domain.
      </paragraph>
      <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.2">
      ...
      <paragraph label="Lemma 3">
       {a mathematical formula}CR[L∧]=CR[L]for all{a mathematical formula}L⊆LPS.
      </paragraph>
      ...
      <paragraph>
       The proof of the lemma is straightforward from our definitions. It is sufficient to observe that an equivalent formulation of our definition of collective rationality can be given by substituting formulas with the set of rational ballots given by their models. Two formulas that are logically equivalent have the same set of models, giving rise to the same requirement of collective rationality.{sup:6}
      </paragraph>
      <paragraph>
       Bringing together the results of Lemma 3, Lemma 4, we can now give the following definition:
      </paragraph>
      <paragraph label="Definition 7">
       A language for integrity constraints{a mathematical formula}L is a subset of {a mathematical formula}LPS that is closed under conjunction and logical equivalence.
      </paragraph>
      <paragraph>
    ...
    <section label="4">
     <section-title>
      Characterisation results for propositional languages
     </section-title>
     <paragraph>
      Once a language for integrity constraints is fixed, e.g., by means of a syntactic restriction on the integrity constraints, we may be interested in the problem of how to guarantee collective rationality with respect to all the integrity constraints that can be expressed in the given language. The aim of this section is to explore the relationship between the two definitions of classes of aggregation procedures introduced in Section 3: collectively rational procedures on one side, and procedures defined by axiomatic requirements on the other. In particular, we look for results of the following form:{a mathematical formula} for languages {a mathematical formula}L and axioms AX. We call such findings characterisation results: they provide necessary and sufficient axiomatic conditions for an aggregation procedure to be collectively rational with respect to a language for integrity constraints. Given the importance of collective rationality in many classical studies of aggregation and for a number of practical applications, such results are central to our study.
     </paragraph>
     <paragraph>
      The focus of this section is on languages: We provide complete characterisation for some basic classes of languages defined in a syntactic fashion, proving the correspondence with some of the main classical axioms from the literature on SCT. We shift the focus to axioms in Section 5.
     </paragraph>
     <paragraph>
      Definitions of all the axiomatic properties we refer to in this section can be found in Section 2.4. Axioms will be denoted with the capital letter associated with them, e.g., we will write U for unanimity and I for independence.
     </paragraph>
     <section label="4.1">
      <section-title>
    ...
    <section label="7">
     ...
     <paragraph>
      In this section we shall discuss the impact of the results of the present paper on both SCT and AI, summarising also some of the results presented in our previous work [21], [19], [18].
     </paragraph>
     <paragraph>
    ...
    <section label="8">
     ...
     <paragraph>
      While the framework of binary aggregation is well-known in the literature on SCT, this paper is the first systematic study of collective rationality with respect to languages for integrity constraints in this setting. Moreover, classical approaches concentrate on Arrovian aggregation procedures, i.e., procedures that are both unanimous and independent, while most of our characterisation results do not make such restrictive assumptions. As we have argued before, while the restriction to Arrovian aggregators is in line with standard assumptions in economics, those assumptions are not always justified in AI applications. In this section we review some of the classical approaches to the problem of collective rationality that can be found in the literature.
     </paragraph>
     <paragraph>
      Wilson [40] has been the first to define and study the framework of binary aggregation, in order to obtain an impossibility result for independent aggregation procedures that generalises the more famous result by Arrow [3]. This setting has been investigated more recently by Dokow and Holzman [11] and Nehring and Puppe [33]. In the former work, the authors study the existence of independent, unanimous and non-dictatorial procedures that are collectively rational with respect to certain domain restrictions. As we have remarked at the beginning of this section, our work differs in that we do not concentrate on independent aggregation procedures. Moreover, we exploit our representation of constraints as propositional formulas by defining languages for integrity constraints in a syntactic way, while Nehring and Puppe [11] use a model-based approach: they assume that the set of rational ballots is specified explicitly.
     </paragraph>
     <paragraph>
      Results in line with our work of Section 6 can be obtained by restricting the set of aggregation procedures to independent rules alone, focusing on the study of winning coalitions, i.e., those subsets of {a mathematical formula}N that can force acceptance on a given issue. Results in this setting have been proven in the case of judgment aggregation by Nehring and Puppe [32] and generalised by Dietrich and List [10], giving conditions on the structure of winning coalitions to guarantee collective rationality.
     </paragraph>
     <paragraph>
      As we already remarked in several places throughout the paper, many of the results on quota rules proved in Section 6 are analogous to those proved by Dietrich and List [10] in the framework of judgment aggregation. The use of integrity constraints, however, enables us to use syntax to analyse possible sources of impossibility, resulting in a flexible framework that is arguably easier to implement.
     </paragraph>
     <paragraph>
      A related field of research is that of belief merging [23], [24], in which sets of propositional formulas are aggregated into a collective set. The two frameworks share interesting features, but study different (although related) problems. While binary aggregation imposes an integrity constraint as a rationality assumption for both individual and collective outcomes, in belief merging integrity constraints are enforced only on the outcome. This reflects the view that the integrity constraint is a feasibility requirement rather than a rationality assumption, and should not be enforced on individual preferences or beliefs. Nevertheless, exploring possible ways of combining the two settings constitutes an interesting direction for future work.
     </paragraph>
    </section>
   </content>
  </root>
