   ...
   <content>
    ...
    <section label="3">
     ...
     <section label="3.4">
      ...
      <paragraph>
       Besides the two categories of Follow-the-Winner/Loser, another type of strategies may utilize both winners and losers, which is based on pattern matching. Based on nonparametric prediction [29], this category consists of several pattern-matching based investment strategies [27], [31], [30], [44], [28].
      </paragraph>
      <paragraph>
      ...
      <section label="3.4.2">
       ...
       <paragraph>
        The second step of the Pattern-Matching based Approaches is to construct an optimal portfolio based on the similar set C. Two main approaches are Kelly's capital growth portfolio and Markowitz's mean variance portfolio.
       </paragraph>
       <paragraph>
        Györfi et al. [27] proposed to figure out a log-optimal (Kelly) portfolio based on similar price relatives, which clearly follows the Capital Growth Theory. Given a similarity set, the log-optimal utility function is defined as,{a mathematical formula} where {a mathematical formula}Pi denotes the probability assigned to the similar price relative {a mathematical formula}xi,i∈C(x1t). Györfi et al. [27] assumes a uniform probability among the similar samples, thus it is equivalent to the following utility function,{a mathematical formula}
       </paragraph>
       <paragraph>
        Besides the log-optimal portfolio optimization, other methods can also be adopted, such as semi-log-optimal portfolio [56], [31], Markowitz-type portfolio [51], and GV-type portfolio [32], which we do not detail here.
       </paragraph>
       <paragraph>
        In any above procedure, if the similarity set is empty, we can choose either uniform portfolio or previous portfolio.
       </paragraph>
      </section>
      <section label="3.4.3">
       <section-title>
    ...
    <section label="4">
     ...
     <section label="4.2">
      ...
      <paragraph>
       In this article, we adopt two types of moving average. The first, so-called “Simple Moving Average” (SMA), truncates the historical prices via a window and calculates its arithmetical average,{a mathematical formula} where w denotes the window size. Although we can enlarge the window size such that SMA can cover more historical price relatives, the empirical evaluations in Section 5 show that generally their performance drops as the window size increases.
      </paragraph>
      <paragraph>
       To consider entire price relatives rather than a window, the second type, “Exponential Moving Average” (EMA), adopts all historical prices, and each price is exponentially weighted,{a mathematical formula} where {a mathematical formula}α∈(0,1) denotes a decaying factor.
      </paragraph>
      <paragraph>
       To this end, we can calculate the predicted price relative vector following the idea of so-called “Moving Average Reversion” (MAR). Based on the two types of moving average, we can infer two types of MAR,
      </paragraph>
      <paragraph>
       Moving Average Reversion: MAR-1{a mathematical formula} where w is the window size and ⊗ denotes element-wise product.
      </paragraph>
      <paragraph>
       Moving Average Reversion: MAR-2{a mathematical formula} where {a mathematical formula}α∈(0,1) denotes the decaying factor and the operations are all element-wise.
      </paragraph>
      <paragraph>
       Based on the expected price relative vector in Eq. (1) and Eq. (2), OLMAR further adopts the idea of an effective online learning algorithm, that is, Passive Aggressive (PA) [17] learning, to exploit the moving average reversion. Generally proposed for classification, PA passively keeps the previous solution if the classification is correct, while aggressively approaches a new solution if the classification is incorrect. After formulating the proposed OLMAR optimization, we solve its closed form update and design the proposed algorithm.
      </paragraph>
      <paragraph>
       The proposed formulation, OLMAR, is to exploit moving average reversion via online learning techniques [38]. The basic idea is to maximize the expected return {a mathematical formula}b⋅x˜t+1, and keep last portfolio information via a regularization term, using the online passive–aggressive learning method [17]. Thus, we follow the similar idea of PAMR [46] and formulate an optimization as,
      </paragraph>
      <paragraph>
       Optimization Problem: OLMAR{a mathematical formula} Note that we adopt expected return instead of expected log return.{sup:8} According to Helmbold et al. [37], to solve the optimization with expected log return, one can adopt the first-order Taylor expansion, which is essentially linear.
      </paragraph>
      <paragraph>
       The above formulation explicitly reflects the basic idea of the proposed OLMAR. On the one hand, if its constraint is satisfied, that is, the expected return is higher than a threshold, then the resulting portfolio equals to previous portfolio. On the other hand, if the constraint is not satisfied, then the formulation will figure out a new portfolio such that the expected return is higher than the threshold, while the new portfolio is not far from previous one.
      </paragraph>
      <paragraph>
       Note that although OLMAR's formulation looks similar to that of PAMR, they have some essential differences. In particular, PAMR's key constraint is {a mathematical formula}b⋅xt≤ϵ, while OLMAR's key constraint is {a mathematical formula}b⋅x˜t≥ϵ. First, PAMR uses raw price relative ({a mathematical formula}xt), while OLMAR adopts the price relative calculated by following the moving average reversion principle ({a mathematical formula}x˜t). Second, their inequality signs (“≤” for PAMR, and “≥” for OLMAR) are different. The different signs will make their behaviors different, which distinguish the two algorithms. After making certain transformations, PAMR may be written in a similar form, as shown in Table 2. However, their prediction functions are different, that is, OLMAR adopts multi-period mean reversion, while PAMR exploits single-period mean reversion.
      </paragraph>
     </section>
     <section label="4.3">
      <section-title>
      ...
      <paragraph>
       The above formulations are convex and thus can be solved by convex optimization techniques [9]. We now derive the OLMAR solutions in closed form and show its solution in Proposition 1.
      </paragraph>
      <paragraph label="Proposition 1">
       The solution of OLMAR without considering the non-negativity constraint is{a mathematical formula}where{a mathematical formula}x¯t+1=1m(1⋅x˜t+1)denotes the average predicted price relative and{a mathematical formula}λt+1is the Lagrangian multiplier calculated as,{a mathematical formula}
      </paragraph>
      <paragraph>
       Note that in the derivations we do not consider the non-negativity constraint following Helmbold et al. [37]. Thus, it is possible that the resulting portfolio goes out the portfolio simplex domain. To maintain a proper portfolio, we finally project the portfolio calculated according to the Propositions to the simplex domain.
      </paragraph>
      <paragraph>
       To this end, we can design the proposed algorithms based on the propositions. The on-line portfolio selection procedure is illustrated in Algorithm 2, and the proposed OLMAR procedures are illustrated in Algorithm 3.
      </paragraph>
      <paragraph>
    ...
    <section label="5">
     ...
     <paragraph>
      In this section, we will present an extensive set of empirical studies, including our experimental testbed, experimental protocols, comparison schemes, experimental results and detailed empirical analysis. All the source code and datasets for this work can be found in our website: http://OLPS.stevenhoi.org/OLMAR/.
     </paragraph>
     <section label="5.1">
      <section-title>
      ...
      <paragraph>
       In our study, we focus on historical daily prices in stock markets,{sup:9} which are easy to obtain from public domains (such as Yahoo Finance and Google Finance{sup:10}), and thus publicly available to other researchers. Data from other types of markets, such as high frequency intra-day quotes and Forex markets, are either too expensive or hard to obtain and process, and thus may reduce the experimental reproducibility. Summarized in Table 4, we employ six real and diverse datasets from several financial markets.{sup:11}
      </paragraph>
      <paragraph>
       The first dataset, “NYSE (O)”, is one “standard” dataset pioneered by Cover [14] and followed by others [37], [8], [1], [27], [30]. This dataset contains 5651 daily price relatives of 36 stocks{sup:12} in New York Stock Exchange (NYSE) for a 22-year period from Jul. 3rd 1962 to Dec. 31st 1984.
      </paragraph>
      <paragraph>
       The second dataset is an extended version of the NYSE dataset. For consistency, we collected the latest data in New York Stock Exchange (NYSE) from Jan. 1st 1985 to Jun. 30th 2010, which consists of 6431 trading days. We denote this new dataset as “NYSE (N)”.{sup:13} Note that the new dataset consists of 23 stocks rather than previous 36 stocks owing to amalgamations and bankruptcies. All self-collected price relatives are adjusted for splits and dividends, which is consistent with the previous “NYSE (O)” dataset.
      </paragraph>
      <paragraph>
       The third dataset “TSE” is collected by Borodin et al. [8], which consists of 88 stocks from Toronto Stock Exchange (TSE) containing price relatives of 1259 trading days, ranging from Jan. 4th 1994 to Dec. 31st 1998. The fourth dataset “SP500” is collected by Borodin et al. [8], which consists of 25 stocks with the largest market capitalizations in the 500 SP500 components. It ranges from Jan. 2nd 1998 to Jan. 31st 2003, containing 1276 trading days.
      </paragraph>
      <paragraph>
       The fifth dataset is “MSCI”, which is a collection of global equity indices that constitute the MSCI World Index.{sup:14} It contains 24 indices that represent the equity markets of 24 countries around the world, and consists of a total of 1043 trading days, ranging from Apr. 1st 2006 to Mar. 31st 2010. The final dataset is the “DJIA” dataset [8], which consists of 30 Dow Jones composite stocks. DJIA contains 507 trading days, ranging from Jan. 14th 2001 to Jan. 14th 2003.
      </paragraph>
      <paragraph>
       Unlike previous studies, the above testbed covers much longer trading periods from 1962 to 2010 and much more diversified markets, which enables us to examine the behaviors of the proposed strategies under different events and crises. For example, it covers several well-known events in the stock markets, such as dot-com bubble from 1995 to 2000 and subprime mortgage crisis from 2007 to 2009. The five stock datasets are mainly chosen to test the capability of the proposed algorithms on regional stock markets, while the index dataset aims to test their capability on global indices, which may be potentially applicable to “Fund on Fund” (FOF).{sup:15} As a remark, although we numerically test the proposed algorithms on stock and ETF markets, we note that the proposed strategy could be generally applied to any types of financial markets.
      </paragraph>
     </section>
     <section label="5.2">
      <section-title>
      ...
      <paragraph>
       Besides, we also introduce two practical issues in on-line portfolio selection, that is, margin buying and transaction costs, in Section 5.2.1, and further evaluate them in Section 5.7.
      </paragraph>
      <section label="5.2.1">
       <section-title>
        Practical issues
     ...
     <section label="5.3">
      ...
      <list>
       ...
       <list-item label="4.">
        UP: Cover's Universal Portfolios implemented according to Kalai and Vempala [40], where the parameters are set as {a mathematical formula}δ0=0.004, {a mathematical formula}δ=0.005, {a mathematical formula}m=100, and {a mathematical formula}S=500;
       </list-item>
       <list-item label="5.">
        EG: Exponential Gradient algorithm with the best learning rate {a mathematical formula}η=0.05 as suggested by Helmbold et al. [37];
       </list-item>
       <list-item label="6.">
        ONS: Online Newton Step with the parameters suggested by Agarwal et al. [1], that is, {a mathematical formula}η=0, {a mathematical formula}β=1, {a mathematical formula}γ=18;
       </list-item>
       <list-item label="7.">
        Anticor: {a mathematical formula}BAH30(Anticor(Anticor)) as a variant of Anticor to smooth the performance, which achieves the best performance among the three solutions proposed by Borodin et al. [8];
       </list-item>
       <list-item label="8.">
        {a mathematical formula}BK: Nonparametric kernel-based moving window strategy with {a mathematical formula}W=5, {a mathematical formula}L=10 and threshold {a mathematical formula}c=1.0, which has the best empirical performance according to Györfi et al. [27];
       </list-item>
       <list-item label="9.">
        {a mathematical formula}BNN: Nonparametric nearest neighbor-based strategy with parameters {a mathematical formula}W=5, {a mathematical formula}L=10 and {a mathematical formula}pℓ=0.02+0.5ℓ−1L−1 as the authors suggested [30];
       </list-item>
       <list-item label="10.">
        CORN: Correlation-driven nonparametric learning approaches [44] with parameters {a mathematical formula}W=5, {a mathematical formula}P=1, {a mathematical formula}ρ=0.1;
       </list-item>
       <list-item label="11.">
        PAMR: Passive aggressive mean reversion [46] with parameters {a mathematical formula}ϵ=0.5;
       </list-item>
       <list-item label="12.">
        CWMR: Confidence weighted mean reversion [45] algorithm (exact version) with {a mathematical formula}ϵ=0.5.
       </list-item>
      </list>
     </section>
     <section label="5.4">
      <section-title>
       Experimental results — cumulative wealth
      </section-title>
      <paragraph>
     ...
     <section label="5.5">
      <section-title>
       Experimental results — risk-adjusted return
      </section-title>
      <paragraph>
       We now evaluate the volatility risk and drawdown risk, and the risk-adjusted return in terms of annualized Sharpe ratio and Calmar ratio. Fig. 1 shows the evaluation results on the six datasets. In addition to the proposed algorithms, we also plot two benchmarks (Market and BCRP) and five state-of-the-art algorithms (Anticor, {a mathematical formula}BNN, CORN, PAMR and CWMR).
      </paragraph>
      <paragraph>
       In the preceding results on cumulative wealth, we find that the proposed OLMAR achieves the highest cumulative return on most datasets. However, high return is associated with high risk, as no real financial instruments can guarantee high return without high risk. The volatility risk in Fig. 1(a) shows that the proposed method almost achieves the highest risk in terms of volatility risk on most datasets. In addition, the drawdown risk in Fig. 1(b) also shows that the proposed method also achieves high drawdown risk in most datasets. These results are not difficult to understand since high return is often associated with high risk.
      </paragraph>
      <paragraph>
