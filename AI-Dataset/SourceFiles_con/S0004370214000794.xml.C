   ...
   <content>
    <section label="1">
     ...
     <section label="1.1">
      ...
      <paragraph>
       Recently, Liu and Shen [15] proposed a so-called multicategory ψ-learning algorithm by using a multicategory ψ loss, and Wu and Liu [27] devised robust truncated-hinge-loss SVMs. These two algorithms are parallel to the multiclass SVM of Crammer and Singer [5] and enjoy a generalized pairwise comparison setting.
      </paragraph>
      <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.1">
      ...
      <paragraph label="Proposition 5">
       For{a mathematical formula}(x,c), let{a mathematical formula}g(x)be a margin vector atxand the induced classifier be{a mathematical formula}ϕ(x)=arg⁡maxj⁡gj(x). Then{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula}
      </paragraph>
      <paragraph>
       Proposition 5 shows that {a mathematical formula}gc(x)≤0 is the sufficient condition of {a mathematical formula}ϕ(x)≠c, while {a mathematical formula}gj(x)&gt;0 for some {a mathematical formula}j≠c is its necessary condition. The following theorem shows that they become sufficient and necessary when g has one and only one positive element.
      </paragraph>
      <paragraph label="Proposition 6">
       Under the conditions inProposition 5. The relationship of{a mathematical formula}holds if and only if the margin vector{a mathematical formula}g(x)has only one positive element.
      </paragraph>
      <paragraph>
     ...
     <section label="3.4">
      ...
      <paragraph>
       Although there is no explicit expression for {a mathematical formula}gˆ(x) in Problem (7), Theorem 9 shows that its limit at {a mathematical formula}T=0 is equal to the minimizer of {a mathematical formula}∑c=1m∑j≠c(1+gj(x))+Pc(x), which was studied by Lee et al. [13].
      </paragraph>
      <paragraph>
       Based on the pairwise comparison, we have a smooth alternative to multiclass hinge loss {a mathematical formula}∑j≠c(1+gc(x)−gj(x))+, which is{a mathematical formula} It is also immediate that {a mathematical formula}GT(g(x),c)≥∑j≠c(1+gc(x)−gj(x))+ and {a mathematical formula}limT→0⁡GT(g(x),c)=∑j≠c(1+gc(x)−gj(x))+.
      </paragraph>
    ...
    <section label="4">
     ...
     <section label="4.1">
      ...
      <paragraph>
       In the case of {a mathematical formula}u=0 and {a mathematical formula}T=1, it follows from Theorem 13 that {a mathematical formula}Pc(x)=exp⁡(gˆc(x))∑j=1mexp⁡(gˆj(x)). This is identical to the solution for logistic regression.
      </paragraph>
      <paragraph label="Theorem 14">
       Let{a mathematical formula}gˆ(x)=(gˆ1(x),…,gˆm(x))Tbe the solution of optimization problem(15)where{a mathematical formula}Pc(x)&gt;0for{a mathematical formula}c=1,…,m, and let{a mathematical formula}Pl(x)=maxc⁡Pc(x).
      </paragraph>
      <list>
       <list-item label="(1)">
        If{a mathematical formula}Pl(x)&gt;1/2, then{a mathematical formula}
       </list-item>
       <list-item label="(2)">
        If{a mathematical formula}Pl(x)&lt;1/2, then{a mathematical formula}
       </list-item>
   ...
   <appendices>
    ...
    <section label="Appendix B">
     ...
     <section label="B.1">
      ...
      <paragraph>
       Without loss of generality, we assume that {a mathematical formula}P1&gt;1/2&gt;P2≥⋯Pm&gt;0.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       In the first case where {a mathematical formula}0≤ρ≤1, we have{a mathematical formula} which implies that {a mathematical formula}L(u)−L(f)&gt;0 whenever {a mathematical formula}ρ≠1.
      </paragraph>
      <paragraph>
       In the second case where {a mathematical formula}ρ&gt;1, we then have{a mathematical formula} In summary, the minimizer {a mathematical formula}gˆj should satisfy {a mathematical formula}gˆ1=1+gˆ2 whenever {a mathematical formula}P1&gt;12.
      </paragraph>
      <paragraph>
     ...
     <section label="B.5">
      ...
      <paragraph label="Case 2">
       {a mathematical formula}Pl&lt;1/2. In this case, we always have {a mathematical formula}limT→0⁡(gi−gl)=0. Otherwise, the second term is {a mathematical formula}1−Pl which is greater than 1/2.
      </paragraph>
     </section>
    </section>
    <section label="Appendix C">
     The properties of the multiclass {a mathematical formula}C-loss
     <paragraph>
      We first prove that {a mathematical formula}CT,u(g(x),c) is convex. From Appendix B.5, we can obtain the Hessian matrix of {a mathematical formula}CT,u(g(x),c) w.r.t. {a mathematical formula}g(x). That is,{a mathematical formula} where {a mathematical formula}Δc=diag(β1c,…,βmc) and {a mathematical formula}βc=(β1c,…,βmc)T. We also have from Appendix B.5 that {a mathematical formula}Hc is positive semidefinite (in fact, it is conditionally positive definite). Thus, {a mathematical formula}CT,u(g(x),c) is convex.
     </paragraph>
     <section label="C.1">
      The proof of Proposition 11
      <paragraph>
       Noting that{a mathematical formula} we have {a mathematical formula}CT,1(g(x),c)&lt;GT(g(x),c).
      </paragraph>
      <paragraph>
       Now assume that {a mathematical formula}l=argmaxj{gj(x)+u−uI{j=c}}. Then{a mathematical formula}
      </paragraph>
     </section>
     <section label="C.2">
      The proof of Proposition 12
      <paragraph>
       First, it is easily obtained that {a mathematical formula}limT→∞⁡ωjc(x)=1m. Second, consider that{a mathematical formula} It immediately follows from {a mathematical formula}Hu(g(x),c)≤CT,u(g(x),c)≤Hu(g(x),c)+Tlog⁡(m) that {a mathematical formula}limT→0⁡CT,u(g(x),c)=Hu(g(x),c).
      </paragraph>
      <paragraph>
       Third, the derivative of {a mathematical formula}CT,u(g(x),c) w.r.t. T is given by{a mathematical formula} Thus, {a mathematical formula}CT,u(g(x),c) is an increasing function of T.
      </paragraph>
     </section>
    </section>
    <section label="Appendix D">
     <section-title>
      The learning algorithm for MCL
     </section-title>
     <paragraph>
