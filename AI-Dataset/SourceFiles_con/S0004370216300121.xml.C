   ...
   <content>
    ...
    <section label="2">
     ...
     <paragraph label="Definition 1">
      A binary CSP instance I consists of
     </paragraph>
     <list>
      <list-item label="•">
      ...
      <list-item label="•">
       a domain {a mathematical formula}D(x) of possible values for each variable {a mathematical formula}x∈X,
      </list-item>
      <list-item label="•">
       a relation {a mathematical formula}Rxy⊆D(x)×D(y), for each pair of distinct variables {a mathematical formula}x,y∈X, which consists of the set of compatible pairs of values {a mathematical formula}(a,b) for variables {a mathematical formula}(x,y).
      </list-item>
     </list>
     <paragraph>
      For simplicity of presentation, Definition 1 assumes that there is exactly one constraint relation for each pair of variables. The number of constraints e is the number of pairs of variables {a mathematical formula}x,y such that {a mathematical formula}Rxy≠D(x)×D(y). An instance I is arc consistent if for each pair of distinct variables {a mathematical formula}x,y∈X, each value {a mathematical formula}a∈D(x) has an AC-support at y, i.e. a value {a mathematical formula}b∈D(y) such that {a mathematical formula}(a,b)∈Rxy.
     </paragraph>
     <paragraph>
      In our representation of general-arity CSP instances, we require the notion of tuple which is simply a set of variable-value assignments. For example, in the binary case, the tuple {a mathematical formula}{〈x,a〉,〈y,b〉} is compatible if {a mathematical formula}(a,b)∈Rxy and incompatible otherwise.
     </paragraph>
     <paragraph label="Definition 2">
      A (general-arity) CSP instance I consists of
     </paragraph>
     <list>
      <list-item label="•">
       a set X of n variables,
    ...
    <section label="3">
     <section-title>
      Value merging in binary CSP based on the BTP
     </section-title>
     <paragraph>
    ...
    <section label="5">
     ...
     <paragraph label="Theorem 7">
      The problem of determining if it is possible to perform k BTP-mergings in a boolean binary CSP instance is NP-complete.
     </paragraph>
     <paragraph label="Proof">
     ...
     <paragraph>
      The reduction in the proof of Theorem 7 supposes that no arc-consistency operations are used. We will now show that it is possible to modify the reduction so as to prevent the elimination of any values in the instance {a mathematical formula}ICSP by arc-consistency, even when the maximum size of the domains d is bounded by a constant as small as 3. Recall that an arc-consistent instance remains arc-consistent after any number of BTP-mergings.
     </paragraph>
     <paragraph label="Theorem 8">
      The problem of determining if it is possible to perform k BTP-mergings in an arc-consistent binary CSP instance is NP-complete, even when only considering binary CSP instances where the size of the domains is bounded by 3.
     </paragraph>
     <paragraph label="Proof">
     ...
     <paragraph label="Corollary 9">
      The problem of determining if it is possible to perform k value eliminations by arc consistency and BTP-merging in a binary CSP instance is NP-complete, even when only considering binary CSP instances where the size of the domains is bounded by 3.
     </paragraph>
     <paragraph>
     ...
     <paragraph label="Theorem 10">
      The problem of determining if it is possible to perform k BTP-mergings within a same domain in a binary CSP instance is NP-complete.
     </paragraph>
    </section>
    <section label="6">
     <section-title>
     ...
     <section label="6.3">
      <section-title>
       Comparisons with neighbourhood substitution and virtual interchangeability
      </section-title>
      <paragraph>
    ...
    <section label="7">
     <section-title>
      Generalising BTP-merging to constraints of arbitrary arity
     </section-title>
     <paragraph>
      In the remainder of the paper, we assume that the constraints of a general-arity CSP instance I are given in the form described in Definition 2, i.e. as a set of incompatible tuples NoGoods(I), where a tuple is a set of variable-value assignments. For simplicity of presentation, we use the predicate Good({a mathematical formula}I,t) which is true iff the tuple t is a partial solution, i.e. t does not contain any pair of distinct assignments to the same variable and {a mathematical formula}∄t′⊆t such that {a mathematical formula}t′∈ NoGoods(I). We first generalise the notion of broken triangle and merging to the general-arity case, before showing that absence of broken triangles allows merging.
     </paragraph>
     <paragraph label="Definition 11">
      A general-arity broken triangle (GABT) on values {a mathematical formula}a,b∈D(x) consists of a pair of tuples {a mathematical formula}t,u (containing no assignments to variable x) satisfying the following conditions:
     </paragraph>
     <list>
      <list-item label="1.">
       Good({a mathematical formula}I,t∪u) ∧ Good({a mathematical formula}I,t∪{〈x,a〉}) ∧ Good({a mathematical formula}I,u∪{〈x,b〉})
      </list-item>
      <list-item label="2.">
       {a mathematical formula}t∪{〈x,b〉}∈ NoGoods(I) ∧ {a mathematical formula}u∪{〈x,a〉}∈ NoGoods(I)
      </list-item>
     </list>
     <paragraph>
    ...
    <section label="8">
     <section-title>
      BTP-merging in the presence of global constraints
     </section-title>
     <paragraph>
    ...
    <section label="9">
     <section-title>
      A tractable class of general-arity CSP
     </section-title>
     <paragraph>
     ...
     <section label="9.1">
      <section-title>
       Directional general-arity BTP
      </section-title>
      <paragraph>
       Recall that we assume that a CSP instance I is given in the form of a set of incompatible tuples NoGoods(I), where a tuple is a set of variable-value assignments, and that the predicate Good({a mathematical formula}I,t) is true iff the tuple t does not contain any pair of distinct assignments to the same variable and {a mathematical formula}∄t′⊆t such that {a mathematical formula}t′∈ NoGoods(I). We suppose given a total ordering &lt; of the variables of a CSP instance I. We write {a mathematical formula}t&lt;x to represent the subset of the tuple t consisting of assignments to variables occurring before x in the order &lt;, and {a mathematical formula}Vars(t) to denote the set of all variables assigned by t.
      </paragraph>
      <paragraph label="Definition 21">
       A directional general-arity (DGA) broken triangle on assignments {a mathematical formula}a,b to variable x in a CSP instance I is a pair of tuples {a mathematical formula}t,u (containing no assignments to variable x) satisfying the following conditions:
      </paragraph>
      <list>
       <list-item label="1.">
        {a mathematical formula}t&lt;x and {a mathematical formula}u&lt;x are non-empty
       </list-item>
       <list-item label="2.">
        Good({a mathematical formula}I,t&lt;x∪u&lt;x) ∧ Good({a mathematical formula}I,t&lt;x∪{〈x,a〉}) ∧ Good({a mathematical formula}I,u&lt;x∪{〈x,b〉})
       </list-item>
       <list-item label="3.">
        {a mathematical formula}∃t′ s.t. {a mathematical formula}Vars(t′)=Vars(t)∧(t′)&lt;x=t&lt;x∧{a mathematical formula}t′∪{〈x,a〉}∉ NoGoods(I)
       </list-item>
       <list-item label="4.">
        {a mathematical formula}∃u′ s.t. {a mathematical formula}Vars(u′)=Vars(u)∧(u′)&lt;x=u&lt;x∧u′∪{〈x,b〉}∉ NoGoods(I)
       </list-item>
       <list-item label="5.">
        {a mathematical formula}t∪{〈x,b〉}∈ NoGoods(I) ∧ {a mathematical formula}u∪{〈x,a〉}∈ NoGoods(I)
       </list-item>
     ...
     <section label="9.3">
      <section-title>
       Tractability of DGABTP for a known variable ordering
      </section-title>
      <paragraph>
