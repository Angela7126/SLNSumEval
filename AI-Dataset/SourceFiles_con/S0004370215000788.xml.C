  ...
  <root>
   ...
   <content>
    <section label="1">
     ...
     <section label="1.2">
      <section-title>
       Learning models for white-box and black-box optimization
      </section-title>
      <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.1">
      ...
      <paragraph>
       In this paper, we use regression trees [17] and least absolute shrinkage and selection operator (LASSO) [18] as regression functions, and train them using features based on the items auctioned before and after the current item. We first briefly introduce these regressors.
      </paragraph>
      <section>
       <section>
        <section-title>
     ...
     <section label="3.2">
      <section-title>
       Learning regression functions for predicting revenues
      </section-title>
      <paragraph>
    ...
    <section label="4">
     ...
     <section label="4.1">
      <section-title>
       White-box optimization: an ILP model
      </section-title>
      <paragraph>
      ...
      <section>
       ...
       <section>
        <section-title>
         Translating feature values
        </section-title>
        <paragraph>
         In order to compute the prediction of a regression model, we not only need to translate the models into ILP constraints, but also the values of the features used by these models.{sup:4} Feature 1, 2, 3, and 5 can be computed using linear functions from the x variables:{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula}
        </paragraph>
        <paragraph>
         For the fourth type of feature, we use an additional variable {a mathematical formula}pj,r, which encodes the expected value of the item auctioned at index j of type r. If the item at index j is not of type r, {a mathematical formula}pj,r is equal to 0. Since the p variables are the predictions of the regression functions, we provide their definition after defining the regression models.{a mathematical formula}
        </paragraph>
        <paragraph>
         To aid the ILP solver, we also pre-compute the minimum {a mathematical formula}mf,i and maximum {a mathematical formula}Mf,i obtainable values of every feature f at every index i and provide these as bounds to the solver.
        </paragraph>
       </section>
       <section>
        <section-title>
        ...
        <paragraph>
         We aim to maximize the expected values {a mathematical formula}pi,r:{a mathematical formula}
        </paragraph>
        <paragraph>
       ...
       <section>
        <section-title>
         Encoding linear regression model
        </section-title>
        <paragraph>
    ...
    <section label="5">
     ...
     <section label="5.1">
      ...
      <section>
       ...
       <section>
        ...
        <paragraph>
         For the same set of 20 bidders, we generate 5 sets of 40 items, which are used for testing. First, the regressors are tested by comparing their predictions with the revenues generated by the simulator on 50 random orderings of each of these item sets. Second, we translate each of the item sets into constraints for both the black-box and white-box optimization solvers. The best ordering found by these solvers are compared based on their values on the regression model, and in the simulator.
        </paragraph>
       </section>
      </section>
     </section>
     ...
     <section label="5.3">
      <section-title>
       Experiment 1: first-price auctions
      </section-title>
      <paragraph>
      ...
      <section>
       ...
       <section>
        ...
        <paragraph>
         Moreover, the small difference in {a mathematical formula}R2 scores between trees with depth 5 and depth 8 also has a significant effect on the difference between their model and simulator evaluations (e.g., due to cascading errors). To demonstrate this, we report in Fig. 7 the solution differences of the same ordering methods when being evaluated by the model and the simulator. The purpose of this comparison is to test whether the predicted outcome (using learned models) corresponds to the actual outcome (using simulator). This test is important as in general, there are no simulators available to evaluate the solutions. The figure demonstrates that the linear regression based optimization methods return more reliable solutions, i.e., their solutions evaluated on the learned linear models are closer to the solution values returned by the simulator. Note that it is logical that most values are over estimated because we the optimization tries to solve a maximization problem. The trees with depth 5 and 8 show a significant difference in this evaluation. The depth 3 trees end up with the highest evaluation difference, and overestimate the solution values the most.
        </paragraph>
       </section>
      </section>
     </section>
     <section label="5.4">
      <section-title>
       Experiments 2 and 3: Vickrey auction with myopic and smart agents
      </section-title>
      <paragraph>
      ...
      <paragraph>
       In the second-price experiment, we test with truth-telling bidders in second-price auctions. The only differences with the setting in the first experiment are the bidding values and the payments. As Fig. 8 shows, the results are very similar to those in the first experiments: (1) all methods outperform the naive ordering strategies from the literature, (2) the white-box outperforms the black-box methods, and (3) the linear regression models perform best. However, the difference between the best performing methods tree5 and lasso2 is no longer significant.
      </paragraph>
      <paragraph>
     ...
     <section label="5.5">
      ...
      <section label="5.5.2">
       ...
       <paragraph>
        In this set of experiments, we obtain different sets of agents by creating a large population of size 60, and drawing 20 of them at random for every auction. In addition, we generate a random set of 20 to 60 items for every auction, sampled uniformly at random. Other than that, we use the exact same settings as the main experiments. We show the results of 19 sets of agents (we increased the set to reduce noise in the results), each being tested with 5 sets of items, see Fig. 12. The {a mathematical formula}R2-scores obtained in these experiments are shown in Fig. 11.
       </paragraph>
       <paragraph>
    ...
    <section label="6">
     ...
     <section label="6.1">
      <section-title>
       Interplay between mathematical optimization and machine learning
      </section-title>
      <paragraph>
       Many studies have investigated the interplay of data mining and machine learning with mathematical modeling techniques, see overview in e.g. [25], [26], [27]. Most of these investigate how to use data mining to estimate the value of parameters in decision making models or to replace decision model structure when it cannot be fully determined from the hypotheses at hand. For instance, Brijs et al. [28] build a decision model as an integer program that maximizes product assortment of a retail store. The decision model is then refined by incorporating additional decision attributes that are the learned patterns from recorded sales data. Li and Olafsson [3] use a decision tree to learn dispatching rules that are then used to decide which job should be dispatched first. These dispatching rules are previously unknown, and it is assumed that it is worthwhile to capture the current practices from previous data. Gabel and Riedmiller [1] model production scheduling problem as multi-agent reinforcement learning where each agent makes its dispatching decisions using a reinforcement learning algorithm based on a neural network function approximation.
      </paragraph>
      <paragraph>
       Another line of work investigates how to use learning techniques during optimization in order to learn properties of good solutions. For instance, Defourny et al. [29] combine the estimation of statistical models for returning a decision rule given a state with scenario tree techniques from multi-stage stochastic programming. This line of work shares similarities with the field of black-box optimization, see, e.g., [14], [15], [30]. In black-box optimization, methods are used to approximate a function with unknown analytical form and which typically is expensive to execute. In contrast, in multi-stage stochastic programming this form is known but stochastic. An often applied technique for black-box optimization is the use of surrogate methods, see, e.g., [31]. Surrogates are approximations of the black-box function that are less expensive to execute. Typical examples include linear/polynomial regression, neural networks, and other methods from machine learning. These functions are trained during optimization from (as few as possible) black-box function calls.
      </paragraph>
      <paragraph>
   ...
   <appendices>
    <section label="Appendix A">
     <section-title>
      Hardness of auction design using learned predictors
     </section-title>
     <paragraph>
     ...
     <paragraph label="Lemma 1">
      Using regression trees, the problem of whether there exists an ordering that has a total predicted value of at least K is NP-complete.
     </paragraph>
     <paragraph label="Proof">
      The proof follows from the fact that we can use simple regression trees to model the preferences of the two agents from Theorem 1, and evaluating an ordering using these trees can be done in polynomial time. The regression tree for every item type {a mathematical formula}ri is shown in Fig. 15. □
     </paragraph>
     <paragraph label="Lemma 2">
      Using linear regression predictors, the problem of whether there exists an ordering that has a total predicted value of at least K is NP-complete.
     </paragraph>
     <paragraph label="Proof">
     ...
     <paragraph label="Remarks">
      We proved the NP-completeness for the general case of Lemma 2. However, we do not know whether the complexity holds for more realistic valuation functions that bidders have.
     </paragraph>
    </section>
   </appendices>
  </root>
