  ...
  <root>
   ...
   <content>
    <section label="1">
     ...
     <list>
      <list-item label="(1)">
       Simultaneous decision making, in which agents A and B make their decisions at the same time. In this case, Agent A could lead Agent B to perform actions resulting in long-term higher team utility. This interaction between the agents is modeled using game theoretic tools, specifically, by a matrix game representation.
      </list-item>
      <list-item label="(2)">
       Sequential decision making, in which Agent B selects its action after observing the outcome of Aʼs (and possibly its own past) actions. Here, the actions chosen by Agent A can teach Agent B of the optimal action it should choose, yielding highest possible team utility in the long run. In this case, we model the interaction between the agents by a novel cooperative k-armed bandit formalism.
      </list-item>
     </list>
     <paragraph>
     ...
     <section label="1.1">
      ...
      <paragraph>
       The challenge of ad hoc teamwork, as presented in the ad hoc teamwork introductory paper [1], is: To create an autonomous agent that is able to efficiently and robustly collaborate with previously unknown teammates on tasks to which they are all individually capable of contributing as team members.
      </paragraph>
      <paragraph>
    ...
    <section label="2">
     ...
     <section label="2.2">
      ...
      <section label="2.2.3">
       ...
       <paragraph>
        Until this point, we have assumed that Agent B acts deterministically: Agent A could predict Agent Bʼs next action with certainty based on its own previous actions. In this section we relax that assumption by allowing Bʼs ϵ to be greater than 0.
       </paragraph>
       <paragraph>
     ...
     <section label="2.4">
      ...
      <list>
       <list-item label="•">
        Can find optimal action sequence efficiently: {a mathematical formula}O(d3)
       </list-item>
       <list-item label="•">
        Maximum length of optimal sequence: {a mathematical formula}min(x,y)
       </list-item>
      </list>
      <paragraph>
       Longer teammate memory:
      ...
      <list>
       ...
       <list-item label="•">
        Maximum length of optimal sequence: open problem—between {a mathematical formula}(min(x,y)−1)⁎mem+1 and {a mathematical formula}min(x,y)⁎xmem−1
       </list-item>
      </list>
      <paragraph>
       Random teammate:
    ...
    <section label="3">
     <section-title>
      Teaching a teammate: Sequential scenarios with differing abilities
     </section-title>
     <paragraph>
     ...
     <section label="3.1">
      ...
      <paragraph>
       We assume that the learnerʼs behavior (Agent B) is fixed and known: it acts greedily, always selecting the arm with the highest observed sample average so far. Any arm that has never been pulled is assumed to have a sample average of ∞. Thus, the learner always prefers selecting an arm that has not been selected previously. If there is more than one such arm, it selects randomly from among them. This assumption reflects optimism in the face of uncertainty on the part of the learner (optimistic initialization).
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       By these assumptions, the learner is both less capable and less knowledgeable than the teacher, and it does not understand direct communication from the teacher. It is tempting to think that we should begin by improving the learner. But in the ad hoc team setting, that is not an option. The learner “is what it is” either because it is a legacy agent, or because it has been programmed by others. Our task is to determine the teacherʼs best actions given such learner behavior.
      </paragraph>
      <paragraph>
       We use the following notation for the three arms. The learner selects between Arm1 and Arm2, while the teacher can additionally choose {a mathematical formula}Arm⁎. While we consider two different forms of distributions for the payoffs, throughout the section we use the following notation:
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}μi is the expected payoff of {a mathematical formula}Armi ({a mathematical formula}i∈{1,2,⁎}).
       </list-item>
       <list-item label="•">
        {a mathematical formula}ni is the number of times {a mathematical formula}Armi has been pulled (observed) in the past.
       </list-item>
       <list-item label="•">
        {a mathematical formula}mi is the cumulative payoff from all the past pulls of {a mathematical formula}Armi.
       </list-item>
       <list-item label="•">
        {a mathematical formula}x¯i=mini is the observed sample average so far.
       </list-item>
       <list-item label="•">
     ...
     <section label="3.2">
      ...
      <section label="3.2.1">
       ...
       <paragraph>
        Therefore there are situations in which it is better for the teacher to pull Arm1 than {a mathematical formula}Arm⁎. This article is devoted to characterizing exactly what those situations are.
       </paragraph>
      </section>
      <section label="3.2.2">
       The teacher should never pull Arm2
       <paragraph>
       ...
       <paragraph label="Theorem 3.1">
        It is never optimal for the teacher to pull{a mathematical formula}Arm2.
       </paragraph>
       <paragraph label="Proof sketch">
        The proof uses induction on r.
       </paragraph>
       <paragraph label="Base case">
      ...
      <section label="3.2.3">
       Never teach when {a mathematical formula}x¯1&gt;x¯2
       <paragraph>
        Third, we show that the teacherʼs choice is clear whenever {a mathematical formula}x¯1&gt;x¯2. That is, if the current sample average of Arm1 is greater than that of Arm2 such that the learner will choose Arm1 next, then the teacher should always choose {a mathematical formula}Arm⁎: it should not teach.
       </paragraph>
       <paragraph label="Theorem 3.2">
        When{a mathematical formula}x¯1&gt;x¯2, it is always optimal for the teacher not to teach (to pull{a mathematical formula}Arm⁎).
       </paragraph>
       <paragraph label="Proof">
      ...
      <section label="3.2.4">
       Do not teach when {a mathematical formula}n1=0 and/or {a mathematical formula}n2=0
       <paragraph>
     ...
     <section label="3.3">
      ...
      <section label="3.3.1">
       ...
       <list>
        <list-item label="1.">
         {a mathematical formula}m1+1n1+1&gt;m2n2
        </list-item>
        <list-item label="2.">
         {a mathematical formula}p⁎−p1&lt;p1(p1−p2)
        </list-item>
       </list>
       <paragraph>
        The right hand side of the second inequality is the probability that Arm1 will yield a 1 multiplied by the difference in expected values between Arm1 and Arm2.
       </paragraph>
       <paragraph>
        Note that we can also explicitly calculate the expected values of both not teaching ({a mathematical formula}EVnt) and teaching ({a mathematical formula}EVt). {a mathematical formula}EVnt=p⁎+p2 and {a mathematical formula}EVt=p1+p12+(1−p1)p2.
       </paragraph>
      </section>
      <section label="3.3.2">
       <section-title>
       ...
       <paragraph>
        Building on the intuition from Section 3.3.1, this section presents our fully-implemented polynomial memory and time dynamic programming algorithm for determining the teacherʼs optimal action with any number of rounds left. It takes as input initial values for {a mathematical formula}m1,n1,m2,n2, and r, which we denote as {a mathematical formula}M1,N1,M2,N2, and R respectively, and it outputs whether the teacherʼs expected value is higher if it teaches by pulling Arm1 or if it exploits by pulling {a mathematical formula}Arm⁎.
       </paragraph>
       <paragraph>
        The dynamic programming algorithm works backwards from smaller to bigger values of r, computing the expected value of the optimal action from any possible values of {a mathematical formula}m1, {a mathematical formula}n1, {a mathematical formula}m2, and {a mathematical formula}n2 that could be reached from the initial values.
       </paragraph>
       <paragraph>
        First, consider the values that {a mathematical formula}m1, {a mathematical formula}n1, {a mathematical formula}m2, and {a mathematical formula}n2 can take on when there are r rounds left.
       </paragraph>
       <list>
        <list-item label="•">
         Because both agents can pull Arm1 any number of times, with r rounds left (after {a mathematical formula}R−r rounds have passed), {a mathematical formula}n1 can range from {a mathematical formula}N1 (if Arm1 was never selected) to {a mathematical formula}N1+2(R−r).
        </list-item>
        <list-item label="•">
         Any number of the {a mathematical formula}n1−N1 times that Arm1 was pulled, {a mathematical formula}m1 could have increased by 1. Thus {a mathematical formula}m1 can range from {a mathematical formula}M1 to {a mathematical formula}M1+(n1−N1).
        </list-item>
        <list-item label="•">
         Because only the learner pulls Arm2, it will be pulled at most once per round. But the range of {a mathematical formula}n2 depends on the value {a mathematical formula}n1, because the learner only pulls Arm2 when it does not pull Arm1. Thus {a mathematical formula}n2 can range from {a mathematical formula}N2+max(0,R−r−(n1−N1)) to {a mathematical formula}N2+(R−r)−max(0,n1−N1−(R−r)).
        </list-item>
        <list-item label="•">
         Similarly to {a mathematical formula}m1, {a mathematical formula}m2 can range from {a mathematical formula}M2 to {a mathematical formula}M2+(n2−N2).
        </list-item>
       </list>
       <paragraph>
      ...
      <section label="3.3.5">
       ...
       <list>
        <list-item label="1.">
         Are there any patterns in the optimal action as a function of r when all other parameters are held constant?
        </list-item>
        <list-item label="2.">
         How sensitive is the expected value computation to the relationship between {a mathematical formula}m1, {a mathematical formula}n1, {a mathematical formula}m2, {a mathematical formula}n2, {a mathematical formula}p1, {a mathematical formula}p2, and {a mathematical formula}p⁎?
        </list-item>
        <list-item label="3.">
         When Algorithm 2 is run, how many of the states tend to have Arm1 (teaching) as the optimal action?
        </list-item>
       </list>
       <paragraph>
     ...
     <section label="3.4">
      ...
      <section label="3.4.1">
       {a mathematical formula}x¯1&lt;x¯2, {a mathematical formula}r=1
       <paragraph>
        In order to analyze this case, we make use of the cumulative distribution function (CDF) of the normal distribution, denoted as {a mathematical formula}Φμ,σ(v). Exactly as in the binary case, with one round left, the teacher should teach when the expected cost of teaching, {a mathematical formula}μ⁎−μ1, is less than the probability that teaching will successfully cause the learner to switch its choice from Arm2 to Arm1, {a mathematical formula}Φμ1,σ1(y), multiplied by the benefit of successful teaching, {a mathematical formula}μ1−μ2. Here y is the minimum return from Arm1 that would cause the sample average of Arm1 to surpass that of Arm2: {a mathematical formula}m1+yn1+1=x¯2.
       </paragraph>
       <paragraph>
        Therefore, the teacher should pull Arm1 if and only if{a mathematical formula} (recall that {a mathematical formula}x¯1=m1n1 by definition). Otherwise, the teacher should pull {a mathematical formula}Arm⁎. We can then compute the expected value of the optimal action as:
       </paragraph>
       <list>
        <list-item label="•">
         If {a mathematical formula}x¯1&gt;x¯2, {a mathematical formula}EVnt=μ⁎+μ1
        </list-item>
        <list-item label="•">
         Else, if the optimal action is to teach, {a mathematical formula}EVt=μ1+μ2Φmu1,σ1(x¯2(n1+1)−x¯1n1)+μ1(1−Φmu1,σ1(x¯2(n1+1)−x¯1n1))
        </list-item>
        <list-item label="•">
         Else {a mathematical formula}EVnt=μ⁎+μ2.
        </list-item>
       </list>
       <paragraph>
        Since there are readily available packages, for example in Java, for computing {a mathematical formula}Φμ1,σ1(y), this result can be considered a closed form solution for finding the optimal teacher action and its expected value when {a mathematical formula}r=1.
       </paragraph>
      </section>
      <section label="3.4.2">
       {a mathematical formula}x¯1&lt;x¯2, {a mathematical formula}r⩾2
       <paragraph>
      ...
      <section label="3.4.3">
       ...
       <paragraph>
        Even if we cannot practically determine in general what the teacherʼs optimal action is, it may be possible to find some reasonable heuristics. To this end, in this section we consider the following questions, the first of which is parallel to the first question considered in Section 3.3.5:
       </paragraph>
       <list>
        <list-item label="1.">
         Are there any rules or patterns in the optimal action as a function of r (when all other parameters are held constant)?
        </list-item>
        <list-item label="2.">
         How do various teacher heuristics compare to one another in performance?
        </list-item>
       </list>
       <paragraph>
        First, just as in the binary case, intuition suggests that increasing r should make it more beneficial to teach since there is more time for the added information to be used by the learner. However again, we can find a counterexample even with {a mathematical formula}r=1 and 2.
       </paragraph>
       <paragraph>
        Consider the case in which {a mathematical formula}(μ⁎,σ⁎)=(10,0), {a mathematical formula}(μ1,σ1)=(9,2), and {a mathematical formula}(μ2,σ2)=(7,2). Suppose that the learner has observed Arm1 being pulled once when it got a payoff of 6.99 ({a mathematical formula}x¯1=6.99, {a mathematical formula}n1=1), and it observed Arm2 once for a payoff of 8 ({a mathematical formula}x¯2=8, {a mathematical formula}n2=1).
       </paragraph>
       <paragraph>
        With these values it is barely not worth it for the teacher to teach with {a mathematical formula}r=1. That is, with these values, Inequality (1) is not satisfied, but if {a mathematical formula}x¯1 were 7.01, then it would be satisfied. Thus we know with certainty that the teacherʼs optimal action is {a mathematical formula}Arm⁎.
       </paragraph>
       <paragraph>
       ...
       <paragraph>
        On the other hand, when the teacher selects Arm1 with two rounds remaining, we see the following breakdown after the first round:
       </paragraph>
       <list>
        <list-item label="•">
         {a mathematical formula}x¯1&gt;x¯2: 64.0%
        </list-item>
        <list-item label="•">
         {a mathematical formula}x¯1&lt;x¯2, Inequality 1 true (worth teaching): 14.1%
        </list-item>
        <list-item label="•">
         {a mathematical formula}x¯1&lt;x¯2, Inequality 1 false (not worth teaching): 22.0%
        </list-item>
       </list>
       <paragraph>
       ...
       <list>
        ...
        <list-item label="2.">
         Teach iff {a mathematical formula}x¯1&lt;x¯2;
        </list-item>
        <list-item label="3.">
         Teach iff it would be optimal to teach if {a mathematical formula}r=1 and all other parameters were unchanged.
        </list-item>
       </list>
       <paragraph>
     ...
     <section label="3.5">
      ...
      <paragraph>
       Observe that adding additional arms that are only available to the teacher does not change anything. Only the best such arm (the one with the greatest expected value) should ever be considered by the teacher. We continue to call that arm {a mathematical formula}Arm⁎; the others can be ignored entirely.
      </paragraph>
      <paragraph>
      ...
      <section label="3.5.2">
       ...
       <paragraph>
        For the remainder of this section, we continue to refer to pulling {a mathematical formula}Arm⁎ as “not teaching,” but now must specify with which arm when referring to “teaching.”
       </paragraph>
      </section>
      <section label="3.5.3">
       Never teach with {a mathematical formula}Armi when {a mathematical formula}x¯i&gt;x¯j, {a mathematical formula}∀j≠i
       <paragraph>
        The proof of Theorem 3.2 from Section 3.2.3 generalizes directly to the following statement. The teacher should never take the action that the learner would take next on its own if the teacher were to pull {a mathematical formula}Arm⁎.
       </paragraph>
      </section>
      <section label="3.5.4">
       Do not teach when {a mathematical formula}n1=n2=⋯=nz=0
       <paragraph>
      ...
      <section label="3.5.6">
       Discrete distributions, {a mathematical formula}x¯1&lt;x¯i for some {a mathematical formula}i,r=1
       <paragraph>
        The results from Section 3.3.1 generalize directly. In particular, let {a mathematical formula}Armi be the learnerʼs arm with the highest sample average {a mathematical formula}x¯i. The teacher should consider teaching with any {a mathematical formula}Armj, {a mathematical formula}j&lt;z, {a mathematical formula}j≠i such that:
       </paragraph>
       <list>
        <list-item label="1.">
         {a mathematical formula}mj+1nj+1&gt;mini
        </list-item>
        <list-item label="2.">
         {a mathematical formula}p⁎−pj&lt;pj⁎(pj−pi)
        </list-item>
       </list>
       <paragraph>
        Those are the arms with higher expected value than {a mathematical formula}Arm⁎. From among those arms, it should select the {a mathematical formula}Armj with the highest expected value {a mathematical formula}EV=pj+pj2+(1−pj)pi.
       </paragraph>
      </section>
      <section label="3.5.7">
       <section-title>
        Discrete distributions, algorithm
       </section-title>
       <paragraph>
      ...
      <section label="3.5.8">
       <section-title>
        Discrete distributions, algorithm analysis and generalization
       </section-title>
       <paragraph>
        Both the memory and runtime bounds of the extended algorithm generalize naturally to {a mathematical formula}O(R2z+1). The extended algorithm generalizes to arbitrary success and failure payoffs exactly as in Section 3.3.4.
       </paragraph>
      </section>
      <section label="3.5.9">
       Normal distributions, {a mathematical formula}x¯1&lt;x¯i for some {a mathematical formula}i,r=1
       <paragraph>
        Exactly as the results from Section 3.3.1 generalize as described in Section 3.5.6, the results from Section 3.4.1 generalize as well. Specifically, let {a mathematical formula}Armi be the learnerʼs arm with the highest sample average {a mathematical formula}x¯i. The teacher should consider teaching with any {a mathematical formula}Armj, {a mathematical formula}j&lt;z, {a mathematical formula}j≠i such that the equivalent of Inequality 1 is satisfied:{a mathematical formula} Those are the arms with higher expected value than {a mathematical formula}Arm⁎. From among those arms, it should select the {a mathematical formula}Armj with the highest expected value {a mathematical formula}EV=μj+μiΦmuj,σj(x¯i(nj+1)−x¯jnj)+μj(1−Φmuj,σj(x¯i(nj+1)−x¯jnj)).
       </paragraph>
      </section>
      <section label="3.5.10">
       Normal distributions, {a mathematical formula}x¯1&lt;x¯i for some {a mathematical formula}i,r⩾2
       <paragraph>
     ...
     <section label="3.6">
      ...
      <list>
       <list-item label="•">
        {a mathematical formula}x¯1&gt;x¯2: do not teach
       </list-item>
       <list-item label="•">
        {a mathematical formula}n1=0 and/or {a mathematical formula}n2=0: do not teach
       </list-item>
      </list>
      <paragraph>
      ...
      <list>
       <list-item label="•">
        {a mathematical formula}x¯1&lt;x¯2, {a mathematical formula}r=1: closed form solution for optimal teacher action
       </list-item>
       <list-item label="•">
        {a mathematical formula}x¯1&lt;x¯2, {a mathematical formula}r⩾2: only numerical solutions
       </list-item>
      </list>
     </section>
    </section>
    <section label="4">
     ...
     <section label="4.1">
      <section-title>
       Repeated scenarios with simultaneous actions
      </section-title>
      <paragraph>
      ...
      <paragraph>
       Axelrod [36] conducted several well-known computer tournament experiments on repeated play of the Prisonerʼs Dilemma, pitting computer programs playing various strategies against one another. These strategies were evaluated on the basis of their overall success in the tournaments, as well as other factors (e.g., given a population that is playing some strategy, what is that populationʼs resistance to invasion by a competing strategy, assuming that winning strategies reproduce more successfully).
      </paragraph>
      <paragraph>
     ...
     <section label="4.2">
      ...
      <paragraph>
       Cooperative (coalitional) game theory is concerned with groups of self-interested agents that work together to increase their utility; much of the research in this area is concerned with how a groupʼs “profit” from joint activity can be divided among its members in a way that motivates them to remain in the group. The models used differ from those explored in this paper, but future work could profitably explore connections between these areas. Classic foundational work in this area includes [71], but there continues to be important research in recent years exploring new models of coalitional games (including from a computational perspective) [72].
      </paragraph>
      <paragraph>
       Finally, there are classic game theory solution concepts that appear to have relevance in future research on ad hoc teams. For example, Aumannʼs notion of “strong Nash equilibrium” [73], a Nash equilibrium where no coalition can cooperatively deviate in a way that benefits all members assuming that non-member actions are fixed (i.e., an equilibrium defined in terms of all possible coalitional deviations, rather than all possible unilateral deviations), could be applied to interactions among agents in ad hoc encounters. In addition, Aumannʼs later solution concept of “correlated equilibrium” [74], where agents do not want to deviate from a strategy recommended by (or associated with) the value of a public signal (assuming that others do not deviate), could also be applied to ad hoc cooperation.
      </paragraph>
     </section>
    </section>
   </content>
   <appendices>
    <section label="Appendix A">
     ...
     <list>
      <list-item label="(a)">
       These payoffs are the 1ʼs for each “correct” step in the path.
      </list-item>
      <list-item label="(b)">
       These large negative payoffs prevent taking a step when there is no corresponding edge in the graph.
      </list-item>
      <list-item label="(c)">
       These payoffs lure Agent B to do what Agent A did last.
      </list-item>
      <list-item label="(d)">
       These payoffs prevent Agent A from skipping to an action corresponding to a later time step.
      </list-item>
      <list-item label="(e)">
       These payoffs ensure that it is still attractive for Agent B to copy Agent Aʼs last move.
      </list-item>
      <list-item label="(f)">
       These payoffs are chosen carefully so that it Agent B doesnʼt move to a trap action after Agent A takes just a single action corresponding to a given node, but if it ever takes two such actions, then Agent B will be lured into the trap.
      </list-item>
      <list-item label="(g)">
       The payoffs for other trap actions are 0.
      </list-item>
      <list-item label="(h)">
       The payoff for selecting done only comes at {a mathematical formula}m⁎.
      </list-item>
      <list-item label="(i)">
       The payoff that induces Agent B to take its initialize action on the first step.
      </list-item>
      <list-item label="(j)">
       A payoff that prevents Agent A from taking an action corresponding to the initial node ever again (lest Agent B take the trap action).
      </list-item>
      <list-item label="(k)">
       This payoff prevents Agent B from taking the done action until all memory of Agent A taking the start action is past, i.e. after at least {a mathematical formula}n=mem steps.
      </list-item>
      <list-item label="(l)">
       These payoffs play no special role.
      </list-item>
      <list-item label="(m)">
       These payoffs are for taking the last step on the Hamiltonian path (reaching the destination node).
      </list-item>
      <list-item label="(n)">
       These payoffs ensure that if Agent A takes the done action before step n, then the cost is already higher than the target of {a mathematical formula}n⁎(n4−1).
      </list-item>
     </list>
     <paragraph>
      Therefore, if we could find the optimal sequence through any matrix in polynomial time, then we could use this ability to also solve the Hamiltonian path problem. That is, finding {a mathematical formula}S⁎ when {a mathematical formula}mem&gt;1 is NP-hard. □
     </paragraph>
    </section>
