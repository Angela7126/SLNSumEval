  ...
  <root>
   ...
   <content>
    ...
    <section label="2">
     ...
     <paragraph label="Definition 1">
      Before defining the SPR process, we present some background. There are multiple approaches in the literature to defining PR problem [42], [45]. In this paper we assume the existence of a plan library that defines the possible behaviors of the observed agent [28]. Plan libraryA plan library is a tuple {a mathematical formula}L=〈B,C,G,R〉, where B is a set of basic actions, C is a set of complex actions, {a mathematical formula}G⊆C is a list of goals and R is a set of refinement methods of the form {a mathematical formula}c→(τ,O), where (1) {a mathematical formula}c∈C; (2) {a mathematical formula}τ∈(B∪C)⁎; (3) and O is a partial order over τ representing ordering constraints over the actions in τ.
     </paragraph>
     <paragraph>
      The refinement methods represent how complex actions can be decomposed into (basic or complex) actions.
     </paragraph>
     <paragraph>
      A plan library can also be viewed as equivalent to a context-free grammar extended with partial-ordering constraints or to a Hierarchical Task Network [19]. There is a natural analogy between PR and parsing as well [36], [25]. The non-terminals of the grammars are complex actions, the start symbols are mapped to goals, the terminals are the basic actions (observations).
     </paragraph>
     <paragraph label="Definition 2">
      A plan in a plan library L is a labeled tree {a mathematical formula}p=(V,E,L), where (1) V and E are the nodes and edges of the tree, respectively, (2) {a mathematical formula}L is a labeling function {a mathematical formula}L:V→B∪C mapping every node in the tree to either a basic or a complex action in L, and (3) the children of a node labeled by a complex action are a decomposition of this complex action into constituent actions, according to one of the refinement methods.
     </paragraph>
     <paragraph>
      The set of all leaves of a plan p is denoted by {a mathematical formula}leaves(p). Note that sibling nodes in a plan can have implicit ordering constraints between them, according to the refinement method used to create them.
     </paragraph>
     <paragraph label="Definition 3">
      A plan is said to be complete iff all its leaf nodes are labeled basic actions, i.e., {a mathematical formula}∀v∈leaves(p),L(v)∈B.
     </paragraph>
     <paragraph>
      An observation sequence is an ordered set of basic actions that represents actions carried out by the observed agent. A plan pexplains an observation sequence O iff every observation is mapped to a leaf in the tree.
     </paragraph>
     <paragraph>
      Formally, there exists an injective function {a mathematical formula}f:O→leaves(p)∩B such that {a mathematical formula}f(o)=v with {a mathematical formula}v∈leaves(p).
     </paragraph>
     <paragraph>
      The observed agent chooses a subset of complex actions as intended goals and then carries out a separate plan for completing each of these goals. We assume that the actions carried out by the agent are deterministic and fully observable.
     </paragraph>
     <paragraph label="Definition 4">
      A hypothesis that explains an observation sequence is a set of plans such that each plan explains a mutually exclusive subset of the observation sequence and taken together the plans explain all of the observations.
     </paragraph>
     <paragraph>
      We will illustrate these concepts using an open-ended educational software package for chemistry called VirtualLabs, which also comprises part of our empirical analysis [58]. VirtualLabs allows students to design and carry out their own experiments for investigating chemical processes by simulating the conditions and effects that characterize scientific inquiry in the physical laboratory. Such software is open-ended and flexible and is generally used in classes too large for teachers to monitor all students and provide assistance when needed. Thus, there is a need to develop recognition tools to support teachers' understanding of students' activities using the software.
     </paragraph>
     <paragraph>
      We use the following problem (called Oracle) as a running example: Given four substances {a mathematical formula}A;B;C, and D that react in a way that is unknown, design and perform virtual lab experiments to determine which of these substances react, including their stoichiometric coefficients.
     </paragraph>
     <paragraph>
      There are two classes of strategies used by students to solve the above problem in VirtualLabs [22]. The most common strategy, called pairwise, is to mix pairs of solutions (A with B, A with C, etc.) in order to determine which solutions react with one another. In some cases, students stop mixing pairs when a reaction occurs because this is sufficient to identify the reactants. In the four-way solution strategy, all substances are mixed in a single flask, which is sufficient to identify which solution pair were the reactants and which did not react, since the non-reactants are still observable after the reaction. Fig. 1 show a plan for the four-way strategy (left) and pairwise strategy (right).{sup:1}
     </paragraph>
     <paragraph>
      Now suppose that the student is observed to mix solutions A and B together in a single flask. Without receiving additional information, both the pairwise and four-way strategies are hypotheses that are consistent with the observations, and both include an incomplete plan explaining the student's actions. Incomplete plans include nodes labeled with complex level actions that have not been decomposed using a refinement method. These open frontier nodes represent activities that the agent will carry out in future and have yet to be refined. This ambiguity is exemplified in Fig. 1, showing one hypothesis for the four-way solution strategy (left) and one for the pairwise solution strategy (right). Each of these hypotheses contain a single incomplete plan. The nodes representing the observations A and B are underlined. The dashed nodes denote open frontier nodes.
     </paragraph>
     <paragraph>
      We can now define the PR problem using the formalism above.
     </paragraph>
     <paragraph label="Definition 5">
      Plan recognition (PR)A PR problem is defined by the tuple {a mathematical formula}〈L,O〉 where L is a plan library and O is an observation sequence. A PR algorithm accepts a PR problem and outputs a set of hypotheses H. the observation sequence.
     </paragraph>
     <section label="2.1">
      <section-title>
       Plan refinement
     ...
     <section label="2.2">
      ...
      <paragraph>
       This paper assumes a sound and complete PR algorithm that outputs a set of hypotheses that explain the given observation sequence (see Definition 5). We mention representative examples of the state-of-the-art. The PHATT algorithm [24], which is used in the empirical section of this paper generates hypothesis in a top-down manner. SLIM [38] and SBR [4] are similar to PHATT in the format of outputted hypothesis set, and vary only in the approach they are using to construct this set. SBR represents all possible plans in the library as a single tree. Unlike PHATT, it does not enumerate hypotheses for every observations, but construct hypotheses by request.
      </paragraph>
      <paragraph>
       SLIM combines a top-down and bottom-up recognition processes which allow it to commit only to the minimum necessary actions in real-time, but still provide complete hypotheses post factum.
      </paragraph>
      <paragraph>
       ELEXIR [23] differs from these algorithms, as it uses as input a Combinatory Categorial Grammar (CCG), but its underlying formulation is based on similar components. Other PR algorithms do not use plan libraries as their underlying model, such as [44], [52] and [14] and are beyond the scope of this work.
      </paragraph>
      <paragraph>
       To exemplify the mechanics of PR algorithms, we provide here a brief description of PHATT. At the first observation {a mathematical formula}σ0, PHATT constructs hypotheses for explaining this observation. Each hypothesis includes a single plan tree such that 1) the root node is labeled with a goal {a mathematical formula}g∈G; 2) there exists a leaf node that is labeled with {a mathematical formula}σ0; 3) the path from the root to the leaf includes actions that are derived from the refinement methods of the plan library; 4) all other leaves in the plan tree are open frontier nodes. while the plan is referred to as a leftmost tree. When introducing a new observation {a mathematical formula}σ1 into an existing hypothesis, PHATT considers two possibilities:
      </paragraph>
      <list>
       <list-item label="1.">
        Adding {a mathematical formula}σ1 as a new plan in the hypothesis, which requires to construct a leftmost tree that derives {a mathematical formula}σ1 with a root in G, the set of goals.
       </list-item>
       <list-item label="2.">
        Updating an existing plan in the hypothesis with {a mathematical formula}σ1. This option requires to construct another leftmost tree deriving {a mathematical formula}σ1 with a root that matches one of the open frontier nodes of an existing plan.
       </list-item>
      </list>
      <paragraph>
       This process is iterated over all observations, maintaining a set of hypotheses H that explain the sequence of observations seen so far. To this end, PHATT modifies and extends the hypotheses in H, and, if necessary, adds and removes hypotheses, so that H contains hypotheses that explain the new updated sequence of observations {a mathematical formula}σ1…,σn.
      </paragraph>
     </section>
    </section>
    <section label="3">
    ...
    <section label="4">
     ...
     <paragraph>
      We evaluated the query policies described in the previous sections on two separate domains from the PR literature. The first domain involves students' interactions with the VirtualLabs system when solving two different types of problems: the Oracle problem described in Section 2, and a problem called Unknown Acid which required students to determine the concentration level of an unknown acid solution by performing a chemical titration process.
     </paragraph>
     <paragraph>
     ...
     <paragraph>
      In each of the logs, we used domain experts (chemistry teachers and researchers) to tag the correct hypothesis. We sampled 35 logs of students' interactions in VirtualLabs to solve the above problems. The plan library was the one used by Amir and Gal [3] which included parameterized actions and ordering constraints.
     </paragraph>
     <paragraph>
      The second domain is a synthetic domain used by Kabanza et al. [28]. This domain generates plan libraries that can be configured using several features which are known to affect the hypothesis set size [24]:
     </paragraph>
     <list>
      <list-item label="•">
       Or Branching Factor: This is the number of different ways a complex action can be decomposed into a sequence of subactions.
      </list-item>
      <list-item label="•">
       And Branching Factor: This is the number of constituents which decompose a complex action.
      </list-item>
      <list-item label="•">
       Ordering Constraints: This is an indication of how many ordering constraints exist between the constituents of a refinement method. There are various possible configurations, we focus in the paper on two which are easy to compare: Unordered, meaning that the constituents can be observed in any order; and partially ordered, meaning that some of the constituents must come in a particular order, but not all of them. The latter configuration is commonly used in PR literature to represent real world domains.
      </list-item>
      <list-item label="•">
       Number of Goals: Representing the number of different goals an agent might pursue at the same time.
      </list-item>
      <list-item label="•">
       Depth: Representing the depth of the plan library. Specifically, in our configurations it represents how many refinement methods are used, in the largest tree, to construct a path from a leaf representing an observation to a root representing a goal.
      </list-item>
      <list-item label="•">
       Alphabet Size: Representing the number of basic actions possible in the grammar.
      </list-item>
     </list>
     <paragraph>
      We used the same configuration as Kabanza et al. [28] which consisted of 100 different plan libraries with an alphabet of 100 actions and five identified goal. The refinement method were represented as AND-OR trees with an AND branching factor of 3, and an OR branching factor of 2.
     </paragraph>
     <paragraph>
      We compare the performance of the query policies introduced in Section 3.2 (using a random tie-breaking rule). For each policy, we evaluate the performance using three metrics:
     </paragraph>
     <paragraph>
      The runtime of each query policy, the number of queries required until convergence, and the decrease in the size of the hypothesis set. In both domains, the correct hypothesis is known in advance (whether it was determined by the domain expert, as in the VL domain, or by simulation, as in the synthetic domain). Therefore we can generate the query of the agent automatically.
     </paragraph>
     <section label="4.1">
      <section-title>
       Evaluation of query policies
      </section-title>
      <paragraph>
       In this section we show the results of testing a selected set of query policies on the two domains. Specifically, we compared between the following policies: the Most Probable Plan (MPP), the Most Probable Hypothesis (MPH) and the Minimal Entropy (Entropy) approaches, as well as a baseline approach that picked a plan to query at random (Random).
      </paragraph>
      <paragraph>
       In all cases, we ran the PHATT PR algorithm once on a given set of observations, and then proceeded to run the SPR process described in Algorithm 1. We first show the number of hypotheses that were outputted by the PHATT algorithm without query interventions. As can be seen in Table 1, the number of hypotheses in the synthetic domain grows linearly in the number of observations, but for the real-world domain, the number of hypotheses grows exponentially, reaching over 10,000 hypotheses after just 7 actions. Fig. 4 shows the average percentage of hypotheses remaining from the initial hypothesis set ({a mathematical formula}H0) as a function of the number of queries performed. Before the first query, all algorithms start with 100% of the hypotheses in {a mathematical formula}H0, and this number decreases as more queries are performed. For both domains we used the PR output after 7 observations.
      </paragraph>
      <paragraph>
       As seen in Fig. 4, all MPH, MPP and Entropy query policies were able to reduce the number of hypotheses significantly compared to the baseline random policy. In both domains, the Entropy query policy performed better than all other policies. The SPR process was able to converge to a single hypothesis (100% reduction in hypothesis size compared to {a mathematical formula}H0) after about 20 queries in the VirtualLabs domain, while converging to a minimal hypothesis set of about 30% of the number of hypotheses in {a mathematical formula}H0 of the synthetic domain. We attribute this to inherent ambiguity in this domain that cannot be resolved by making further queries. In both domains, the advantage of the Entropy over all other approaches after just five queries strongly significant ({a mathematical formula}p≤0.01). This is important since queries are costly and the number of queries that can be asked in practice may be small.
      </paragraph>
      <paragraph>
       Table 2 shows the average number of queries needed until reaching the minimal set of hypotheses, for each query policy. Notice that the number of hypotheses increase with each new observation. This is due to the fact that for each hypothesis, a new observation can initiate a new plan or complement an existing plan (or both), so the size of the hypothesis set will be at least the size of the original one [41]. This table shows that the Entropy query policy made significantly fewer queries than the other approaches.
      </paragraph>
      <paragraph>
       The runtime was measured in seconds, on a commodity i-7 server, with the initial hypothesis set for the first 7 observations. The table shows that the Entropy policy exhibited higher runtime than the other approaches but not by a significant margin.
      </paragraph>
     </section>
     <section label="4.2">
      <section-title>
       The effect of domain characteristics
      </section-title>
      <paragraph>
       We studied the effect of different domain characteristics on the performance of the SPR process in the synthetic domain. To this end, we varied three factors of the environment: the or-branching, and-branching factors and ordering constraints. For this purpose we generated four domains that differ from the synthetic domain used in the previous section in a single feature: increasing the or-branching factor from two to four; increasing the and-branching factor from three to six; removing the ordering constraints.
      </paragraph>
      <paragraph>
       For each of these synthetic domains, there was a different number of hypotheses generated after five observations: The baseline domain used in the previous section produced 428 hypotheses on average; modifying the or-branching factor produced 5,460 hypotheses; modifying the and-branching 434 hypotheses; removing the ordering constraints produced 942 hypotheses.
      </paragraph>
      <paragraph>
       Fig. 5 shows the decrease in hypothesis set size after five observations for each policy in each of the domains. Interestingly, the best decrease in the hypothesis set size was obtained for the domain with the modified or-branching factor, which was also associated with the highest ambiguity. Thus the ability of the SPR process to reach convergence cannot be attributed solely to the initial number of hypotheses in the domain. We attribute the improved performance in the or-branching domain to the fact that each hypothesis provides a mutually exclusive explanation, so it is easier to separate them.
      </paragraph>
     </section>
     <section label="4.3">
      <section-title>
      ...
      <paragraph>
       In the results presented thus far, the querying process was performed after a fixed number of observations to establish a uniform baseline. In this section we interleave the execution of the PR algorithm with the SPR process between observation processing. Specifically, the recognition process is performed on the updated hypothesis set following the query response, and outputs a new set of possible hypotheses, which is subsequently used to perform the next query. This interleaved process is called online SPR.
      </paragraph>
      <paragraph>
       Fig. 6 shows how online SPR is used with PR. In the original (offline) case, the SPR module comes after the PR module is done (Fig. 3). In the online case, on the other hand, the plan recognizer only receives a subset of the observations, {a mathematical formula}o1,…,ok and the outputted hypotheses explain only these observations. Then, the SPR process is applied to discard unnecessary hypotheses. This reduced set is given back to the plan recognizer together with more observations and this loop continues until all observations have been processed. We note that this variant of the SPR process is only possible if the plan recognizer can handle online recognition of a partial sequence of observations. This is a common requirement and many algorithms fulfill it [23], [24], [28], [38], [41].
      </paragraph>
      <paragraph>
       When online SPR is integrated with PR, there are some parameters that can vary, such as the frequency of SPR between observations and the number of queries asked per SPR execution. We run several types of experiments on the synthetic domain in order to explore these different combinations of online SPR with PR.
      </paragraph>
      <paragraph>
       Interval probing We performed a query and ran the PR algorithm (PHATT) at fixed intervals of observations. The results are shown in Fig. 7. We measured the number of hypotheses (in log scale) remaining after processing all of the observations, as well as the runtime (in seconds). The results show that when the query interval is smaller, there is a greater reduction in the number of hypotheses. In particular, we achieve a significant reduction in the hypothesis space when decreasing the query interval from two observations to one (Fig. 7, left), which corresponds to querying after each observation. On the one hand, decreasing the interval size increases the amount of queries and associated updates to the hypothesis space. On the other hand, this also reduces the number of hypotheses we update at each step, which reduces the run time necessary to perform these actions (Fig. 7, right).
      </paragraph>
      <paragraph>
       Interestingly, when the probing is performed more frequently, this reduces the number of hypotheses. This shows that the benefit from the SPR process can overcome the cost of having to perform queries more frequently. For example, there is a decrease in the number of hypotheses between the 8th and 9th observation when probing after each observation, but not when the probing is done less frequently.
      </paragraph>
      <paragraph>
       One time query Assuming that querying is associated with a cost (e.g., interrupting the user), we investigate when is the best time to perform the probing process, whether earlier or later in the recognition process. We ran the same experiment, but now we initiated the probing process just once, after a predefined number of observations (set to 2, 4 and 6). Fig. 8 shows the runtime and number of hypotheses remaining after the query process. It shows that the earlier we use the probing process, the fewer hypotheses we have to reason about in the future. This means that early disambiguation can be advantageous the PR process. Similarly, the runtime is smaller when there are less hypotheses to consider.
      </paragraph>
     </section>
    </section>
    <section label="5">
     <section-title>
      Subplans and inferred query pruning
     </section-title>
     <paragraph>
      In Section 3.2 we proposed several query policies whose aim is to choose intelligently which plan to query that will result in the smallest hypothesis set. In this section we study the use of subplans, rather than full plans, in the query process.
     </paragraph>
     <paragraph label="Definition 12">
      For a plan p, we define the set of subplans of p to be the set of plans that can be refined to p: {a mathematical formula}Subs(p)={p′|p′∼rp}. We extend this definition to a set of plans P to be {a mathematical formula}Subs(P)=⋃p∈PSubs(p).
     </paragraph>
     <paragraph>
      Note that the root of any subplan of p is the same as the original plan p.
     </paragraph>
     <paragraph>
      Subplans provide a different level of granularity than complete plans. The intuition is that we may be able to prune the hypothesis set more by querying about less specific, rather than more specific plans. For example, in the VirtualLabs domain, consider the sub-plan corresponding to a pairwise mix, but without committing to specify the specific compounds. If the answer to querying the sub-plan is False, we can prune all hypotheses including pairwise mixes. In contrast, if we choose a complete plan specifying exactly which compounds are mixed, we will only be able to prune hypotheses that include these compounds.
     </paragraph>
     <paragraph>
      Allowing the SPR process to perform queries on subplans as well as full plans may result in finding the correct plan with fewer queries.
     </paragraph>
     <paragraph>
      To demonstrate that allowing subplan queries can be helpful, consider the example in Fig. 9. The hypothesis set has 6 hypotheses {a mathematical formula}H4,H5,…H9, each consisting of a plan {a mathematical formula}p4,p5,…,p9, respectively. All these hypotheses are equally likely and none of the plans are refinements of each other.
     </paragraph>
     <paragraph>
      Table 4 shows the possible outcomes of querying about each of the plans in terms of the set of outputted hypotheses. If only one of these six hypotheses is the correct one, it can be beneficial to query about the subplans {a mathematical formula}p2 and {a mathematical formula}p3 instead of the refined ones. If we ask about {a mathematical formula}p4,…,p9, in the worst case we will need to perform five queries to eliminate all the wrong hypotheses. However, if we first ask about either of the subplans {a mathematical formula}p2 or {a mathematical formula}p3, in the worst case we will need to perform three queries to eliminate all the wrong plans.
     </paragraph>
     <paragraph>
      Note that in general, if one had the optimal query policy, then querying about subplans as well as complete plans can only improve the SPR process by performing less queries in total. Our query strategies, however, are not optimal, but we show that querying about subplans can still result in less queries.
     </paragraph>
     <section label="5.1">
      <section-title>
       Query policies
      ...
      <paragraph>
       Let {a mathematical formula}Subs(P) be the set of all subplans and full plans that are refinements of a plan from the set of plans P. To implement the query process using subplans, we only need to change the query function π in Algorithm 1 to choose any subplan in {a mathematical formula}Subs(P). We now show how we adapt the query policies defined in Section 3.2.
      </paragraph>
      <paragraph>
       Most Probable Hypothesis (MPH). Choose a plan or subplan from the hypothesis h that is associated with the highest probability and was not yet queried about, i.e., choose a plan or subplan t such that {a mathematical formula}t∈Subs(Ph) where {a mathematical formula}Ph={p|p∈h=arg⁡maxh∈HiP(h)}.
      </paragraph>
      <paragraph>
       Most Probable Plan (MPP). Choose the plan or subplan that is associated with the highest probability across all hypotheses: {a mathematical formula}arg⁡maxp∈Subs(P)Pr(p), where P is the union set of all plans in all of the hypotheses H.
      </paragraph>
      <paragraph>
       Maximal Information Gain (or Minimal Entropy). The computation of the minimal Entropy is the same as before, only we need to consider all possible subplans:{a mathematical formula} where the standard entropy computation over the resulting hypothesis set remains the same.
      </paragraph>
      <paragraph>
       We will now show that querying about subplans does not hinder soundness and completeness of the SPR process.
      </paragraph>
      <paragraph label="Proposition 1">
       Let {a mathematical formula}H′⊆H be the set of hypotheses remaining after convergence of the SPR process. Any hypothesis that was discarded ({a mathematical formula}h∈H∖H′) by using complete plans will also be discarded by querying with subplans (and vice versa).
      </paragraph>
      <paragraph label="Proof">
       Suppose that a hypothesis is discarded by SPR using only full plans, it means that there is a query p that its update causes the hypothesis to be discarded. Since {a mathematical formula}P⊆Subs(P) and the SPR process does not end while there are still possible queries, when using SPR with subplans, at some point p must be chosen out of the set of all subplans and then the hypothesis will be discarded as well.In addition, all discarded hypotheses using subplans will be discarded when using only full plans: Assume that there is some hypothesis H which is not in the final set of hypotheses using subplans. This means that there is a query about some subplan {a mathematical formula}p′ that can eliminate H. However, since all subplans are constructed from full plans, there is some full plan p such that {a mathematical formula}p′∼rp.If {a mathematical formula}QA(p′)=True and H is discarded, then {a mathematical formula}∀ph∈H¬(ph∼mp′){a mathematical formula}⇒∀ph∈H′¬(ph∼mp){a mathematical formula}⇒∃pp′∼rp such that {a mathematical formula}H′ will be discarded when asking about the full plan p.If {a mathematical formula}QA(p′)=False then it must hold that {a mathematical formula}QA(p)=False, meaning that H is discarded since {a mathematical formula}∃ph∈Hph∼rp′{a mathematical formula}⇒∃ph∈Hph∼rp⇒H will be discarded when asking about the full plan p. □
      </paragraph>
      <paragraph>
       This means that in practice, the number of hypotheses that are left after convergence is equal when using both query approaches. The difference is in the number of queries that is required to reach convergence. The next section empirically explores that using subplans requires less queries than full plans.
      </paragraph>
      <paragraph>
       Also, note that by adding subplans we also introduce more cases in which plans are equally preferable when using the above query policies. An extreme case is the MPH query policy, in which all the subplans of a given plan are equally preferable according to MPH, thus raising the issue of how to break ties in such cases.
      </paragraph>
     </section>
     <section label="5.2">
      <section-title>
      ...
      <paragraph>
       To conclude this section, it seems that using subplans instead of complete trees has a real benefit in domains of high ambiguity. Moreover, updating the set of possible queries at each point proved to be an important and useful addition to the original algorithm.
      </paragraph>
     </section>
    </section>
    <section label="6">
   ...
   <appendices>
    <section label="Appendix A">
     <section-title>
      Pseudo-code for refinement and Match methods
     </section-title>
     <paragraph>
      We specify below the pseudo-code for the refine (Algorithm 2) and match (Algorithm 3) conditions. We use the following methods:
     </paragraph>
     <list>
      <list-item label="•">
       {a mathematical formula}label(p) – takes a plan p and returns the letter mapped to its root node.
      </list-item>
      <list-item label="•">
       {a mathematical formula}childNum(p) – takes a plan p and returns the numbers of child nodes under its root node.
      </list-item>
      <list-item label="•">
       {a mathematical formula}childAt(p,i) – takes a plan p and returns the its ith child node of its root node.
      </list-item>
      <list-item label="•">
       {a mathematical formula}isExpanded(p) – takes a plan p and return True iff its root node has been expanded using a refinement method.
      </list-item>
     </list>
    </section>
    <section label="Appendix B">
     <section-title>
      Computing probabilities for plans
     </section-title>
     <paragraph>
      In this appendix we discuss how to approximate the probability that a query about a plan p will return true, given a probability distribution of a set of hypotheses. This probability, denoted earlier as {a mathematical formula}P⁎(p), is formally defined as follows as the probability that {a mathematical formula}h⁎ contains a plan that is a refinement of p.{a mathematical formula}
     </paragraph>
     <paragraph>
      The challenge in {a mathematical formula}P⁎(p) is that PR algorithms output probabilities over hypotheses, and not over plans. However, as we show below, the probabilities over hypotheses outputted by PR algorithms can be used to approximate {a mathematical formula}P⁎(p).
     </paragraph>
     <paragraph>
