 ...
 <body>
  <root>
   ...
   <content>
    <section label="1">
     ...
     <section label="1.1">
      ...
      <section label="1.1.2">
       ...
       <paragraph>
        Botea et al. [10] proposed planning on an abstraction of the search space. Applied to Sokoban they obtain a non-admissible solver which is able to solve ten instances from the standard set in less than three minutes and exploring less than one million nodes. Demaret et al. [11] describe a non-admissible solver that uses a hierarchical planning strategy along with deadlocks learning to solve Sokoban. Using this approach, they were able to solve 54 instances from the standard set. In this case, the stopping criterion was eight hours of running time per instance.
       </paragraph>
       <paragraph>
        The state-of-the-art non-admissible solvers have been developed by the Sokoban community. Among the ones with the best results, JSoko is able to solve 71, YASC 79, and Takaken 86 instances [12], [13], [14], [15]. Since these results have not been published formally, it is not always clear which techniques have been used to achieve them.
       </paragraph>
      </section>
      <section label="1.1.3">
       <section-title>
    ...
    <section label="2">
     <section-title>
      A heuristic based on pattern databases
     </section-title>
     <paragraph>
    ...
    <section label="3">
     ...
     <paragraph>
      Compared to other problems, Sokoban tends to generate a larger number of ties, since there are multiple combinations of stones on goal squares and the rest of the instance which have the same f-value. For example, in Fig. 7 the initial node (Fig. 7a) has descendants (Figs. 7c and 7d) with {a mathematical formula}f=93. Using the proposed heuristic function and breaking ties by generation order explores more than 5 million nodes to solve the instance, since several possible configurations of the stones on goal squares are explored before reaching the goal node. Some of these configurations are deadlocks like configuration in Fig. 7c.
     </paragraph>
     <paragraph>
     ...
     <paragraph>
      The order is defined algorithmically as follows. Starting from a placement of all stones on the goal squares, we repeatedly remove a set of stones, until all stones have been removed. In each iteration, we process all remaining stones in an arbitrary order. For each stone we test if a reverse move can be applied to it. If a test succeeds we assign a priority of {a mathematical formula}2i to the corresponding goal square, where i is current total number of successful tests. Then we remove the set of all stones for which the test succeeded in the current iteration. On termination a priority value has been assigned to each goal square. The priority values represent a guess of the order in which the goal squares are filled in an optimal solution. The fill order of a node is defined as the sum of the priority values of the goal squares which are occupied by a stone. Thus, the fill order gives preference to nodes that place the stones at the goal squares of highest priority value first. The idea of the fill order is similar to goal macros [9] since both techniques try to use information from the state space related to the placement of goal squares to improve the effectiveness of the solver. The fill order, however, is admissible, since it is only used to break ties.
     </paragraph>
     <paragraph>
      Fig. 7b shows the priorities of the goal squares of the example above. A state with a stone at B5 has priority 256 and a state with eight stones in the other goal squares has priority 255. In this way we get an absolute order for filling the goal squares. Using this rule of tie breaking we solve the example instance exploring less than 5000 nodes.
     </paragraph>
    </section>
    <section label="4">
     <section-title>
     ...
     <section label="4.4">
      ...
      <paragraph>
       We now turn to the analysis of the solvers when breaking ties by rule INFO. Table 7 shows the number of explored nodes and the solving time for all instances that could be solved by at least one of the solvers ({a mathematical formula}RS⁎, EMM, MPDB, IPDB). MPDB and IPDB were tested with abstractions of two and four stones. The test was limited to abstractions of at most four stones, since for some instances a PDB with five stones could not be built in an hour.
      </paragraph>
      <paragraph>
