  ...
  <root>
   ...
   <content>
    ...
    <section label="2">
     ...
     <paragraph>
      There are numerous definitions of ontology over years. But the most frequently referenced one was given by Gruber in 1993 which defined the ontology as an explicit specification of a conceptualization. For the convenience of understanding the work in this paper, the ontology is defined as following:
     </paragraph>
     <paragraph label="Definition 1">
      (See [25].) An ontology is a 9-tuple {a mathematical formula}O=(C,P,I,A,≤C,≤P,ϕCP,ϕCI,ϕPI), where:
     </paragraph>
     <list>
      <list-item label="•">
      ...
      <list-item label="•">
       {a mathematical formula}≤C is a partial order on C, called class hierarchy or taxonomy,
      </list-item>
      <list-item label="•">
       {a mathematical formula}≤P is a partial order on P, called property hierarchy,
      </list-item>
      <list-item label="•">
      ...
      <list-item label="•">
       {a mathematical formula}ϕCI:C→P(I) is a function which associates a concept {a mathematical formula}c∈C with a subset of I representing the instances of the concept c,
      </list-item>
      <list-item label="•">
       {a mathematical formula}ϕPI:P→P(I2) is a function which associates a property {a mathematical formula}p∈P with a subset of Cartesian product {a mathematical formula}I×I representing the pair of instances related through the property p.
      </list-item>
     </list>
     <paragraph>
      At present, ontologies are viewed as a practical way to conceptualize information that is expressed in electronic format, and are used in many applications from different areas. However, certain systems that encompass a large number of components associated with different domains would generally require the use of different ontologies. In such cases, using ontologies would not reduce heterogeneity but rather would recast the heterogeneity problem into a different (and higher) framework wherein the problem becomes one of ontology alignment, thereby allowing the more efficient exchange of information and knowledge derived from different (heterogeneous) data bases, knowledge bases, and the knowledge contained in the ontologies themselves.
     </paragraph>
     <paragraph>
      The tools, designed to automatically identify the correspondences that may exist between entities of different ontologies, are called ontology alignment systems. In particular, an ontology alignment system executes an ontology alignment process which can be defined as follows:
     </paragraph>
     <paragraph label="Definition 2">
      (See [26].) An alignment between two ontologies is a set of mapping elements. A mapping element is a 5-tuple {a mathematical formula}(id,e,e′,n,R), where:
     </paragraph>
     <list>
      <list-item label="•">
    ...
    <section label="4">
     ...
     <section label="4.1">
      ...
      <section label="4.1.2">
       ...
       <paragraph>
        Linguistic based matchers compute a similarity between entity names/labels, based on synonymy, hypernymy, and other linguistic relations [20]. To compute the linguistic similarity or inversely the linguistic distance, a lexicon and a thesauri are needed, and the most popular one being used to identify the relationships between entities is WordNet [11] which is an electronic lexical database where various senses of words are put together into sets of synonyms. Given two words {a mathematical formula}w1 and {a mathematical formula}w2, {a mathematical formula}LinguisticDistance(w1,w2) equals:
       </paragraph>
       <list>
        <list-item label="•">
         1, if words {a mathematical formula}w1 and {a mathematical formula}w2 are synonymous,
        </list-item>
        <list-item label="•">
         0.5, if word {a mathematical formula}w1 is the hypernym of word {a mathematical formula}w2 or vice versa,
        </list-item>
        <list-item label="•">
         0, otherwise.
        </list-item>
     ...
     <section label="4.2">
      ...
      <section label="4.2.2">
       ...
       <paragraph>
        Analogously, the combined match ratio can be defined as follows:{a mathematical formula} High match ratio indicates entities are mapped to many other entities. Match ratio that is close to 1.0 indicates the high precision. Therefore, for the convenience, we define Frequency below as the rough approximation for precision.{a mathematical formula}
       </paragraph>
       <paragraph>
        In our work, we use MatchFmeasure, which is defined as follows, to approximate the traditional f-measure.{a mathematical formula} where α is the relative weight of MatchCoverage and Frequency and is in the range {a mathematical formula}[0,1].
       </paragraph>
      </section>
      <section label="4.2.3">
       <section-title>
    ...
    <section label="5">
     ...
     <section label="5.6">
      ...
      <paragraph>
       In general, the local search strategies perform iterative search for optimal solution in the neighborhood of a candidate. In order to tradeoff between the local search and the global search, the local search process in our work is designed according to the following rules:
      </paragraph>
      <list>
       <list-item label="•">
       ...
       <list-item label="•">
        the local search method is the hill climbing algorithm.
       </list-item>
      </list>
      <paragraph>
       In particular, the hill climbing algorithm is a local search iterative method. During iterations, the algorithm attempts to find a better individual by randomly mutating the current one. If a mutation improves the current individual, then the new individual replaces the current one. The search is repeated until no further improvement can be found or after a maximum number of iterations.
      </paragraph>
      <paragraph>
       Next, we will perform a comparison by experiments among the baseline alignments, which set the weights of each similarity measures as {a mathematical formula}1/num (num is the number of similarity measures) and the threshold as 0.80, and the alignments obtained by MA using MatchFmeasure only and those by MA using both MatchFmeasure and UIR.
      </paragraph>
     </section>
    </section>
    <section label="6">
     ...
     <section label="6.1">
      ...
      <list>
       ...
       <list-item label="•">
        The population size, local Search intensity and fitness evaluation number for termination depend on the scale of the problem, the suggested ranges for them are [30, 300], [10, 50] and [200, 1000], respectively. Since the problem scale in our work is not large (only four parameters need to be determined), we set the size of population, maximum number of iterations and the fitness evaluation number for termination as 30, 20 and 250 respectively.
       </list-item>
      </list>
      <paragraph>
       To summary, in our work, MA uses the following parameters which represent a trade-off setting obtained in empirical way to achieve the highest average alignment quality on all test cases of exploited dataset. Through the configuration of parameters chosen in this way, it has been justified by the experiments in this paper that parameters chosen are robust for all the heterogeneous problems presented in the benchmarks, and it is hopeful to be robust for the common heterogeneous situations in the real world.
      </paragraph>
      <list>
       <list-item label="•">
        Search space for each parameter is the continuous interval {a mathematical formula}[0,1],
       </list-item>
       <list-item label="•">
        {a mathematical formula}Population size=30 individuals,
       </list-item>
       <list-item label="•">
        {a mathematical formula}Crossover probability=0.8,
       </list-item>
       <list-item label="•">
        {a mathematical formula}Mutation probability=0.02,
       </list-item>
       <list-item label="•">
        {a mathematical formula}Local Search intensity=20 iterations,
       </list-item>
       <list-item label="•">
        {a mathematical formula}Local Search Mutation probability=0.5,
       </list-item>
       <list-item label="•">
        {a mathematical formula}Termination condition=250 fitness evaluations.
       </list-item>
      </list>
      <paragraph>
     ...
     <section label="6.2">
      ...
      <paragraph>
       According to this result, a post-hoc statistical analysis is needed to conduct pairwise comparisons in order to detect concrete differences among compared algorithms. Holm's procedure is a multiple comparison procedure that works by setting a control algorithm and comparing it with the remaining ones. Normally, the algorithm which obtains the lowest value of ranking in the Friedman's test is chosen as control algorithm. In our experimentation, as shown in Table 5, our proposal is characterized by the lowest value of ranking.
      </paragraph>
      <paragraph>
       Holm's test works on a family of hypotheses where each one is related to a comparison between the control algorithm and one of the remaining algorithms. In details, the test statistic for comparing the ith and jth algorithms named z value is used for finding the corresponding probability from the table of the normal distribution (the so-called p-value), which is then compared with an appropriate level of significance α[19]. In our experimentation {a mathematical formula}α=0.05 and the results of Holm's test are shown in Table 6. By analyzing the data in Table 6, it is possible to state that our proposal statistically outperforms GOAL and ECOMatch at 5% significance level.
      </paragraph>
      <paragraph>
       Finally, we will compare our proposal with the state of the art ontology alignment systems in terms of F-measure. The compared ontology alignment systems are the participants in OAEI 2013, whose f-measure values of the alignments produced for the benchmark track are from [29]. As can be seen from Table 7, our approach ranks 3rd in all eleven state of the art ontology alignment systems. With respect to YAM++ and MAPSSS, although they obtain better results than our approach in terms of f-measure, they require domain specific corpora being the background knowledge for ontology matching, and such documents should be pre-selected before executing the matching systems. Thus, they require more and further information than the proposed approach. More important of all, these documents are not always available, while our approach utilizes common corpora WordNet and suffers no such constraint. In general, we can draw the conclusion that our proposal is effective.
      </paragraph>
     </section>
    </section>
   </content>
