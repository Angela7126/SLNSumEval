  ...
  <root>
   ...
   <content>
    ...
    <section label="2">
     ...
     <paragraph>
      Probabilistic conditional independence is an important notion in several disciplines [2], [3], forming the theoretical basis for probabilistic graphical models in particular and efficient probabilistic inference in general. Numerous lines of research have been devoted to the study of its mathematical, logical, and algorithmic properties. The first axiomatic approach to conditional independence was introduced by Dawid [2]. A closely related inference system for probabilistic CI was developed by Pearl and Paz [16]. In both lines of work, a three-place relation was characterized using similar axiom systems. While Dawid termed the relation a separoid (cf. Dawid [17]), Pearl and Paz termed it a graphoid. These relations were shown to surface in the context of probabilistic conditional independence and other notions in statistics and artificial intelligence. Shafer [18] provides a comprehensive survey of some of the advances in the use and investigation of probabilistic conditional independence up until the year 1996, published in a special issue of Annals of Mathematics and Artificial Intelligence[19], [20], [21], [22], [23]. A line of work that has stimulated considerable interest in probabilistic conditional independence is Pearlʼs book about probabilistic expert systems [3]. A more recent overview of the principles and practice of graphical models can be found in, e.g., Koller and Friedman [1].
     </paragraph>
     <paragraph>
      A considerable amount of related work exists in the area of uncertainty in artificial intelligence. Studenýʼs work on structural representations of conditional independence [6], for instance, is highly related to the present work. In fact, there is a close connection between the lattice-theoretic representation and Studenýʼs theory of imsets [6], an algebraic framework for the study of supermodular functions on sets. Building upon Studenýʼs work, Hemmecke et al. [7] used, among other tools, integer programming to solve some open problems concerning imsets. Studený discussed the use of the maximization problem over the class of l-standardized supermodular functions which can be posed as a linear program [24]. A different but related linear programming formulation for a validation algorithm for the CI implication problem was introduced by Niepert et al. [25]. Independently, Bouckaert et al. [9] leveraged the theory of imsets [6] to assemble linear programs to verify various instances of the CI implication problem. Their approach is similar to ours, except that they do not use the several orders of magnitude more compact and, therefore, more efficient representation. We will compare the two linear programming formulations in the experimental section (Section 9).
     </paragraph>
     <paragraph>
      Similarly influential to the present work is the work of Geiger and Pearl [14] and Malvestuto [26] about the algorithmic and logical properties of conditional independence. In their work, probabilistic conditional independence is approached from a purely logical point of view and syntactic inference systems and their properties with respect to various classes of probability measures are investigated. The authors also discuss possible algorithms for the probabilistic conditional independence implication problem and the computational complexity of some of the problems and also pose several open problems.
     </paragraph>
     <paragraph>
      This article is a substantial extension, in terms of both theoretical and experimental results, of previous conference papers by the same authors [25], [27]. With this article we extend the theory to fully cover the case of elementary CI statements (Section 5) which serves as the basis for most of the results in Section 9. In addition to additional examples and a refinement of the theoretical results with respect to the additive implication problem (Section 6) and the significant generalization of the validation algorithm (Section 7), the experimental section (Section 9) was expanded with several additional experiments not included in previous publications. The construction of the constraint matrix of the validation algorithm is also a novel contribution increasing the efficiency of the validation algorithm. The lattice-theoretic framework that is developed is a continuation of previous work by three of the present authors and Purdom [28], [29], [30], [31], [32]. The theory is applicable in several different areas of computer science. Sayrafiʼs work, for instance, focused primarily on the class of frequency functions over finite relational databases [31]. The derived inference rules were used in frequent itemset mining algorithms as heuristics to prune the search space [33].
     </paragraph>
     <paragraph>
      Matúš [34] and de Waal and van der Gaag [35], [36] introduce stable independence as a notion to more compactly represent information about conditional independence.{sup:1} To this end, conditional independence statements are partitioned in a stable and a non-stable part allowing for more efficient storage and retrieval of probabilistic conditional independencies. Stable independence was also studied in earlier work of some of the present authors [5]. Though not elaborated upon in this paper, the theory we develop is also applicable to stable sets of CI statements.
     </paragraph>
    </section>
    <section label="3">
     <section-title>
     ...
     <paragraph>
      We first consider the well-known semi-graphoid axiom system [3], denoted here as System {a mathematical formula}G and shown in Fig. 1, and System {a mathematical formula}S[14], [26], shown in Fig. 2. In contrast to System {a mathematical formula}G, the rules of System {a mathematical formula}S are only applicable to saturated CI statements and yield saturated CI statements only. In all rules, the sets are assumed pairwise disjoint. Clearly, we have the following.
     </paragraph>
     <paragraph label="Proposition 2">
      System{a mathematical formula}Scan be inferred from System{a mathematical formula}G; the weak contraction rule can be inferred from the symmetry, decomposition, and contraction inference rules.
     </paragraph>
     <paragraph>
      Finally, we consider the set of inference rules in Fig. 3, denoted as System {a mathematical formula}A. Besides four rules of the semi-graphoid axiom system, it contains the strong union and strong contraction rules. Clearly, {a mathematical formula}G is subsumed by System {a mathematical formula}A.
     </paragraph>
     <paragraph label="Proposition 3">
      System{a mathematical formula}Gcan be inferred from System{a mathematical formula}A; the weak union rule can be inferred from the decomposition and strong union rules.
     </paragraph>
     <paragraph>
      From System {a mathematical formula}A, many other rules can be inferred, such as the composition rule shown in Fig. 4. As System {a mathematical formula}A plays a pivotal role in this work, we use the following abbreviations. If {a mathematical formula}C is a set of CI statements and c is a single CI statement, then {a mathematical formula}C⊢c is short for {a mathematical formula}C⊢Ac; similarly, {a mathematical formula}C+ is short for {a mathematical formula}CA+.
     </paragraph>
     <paragraph>
      We emphasize that in this and the next section we take a purely syntactic viewpoint towards CI statements, inference rules, and inference systems. In particular, we do not imply that the rules exhibited in Fig. 1, Fig. 2, Fig. 3, Fig. 4 are necessarily sound and/or complete. Notice, for example, that strong union is not a sound rule for the inference of CI statements relative to the class of discrete probability measures, as is illustrated by Example 35, Example 38.
     </paragraph>
    </section>
    <section label="4">
     <section-title>
     ...
     <paragraph>
      We first introduce the lattice-theoretic framework which is at the basis of the theory developed in this work. We associate CI statements with so-called meet semi-lattices, i.e., partially ordered sets in which each pair of elements has an infimum (a meet) [37]. If the semi-lattice is a class of sets, the infimum of a pair of its elements is their set intersection. In the following, the term “semi-lattice” is used for a meet semi-lattice on a class of sets. Subsequently, we characterize derivability of CI statements in terms of their associated semi-lattices.
     </paragraph>
     <paragraph>
      Given subsets A and B of S, we write {a mathematical formula}[A,B] for {a mathematical formula}{U|A⊆U⊆B}. We now associate semi-lattices with conditional independence statements as follows.
     </paragraph>
     <paragraph label="Definition 4">
      Let {a mathematical formula}I(A,B|C) be a CI statement. The semi-lattice of {a mathematical formula}I(A,B|C), denoted {a mathematical formula}L(A,B|C), equals {a mathematical formula}[C,S]−([A,S]∪[B,S])={U⊆S|C⊆U,A−U≠∅,B−U≠∅}.
     </paragraph>
     <paragraph>
      Notice that {a mathematical formula}L(A,B|C) is indeed closed under intersection.
     </paragraph>
     <paragraph label="Example 5">
      First, let {a mathematical formula}S={a,b,c}. Then, {a mathematical formula}L(a,b|c)={c}, as illustrated in Fig. 5. Next, let {a mathematical formula}S={a,b,c,d}. Then,{a mathematical formula}{a mathematical formula}{a mathematical formula}
     </paragraph>
     <paragraph>
      For a CI statement c, {a mathematical formula}L(c) denotes the semi-lattice of c, and, for a set of CI statements {a mathematical formula}C, {a mathematical formula}L(C)=⋃c′∈CL(c′).{sup:2}
     </paragraph>
     <paragraph label="Definition 6">
      Let {a mathematical formula}I(A,B|C) be a CI statement. The set of witness sets of {a mathematical formula}I(A,B|C), denoted {a mathematical formula}W(A,B|C), equals {a mathematical formula}{{a,b}|a∈A&amp;b∈B}.
     </paragraph>
     <paragraph>
    ...
    <section label="5">
     ...
     <paragraph>
      A witness decomposition of a CI statement (Definition 9) is equivalent to the original statement for derivations under System {a mathematical formula}A (Proposition 10). Therefore, the CI statements involved in witness decompositions deserve further study.
     </paragraph>
     <paragraph label="Definition 13">
      A CI statement is elementary if it is of the form {a mathematical formula}I(a,b|C), with a and b distinct elements of S and C a subset of {a mathematical formula}S−{a,b}.
     </paragraph>
     <paragraph>
      Elementary CI statements play an important role in Section 8 and following, where we apply the developed theory to obtain falsification and validation algorithms for the probabilistic implication problem. Here, we study some of their properties.
     </paragraph>
     <paragraph label="Definition 14">
      Let {a mathematical formula}c=I(A,B|C) be an arbitrary CI statement. We define {a mathematical formula}Del(c)={I(a,b|V)|C⊆V⊆ABC−{a,b},a∈A,b∈B}. Moreover, {a mathematical formula}Del(C)=⋃c′∈CDel(c′).
     </paragraph>
     <paragraph>
      Given a set of arbitrary CI statements {a mathematical formula}C and an arbitrary CI statement c, we consider the sets of elementary CI statements {a mathematical formula}Del(C) and {a mathematical formula}Del(c). In Theorem 15, we state that {a mathematical formula}CG+=Del(C)G+ and {a mathematical formula}{c}G+=Del(c)G+ (and hence also {a mathematical formula}C+=Del(C)+ and {a mathematical formula}{c}+=Del(c)+).{sup:3} We also present System {a mathematical formula}E, shown in Fig. 6, which involves elementary CI statements only. In Theorem 16, we show that {a mathematical formula}C⊢c if and only if, for all {a mathematical formula}cel∈Del(c), {a mathematical formula}Del(C)⊢Ecel.
     </paragraph>
     <paragraph>
      These results yield a normal form for derivations under System {a mathematical formula}A: first, from the given set of CI statements, derive a set of elementary CI statements from the given set of CI statements under System {a mathematical formula}G; then, from this set, derive a new set of elementary CI statements under System {a mathematical formula}E; and, finally, from this last set, derive the targeted CI statement under inference System {a mathematical formula}G. (Notice that System {a mathematical formula}E can be inferred from System {a mathematical formula}A.)
     </paragraph>
     <paragraph>
      We start with the first and last stage of this normalization, which is straightforward.
     </paragraph>
     <paragraph label="Theorem 15">
      Let c be an arbitrary CI statement and let{a mathematical formula}Cbe a set of arbitrary CI statements. Then,{a mathematical formula}{c}G+=Del(c)G+and{a mathematical formula}CG+=Del(C)G+, and hence also{a mathematical formula}{c}+=Del(c)+and{a mathematical formula}C+=Del(C)+.
     </paragraph>
     <paragraph>
      We now turn to the second stage of the normalization, which involves System {a mathematical formula}E, shown in Fig. 6. System {a mathematical formula}E is constructed in such a way that only elementary CI statements can be derived. In addition, it has the following property.
     </paragraph>
     <paragraph label="Theorem 16">
      Let{a mathematical formula}Cbe a set of elementary CI statements, and let c be a single elementary CI statement. Then{a mathematical formula}C⊢Ecif and only if{a mathematical formula}L(c)⊆L(C).
     </paragraph>
     <paragraph label="Proof">
      System {a mathematical formula}E can be inferred from System {a mathematical formula}A. The “only if” direction then follows from Theorem 11, Theorem 15. The proof of the “if” direction is a straightforward adaptation of the proof of the “if” direction of Theorem 11.  □
     </paragraph>
     <paragraph>
      Theorem 11, Theorem 15, Theorem 16 yield the following corollary.
     </paragraph>
     <paragraph label="Corollary 17">
      Let{a mathematical formula}Cbe an arbitrary set of CI statements, and let c be a single arbitrary CI statement. Then the following statements are equivalent:
     </paragraph>
     <list>
      <list-item label="•">
    ...
    <section label="6">
     ...
     <section label="6.1">
      ...
      <paragraph>
       We first introduce some terminology. In this work, a real-valued function is a function {a mathematical formula}F:2S→R, associating a real number to each subset of S.
      </paragraph>
      <paragraph label="Definition 18">
       Let {a mathematical formula}I(A,B|C) be a CI statement, and F be a real-valued function. We say that F a-satisfies{a mathematical formula}I(A,B|C), denoted {a mathematical formula}⊨FaI(A,B|C), if {a mathematical formula}F(C)+F(ABC)=F(AC)+F(BC).
      </paragraph>
      <paragraph>
       For a real-valued function F and a class of CI statements {a mathematical formula}C, we say that F a-satisfies {a mathematical formula}C, denoted {a mathematical formula}⊨FaC, if F a-satisfies each CI statement in {a mathematical formula}C.
      </paragraph>
      ...
      <paragraph>
       Now, given a class of real-valued functions {a mathematical formula}F and classes of CI statements {a mathematical formula}C1 and {a mathematical formula}C2, the additive implication problem{a mathematical formula}(C1,C2)F is to decide whether {a mathematical formula}C⊨Fac, for {a mathematical formula}C⊆C1 and {a mathematical formula}c∈C2.
      </paragraph>
      <paragraph>
       We proceed by defining the notion of density of a real-valued function.
      </paragraph>
      <paragraph label="Definition 20">
       The density of the real-valued function F is the real-valued function ΔF defined by {a mathematical formula}ΔF(X)=∑U∈[X,S](−1)|U|−|X|F(U), for each {a mathematical formula}X⊆S.
      </paragraph>
      <paragraph>
       The following well-known relationship between a real-valued function and its density, also known as its Möbius inversion, justifies the name (see, e.g., [38]).
      </paragraph>
      <paragraph label="Proposition 21">
       Let F be a real-valued function. Then, for each{a mathematical formula}X⊆S,{a mathematical formula}F(X)=∑U∈[X,S]ΔF(U).
      </paragraph>
      <paragraph>
       The a-satisfaction of a real-valued function for a CI statement can be characterized in terms of an equation involving its density function. This characterization is central in developing our results and follows from a more general result in [31].
      </paragraph>
      <paragraph label="Proposition 22">
       Let{a mathematical formula}I(A,B|C)be a CI statement and F be a real-valued function. Then,{a mathematical formula}⊨FaI(A,B|C)if and only if{a mathematical formula}∑U∈L(A,B|C)ΔF(U)=0.
      </paragraph>
      <paragraph>
       In the remainder of this section, we study properties of classes of real-valued functions that guarantee soundness and/or completeness of inference systems subsumed by System {a mathematical formula}A for additive implication. These results are related to probabilistic conditional independence implication in Section 7.
      </paragraph>
     </section>
     <section label="6.2">
      <section-title>
       General soundness properties
      </section-title>
      <paragraph>
       We first define soundness, and then exhibit several characterizations.
      </paragraph>
      ...
      <paragraph>
       In the remainder of Section 6, we shall consider {a mathematical formula}Call, the class of all CI statements over the universe S, and {a mathematical formula}Csat, the class of all saturated CI statements over S.
      </paragraph>
      <paragraph>
       In order to characterize soundness for additive implication for several inference systems, we introduce the zero-density property.
      </paragraph>
      ...
      <paragraph>
       Example 30 lists several real-valued functions illustrating the zero-density property.
      </paragraph>
      <paragraph>
       We now provide characterizations of soundness for System {a mathematical formula}A:
      </paragraph>
      <paragraph label="Theorem 25">
       Let{a mathematical formula}Iconsist of the symmetry, weak union, and strong union rules. Let{a mathematical formula}Fbe a class of real-valued functions. The following statements are equivalent:
      </paragraph>
      <list>
       <list-item label="(1)">
        System{a mathematical formula}Iis sound for the additive implication problem{a mathematical formula}(Call,Call)F;
       </list-item>
       <list-item label="(2)">
        {a mathematical formula}Fhas the zero-density property on{a mathematical formula}Call; and
       </list-item>
       <list-item label="(3)">
        System{a mathematical formula}Ais sound for the additive implication problem{a mathematical formula}(Call,Call)F.
       </list-item>
      </list>
      <paragraph label="Proof">
     ...
     <section label="6.3">
      <section-title>
       Soundness properties for saturated CI statements
      </section-title>
      <paragraph>
       We now discuss the additive implication problem {a mathematical formula}(Csat,Call)F.
      </paragraph>
      <paragraph label="Theorem 26">
       Let{a mathematical formula}Iconsist of the symmetry and weak union rules. Let{a mathematical formula}Fbe a class of real-valued functions. The following statements are equivalent:
      </paragraph>
      <list>
       <list-item label="(1)">
        System{a mathematical formula}Iis sound for the additive implication problem{a mathematical formula}(Csat,Call)F;
       </list-item>
       <list-item label="(2)">
        {a mathematical formula}Fhas the zero-density property on{a mathematical formula}Csat;
       </list-item>
       <list-item label="(3)">
        System{a mathematical formula}Ais sound for the additive implication problem{a mathematical formula}(Csat,Call)F; and
       </list-item>
       <list-item label="(4)">
        System{a mathematical formula}Gis sound for the additive implication problem{a mathematical formula}(Csat,Call)F.
       </list-item>
      </list>
      <paragraph label="Proof">
     ...
     <section label="6.4">
      ...
      <paragraph>
       We first define completeness, and then study it as in Section 6.2.
      </paragraph>
      ...
      <paragraph>
       In order to characterize completeness for additive implication for System {a mathematical formula}A, we introduce the Kronecker property.
      </paragraph>
      ...
      <paragraph>
       The name Kronecker property is inspired by the observation that, on Ω, {a mathematical formula}ΔFU behaves as a Kronecker delta function, which is zero everywhere except in one point.
      </paragraph>
      <paragraph>
       Let {a mathematical formula}Ω(2)={V⊂S||V¯|⩾2}. Relative to a class of real-valued functions, the Kronecker property on {a mathematical formula}Ω(2) implies the completeness of System {a mathematical formula}A:
      </paragraph>
      <paragraph label="Theorem 29">
       Let{a mathematical formula}Fbe a class of real-valued functions. If{a mathematical formula}Fhas the Kronecker property on{a mathematical formula}Ω(2), then System{a mathematical formula}Ais complete for the additive implication problem{a mathematical formula}(Call,Call)F.
      </paragraph>
      <paragraph label="Proof">
      ...
      <paragraph>
       The following example illustrates the zero-density and Kronecker properties.
      </paragraph>
      ...
      <paragraph>
       Theorem 29 goes only in one direction, but has the following weak converse.
      </paragraph>
      ...
      <paragraph>
       Theorem 31 and Theorem 29 immediately yield the following corollary.
      </paragraph>
      <paragraph label="Corollary 32">
    ...
    <section label="7">
     <section-title>
      The probabilistic CI implication problem
     </section-title>
     <paragraph>
      Although the theory developed in Section 6 concerns the additive implication problem for CI statements, it is also applicable to the probabilistic implication problem. The link between both is made with the notion of multi-information functions (Studený [6]) induced by probability measures.{sup:4}
     </paragraph>
     <section label="7.1">
      <section-title>
       Preliminaries and problem statement
     ...
     <section label="7.2">
      ...
      <paragraph>
       Definition 23, Definition 27 for soundness and completeness of an inference system for the additive implication problem {a mathematical formula}(C1,C2)F ({a mathematical formula}C1 and {a mathematical formula}C2 are classes of CI statements and {a mathematical formula}F is a class of real-valued functions) can be adapted to the setting of probabilistic implication, provided {a mathematical formula}F is replaced by a class of discrete probability measures {a mathematical formula}P, and a-implication ({a mathematical formula}⊨a) is replaced by m-implication ({a mathematical formula}⊨m).
      </paragraph>
      <paragraph label="Example 38">
       From Example 35, we may conclude that strong union is not sound for the probabilistic implication problem {a mathematical formula}(Call,Call)Pbin. Hence, it is also not sound for the probabilistic implication problem {a mathematical formula}(Call,Call)Pall.
      </paragraph>
      <paragraph>
       It is known (e.g., [40]) that the semi-graphoid axioms are sound for the probabilistic implication problem {a mathematical formula}(Call,Call)Pall. However, System {a mathematical formula}G is not complete. Moreover, there cannot exist a finite set of inference rules that is sound and complete for the implication problem [6]. In addition, it is unknown whether the problem is decidable. For saturated CI statements, the situation is different. Malvestuto [26] and Geiger and Pearl [14] independently showed that System {a mathematical formula}S is sound and complete for the probabilistic implication problem {a mathematical formula}(Csat,Csat)Pall (see also [40]). Using the results of Section 6, we can bootstrap these results, as follows.
      </paragraph>
      <paragraph label="Theorem 39">
       <list>
        <list-item label="(1)">
         The class of multi-information functions{a mathematical formula}MPallhas the zero-density property on{a mathematical formula}Csat.
        </list-item>
        <list-item label="(2)">
         System{a mathematical formula}Ais sound for the probabilistic implication problem{a mathematical formula}(Csat,Call)Pall.
        </list-item>
       </list>
      </paragraph>
      <paragraph label="Proof">
       Since System {a mathematical formula}G is sound for the general probabilistic implication problem {a mathematical formula}(Call,Call)Pall, it is also sound for {a mathematical formula}(Csat,Call)Pall. The result now follows immediately from Theorem 26.  □
      </paragraph>
      <paragraph label="Theorem 40">
       <list>
        <list-item label="(1)">
         The class of multi-information functions{a mathematical formula}MPallhas the Kronecker property on{a mathematical formula}Ω(2).
        </list-item>
        <list-item label="(2)">
         System{a mathematical formula}Ais complete for the probabilistic implication problem{a mathematical formula}(Call,Call)Pall.
        </list-item>
       </list>
      </paragraph>
      <paragraph label="Proof">
       By Theorem 39(2), System {a mathematical formula}A is sound for the probabilistic implication problem {a mathematical formula}(Csat,Call)Pall. Since System {a mathematical formula}S is already complete for the probabilistic implication problem {a mathematical formula}(Csat,Csat)Pall, it follows in particular that System {a mathematical formula}A is. The result now follows immediately from Theorem 31 and Corollary 32.  □
      </paragraph>
      <paragraph>
       It can be shown by direct calculation that the class of multi-information functions induced by the class of binary probability measures has also the Kronecker property on {a mathematical formula}Ω(2). Therefore, System {a mathematical formula}A is also complete for the probabilistic implication problem {a mathematical formula}(Call,Call)Pbin. Please note that completeness of System {a mathematical formula}A with respect to a class of probability functions {a mathematical formula}P does not automatically imply completeness with respect to a subclass {a mathematical formula}P′⊆P.
      </paragraph>
      <paragraph>
       We conclude this section by a sound-and-completeness result.
      </paragraph>
      <paragraph label="Theorem 41">
       Both System{a mathematical formula}Aand System{a mathematical formula}Gare sound and complete for the probabilistic implication problems{a mathematical formula}(Csat,Call)Palland{a mathematical formula}(Csat,Call)Pbin.
      </paragraph>
      <paragraph label="Proof">
       It only remains to show that System {a mathematical formula}G is complete for both probabilistic implication problems. This is because System {a mathematical formula}G is complete for the probabilistic implication problem {a mathematical formula}(Csat,Call)P, where {a mathematical formula}P is an arbitrary class of probability measures, if and only if System {a mathematical formula}A is [41].  □
      </paragraph>
      <paragraph>
       Notice that, in Theorem 41, we do not assume that the CI statements to be derived are saturated.
      </paragraph>
     </section>
    </section>
    <section label="8">
     <section-title>
      Falsification and validation criteria
     </section-title>
     <paragraph>
      Decidability of the CI implication problem over the class of discrete probability measures is open, but it is known it has no finite sound and complete inference system [15]. Nonetheless, Theorem 40 demonstrates it has a finite complete inference system, namely System {a mathematical formula}A, which is useful since it provides a falsification criterion to decide some non-valid instances (Section 8.2). Of course, for instances which cannot so be falsified, it would be desirable to have validation criteria to decide some valid ones among these. We develop such a validation criterion, based on linear programming techniques (Section 8.3). In preparation, we explore two simplifications (Section 8.1). First, we show that we can reduce the general probabilistic implication problem to the probabilistic implication problem for elementary CI statements only. Then, we demonstrate that it suffices to consider only those variables that occur in the CI statements under consideration. In Section 9, we will give experimental evidence that the combination of the falsification and validation criteria is useful for deciding a large set of instances of the probabilistic implication problem.
     </paragraph>
     <section label="8.1">
      <section-title>
       Simplifications
      ...
      <paragraph>
       To reduce the probabilistic CI implication problem to one on elementary CI statements only, we build upon the work in Section 5, where elementary CI statements were considered from a purely syntactical point of view.
      </paragraph>
      <paragraph label="Proposition 42">
       Let{a mathematical formula}Cbe a set of CI statements, and c be a single CI statement. Then,
      </paragraph>
      <list>
       <list-item label="(1)">
        {a mathematical formula}C⁎=Del(C)⁎and{a mathematical formula}c⁎=Del(c)⁎;
       </list-item>
       <list-item label="(2)">
        {a mathematical formula}C⊨Pallmcif and only if, for all{a mathematical formula}cel∈Del(c),{a mathematical formula}Del(C)⊨Pallmcel.
       </list-item>
      </list>
      <paragraph label="Proof">
       It suffices to prove the first statement for a set of CI statements {a mathematical formula}C. By Theorem 15, {a mathematical formula}Del(C)⊆CG+. Since System {a mathematical formula}G is sound for the probabilistic implication problem {a mathematical formula}(Call,Call)Pall, we have that {a mathematical formula}Del(C)⊆C⁎, hence {a mathematical formula}Del(C)⁎⊆C⁎. Theorem 15 also yields that {a mathematical formula}C⊆Del(C)G+, from which we derive {a mathematical formula}C⁎⊆Del(C)⁎.  □
      </paragraph>
      <paragraph>
       We now turn to the second simplification, namely that variables that do not occur in the given CI statements need not be considered for the implication problem. In particular, this is also the case for hidden variables. For a set of CI statements {a mathematical formula}C′ over a finite universe S (the set of variables), let {a mathematical formula}var(C′) be the set of variables occurring in at least one CI statement of {a mathematical formula}C′. Now consider an instance “{a mathematical formula}C⊨Pmc?” of the probabilistic implication problem relative to S. Intuitively, the variables of S outside {a mathematical formula}var(C∪{c}) play no role. Nevertheless, restricting this instance of the probabilistic implication problem to the relevant variables also means restricting the probability measures considered. Therefore, we need to prove formally that our intuition is correct.
      </paragraph>
      <paragraph label="Lemma 43">
       Let U and V be disjoint sets of variables. Let{a mathematical formula}PUbe a probability measure over U. Then there exists a probability measure{a mathematical formula}PUVover UV such that, for each subset X of U,{a mathematical formula}PUVX=PUX.
      </paragraph>
      <paragraph label="Proof">
       Designate by “0” one element in the domain of each variable in UV. Let u be a domain vector of U and v a domain vector of V. Let {a mathematical formula}0V be the domain vector of V of which each component is 0. We define{a mathematical formula} Clearly, {a mathematical formula}PUV is a probability measure over UV and, for {a mathematical formula}X⊆U, {a mathematical formula}PUVX=PUX.  □
      </paragraph>
      <paragraph>
       For {a mathematical formula}U⊆S, {a mathematical formula}PU denotes class of all probability measures over U.
      </paragraph>
      <paragraph label="Proposition 44">
       Let U and V be disjoint sets of variables,{a mathematical formula}Cbe a set of CI statements over U, and c be a single CI statement over U. Then,{a mathematical formula}C⊨PUmcif and only if{a mathematical formula}C⊨PUVmc.
      </paragraph>
      <paragraph label="Proof">
       We first prove the “only if”. Thereto, let {a mathematical formula}PUV∈PUV be such that {a mathematical formula}PUV m-satisfies all CI statements in {a mathematical formula}C. Let {a mathematical formula}PU=PUVU. Obviously, {a mathematical formula}PU∈PU, and, for {a mathematical formula}X⊆U, {a mathematical formula}PUX=PUVX. Hence, {a mathematical formula}PU m-satisfies all CI statements in {a mathematical formula}C. From {a mathematical formula}C⊨PUmc, it now follows that {a mathematical formula}PU=PUVU m-satisfies c, and hence that {a mathematical formula}PUV m-satisfies c. We now turn to the “if.” Thereto, let {a mathematical formula}PU∈P be such that {a mathematical formula}PU m-satisfies all CI statements in {a mathematical formula}C. By Lemma 43, there exists a probability function {a mathematical formula}PUV∈PUV such that {a mathematical formula}PU=PUVU. The remainder of the proof now goes along the same lines as for the “only if.”  □
      </paragraph>
      <paragraph>
       An immediate corollary to Proposition 44 is that, for a set of CI statements {a mathematical formula}C and a single CI statement c, both over S, {a mathematical formula}C⊨PSmc if and only if {a mathematical formula}C⊨Pvar(C∪{c})mc, i.e., the targeted simplification. We shall, therefore, disregard irrelevant variables.
      </paragraph>
     </section>
     <section label="8.2">
      <section-title>
     ...
     <section label="8.3">
      ...
      <paragraph>
       A validation criterion for the probabilistic implication problem accepts an instance of the problem only if the implication is valid. If the criterion does not accept the instance, however, this does not imply that the instance is invalid. One of the most prominent validation criteria is the algorithm that computes the closure of the semi-graphoid axioms [2], [3], which, unfortunately, can only validate a small fraction of the set of verifiable instances. Here, we introduce a much more powerful validation criterion, based on the lattice-theoretic framework. We harness our results to represent a set of CI statements {a mathematical formula}C as a sparse 0–1 matrix {a mathematical formula}AC in which the rows correspond to elements of {a mathematical formula}L(C) and the columns correspond to certain CI statements of the form {a mathematical formula}I(a,b|C) that are induced from {a mathematical formula}C. Subsequently, for a given CI statement c, we will associate with the instance {a mathematical formula}C⊨Pallmc a linear program with equality constraints involving an encoding of c in terms of its associated semi-lattice {a mathematical formula}L(c) and the matrix {a mathematical formula}AC. We then show that {a mathematical formula}C⊨Pallmc is valid if this linear program has a solution. To achieve this, we introduce relevant elementary CI statements:
      </paragraph>
      <paragraph label="Definition 47">
       An elementary CI statement c is relevant to a set of CI statements {a mathematical formula}C if {a mathematical formula}L(c)⊆L(C). The set of all such CI statements is denoted {a mathematical formula}R(C).
      </paragraph>
      <paragraph>
       Since System {a mathematical formula}A is complete but not sound for the probabilistic implication problem (Theorem 40), {a mathematical formula}C⊢c or, equivalently, {a mathematical formula}L(c)⊆L(C) (Theorem 11) is necessary but not sufficient for {a mathematical formula}C⊨Pmc, not all relevant statements associated with {a mathematical formula}C need be probabilistically implied by {a mathematical formula}C. By Proposition 42, however, there is a subset of {a mathematical formula}R(C) that is probabilistically equivalent to {a mathematical formula}C.
      </paragraph>
      <paragraph>
       With a CI statement c over the universe S, we associate a vector {a mathematical formula}vc over {a mathematical formula}2S:{a mathematical formula} In other words, {a mathematical formula}vc is a representation of the characteristic function of {a mathematical formula}L(c).
      </paragraph>
      <paragraph>
       Apart from being the set of all elementary CI statements equivalent to the given set of CI statements {a mathematical formula}C for derivations in System {a mathematical formula}A, {a mathematical formula}R(C) has the following attractive property which justifies calling {a mathematical formula}R(C) a “basis” for {a mathematical formula}C.
      </paragraph>
      ...
      <paragraph>
       We still need a couple of additional results before we can move on to the construction of the linear program. The first one involved the multi-information function {a mathematical formula}MP of a discrete probability measure P.
      </paragraph>
      <paragraph label="Proposition 49">
       Let A, B, and C be pairwise disjoint subsets of S, and let P be a discrete probability distribution. Then,{a mathematical formula}∑U∈L(A,B|C)ΔMP(U)⩾0.
      </paragraph>
      <paragraph label="Proof">
       For a real-valued function F, we have {a mathematical formula}F(C)−F(AC)−F(BC)+F(ABC)=∑U∈L(A,B|C)ΔF(U)[31]. If F is supermodular, the left-hand side is nonnegative. Since {a mathematical formula}MP is supermodular [6], Proposition 49 immediately follows.  □
      </paragraph>
      <paragraph>
       We have already defined the vector associated with a single CI statement. We now define the vector associated with a set of CI statements {a mathematical formula}C as the sum of the vectors associated to the CI statements in {a mathematical formula}C: {a mathematical formula}vC=∑c∈Cvc.
      </paragraph>
