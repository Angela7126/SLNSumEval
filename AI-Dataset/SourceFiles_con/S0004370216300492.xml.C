   ...
   <content>
    ...
    <section label="2">
     ...
     <section label="2.6">
      ...
      <paragraph>
       Hampton therefore reports the following six main results:
      </paragraph>
      <list>
       <list-item label="•">
        The attribute set for combined concepts is the union of the attribute sets of the constituent concepts
       </list-item>
       <list-item label="•">
        The importance of attributes in the combined concept is usually a weighted sum of the importance of the attributes in each individual concept
       </list-item>
       <list-item label="•">
        Necessity and impossibility are preserved
       </list-item>
       <list-item label="•">
        Attribute loss or emergence is observed
       </list-item>
       <list-item label="•">
        Conjunction is not commutative
       </list-item>
       <list-item label="•">
        Dominance effects are observed
       </list-item>
      </list>
      <paragraph>
       We will argue that our proposed model of concepts and concept combination can also account for these phenomena. Furthermore, our framework is a natural extension of the conceptual spaces model in which the importance of certain dimensions is related to their necessity as defined by possibility theory [16].
      </paragraph>
     </section>
    </section>
    <section label="3">
     ...
     <section label="4.3">
      ...
      <section label="4.3.1">
       ...
       <paragraph label="Definition 5">
        Betweenness{a mathematical formula}∀x,y,z∈{0,1}n with distance metric {a mathematical formula}Hλ→, z is between x and y, {a mathematical formula}B(x,y,z) iff {a mathematical formula}Hλ→(x,y)=Hλ→(x,z)+Hλ→(z,y).
       </paragraph>
       <paragraph label="Definition 6">
        ConvexityA set {a mathematical formula}S⊆{0,1}n is convex if {a mathematical formula}∀x,y∈S, every point z lying between x and y also belongs to S, i.e. {a mathematical formula}{z:B(x,y,z)}⊆S.
       </paragraph>
       <paragraph>
        We can now generalise Theorem 4 to the case where {a mathematical formula}θ=⋀i=1k±Li, {a mathematical formula}k≤n. In this case, the prototype P does not specify all values of {a mathematical formula}i=1...n. We firstly introduce some notation allowing us to talk about the set of dimensions in a prototype that remain invariant. We argue that if P does not specify all dimensions of {a mathematical formula}{0,1}n, then the weight vector {a mathematical formula}λ→ must be such that only those dimensions contributing to the concept are weighted. For example, suppose {a mathematical formula}n=3, {a mathematical formula}P={(1,1,1),(1,1,0)}. Then {a mathematical formula}λ→=(0.4,0.3,0). We go on to prove that a similar result to that shown in Theorem 4 holds for θ.
       </paragraph>
       <paragraph label="Definition 7">
        We now introduce some notation to enable us to talk about the dimensions of a set of points {a mathematical formula}S⊆{0,1}n that take a fixed value across the subset.
        <section-title>
         Fixed dimensions
        </section-title>
       </paragraph>
       <list>
        <list-item label="•">
         For {a mathematical formula}S⊆{0,1}n, {a mathematical formula}Z(S)⊆{1,…,n} such that {a mathematical formula}Z(S)={i:∀x→∈S,xi=0} (the zeros of S)
        </list-item>
        <list-item label="•">
         For {a mathematical formula}S⊆{0,1}n, {a mathematical formula}O(S)⊆{1,…,n} such that {a mathematical formula}O(S)={i:∀x→∈S,xi=1} (the ones of S)
        </list-item>
        <list-item label="•">
         For {a mathematical formula}S⊆{0,1}n, {a mathematical formula}E(S)⊆{1,…,n} such that {a mathematical formula}E(S)={i:∀x→,y→∈S,xi=yi}. Note that {a mathematical formula}E(S)=Z(S)∪O(S) (the set of points in S that take fixed values)
        </list-item>
       </list>
       <paragraph label="Proof">
       ...
       <paragraph>
        So with respect to membership in θ in the binary space, the weight vector {a mathematical formula}λ′ produces the same results as the weight vector λ. We now show that an analogue to Theorem 4 holds.
       </paragraph>
       ...
       <paragraph label="Proof">
        Since P is convex, {a mathematical formula}Hλ→(P,X→)=Hλ→′(P,X→).So{a mathematical formula}Now,{a mathematical formula} □
       </paragraph>
       <paragraph>
        We can therefore convert any {a mathematical formula}θ=&lt;P,Hλ→,δ&gt; with {a mathematical formula}δ=U(0,λT) into {a mathematical formula}θ′, such that {a mathematical formula}i∉E(P)→λi=0 via a suitable scaling of {a mathematical formula}μθ(X→) and {a mathematical formula}μθ(Y→).
       </paragraph>
       <paragraph>
      ...
      <section label="4.3.2">
       ...
       <paragraph>
        Let {a mathematical formula}θ=&lt;P1,d1,δ1&gt; and {a mathematical formula}φ=&lt;P2,d2,δ2&gt; be two compound concepts consisting of a conjunction of attribute labels from individual conceptual spaces {a mathematical formula}Ωi. As in section 4.2 we define the following two binary variables:{a mathematical formula}
       </paragraph>
       <paragraph>
      ...
      <section label="4.3.3">
       ...
       <paragraph>
        The necessity of an event A is then {a mathematical formula}N(A)=1−Π(Ac), i.e. 1 − the possibility that A does not occur.
       </paragraph>
       <paragraph>
        Within our model, a state s is considered to be a particular point {a mathematical formula}X→ in the binary combination space {a mathematical formula}{0,1}n and {a mathematical formula}π(X→):=μθ(X→). To compute the necessity of a dimension i to a concept {a mathematical formula}θ=&lt;P,Hλ→,δ&gt;, where {a mathematical formula}P⊆{0,1}n, we consider the necessity of set {a mathematical formula}Si={X→:Xi=pi}, i.e. the set of all points that have equal value to the prototype on dimension i.{a mathematical formula} where {a mathematical formula}Nθε is the neighbourhood of θ as defined in section 3. It may be the case that P contains both points with value 0 on dimension i and points with value 1 on dimension i. In this case we say that {a mathematical formula}Si=∅, since {a mathematical formula}Si must satisfy {a mathematical formula}Si={X→:Xi=0∧Xi=1}.
       </paragraph>
       <paragraph label="Definition 13">
        Necessity of a dimensionGiven {a mathematical formula}θ=&lt;P,Hλ→,δ&gt;, the necessity of a dimension i to θ is defined as the necessity of the set {a mathematical formula}Si={X→:Xi=pi}, where {a mathematical formula}pi is the ith dimension of {a mathematical formula}{0,1}n, by {a mathematical formula}N(Si)=δ({ε:Nθε⊆Si}).
       </paragraph>
       <paragraph>
        If P contains both vectors with value 0 on dimension i and vectors with value 1 on dimension i, then {a mathematical formula}N(Si)=0, since {a mathematical formula}Si=∅ and therefore {a mathematical formula}N(Si)=δ({ε:Nθε⊆Si})=P(Nθε⊆∅)=0.
       </paragraph>
       <paragraph label="Proof">
        For{a mathematical formula}α=&lt;P,Hλ→,δ&gt;,{a mathematical formula}N(Si)=1−Δ(λi).{a mathematical formula}N(Si)=P(Nαε⊆Si)=P(ε&lt;λi)=1−Δ(λi).  □
       </paragraph>
       <paragraph>
        In particular, when {a mathematical formula}ε∼U(0,λT), {a mathematical formula}N(Si)=λiλT.
       </paragraph>
       <paragraph>
       ...
       <paragraph>
        We begin by defining the neighbourhood of a conjunction of two concepts. Suppose that two concepts θ and φ are combined in the second level space {a mathematical formula}{0,1}2, with weight vector {a mathematical formula}w→=(w1,w2) where {a mathematical formula}w1 is associated with θ and {a mathematical formula}w2 with φ. Suppose {a mathematical formula}w2≤w1. Now, if the threshold ε in the second level space is less than {a mathematical formula}w2, then an element {a mathematical formula}X→ of the first level binary space must belong to the neighbourhoods of both concepts θ and φ, i.e. {a mathematical formula}X→∈Nθεθ∩Nφεφ. When {a mathematical formula}w2≤ε≤w1, the points within ε in the second level space are {a mathematical formula}{(1,1),(1,0)}, so {a mathematical formula}X→ must belong to {a mathematical formula}Nθεθ, but does not have to belong to {a mathematical formula}Nφεφ. When {a mathematical formula}w1≤ε≤wT, where {a mathematical formula}wT=w1+w2, {a mathematical formula}X→ may belong to either neighbourhood, i.e. {a mathematical formula}X→∈Nθεθ∪Nφεφ. This is summarised in the following definition:
       </paragraph>
       <paragraph label="Definition 15">
        For a conjunction {a mathematical formula}θ∧φ=&lt;{(1,1)},Hw→,δ&gt;, where {a mathematical formula}w→=(w1,w2) and {a mathematical formula}w2≤w1, the neighbourhood R of {a mathematical formula}θ∧φ is:{a mathematical formula}
       </paragraph>
       <paragraph>
        This allows us to relate the necessity of a dimension to the concept {a mathematical formula}θ∧φ to the necessity of the dimension to the constituent concepts θ, φ since {a mathematical formula}Nθ∧φ(Si)=δθ∧φ(εθ,εφ:R⊂Si).
       </paragraph>
       <paragraph label="Theorem 16">
        For a conjunction of compound concepts{a mathematical formula}θ∧φ=&lt;{(1,1)},Hw→,δ&gt;, where θ, φ are defined as forTheorem 10{a mathematical formula}
       </paragraph>
       <paragraph label="Proof">
        {a mathematical formula} □
       </paragraph>
       <paragraph label="Corollary 17">
        If{a mathematical formula}δ=Uniform(0,wT)then{a mathematical formula}Nθ∧φ(Si)=w1wTNθ(Si)+w2wTNφ(Si).
       </paragraph>
       <paragraph label="Proof">
        {a mathematical formula}δ=Uniform(0,wT)⟹Δ(w1)=w2wT and {a mathematical formula}Δ(w1)=w1wT{a mathematical formula} □
       </paragraph>
       <paragraph>
        This allows us to choose a boundary distribution δ which gives us the property that high necessity is carried through into the conjunction. This is illustrated in the following example.
       </paragraph>
       ...
       <paragraph>
        Lastly, Hampton finds that concepts with more attributes tend to have higher weightings. This can also be accounted for by weighting concepts θ relative to the cardinality of {a mathematical formula}E(Pθ).
       </paragraph>
      </section>
     </section>
     <section label="4.4">
