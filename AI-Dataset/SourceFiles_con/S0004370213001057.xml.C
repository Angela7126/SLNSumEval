  ...
  <root>
   ...
   <content>
    <section label="1">
     ...
     <paragraph>
      The paper is structured as follows: in Section 2 we formally define the considered problem, while in Section 3 we describe all related works highlighting the differences w.r.t. our approach. Section 4 formally describes the modular formulation adopted in this work. In Sections 5 and 6 we respectively describe the model with the novel constraints and their filtering algorithms. Section 7 discusses two search strategies and the restricted version of the solver, and formally defines a dominance rule used to narrow the search space. Experimental results are reported in Section 8 and Section 9 concludes the paper.
     </paragraph>
    </section>
    <section label="2">
     <section-title>
     ...
     <paragraph label="Definition 2">
      The average inter-execution distance {a mathematical formula}λ(i) of an activity i is defined as the following limit:{a mathematical formula}
     </paragraph>
     <paragraph>
      Note that, since the activities are repeated indefinitely, the sum can start for {a mathematical formula}ω=0 without loss of generality, even if ω is in {a mathematical formula}Z as from the problem definition.
     </paragraph>
     <paragraph label="Definition 3">
      The average inter-execution distance λ of a set of activities {a mathematical formula}V is the worst case {a mathematical formula}λ(i).{a mathematical formula}
     </paragraph>
     <paragraph>
      The throughput of an activity i is defined as the average number of executions of i per time unit, and corresponds to the inverse of the average distance:{a mathematical formula} Analogously, the throughput of a set of activities is the inverse of λ. Lower period (i.e. higher throughput) values are to be preferred since they correspond to more efficient schedules.
     </paragraph>
     <paragraph>
      Note that a schedule has infinite size in principle, because each activity executes an infinite number of times. However, since building an infinite schedule is impossible in practice, we should find a way to compute a more compact problem solution. Typically, one wants to build a periodic schedule, i.e. a schedule where activities are executed regularly with a fixed period. Such an approach requires to specify only a start time for each activity, plus the period value. Periodic schedules will be formally defined in Section 2.3.
     </paragraph>
     <paragraph>
      In practical cases, activities are subject to several restrictions. In particular, there may be resource constraints and temporal dependencies, described in the following sections.
     </paragraph>
     <section label="2.1">
      <section-title>
     ...
     <section label="2.2">
      ...
      <paragraph>
       Temporal dependencies between activities corresponds to arcs in the project graph and may connect executions with different ω number.
      </paragraph>
      <paragraph label="Definition 4">
       Each arc {a mathematical formula}(i,j)∈A is a tuple {a mathematical formula}〈i,j,θ(i,j),δ(i,j)〉, where:
      </paragraph>
      <list>
       <list-item label="•">
     ...
     <section label="2.3">
      ...
      <paragraph>
       As stated in Section 2, a way to compute a compact solution to the problem is to consider schedules where the activities are executed regularly with a fixed period (i.e. periodic schedules). Note that in this case the fixed period (called iteration period) is the same as the maximum average inter-execution distance λ from Definition 3. Formally:
      </paragraph>
      <paragraph label="Definition 5">
       A periodic schedule is pair {a mathematical formula}〈L,λ〉 where λ is the iteration period and {a mathematical formula}L is a vector containing the start times for the execution 0 of all the activities, i.e.:{a mathematical formula} A periodic schedule obeys the following restriction:{a mathematical formula}
      </paragraph>
      <paragraph>
      ...
      <section label="2.3.1">
       ...
       <paragraph label="Definition 6">
        The cycle bound {a mathematical formula}CB(c) of a cycle c is the minimum time required in a periodic schedule to execute all the activities in c. This is equal to:{a mathematical formula} where:
       </paragraph>
       <list>
        <list-item label="•">
         {a mathematical formula}T(c)=∑i∈V(c)di is the sum of the durations of the activities in {a mathematical formula}V(c).
        </list-item>
        <list-item label="•">
         {a mathematical formula}D(c)=∑(i,j)∈A(c)δ(i,j) is the sum of the repetition distances of arcs in {a mathematical formula}A(c).
        </list-item>
       </list>
       <paragraph>
       ...
       <paragraph>
        The iteration bound is related to the concept of critical path in traditional scheduling and can be computed in polynomial time with the algorithm presented in [21]. The iteration bound IB is an intrinsic lower bound on the iteration period: we can never achieve a λ less than IB, even if unlimited resources are available. Periodic schedules are said to be periodically optimal if the iteration period λ is the same as the iteration bound IB. If there are no resource restrictions, then a periodically optimal schedule is guaranteed to exist. This is not true once we add resource constraints to the problem. Further details can be found in [12], [16].
       </paragraph>
      </section>
     </section>
     <section label="2.4">
      ...
      <paragraph>
       We can now define the CRCSP as follows. Given:
      </paragraph>
      <list>
       <list-item label="•">
        a Project Graph {a mathematical formula}G=〈V,A〉 with:
       </list-item>
       <list-item label="•">
        A set R of limited capacity resources, where each resource {a mathematical formula}k∈R has capacity {a mathematical formula}CAPk.
       </list-item>
       <list-item label="•">
        A fixed duration {a mathematical formula}di for each activity.
       </list-item>
       <list-item label="•">
        A resource requirement {a mathematical formula}ri,k⩾0 for each activity i and resource k.
       </list-item>
      </list>
      <paragraph>
       The CRCSP consists in finding a periodic schedule: i.e. an iteration period λ and a feasible assignment of {a mathematical formula}start(i,0){a mathematical formula}∀i∈V such that all dependencies are consistent, no resource capacity is exceeded and the iteration period λ is minimized.
      </paragraph>
     </section>
     <section label="2.5">
      <section-title>
      ...
      <paragraph>
       We end this section with a couple of useful definitions. Cyclic schedules can be either non-overlapped or overlapped.
      </paragraph>
      <list>
       <list-item label="•">
        A schedule is said to be non-overlapped if repetition {a mathematical formula}ω+1 starts only once repetition ω is over. Formally:{a mathematical formula}
       </list-item>
       <list-item label="•">
        A schedule is overlapped if Eq. (8) does not hold, i.e.:{a mathematical formula}
       </list-item>
      </list>
     </section>
    </section>
    <section label="3">
     ...
     <paragraph>
      The cyclic scheduling literature mainly arises in industrial and computing contexts. While there is a considerable body of work on cyclic scheduling in the OR literature [1], the problem has not received much attention from the AI community ([13] is one of the few related papers).
     </paragraph>
     <paragraph>
     ...
     <list>
      <list-item label="•">
       The so-called blocked scheduling approach (see Section 3.1) builds a schedule for a single repetition and assumes that the period is equal to the schedule length. As a consequence, consecutive repetitions are not allowed to overlap, with an obvious loss in terms of solution quality.
      </list-item>
      <list-item label="•">
       The unfolding approach (see Section 3.2) schedules a number of consecutive repetitions and then repeats the block similarly to the previous technique. Unfolding often leads to higher quality schedules, but it also requires to solve problem instances with artificially increased size.
      </list-item>
      <list-item label="•">
       The modulo scheduling approach (see Section 3.3) schedules a single repetition, which is however repeated every λ time units. The value λ is called the modulus and it is the same as the iteration period. By exploiting repetition overlaps, the modulus can be made much smaller than the schedule length, obtaining considerable speed-ups.
      </list-item>
     </list>
     <paragraph>
      Fig. 3 depicts a simple example of a CRCSP. All minimum time lags {a mathematical formula}θi,j are assumed to be 0 and the repetition distance {a mathematical formula}δi,j is 0 whenever not explicitly mentioned. In the following subsections we use this instance to show how the three approaches described above work.
     </paragraph>
     <section label="3.1">
      <section-title>
       Blocked scheduling
      ...
      <paragraph>
       Traditional static resource-constrained scheduling techniques for cyclic problems are non-overlapped [24], [7]. These methods optimize the performance of a single repetition of the project graph and then repeat the schedule periodically.
      </paragraph>
      <paragraph>
       Fig. 4 shows the optimal blocked schedule for the simple problem described in Fig. 3. The output of the solution approach is the schedule (or block) identified by the black arrows, which is then repeated every iteration period {a mathematical formula}λ=27. Hence, the throughput is one over the length of the schedule {a mathematical formula}Thp=1λ=0.037. The horizontal dotted line represents the resource capacity: note that this schedule leaves most of the resource idle. With blocked schedules, this may happen quite frequently.
      </paragraph>
     </section>
     <section label="3.2">
      <section-title>
      ...
      <paragraph>
       Overlapped schedules exploit the repetitive nature of periodic schedules to achieve higher throughput. On this purpose, they take into account inter-repetition dependencies in addition to intra-repetition ones. The unfolding technique, presented in [32] and [33], consists in scheduling u consecutive repetitions of the graph, where u is referred to as unfolding factor (or as blocking factor). Then, the resulting schedule (say with total length L) is treated as a normal blocked schedule and repeated every L time units. Note that the schedule length acts as a sort of period, but since u repetitions are considered, the actual average inter-repetition distance is {a mathematical formula}λ=Lu. In fact, u executions are completed in L time units.
      </paragraph>
      <paragraph>
       Fig. 5, Fig. 6 show the optimal unfolded schedule for the simple problem depicted in Fig. 3 with unfolding factor {a mathematical formula}u=2 and {a mathematical formula}u=3 respectively (with {a mathematical formula}u=1 we obtain the blocked schedule from Fig. 4). The schedule is restarted after L time units: for {a mathematical formula}u=2 the optimal L is 35 and {a mathematical formula}λ=17.5, while for {a mathematical formula}u=3, {a mathematical formula}L=47 and {a mathematical formula}λ=15.6. The throughput is higher than in the blocked schedule, but the problem size (and consequently the search space) is bigger because it consists of u different repetitions. Because the problem is NP-hard [25], multiplying the number of activities by the unfolding factor leads to an exponential increase in the solution time.
      </paragraph>
      <paragraph>
       Since the group of repetitions in the schedule is repeated in a blocked fashion, the unfolding approach may be incapable to optimally exploit inter-repetition overlaps to make the best use of the available resources. Moreover, despite increasing u from 2 to 3 led to a lower λ value in our example, it is not true in general that increasing the unfolding factor leads to better schedules. The optimal unfolding factor cannot be trivially computed.
      </paragraph>
      <paragraph>
       On the other hand, it is known [19] that periodic schedules are dominated by K-periodic schedules (i.e. periodic schedules for sequences of K iterations) in the presence of finite capacity resources. Since the unfolding technique takes into account multiple iterations, in some cases an unfolded schedule may be better than a (single-iteration) periodic schedule. Hence, no strict dominance exists between the two approaches. An in-depth survey of traditional and unfolding cyclic scheduling techniques can be found in [5].
      </paragraph>
     </section>
     <section label="3.3">
      <section-title>
    ...
    <section label="4">
     ...
     <section label="4.1">
      ...
      <paragraph>
       Before introducing our constraint model it is necessary to get a better insight into the resource usage profile of a periodic schedule. The most unusual peculiarity of this case is that the value of λ may modify the resource usage over time, as depicted in Fig. 10. The picture presents the profiles corresponding to schedules with the same start times, but with 4 different period values:
      </paragraph>
      <list>
       <list-item label="•">
       ...
       <list-item label="•">
        In b) the modular start time s is equal to the modular end time e, since the period is equal to the length of the activity.
       </list-item>
       <list-item label="•">
        In case c) the period is shorter than the activity, hence the schedule is overlapped and the resource profile exhibits a “pulse” in the middle.
       </list-item>
       <list-item label="•">
        In d) the period is slightly larger than the half of the activity duration. In this profile the modular end time e precedes the modular start time s, which is a bit counterintuitive.
       </list-item>
      </list>
      <paragraph>
       Note that changes in the modulus affect not only the resource usage but also the modular end time {a mathematical formula}ei. In general, in [28] we showed that the amount of resource k required by activity i in a time window having width λ is given by the following expression:
      </paragraph>
      <paragraph>
       {a mathematical formula} In other words, the resource usage is given by a constant factor {a mathematical formula}ri,k⋅⌊diλ⌋, plus an additional {a mathematical formula}ri,k in case the considered time point lies “between” the modular start and the modular end (the quotes are used since we may have {a mathematical formula}ei&lt;si). In Fig. 10, the constant usage factor is always 1 except for a), where it is 0. As a particular case, if {a mathematical formula}βi=ηi the constant usage factor is zero and {a mathematical formula}rqi,k(si,di,t,λ) becomes a classical resource usage function. This suggests that forcing the end times to be within the modulus allows the use of classical resource constraints (see Section 5).
      </paragraph>
     </section>
    </section>
    <section label="5">
     ...
     <section label="5.1">
      ...
      <paragraph>
       In a real world context, a precedence constraint often implies an exchange of intermediate products between activities that should be stored in buffers. For example, in the context of embedded systems two activities may exchange data packets that should be stored in memory.
      </paragraph>
      <paragraph>
       Every time the activity i ends, its product is accumulated in a buffer and whenever the activity j starts, a product in the buffer is consumed. It is common to have on each buffer a size limit, which can be modeled through the following constraint:{a mathematical formula} where {a mathematical formula}B(i,j) is the size limit and the reified constraint {a mathematical formula}(ei⩽sj) evaluates to one if the condition is satisfied. Inequality (15) limits the number of executions of activity i (the producer) before the first execution of j (the consumer): this ensures that the buffer capacity is always respected. Obviously {a mathematical formula}B(i,j)⩾δ(i,j), otherwise the problem is infeasible. In fact, the value {a mathematical formula}δ(i,j) can be thought as the number of products already accumulated in the buffer{a mathematical formula}〈i,j,θ(i,j),δ(i,j)〉 when the project starts for the first time.
      </paragraph>
     </section>
    </section>
    <section label="6">
     ...
     <section label="6.1">
      ...
      <list>
       <list-item label="•">
        The filtering rules for the iteration variables, which updates the bounds of the {a mathematical formula}ηi and {a mathematical formula}βj variables so that a proper distance (in terms of number of iterations) exists between activity i and j.
       </list-item>
       <list-item label="•">
     ...
     <section label="6.3">
      ...
      <list>
       ...
       <list-item label="•">
        Coherence: the procedure is executed whenever the modulus upper bound changes. The procedure modifies the data structure to guarantee the coherence with the new λ bound.{sup:5}
       </list-item>
      </list>
      <paragraph>
      ...
      <section label="6.3.2">
       ...
       <paragraph>
        However, assuming a real λ variable, it may happen that the computation takes an infinite number of steps. This may occur in case the computed offset Λ tends to zero but never becomes null. Now, it is sufficient to assume a finite precision for the computation to converge. Since in practice we model λ with an integer variable the termination proof holds.
       </paragraph>
       <paragraph>
       ...
       <list>
        <list-item label="•">
         Updating and sorting the modular end time for items in Ω, with asymptotic complexity {a mathematical formula}O(n⋅logn).
        </list-item>
        <list-item label="•">
         Computing the cumulative over-usage. This takes up to {a mathematical formula}2⋅n steps, and thus has asymptotic complexity {a mathematical formula}O(n).
        </list-item>
       </list>
       <paragraph>
        It is difficult to obtain a tight bound on the number of iterations of the main loop (lines 2–16). As λ is an integer, then {a mathematical formula}λ¯−λ̲ provides a trivial bound and the overall asymptotic complexity is {a mathematical formula}O((λ¯−λ̲)⋅n⋅logn). In practice, however, the number of iterations of the main loop is very small.
       </paragraph>
      </section>
     </section>
    </section>
    <section label="7">
     ...
     <paragraph>
      In this section, we introduce two search strategies tailored for two models of the problem. The first (called Path-based method) works for the restricted case where the end time are assumed to be within the modulus, {a mathematical formula}βi=ηi∀i∈V. The second (called random restart method) is used in the generic version and it is based on restarts, although in this case they are employed to ensure correctness rather than to speed up search. As discussed in Section 5, by constraining the end times to be within the modulus, we can model the resource constraints with traditional cumulative constraints.
     </paragraph>
     <paragraph>
      In this section we present both the search strategies, together with their variable and value selection heuristics. Finally we also present a dominance rule on β variables used to narrow the search space.
     </paragraph>
     <section label="7.1">
      <section-title>
       Path-based method
     ...
     <section label="7.3">
      ...
      <paragraph>
       From an implementation perspective, the direct formulation of the rules may be difficult to model and heavy to propagate. It is therefore convenient to modify the right hand side expression in Inequality (30) with an upper bound, obtaining the following, updated rule:{a mathematical formula}{a mathematical formula} For a fixed {a mathematical formula}λ¯ value, the expression {a mathematical formula}⌈di+θ(i,j)λ⌉ can be replaced with a fixed value to obtain a (much simpler to compute) upper bound on {a mathematical formula}βj. This technique is employed in the context of the CROSS search strategy, which works by restarting the search whenever the λ upper bound is modified.
      </paragraph>
     </section>
    </section>
    <section label="8">
     ...
     <paragraph>
      We have evaluated the effectiveness and scalability of both the approaches discussed in Section 5 (CROSS and {a mathematical formula}CROSS⁎) on various sets of instances. Moreover, we have compared them against a state-of-the-art ILP method and a state-of-the-art heuristic.
     </paragraph>
     <paragraph>
      In the first part of this section we will focus on the {a mathematical formula}CROSS⁎ approach: we show that the solver is able to compute good quality solutions in terms of seconds in the context of a real world problem. In the second part of the section we present a comparison between CROSS and {a mathematical formula}CROSS⁎, plus a blocked and an unfolded approach. More in detail, this section contains:
     </paragraph>
     <list>
      <list-item label="1.">
       An experimentation on industrial instances: the main purpose of this experimental evaluation is to show the our {a mathematical formula}CROSS⁎ approach to cyclic scheduling is viable in a practical setting. In particular, the experimentation will focus on instruction scheduling benchmarks, namely an industrial set of 36 instances for the ST200 processor by STmicroelectronics (also employed in [2]). We compare {a mathematical formula}CROSS⁎ with two state-of-the-art approaches: the complete ILP method presented in [2] and Swing Modulo Scheduling (SMS), a heuristic used by the GCC compiler [18].
      </list-item>
      <list-item label="2.">
       An evaluation of the solution quality obtained by the {a mathematical formula}CROSS⁎ approach: the second group of tests is performed on a set of synthetic instances and compares the best solution obtained by {a mathematical formula}CROSS⁎ (within 300 s) with a lower bound on the instance period (namely, the iteration bound from Section 2.3.1).
      </list-item>
      <list-item label="3.">
       A comparison between CROSS/{a mathematical formula}CROSS⁎ and blocked/unfolded scheduling: this experimentation is performed on a set of synthetic instances, for which we compare the solution quality of both the approaches with respect to classic blocked and unfolded scheduling.
      </list-item>
      <list-item label="4.">
       A comparison between CROSS and {a mathematical formula}CROSS⁎, w.r.t. the Throughput / Resource trade-off: the fourth group of results compares our two approaches in terms of Throughput / Resource-usage trade-off.
      </list-item>
     </list>
     <paragraph>
      All our solvers have been implemented in IBM ILOG Solver and Scheduler 6.7. All the experiments are performed on an Intel Core 2 Duo 3.3 GHz with 8 GB of RAM. A 300 s time limit was set on each run. The synthetic instances were built by means of an internally developed task-graph generator, designed to produce graphs with realistic structure and parameters.
     </paragraph>
     <section label="8.1">
      Evaluation of {a mathematical formula}CROSS⁎ on industrial instances
      <paragraph>
       The first set of 36 instances is obtained from a compiler for a VLIW (Very-Long Instruction Word) architecture. In this setting, cyclic scheduling problems arise when optimizing inner loops at instruction level: in this context the activities represent instructions, the precedence constraints model data and control dependencies and the resources are the hardware units required to execute the instructions. In the considered instances, all the resources have unary capacity and all the instructions have unary duration. With the objective to evaluate the approaches on a more diversified setting, in [2] the authors have obtained an additional set of modified instances, by replacing the original resource capacities and consumptions with randomly generated integer values.
      </paragraph>
      <paragraph>
       The smallest instance features 10 nodes and 42 arcs, while the largest one has 214 nodes and 1063 arcs. In [2] the authors present two ILP formulations for the resource-constrained modulo scheduling problem (RCMSP), which is equivalent to the CRCSP discussed in this paper, with the (easy to enforce) exception that the period must be integer. As described in [2], both the ILP approaches adopt a dual process by iteratively increasing an infeasible lower bound; as a consequence, the method does not provide any feasible solution before the optimum is reached. Given a large time limit (604 800 s) their solvers found the optimal solution for almost all the instances: our experiments compare the optimal value with the solution found by our method within a 300 s time limit.{sup:8} We also compare our approach with an incomplete method, namely a state of the art heuristic approach called Swing Modulo Scheduling (SMS), presented in [26] and used by the gcc compiler [18].
      </paragraph>
      <paragraph>
       Note that the ILP and the SMS approaches were developed to tackle instruction scheduling problems; hence both work with activities with unary duration.
      </paragraph>
      <paragraph>
       As we mentioned, our benchmarks consists of two subsets of instances: the original problems extracted by the compiler and the modified ones (which tend to be more challenging). The results of the experiments performed with both the sets are reported respectively in Table 1 and Table 2. In the tables the first three columns describe the instance (name, number of nodes and arcs), the third shows the run-time of the ILP approach in [2], the fourth and the fifth respectively report the solution time and the quality of our solutions, evaluated by means of the gap w.r.t. the optimal solution.{sup:9} The last two columns present the solution time and the quality gap for the SMS heuristic approach. For some instances (see the missing time information) the ILP approach was not able to find a solution: in such cases we report the best solution found by {a mathematical formula}CROSS⁎, which is used as a reference for computing the SMS gap.
      </paragraph>
      <paragraph>
       On the industrial set, {a mathematical formula}CROSS⁎ is able to find the optimal solution within one second for all but one instance (adpcm-st231.2, whose optimality gap is 2.44%). We also found a solution for the gsm-st231.18 instance that was previously unsolved. The SMS heuristic method obtains an average optimality gap of 14.58%. Its average run time is less than few seconds, with a maximum of 58.36 s, corresponding to instance gsm-st231.18. For the previously unsolved instance (i.e. gsm-st231.18), the gap between SMS and {a mathematical formula}CROSS⁎ is 3.8%.
      </paragraph>
      <paragraph>
       Table 2 report the experimental results on the modified set of instances. Our approach finds the optimal solution within a second for all of the instances but two (for which the average gap is 0.61%). SMS achieves an average gap of 3.03%: again, the highest computation time corresponds to instance gsm-st231.18, but it is considerably larger compared to that of the industrial set (i.e. 429.36 s).
      </paragraph>
      <paragraph>
       Finally, note that for the instances gsm-st231.25 and gsm-st231.33 no optimal solution is available, since in [2] the authors were only able to find a suboptimal schedule within the time limit of 604 800 s (no details are given on how the dual process they propose converges to sub-optimal solutions). On the instance gsm-st231.25 both the modular and the SMS approaches find the same solution as [2]. On gsm-st231.33 the best period value found in [2] has value 52 and the SMS solver finds the same solution; our method however finds in one second a solution with value 47 and within 56 s a solution of value 46.
      </paragraph>
      <paragraph>
       Discussion on the optimality proof The results of our experimentation show that {a mathematical formula}CROSS⁎ is able to find very good solutions in a very short time (a few seconds). The method appears however much less effective in proving optimality: despite the optimal solution was found on most instances, an optimality proof was achieved only for 12.5% of them within the time limit.
      </paragraph>
      <paragraph>
       This is mainly due to the fact that cyclic instances are frequently characterized by a large number of precedences having high repetition distance δ values. Unfortunately this makes the precedence constraints propagation less effective (see Section 6.1).
      </paragraph>
      <paragraph>
       As formulated in Section 2.3.1 a cycle c with a high {a mathematical formula}D(c) (we recall that {a mathematical formula}D(c)=∑(i,j)∈A(c)δ(i,j)) has less impact on the iteration bound than a cycle with a lower {a mathematical formula}D(c).
      </paragraph>
      <paragraph>
       A simple example is considering two activities {a mathematical formula}i,j connected in a cycle with: {a mathematical formula}D(c)=1, or {a mathematical formula}D(c)&gt;1 or without precedences. Let also the activity i be scheduled. In the first case (where {a mathematical formula}D(c)=1) the precedence constraint propagation has a strong impact forcing the activity j to be scheduled after the end of i. In the other two cases (where {a mathematical formula}D(c)&gt;1 or without precedences) the precedence constraint cannot filter on the start time domain of activity j. In fact the two activities might overlap their executions with different iteration values (i.e. β).
      </paragraph>
      <paragraph>
       To conclude, high {a mathematical formula}D(c) values mean that most of activities of c can overlap their executions. There is therefore a strong analogy with those instances and acyclic scheduling problems with few precedence constraints (or even no precedences). It is well known that CP approaches based on start time variables and cumulative constraints have difficulties in proving optimality for this class of problem instances.
      </paragraph>
      <paragraph>
       We are currently investigating how to improve the efficiency of the proof of optimality through no-good learning. Note however that the optimality gap is so small that the method is very appealing even if used as a heuristic.
      </paragraph>
     </section>
     <section label="8.2">
      Evaluating {a mathematical formula}CROSS⁎ solution quality on synthetic benchmarks
      <paragraph>
     ...
     <section label="8.3">
      ...
      <paragraph>
       Table 5 shows the result of the comparison, grouped by the number of activities in the graph. In particular, the whole set of 200 instances has been partitioned into two subsets, respectively with 5–14 and 15–25 activities. The Time column shows the average time taken by the CROSS approach to reach the best solution found within 300 s. The table then reports, for all the considered approaches, the (average) quality gap w.r.t. the CROSS solution (i.e. {a mathematical formula}λ−λCROSSλCROSS) and an (average) {a mathematical formula}Δtime value, representing the difference between the time taken by the considered approach and by CROSS to reach the best solution found within 300 s.
      </paragraph>
      <paragraph>
       The results show the existence of a relevant trade-off between solution time and quality. The blocked scheduling approach represents one of the extremes, providing its best solution in a fraction of second, but with a quality gap as large as 216%. Increasing the unfolding factor leads to better λ values, at the cost of increased solution time. The Unfold5 configuration eventually manages to produce better solutions compared to CROSS (1.3% improvement), but the process is up to 6 times longer. Our approach seems to provide a very good compromise, providing high quality solutions in a short amount of time.
      </paragraph>
     </section>
     <section label="8.4">
      <section-title>
       Throughput/resource trade-off investigation
      </section-title>
      <paragraph>
