   ...
   <content>
    <section label="1">
     ...
     <list>
      ...
      <list-item label="•">
       Cyclic support is disallowed by stable semantics for logic programs [20] and in extensions of default logic [35].{sup:1}
      </list-item>
     </list>
     <paragraph>
     ...
     <list>
      ...
      <list-item label="•">
       ADFs provide all of Dungʼs standard semantics for AFs, so there is no loss in semantical richness. On the contrary, each of the standard AF semantics (stable, preferred, complete, grounded) has at least two ADF generalisations.
      </list-item>
     </list>
     <paragraph>
     ...
     <paragraph label="Structure of the paper">
    ...
    <section label="2">
     ...
     <section label="2.1">
      The algebraic framework of Denecker et al. [11]
      <paragraph>
      ...
      <paragraph>
       The main contribution of Denecker et al. [11] was the association of the stable operator{a mathematical formula}SO to an approximating operator {a mathematical formula}O. Below, the expression {a mathematical formula}O′(⋅,y):L→L denotes the operator given by {a mathematical formula}x↦O′(x,y) for {a mathematical formula}x∈L.
      </paragraph>
      <paragraph label="Definition 2.1">
       For a complete lattice {a mathematical formula}(L,⊑) and an approximating operator {a mathematical formula}O:L2→L2, define the
      </paragraph>
      <list>
       <list-item label="•">
        complete stable operator for{a mathematical formula}O as {a mathematical formula}cO:L→L by {a mathematical formula}cO(y)=deflfp(O′(⋅,y));
       </list-item>
       <list-item label="•">
        stable operator for{a mathematical formula}O as {a mathematical formula}SO:L2→L2 by {a mathematical formula}SO(x,y)=def(cO(y),cO(x)).
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph label="Definition 2.2">
       Let {a mathematical formula}(L,⊑) be a complete lattice and {a mathematical formula}O:L2→L2 be an approximating operator. Furthermore, let {a mathematical formula}x,y∈L with {a mathematical formula}x⊑y. Define the following semantical notions for {a mathematical formula}O:{a mathematical formula}
      </paragraph>
      <paragraph>
     ...
     <section label="2.4">
      ...
      <paragraph label="Definition 2.4">
       An abstract dialectical framework (ADF) is a triple {a mathematical formula}Ξ=(S,L,C) where
      </paragraph>
      <list>
       <list-item label="•">
      ...
      <list>
       ...
       <list-item label="•">
        Statement c has parents {a mathematical formula}par(c)={a,b}. They jointly support c, as is witnessed by {a mathematical formula}Ccin={par(c)}. Note that joint support here indeed means that the support only becomes effective if both parents are in.
       </list-item>
       <list-item label="•">
      ...
      <list>
       <list-item label="1.">
        the grounded extension of Θ and the BW-well-founded model of{a mathematical formula}Ξ(Θ);
       </list-item>
       <list-item label="2.">
        conflict-free sets of Θ and conflict-free sets of{a mathematical formula}Ξ(Θ);
       </list-item>
       <list-item label="3.">
        stable extensions of Θ and models of{a mathematical formula}Ξ(Θ);
       </list-item>
       <list-item label="4.">
        stable extensions of Θ and BW-stable models of{a mathematical formula}Ξ(Θ);
       </list-item>
       <list-item label="5.">
        preferred extensions of Θ and BW-preferred models of{a mathematical formula}Ξ(Θ).
       </list-item>
      </list>
      <paragraph label="Proof">
       Propositions 3, 1, 7 and 12 of [3].  □
      </paragraph>
      <paragraph>
    ...
    <section label="3">
     <section-title>
      Approximating semantics of abstract dialectical frameworks
     </section-title>
     <paragraph>
     ...
     <section label="3.1">
      ...
      <paragraph>
       But first, we do the obviously necessary and show that {a mathematical formula}GΞ is indeed an approximating operator. From Definition 3.1 it is immediate that {a mathematical formula}GΞ is symmetric. It is easy to prove that the operator is also {a mathematical formula}⩽i-monotone.
      </paragraph>
      <paragraph label="Proof">
       For any ADF{a mathematical formula}Ξ=(S,L,C), the operator{a mathematical formula}GΞis{a mathematical formula}⩽i-monotone.Let {a mathematical formula}(X1,Y1)⩽i(X2,Y2), that is, {a mathematical formula}X1⊆X2 and {a mathematical formula}Y2⊆Y1. We have to show {a mathematical formula}GΞ(X1,Y1)⩽iGΞ(X2,Y2), that is, (1) {a mathematical formula}GΞ′(X1,Y1)⊆GΞ′(X2,Y2) and (2) {a mathematical formula}GΞ′(Y2,X2)⊆GΞ′(Y1,X1).
      </paragraph>
      <list>
       <list-item label="1.">
      ...
      <section label="3.1.1">
       ...
       <paragraph>
        First of all, we find a nice characterisation of conflict-freeness: a set M is conflict-free for an ADF Ξ iff application of the two-valued immediate consequence operator {a mathematical formula}GΞ leads to a superset of M, that is, M is a postfixpoint of {a mathematical formula}GΞ. Intuitively speaking, each statement that is contained in a conflict-free set M has a reason to be contained in M.
       </paragraph>
       <paragraph label="Proof">
        For any abstract dialectical framework{a mathematical formula}Ξ=(S,L,C), a set{a mathematical formula}M⊆Sis conflict-free for Ξ iff{a mathematical formula}M⊆GΞ(M).{a mathematical formula}
       </paragraph>
       <paragraph>
      ...
      <section label="3.1.2">
       ...
       <paragraph label="Proof">
        Much in accordance with logic programming, a model of an ADF is simply a two-valued fixpoint of its associated consequence operator: For any abstract dialectical framework{a mathematical formula}Ξ=(S,L,C), a set{a mathematical formula}M⊆Sis a model of Ξ iff{a mathematical formula}GΞ(M,M)=(M,M).{a mathematical formula}
       </paragraph>
       <paragraph>
      ...
      <section label="3.1.3">
       ...
       <paragraph>
        So while there are ADFs with BW-stable models which are not two-valued stable models of the ADFʼs approximating operator, we can establish an inclusion relation for the converse direction: any operator-based two-valued stable model of an ADF is also a BW-stable model of the ADF. To show this, we first need a lemma that relates the operators {a mathematical formula}GΞ′(⋅,M) and {a mathematical formula}GΞM whenever M is a model of Ξ.
       </paragraph>
       <paragraph label="Proof">
       ...
       <paragraph>
        The problem with this example is that the ADF ξ allows for the BW-stable model {a mathematical formula}M2 in which statement b cyclicly supports itself. This violates the intuitive requirement of stable semantics that whatever it takes to be true must have a non-cyclic justification. Furthermore, in logic programming, two distinct stable models of normal logic programs cannot be in a subset-relationship; likewise in Reiterʼs default logic, two distinct extensions of a default theory cannot be in a subset-relationship. With our operator-based definition of two-valued stable models for ADFs, this property comes for free:
       </paragraph>
       <paragraph label="Proof">
       ...
       <paragraph label="Definition 3.2">
        Let {a mathematical formula}Ξ=(S,L,Cin) be an abstract dialectical framework. A set {a mathematical formula}M⊆S is a stable model of Ξ iff it is the unique least model of the reduced ADF {a mathematical formula}ΞM=(S,L,CMin) with{a mathematical formula}
       </paragraph>
       <paragraph>
      ...
      <section label="3.1.5">
       ...
       <paragraph label="Corollary 3.11">
        Let Ξ be an abstract dialectical framework. Any{a mathematical formula}⩽i-maximal admissible pair is a three-valued supported model.
       </paragraph>
       <paragraph>
       ...
       <paragraph label="Corollary 3.13">
        For any ADF Ξ, its BW-well-founded semantics coincides with its ultimate Kripke–Kleene semantics.
       </paragraph>
       <paragraph>
        The well-founded semantics of Ξ in the usual sense (the least fixpoint of the stable operator {a mathematical formula}SGΞ) hence may differ from the BW-well-founded semantics.
       </paragraph>
       <paragraph label="Example 2.1">
        ContinuedRecall that the ultimate Kripke–Kleene semantics of D is given by the pair {a mathematical formula}({a},{a,b,c,d}) (the least model of the operator {a mathematical formula}UD=ΓD). The well-founded semantics of D in the logic-programming sense is given by the pair {a mathematical formula}({a,d},{a,d}). Since this pair is exact, it also represents the unique two-valued stable model of D. (Recall that {a mathematical formula}M2={a,d} is the supported model of D where the self-support of b was rejected.)
       </paragraph>
       <paragraph>
     ...
     <section label="3.2">
      ...
      <paragraph label="Proof">
       For any ADF{a mathematical formula}Ξ=(S,L,Cin), we find that{a mathematical formula}GΞ=TΠ(Ξ).Let {a mathematical formula}X,Y⊆S. We show {a mathematical formula}GΞ′(X,Y)=TΠ(Ξ)′(X,Y).{a mathematical formula}
      </paragraph>
      <paragraph label="Theorem 3.15">
       This result yields immediate correspondence of all operator-based semantics of an ADF Ξ with the respective semantics of its standard logic program {a mathematical formula}Π(Ξ). Let{a mathematical formula}Ξ=(S,L,Cin)be an abstract dialectical framework and{a mathematical formula}Π(Ξ)its standard logic program. Then Ξ and{a mathematical formula}Π(Ξ)coincide on all semantics based on their approximation operators.
      </paragraph>
      <paragraph>
     ...
     <section label="3.3">
      ...
      <paragraph label="Definition 3.5">
       (See Brewka and Woltran [3].) Let Π be a normal logic program over a set A of atoms. Define an ADF {a mathematical formula}Ξ(Π)=(A,L,Cin) as follows.
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}L=def{(b,a)|a←M∈Π,b∈M+∪M−}.
       </list-item>
       <list-item label="•">
        For {a mathematical formula}a∈A, set {a mathematical formula}Cain=def{B⊆par(a)|a←M∈Π,M+⊆B,M−∩B=∅}.
       </list-item>
      </list>
      <paragraph>
       Alternatively, we could define the acceptance condition of each {a mathematical formula}a∈A by{a mathematical formula}
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Example 3.8">
       Lost in translationConsider the following two logic programs over the signature {a mathematical formula}A={a,b,c} that have a common subprogram {a mathematical formula}π={c←∅,b←notb}:
      </paragraph>
      <list>
       <list-item label="1.">
        {a mathematical formula}π1=π∪{a←b,a←c}.
       </list-item>
       <list-item label="2.">
        {a mathematical formula}π2=π∪{a←{b,notc},a←{c,notb},a←{b,c}}.
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph label="Proof">
       For any normal logic program Π, we have{a mathematical formula}TΠ=GΞ(Π).Abbreviate {a mathematical formula}Ξ(Π)=Ξ, let A be the signature of Π and let {a mathematical formula}X,Y⊆A. We show something slightly more general than {a mathematical formula}GΞ(X)=GΞ′(X,X)=TΠ′(X,X)=TΠ(X).
      </paragraph>
      <list>
       <list-item label="1.">
      ...
      <paragraph>
       From the proof we can read off that Π can derive anything that {a mathematical formula}Ξ(Π) can derive, for any three-valued pair; in the converse direction, this only works for two-valued pairs. As an immediate consequence, we get correspondence of two-valued supported models.
      </paragraph>
      <paragraph label="Corollary 3.17">
       Let Π be a normal logic program over a set A of atoms and{a mathematical formula}Ξ=Ξ(Π)be its associated abstract dialectical framework. For any set{a mathematical formula}X⊆A,{a mathematical formula}
      </paragraph>
      <paragraph>
       As another consequence of the proof of Lemma 3.16, we can also show that LP-based ADFs are sound with respect to two-valued stable models of the LP, that is, any stable model of {a mathematical formula}Ξ(Π) is a stable model of Π.
      </paragraph>
      <paragraph label="Proof">
    ...
    <section label="4">
     ...
     <section label="4.1">
      <section-title>
       Fixpoint semantics for abstract argumentation frameworks
      </section-title>
      <paragraph>
      ...
      <paragraph label="Lemma 4.3">
       (See[14, Lemma 45].) For any AF Θ, we have{a mathematical formula}FΘ=UΘ2.
      </paragraph>
      <paragraph>
       For our operator {a mathematical formula}FΘ, this means that for any {a mathematical formula}X,Y⊆A we have{a mathematical formula}
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       In the sequel, we use the term “complete extension” for the set E and the pair {a mathematical formula}(E,E′) interchangeably. It follows by definition that preferred extensions are exactly those consistent fixpoints where E is ⊆-maximal – the M-supported models of {a mathematical formula}FΘ.
      </paragraph>
      <paragraph label="Proposition 4.7">
       Let{a mathematical formula}Θ=(A,R)be an argumentation framework and{a mathematical formula}E⊆A. Then E is a preferred extension of Θ iff for some{a mathematical formula}E′⊆Athe pair{a mathematical formula}(E,E′)is a consistent fixpoint of{a mathematical formula}FΘwhere E is ⊆-maximal.
      </paragraph>
      <paragraph>
       Alternatively, we can say that for a consistent pair {a mathematical formula}(E,E′) the lower bound E is a preferred extension if and only if the pair is M-supported/M-stable for {a mathematical formula}FΘ. This immediately yields a “preferred” semantics for default logic, which is an improvement upon a result by Dung [14, Theorem 43], who defined preferred semantics for default logic only through a translation to infinite AFs.
      </paragraph>
      <paragraph label="Proof">
       Semi-stable extensions are those complete ones where the set of arguments in the upper but not in the lower bound (the undecided arguments) is minimal – L-supported/L-stable models. Let{a mathematical formula}Θ=(A,R)be an argumentation framework and{a mathematical formula}E⊆A. Then E is a semi-stable extension of Θ iff for some{a mathematical formula}E′⊆Athe pair{a mathematical formula}(E,E′)is a consistent fixpoint of{a mathematical formula}FΘwhere{a mathematical formula}E′∖Eis ⊆-minimal.{a mathematical formula}E∪AttackedΘ(E) is ⊆-maximal iff {a mathematical formula}E∪E′¯ is ⊆-maximal iff {a mathematical formula}E∪E′¯¯ is ⊆-minimal iff {a mathematical formula}E¯∩E′ is ⊆-minimal iff {a mathematical formula}E′∖E is ⊆-minimal.  □
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Definition 4.1">
       (See [24, Definition 3.1].) Let {a mathematical formula}Θ=(A,R) be an AF. A JV-labelling is a function {a mathematical formula}l:A→2{+,−} such that for all {a mathematical formula}a∈A:
      </paragraph>
      <list>
       <list-item label="1.">
        If {a mathematical formula}−∈l(a), then there is a {a mathematical formula}b∈A with {a mathematical formula}(b,a)∈R and {a mathematical formula}+∈l(b).
       </list-item>
       <list-item label="2.">
        If {a mathematical formula}+∈l(a), then {a mathematical formula}(b,a)∈R implies {a mathematical formula}−∈l(b).
       </list-item>
       <list-item label="3.">
        If {a mathematical formula}+∈l(a), then {a mathematical formula}(a,c)∈R implies {a mathematical formula}−∈l(c).
       </list-item>
      </list>
      <paragraph>
       It is easy to establish a correspondence between pairs {a mathematical formula}(X,Y) and functions {a mathematical formula}l:A→2{+,−}. It only remains to reformulate the three conditions in terms of AF operators, which results in the proposition below.
      </paragraph>
      <paragraph label="Proposition 4.10">
       Let{a mathematical formula}Θ=(A,R)be an AF and{a mathematical formula}X,Y⊆A.
      </paragraph>
      <list>
       <list-item label="(A)">
        The pair{a mathematical formula}(X,Y)corresponds to a JV-labelling iff{a mathematical formula}X=FΘ′(X,Y)and{a mathematical formula}Y⊆FΘ″(X,Y).
       </list-item>
       <list-item label="(B)">
        The pair{a mathematical formula}(X,Y)is consistent iff its corresponding JV-labelling is total.
       </list-item>
      </list>
      <paragraph label="Proof">
       <list>
     ...
     <section label="4.2">
      <section-title>
       From argumentation frameworks to logic programs
      </section-title>
      <paragraph>
      ...
      <section label="4.2.1">
       ...
       <paragraph label="Corollary 4.11">
        It is immediate from Lemma 3.14 that the associated operators of AFs Θ and their translated logic program {a mathematical formula}Π(Θ) are the same. For any argumentation framework Θ, we have{a mathematical formula}FΘ=TΠ(Θ).
       </paragraph>
       ...
       <list>
        <list-item label="1.">
         the grounded extension of Θ, the Kripke–Kleene model of{a mathematical formula}Π(Θ)and the well-founded model of{a mathematical formula}Π(Θ);
        </list-item>
        <list-item label="2.">
         complete extensions of Θ, three-valued supported models of{a mathematical formula}Π(Θ)and three-valued stable models of{a mathematical formula}Π(Θ);
        </list-item>
        <list-item label="3.">
         preferred extensions of Θ, M-supported models of{a mathematical formula}Π(Θ)and M-stable models of{a mathematical formula}Π(Θ);
        </list-item>
        <list-item label="4.">
         semi-stable extensions of Θ, L-supported models of{a mathematical formula}Π(Θ)and L-stable models of{a mathematical formula}Π(Θ);
        </list-item>
        <list-item label="5.">
         stable extensions of Θ, two-valued supported models of{a mathematical formula}Π(Θ)and two-valued stable models of{a mathematical formula}Π(Θ).
        </list-item>
       </list>
       <paragraph label="Proof">
        The first item is obvious, since they are the least fixpoint of the same operator; the rest follows from Lemma 4.12 and Proposition 4.5, Proposition 4.7, Proposition 4.8, Proposition 4.4.  □
       </paragraph>
       <paragraph>
      ...
      <section label="4.2.2">
       ...
       <paragraph>
        Hence for any semantics derived from the operator {a mathematical formula}TΠD which is only “interested” in atoms from A, the choice between standard translation and Dung translation is semantically inessential. We remark that Dungʼs translation has the advantage of producing a logic program where each rule has a finite body.
       </paragraph>
       <paragraph label="Theorem 4.16">
        Theorem 4.14 and Proposition 4.15 immediately yield the same nice correspondence picture from the standard translation (Theorem 4.13) for Dungʼs translation. The first and last items below are again obvious for our setting, parts of them were also proved by Dung [14, Theorem 62]. Correspondence results 2, 3 and 4 are completely new. Let{a mathematical formula}Θ=(A,R)be an argumentation framework. The following are in one-to-one correspondence:
       </paragraph>
       <list>
        <list-item label="1.">
         the grounded extension of Θ, the Kripke–Kleene model of{a mathematical formula}ΠD(Θ)and the well-founded model of{a mathematical formula}ΠD(Θ);
        </list-item>
        <list-item label="2.">
         complete extensions of Θ, three-valued supported models of{a mathematical formula}ΠD(Θ)and three-valued stable models of{a mathematical formula}ΠD(Θ);
        </list-item>
        <list-item label="3.">
         preferred extensions of Θ, M-supported models of{a mathematical formula}ΠD(Θ)and M-stable models of{a mathematical formula}ΠD(Θ);
        </list-item>
        <list-item label="4.">
         semi-stable extensions of Θ, L-supported models of{a mathematical formula}ΠD(Θ)and L-stable models of{a mathematical formula}ΠD(Θ);
        </list-item>
        <list-item label="5.">
         stable extensions of Θ, two-valued supported models of{a mathematical formula}ΠD(Θ)and two-valued stable models of{a mathematical formula}ΠD(Θ).
        </list-item>
       </list>
       <paragraph label="Proof">
        Follows from Theorem 4.14, Proposition 4.15 and Proposition 4.5, Proposition 4.7, Proposition 4.8, Proposition 4.4.  □
       </paragraph>
       <paragraph>
        This theorem conclusively shows that Dungʼs modular translation from AFs to LPs is faithful with respect to all operator-based semantics. We infer that propositional normal logic programs are at least as expressive as abstract argumentation frameworks.
       </paragraph>
      </section>
     </section>
     <section label="4.3">
    ...
    <section label="5">
     <section-title>
      General semantics for approximating operators
     </section-title>
     <paragraph>
     ...
     <section label="5.2">
      ...
      <paragraph>
       In the sequel, for a complete lattice {a mathematical formula}(L,⊑) with join ⊔ and meet ⊓, we assume the existence of a function {a mathematical formula}⋅−1:L→L such that for any {a mathematical formula}x,y∈L,
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}(x−1)−1=x ({a mathematical formula}⋅−1 is involutive),
       </list-item>
       <list-item label="•">
        {a mathematical formula}(x⊔y)−1=x−1⊓y−1 and {a mathematical formula}(x⊓y)−1=x−1⊔y−1 (de Morganʼs laws).
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph>
       For the special case of argumentation, these general definitions of L-supported and L-stable reduce to a consistent fixpoint {a mathematical formula}(S,P) of {a mathematical formula}FΘ=SFΘ such that {a mathematical formula}P∩S¯=P∖S (the set of undecided arguments) is ⊆-minimal – a semi-stable extension.
      </paragraph>
     </section>
     <section label="5.3">
      <section-title>
      ...
      <paragraph>
       In classical abstract argumentation, a set of arguments is conflict-free if there are no attacks amongst its members. For abstract dialectical frameworks, a set of statements is conflict-free if each statement – informally speaking – has a reason to be in the set. This reason for one entails absence of any attackers as well as presence of supporters in case the statementʼs acceptance conditions so requires.
      </paragraph>
      <paragraph>
       To generalise this notion to three-valued pairs {a mathematical formula}(X,Y), we require two things:
      </paragraph>
      <list>
       <list-item label="•">
      ...
      <paragraph label="Definition 5.3">
       Let {a mathematical formula}(L,⊑) be a complete lattice and {a mathematical formula}O an approximating operator on the bilattice {a mathematical formula}(L2,⩽i). A consistent pair {a mathematical formula}(x,y)∈L2 is conflict-free for{a mathematical formula}O iff {a mathematical formula}x⊑O″(x,y)⊑y.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Proof">
       Let{a mathematical formula}Θ=(A,R)be an AF and{a mathematical formula}X⊆A. X is conflict-free iff{a mathematical formula}(X,UΘ(X))is a conflict-free pair.{a mathematical formula}
      </paragraph>
      <paragraph>
       Indeed, this notion of conflict-free pairs for AFs coincides with Caminadaʼs definition of conflict-free labellings [7].
      </paragraph>
      <paragraph label="Proposition 5.3">
       For any AF{a mathematical formula}Θ=(A,R)and{a mathematical formula}X⊆Y⊆A, the pair{a mathematical formula}(X,Y)is conflict-free iff the labelling{a mathematical formula}l:A→{in,out,undec}with{a mathematical formula}X↦in,{a mathematical formula}Y∖X↦undec,{a mathematical formula}A∖Y↦outis conflict-free in the sense of[7, Definition 3].
      </paragraph>
     </section>
     <section label="5.4">
      <section-title>
      ...
      <paragraph>
       It is clear that conflict-free pairs are amenable to the usual maximisation criteria that lead from admissible to preferred and semi-stable semantics. We begin with naive semantics, which for AFs are just ⊆-maximal conflict-free sets.
      </paragraph>
     ...
     <section label="5.5">
      ...
      <paragraph label="Proof">
       Let{a mathematical formula}Θ=(A,R)be an AF. A set{a mathematical formula}X⊆Ais a stage extension of Θ iff the pair{a mathematical formula}(X,UΘ(X))is an L-stage pair of{a mathematical formula}FΘ.{a mathematical formula}
      </paragraph>
      <paragraph>
       When trying to define two-valued conflict-free pairs we end up with pairs {a mathematical formula}(x,x) where {a mathematical formula}x⊑O″(x,x)=O′(x,x)⊑x, that is, two-valued supported models.
      </paragraph>
     </section>
    </section>
   </content>
