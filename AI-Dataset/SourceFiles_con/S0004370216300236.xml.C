  ...
  <root>
   ...
   <content>
    ...
    <section label="3">
     ...
     <paragraph>
      This section introduces the general model and algorithm used in our work, and further elaborates on connections to other related works.
     </paragraph>
     <section label="3.1">
      <section-title>
      ...
      <paragraph>
       We model the interaction process as a stochastic Bayesian game (SBG) [4] which can be viewed as a combination of the Bayesian game [56] and the stochastic game [81]. This combination is useful because it allows us to study the type-based method of interaction via the established framework of Bayesian games while also providing a means to specify an environment (via states) and the task to be completed. The structural definition of SBGs is as follows:
      </paragraph>
      ...
      <list>
       <list-item label="•">
        finite state space S with initial state {a mathematical formula}s0∈S and terminal states {a mathematical formula}S¯⊂S
       </list-item>
       <list-item label="•">
        players {a mathematical formula}N={1,…,n} and for each {a mathematical formula}i∈N:
       </list-item>
       <list-item label="•">
        state transition function {a mathematical formula}T:S×A×S→[0,1]
       </list-item>
       <list-item label="•">
        type distribution {a mathematical formula}ϒ:Θ+→[0,1], where {a mathematical formula}Θ+ is a finite subset of Θ
       </list-item>
      </list>
      <paragraph>
    ...
    <section label="4">
     ...
     <paragraph>
      A central aspect of the type-based method are the beliefs over types. Beginning with some initial beliefs about the relative likelihood of types, we compare the predictions of types with the observed actions and update our beliefs to reflect the given evidence. Associated with this process are two key questions: how may evidence be incorporated into beliefs, and under what conditions will the beliefs be correct? As can be seen in Algorithm 1, these are important questions since the accuracy of the expected payoffs (3) depends on the accuracy of the posterior belief Pr.
     </paragraph>
     <paragraph>
      In this section, we consider three classes of type distributions to cover a broad spectrum of scenarios: pure distributions, in which all agents have a fixed type; mixed distributions, in which types are randomly re-allocated; and correlated distributions, in which type assignments may be correlated. Corresponding to these classes, we consider three formulations of posterior beliefs which prescribe different ways to incorporate evidence into beliefs. We provide theoretical conditions under which these formulations produce correct beliefs, and we provide examples to show when they may fail to do so.
     </paragraph>
     <paragraph>
      Our definition of correctness is with respect to the type distribution ϒ: beliefs are said to be correct if they assign the same probabilities to true types as ϒ. This requires that the beliefs can point to the types in the support of the type distribution. Therefore, the results in this section pertain to a situation in which the user knows that the true type space {a mathematical formula}Θj+ must be a subset of the hypothesised type space {a mathematical formula}Θj⁎. Formally, we assume:
     </paragraph>
     <paragraph label="Assumption 1">
      {a mathematical formula}∀j≠i:Θj+⊆Θj⁎.
     </paragraph>
     <paragraph>
      The case in which beliefs cannot be correct as defined above, due to incomplete or incorrect hypothesised types, is examined in Sections 6 and 7.
     </paragraph>
     <paragraph>
      Finally, recall from Section 3.2 that posterior beliefs of the form (1) assume independence of player types. That is, they assume that the type distribution ϒ can be represented as a product of n independent factors {a mathematical formula}ϒj (one for each player), such that {a mathematical formula}ϒ(θ)=∏jϒj(θj). Hence, in the following, unless states otherwise, we assume that ϒ satisfies this independence property. Section 4.3 also considers the case of correlated type distributions.
     </paragraph>
     <section label="4.1">
      <section-title>
       Product posterior
     ...
     <section label="4.3">
      ...
      <paragraph label="Theorem 3">
       Let Γ be a SBG with a correlated type distribution ϒ. If HBA uses the correlated posterior, then, for{a mathematical formula}t→∞: If{a mathematical formula}AOj(Ht)=0and{a mathematical formula}ASj(Ht)=0for all players{a mathematical formula}j≠i, then{a mathematical formula}Pr(θ−i|Ht)=ϒ(θ−i)for all{a mathematical formula}θ−i∈Θ−i+.
      </paragraph>
      <paragraph label="Proof">
    ...
    <section label="5">
     ...
     <paragraph>
      The previous section was concerned with the evolution of posterior beliefs as we observe more evidence. However, before we observe any evidence based on which to form our posterior beliefs, we will have to make an initial judgement as to the relative likelihood of types. This initial judgement is called the prior belief.
     </paragraph>
     <paragraph>
      Given the lack of evidence, it may be tempting to use uniform prior beliefs in which all types have equal probability. Indeed, the fact that beliefs can change rapidly after only a few observations suggests that prior beliefs may have negligible effect. On the other hand, there is a substantial body of work in the game theory literature arguing the importance of prior beliefs (cf. Section 2). However, these works consider the impact of prior beliefs on equilibrium attainment when all players use the same type-based reasoning. In contrast, our interest is in the practical impact of prior beliefs, i.e. payoff maximisation, for a single agent using the type-based method.
     </paragraph>
     <paragraph>
      In addition, there is the work of Bernardo [17], Jaynes [62], and others on “uninformed” priors. The purpose of such priors is to express a state of complete uncertainty, whilst possibly incorporating subjective prior information. (What this means and whether this is possible has been the subject of a long debate, e.g. De Finetti [33].) However, this again differs from our interest in the impact of prior beliefs on payoff maximisation.
     </paragraph>
     <paragraph>
     ...
     <paragraph>
      The following subsections describe the experimental setup used in our study. The results are discussed in Section 5.7.
     </paragraph>
     <section label="5.1">
      <section-title>
       Games
     ...
     <section label="5.2">
      ...
      <paragraph>
       Average payoff Average of payoffs an agent received in the time slice. Returns value in {a mathematical formula}[1,4] for each agent.
      </paragraph>
      <paragraph>
       Welfare and fairness Average sum and product, respectively, of the joint payoffs received in the time slice. Returns values in {a mathematical formula}[2,8] and {a mathematical formula}[1,16], respectively.
      </paragraph>
      <paragraph>
       Game solutions Tests if the averaged action probabilities in the time slice formed an approximate stage-game Nash equilibrium, Pareto optimum, Welfare optimum, or Fairness optimum. Returns 1 (true) or 0 (false) for each game solution.
      </paragraph>
      <paragraph>
       Precise formal definitions of these performance criteria can be found in [3].
      </paragraph>
     </section>
     <section label="5.3">
      <section-title>
     ...
     <section label="5.5">
      ...
      <paragraph>
       While this is a mathematically rigorous formulation, it is important to note that it is a simplification of how HBA really works. HBA incorporates its beliefs in every recursion of its planning procedure, whereas the LP formulation implicitly assumes that HBA uses its prior beliefs to randomly sample one of the types against which it then plans optimally. Nonetheless, this is often a reasonable approximation.
      </paragraph>
     </section>
     <section label="5.6">
      <section-title>
      ...
      <paragraph>
       We performed identical experiments for every type generation method described in Section 5.4. Each of the 78 games was played 10 times with different random seeds, and each play was repeated against three opponents (30 plays in total): (RT) A randomly generated type was used to control player 2 and the play lasted 100 rounds. (FP) A fictitious player [23] was used to control player 2 and the play lasted 10 000 rounds. (CFP) A conditioned fictitious player (which learns action distributions conditioned on the previous joint action) was used to control player 2 and the play lasted 10 000 rounds.
      </paragraph>
      <paragraph>
     ...
     <section label="5.7">
      ...
      <paragraph label="Observation 1">
       Prior beliefs can have a significant impact on the long-term performance of HBA.
      </paragraph>
      <paragraph>
       This was observed in all classes of types, against all classes of opponents, and in all classes of games used in this study. Fig. 2 provides three representative examples from a range of scenarios. Many of the relative differences due to prior beliefs were statistically significant, based on paired two-sided t-tests with a 5% significance level.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Observation 2">
       Deeper planning horizons can diminish and amplify the impact of prior beliefs.
      </paragraph>
      <paragraph>
       Again, this was observed in all tested scenarios. Fig. 3, Fig. 4 show examples in which deeper planning horizons diminish and amplify the impact of prior beliefs, respectively.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Observation 3">
       Automatic methods can compute prior beliefs with consistent performance effects.
      </paragraph>
      <paragraph>
       Fig. 5 shows that the prior beliefs had consistent performance effects across a wide variety of scenarios. For example, the Utility prior produced consistently higher payoffs for player 1 (i.e. HBA) while the Stackelberg prior produced consistently higher payoffs for player 2 as well as higher welfare and fairness. The Welfare and Fairness priors were similar to the Stackelberg prior, but not quite as consistent. Similar results were observed for the LP variants of the priors, despite the fact that the LP formulation is a simplification of how HBA works (cf. Section 5.5).
      </paragraph>
      <paragraph>
       We note that none of the prior beliefs, including the Uniform prior, produced high rates for the game solutions (i.e. Nash equilibrium, Pareto optimality, etc.). This is because we measured stage-game solutions, which have no notion of time. These can be hard to attain in repeated games, especially if the other player does not actively seek a specific solution, as was often the case in our study.
      </paragraph>
      <paragraph>
    ...
    <section label="6">
     ...
     <section label="6.3">
      <section-title>
       Critical type spaces
      </section-title>
      <paragraph>
       In our analysis, we will sometimes assume that the hypothesised type spaces {a mathematical formula}Θj⁎ are uncritical:
      </paragraph>
      <paragraph label="Definition 9">
       The hypothesised type spaces {a mathematical formula}Θj⁎ are critical if there is a set {a mathematical formula}Sc⊆S∖S¯ which satisfies all of the following:
      </paragraph>
      <list>
       <list-item label="1.">
        For each {a mathematical formula}Ht∈H with {a mathematical formula}st∈Sc, there is {a mathematical formula}ai∈Ai such that {a mathematical formula}Estai(Ht|Y)&gt;0 and {a mathematical formula}Estai(Ht|X)&gt;0.
       </list-item>
       <list-item label="2.">
        There is a positive probability that Y may eventually get into a state {a mathematical formula}sc∈Sc from {a mathematical formula}s0.
       </list-item>
       <list-item label="3.">
        If Y is in a state in {a mathematical formula}Sc, then with probability 1 it will always be in a state in {a mathematical formula}Sc.
       </list-item>
      </list>
      <paragraph>
       Intuitively, critical type spaces have the potential to lead HBA into a state space in which it believes it chooses the right actions to complete the task, while other actions are actually required to complete the task. The only effect that its actions have is to induce an infinite cycle, due to a critical inconsistency between the hypothesised and true type spaces. The following example demonstrates this:
      </paragraph>
      <paragraph label="Example 7">
       Recall Example 6 and let the task be to choose the same action as player j. Then, {a mathematical formula}Θj⁎ is uncritical because HBA will always complete the task at {a mathematical formula}t=1, regardless of its posterior beliefs and despite the fact that {a mathematical formula}Θj⁎ is inaccurate. Now, assume that {a mathematical formula}Θj⁎={θRL⁎} where {a mathematical formula}θRL⁎ chooses actions R,L,R,L etc. Then, {a mathematical formula}Θj⁎ is critical since HBA will always choose the opposite action of player j, thinking that it would complete the task, when a different action would actually complete it.
      </paragraph>
      <paragraph>
     ...
     <section label="6.4">
      ...
      <paragraph label="Property 3">
       {a mathematical formula}s0⊨XF≥p&lt;∞term⇒s0⊨YF≥p&lt;∞term.
      </paragraph>
      <paragraph>
       We can show that Property 3 holds if the hypothesised type spaces {a mathematical formula}Θj⁎ are uncritical and if Y only chooses actions for player i which X might have chosen as well.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Property 4">
       {a mathematical formula}s0⊨XF≥p≤tterm⇒s0⊨YF≥p≤tterm.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       What relation must Y have to X in order to satisfy Property 4? The fact that Y and X are processes over state transition systems means that we can draw on methods from the model checking literature to answer this question. Specifically, we will use the concept of probabilistic bisimulation[69], which we here define within the context of our work:
      </paragraph>
      <paragraph label="Definition 10">
       A probabilistic bisimulation between X and Y, denoted {a mathematical formula}X∼Y, is an equivalence relation {a mathematical formula}B⊆S×S such that
      </paragraph>
      <list>
       <list-item label="(i)">
        {a mathematical formula}(s0,s0)∈B
       </list-item>
       <list-item label="(ii)">
        {a mathematical formula}sX⊨Xterm⇔sY⊨Yterm for all {a mathematical formula}(sX,sY)∈B
       </list-item>
       <list-item label="(iii)">
        {a mathematical formula}μ(HXt,Sˆ|X)=μ(HYt,Sˆ|Y) for any histories {a mathematical formula}HXt,HYt with {a mathematical formula}(sXt,sYt)∈B and all equivalence classes {a mathematical formula}Sˆ under B.
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph>
       The main contribution in this section is to show that the optimality criterion expressed by Property 4 holds in both directions if there exists a probabilistic bisimulation between X and Y. Thus, we offer an alternative formal characterisation of optimality for the hypothesised type spaces {a mathematical formula}Θj⁎:
      </paragraph>
      <paragraph label="Theorem 7">
       Property 4holds in both directions if there exists a probabilistic bisimulation{a mathematical formula}X∼Y.
      </paragraph>
      <paragraph label="Proof">
    ...
    <section label="7">
     ...
     <section label="7.1">
      <section-title>
       Individual hypotheses and beliefs
      </section-title>
      <paragraph>
     ...
     <section label="7.2">
      ...
      <paragraph>
       In the following subsections, we describe the test statistic T and its asymptotic properties, and how our algorithm learns the distribution of {a mathematical formula}T(a˜jt,aˆjt). A summary of the algorithm is given in Algorithm 2.
      </paragraph>
      <section label="7.2.1">
       <section-title>
        Test statistic
      ...
      <section label="7.2.3">
       ...
       <paragraph>
        What is needed is a distribution which can represent any normal distribution, and which is flexible enough to faithfully represent the gradual shaping. One distribution which has these properties is the skew-normal distribution[9], [77]. Given the PDF ϕ and CDF Φ of the standard normal distribution, the skew-normal PDF is defined as{a mathematical formula} where {a mathematical formula}ξ∈R is the location parameter, {a mathematical formula}ω∈R+ is the scale parameter, and {a mathematical formula}β∈R is the shape parameter. Note that this reduces to the normal PDF for {a mathematical formula}β=0, in which case ξ and ω correspond to the mean and standard deviation, respectively. Hence, the normal distribution is a sub-class of the skew-normal distribution.
       </paragraph>
       <paragraph>
       ...
       <paragraph>
        Given the asymmetry of the skew-normal distribution, the semantics of “as extreme as” in (24) may no longer be obvious (e.g. is this with respect to the mean or mode?). In addition, the usual tail-area calculation of the p-value requires the CDF, but there is no closed form for the skew-normal CDF and approximating it is rather cumbersome. To circumvent these issues, we approximate the p-value as{a mathematical formula} where μ is the mode of the fitted skew-normal distribution. This avoids the asymmetry issue and is easier to compute.
       </paragraph>
      </section>
     </section>
     <section label="7.3">
