  ...
  <root>
   ...
   <content>
    <section label="1">
     ...
     <paragraph>
      The ‘evidence to the contrary’ can vary. Sometimes it simply consists in the empirical observation that the object concerned is in fact an exception to the rule. On other occasions the evidence may be more indirect. Consider:{a mathematical formula}
     </paragraph>
     <paragraph>
    ...
    <section label="2">
     ...
     <paragraph label="Definition 2.1">
      <list>
       <list-item label="(i)">
        Let {a mathematical formula}L be a language as described above, and let {a mathematical formula}A=〈A,I〉 and {a mathematical formula}A′=〈A′,I′〉 be two models for {a mathematical formula}L with the following properties:
       </list-item>
       <list-item label="(ii)">
        Let {a mathematical formula}C be a class of models. Then {a mathematical formula}A=〈A,I〉 is an optimal model in {a mathematical formula}C iff {a mathematical formula}A∈C and there is no model in {a mathematical formula}C that is more normal than {a mathematical formula}A.
       </list-item>
       <list-item label="(iii)">
        Let Δ be a set of sentences. Then {a mathematical formula}Δ⊨cφ iff φ is true in all optimal models of Δ.
       </list-item>
      </list>
     </paragraph>
     <paragraph>
      Notice that in (i) of this definition nothing is said about the interpretation of ordinary predicates. {a mathematical formula}A can be at least as normal as {a mathematical formula}A′, while for all {a mathematical formula}P∈L0, the interpretations {a mathematical formula}I(P) and {a mathematical formula}I′(P) are totally different. However, in practice we are always looking for the most normal models within a given class {a mathematical formula}C, and it may very well happen that within {a mathematical formula}C the interpretation of the ordinary predicates is heavily constrained or even fixed.
     </paragraph>
     <paragraph>
      If {a mathematical formula}Δ⊨cφ, we say that φ follows by circumscription from Δ. Here is an example.{a mathematical formula}
     </paragraph>
     <paragraph>
      This can be formalized as{a mathematical formula}
     </paragraph>
     <paragraph>
     ...
     <paragraph>
      In the next section we will see how one can enforce that in all models in which these three defaults hold, also the formula {a mathematical formula}∀x(Sx→AbAEx) will be true. Once we have this, the only optimal models will be models in which {a mathematical formula}¬AbSAc, {a mathematical formula}¬AbS¬Ec, and {a mathematical formula}¬AbAEc are true, which means that the conclusion follows.
     </paragraph>
     <paragraph>
      The circumscriptive theory developed below differs from other circumscriptive theories in various ways. We will illustrate these differences by comparing our set up with the set up of the theory developed in [4] and [5] by Bonatti et al. for description logic.{sup:5}
     </paragraph>
     <paragraph>
      Bonatti et al. introduce abnormality predicates with only one index, and write for example Whale ⊑ {a mathematical formula}Abmammal to indicate that whales are abnormal mammals. As we pointed out above, this way it gets difficult, if not impossible, to deal properly with different – independent – default properties of the same kind of objects. Unfortunately, the paper does not discuss this problem.{sup:6}
     </paragraph>
     <paragraph>
    ...
    <section label="3">
     ...
     <list>
      <list-item label="(i)">
       Suppose {a mathematical formula}A⊨∀x(φ(x)↝ψ(x)), and let d be an element of the domain{sup:8} of {a mathematical formula}A. Then d complies with{a mathematical formula}∀x(φ(x)↝ψ(x)) (in {a mathematical formula}A) iff d does not satisfy {a mathematical formula}Abφψx.
      </list-item>
      <list-item label="(ii)">
       Let Σ be a set of rules, and let d be some element of the domain of some model {a mathematical formula}A of Σ. Then d complies with Σ (in {a mathematical formula}A) iff d complies with all the default rules in Σ.
      </list-item>
     </list>
     <paragraph>
      So, if an object satisfying {a mathematical formula}φ(x) complies with {a mathematical formula}∀x(φ(x)↝ψ(x)), it will also satisfy {a mathematical formula}ψ(x). But notice that the definition allows for the following situations:
     </paragraph>
     <list>
      <list-item label="•">
       The object d complies with {a mathematical formula}∀x(φ(x)↝ψ(x)), but d does not satisfy {a mathematical formula}φ(x).
      </list-item>
      <list-item label="•">
       The object d satisfies {a mathematical formula}φ(x) and {a mathematical formula}ψ(x), but d does not comply with {a mathematical formula}∀x(φ(x)↝ψ(x)).
      </list-item>
     </list>
     <paragraph>
     ...
     <section label="3.1">
      ...
      <paragraph>
       The constraint we will impose on {a mathematical formula}UI is motivated by the following minimal requirement. If the only information about some object is that it has property P, it must be valid to infer by default that this object complies with all the default rules for objects with property P.{sup:9} What would be the use of these rules if they would not at least allow this inference?
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Definition 3.2">
       <list>
        <list-item label="(i)">
         An exemption clause is a formula of the form {a mathematical formula}∀x(φ(x)→⋁δ∈ΔAbδx).Here we write ‘{a mathematical formula}Abδ’ to refer to the abnormality predicate of the default rule δ. Δ can be any finite set of default rules. So, {a mathematical formula}⋁δ∈ΔAbδx is a disjunction of a finite number of abnormality predicates.{sup:10}
        </list-item>
        <list-item label="(ii)">
         Let Σ be a set of rules. {a mathematical formula}Σφ(x) is the set of rules in Σ with antecedent {a mathematical formula}φ(x).
        </list-item>
        <list-item label="(iii)">
         The exemption clause {a mathematical formula}∀x(φ(x)→⋁δ∈ΔAbδx) is an exemption clause for Σ iff {a mathematical formula}Σ⊨∀x(φ(x)→⋁δ∈Δ∪Σφ(x)Abδx).{sup:11}
        </list-item>
       </list>
      </paragraph>
      <paragraph>
       To see how these definitions work, consider again{a mathematical formula} Here {a mathematical formula}ΣSx={∀x(Sx↝Ax),∀x(Sx↝¬Ex)}. Let {a mathematical formula}Δ={∀x(Ax↝Ex)}. Clearly, there is no model such that some object in its domain satisfies Sx and complies with {a mathematical formula}Δ∪ΣSx. So,{a mathematical formula}
      </paragraph>
      <paragraph>
       By (iii) above this means that {a mathematical formula}∀x(Sx→⋁δ∈ΔAbδx), i.e. {a mathematical formula}∀x(Sx→AbAEx), is an exemption clause for Σ, the idea being that objects with property S are, so to speak, exempted from the rule that A's are normally E.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       ‘Being exempted from a rule’ does not mean ‘being an exception to the rule’: The {a mathematical formula}S's don't have to be E, but this does not mean they are, in fact, not E.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Example">
       Applying Proposition 3.4 we find that{a mathematical formula}
      </paragraph>
     </section>
     <section label="3.2">
      <section-title>
      ...
      <list>
       <list-item>
        Let Σ be a set of rules. Suppose that {a mathematical formula}Σ∈⊨∀x(φ(x)↝ψ(x))
       </list-item>
       <list-item>
        and {a mathematical formula}Σ∈⊨∀x(ψ(x)→Abχθx). Then {a mathematical formula}Σ∈⊨∀x(φ(x)→Abχθx).
       </list-item>
      </list>
      <paragraph>
       To see how this works, consider the theory Σ consisting of the following five rules{a mathematical formula}
      </paragraph>
      <paragraph>
       Consider the first three rules, and notice that the exemption principle enforces that {a mathematical formula}∀x(Px→AbQRx)∈Σ∈. Now, it follows in one step from the inheritance principle that {a mathematical formula}∀x(Sx→AbQRx)∈Σ∈.
      </paragraph>
      <paragraph>
       Without an appeal to the inheritance principle the proof is a bit longer. We give it here because it illustrates why Definition 3.3 (i) is inductive. Note first that {a mathematical formula}∀x(Px→AbQRx)∈Σ1∈. Given that {a mathematical formula}⊨∀x(Sx→(Px∨¬Px)), it follows that {a mathematical formula}Σ1∈⊨∀x(Sx→(AbQRx∨AbSPx)). But then {a mathematical formula}∀x(Sx→AbQRx)∈Σ2∈.
      </paragraph>
      <paragraph>
       By applying the exemption principle to the last three rules we find that {a mathematical formula}∀x(Sx→AbP¬Rx)∈Σ∈. So, the S's, which in optimal circumstances are both P and Q, are exempted both from the rule that P's are normally not R and from the rule that Q's are normally R. They are normally R for independent reasons. As such, the example illustrates the fact that it is possible for an object not to comply with a rule whereas both the antecedent and the consequent of the rule hold for it. Objects with the property S do not comply with the rule {a mathematical formula}∀x(Qx↝Rx), but in optimal circumstances they will have both the properties Q and R.
      </paragraph>
      <paragraph>
       In its general form the inheritance principle runs as follows.
      </paragraph>
      <paragraph label="Proposition 3.5">
       Inheritance principleLet{a mathematical formula}〈UI,FI〉be the state generated by the information{a mathematical formula}I=〈Σ,Π〉. Let{a mathematical formula}Δ⊆Σbe a set of default rules.Suppose{a mathematical formula}Then{a mathematical formula}
      </paragraph>
      <paragraph label="Proof">
     ...
     <section label="3.3">
      ...
      <section label="3.3.2">
       <section-title>
        Defeasible modus tollens
       </section-title>
       <paragraph>
        Both Defeasible Modus Ponens and Defeasible Modus Tollens are valid.{a mathematical formula}
       </paragraph>
       <paragraph>
        The latter shows that an object need not have property S to count as an object that complies with the rule {a mathematical formula}∀x(Sx↝Px). Intuitively, if the object c had property S, it would be an abnormal S. So, assuming that the object c is normal and complies with the rule, it will not have property S.
       </paragraph>
      </section>
      <section label="3.3.3">
       <section-title>
     ...
     <section label="3.4">
      ...
      <paragraph>
       Every set Σ of default rules is consistent in the strict logical sense of the word.{sup:14} This does not mean that every such set is okay. Here are some examples.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Definition 3.6">
       A set of rules Σ is coherent iff for every {a mathematical formula}φ(x) which is the antecedent of some rule in Σ, {a mathematical formula}Σ∈∪{∃xφ(x)} is consistent.
      </paragraph>
      <paragraph>
     ...
     <section label="3.5">
      <section-title>
       Is the exemption principle all there is to it?
      </section-title>
      <paragraph>
      ...
      <paragraph>
       One might think that the next example shows that the answer is ‘no’.{sup:15} Consider the following set of premises.{a mathematical formula}
      </paragraph>
      <paragraph>
    ...
    <section label="4">
     ...
     <paragraph label="Definition 4.2">
      Let {a mathematical formula}〈V,Σ〉 be an inheritance network, with {a mathematical formula}a,b∈V.
     </paragraph>
     <list>
      <list-item label="(i)">
       A positive path from a to b is a subset {a mathematical formula}{α1,…,αn}⊆Σ such that there exist {a mathematical formula}v1,…,vn−1∈V such that:
       <list>
        {a mathematical formula}α1 is a positive (or strict positive) arc from a to {a mathematical formula}v1;{a mathematical formula}αi is a positive (or strict positive) arc from {a mathematical formula}vi−1 to {a mathematical formula}vi, where {a mathematical formula}1&lt;i&lt;n;{a mathematical formula}αn is a positive (or strict positive) arc from {a mathematical formula}vn−1 to b.Moreover, the empty set is considered a positive path from any
       </list>
       <paragraph>
        {a mathematical formula}v∈V to itself.
       </paragraph>
      </list-item>
      <list-item label="(ii)">
       {a mathematical formula}X⊆Σ is a negative path from a to b if there are {a mathematical formula}X1, {a mathematical formula}X2, {a mathematical formula}a′, {a mathematical formula}b′, α such that
      </list-item>
     </list>
     <paragraph>
     ...
     <section label="4.1">
      <section-title>
       Making inferences in inheritance networks
      </section-title>
      <paragraph>
       Let {a mathematical formula}〈V,Σ〉 be an inheritance network, and {a mathematical formula}u,v∈V. We will write ‘{a mathematical formula}u⇌v’ to indicate that there is both a positive path from u to v and a positive path from v to u.
      </paragraph>
      <paragraph label="Definition 4.4">
       Where {a mathematical formula}〈V,Σ〉 is an inheritance network and {a mathematical formula}a∈V, let {a mathematical formula}EssΣ(a) be set of arcs defined by
      </paragraph>
      <list>
       <list-item>
        {a mathematical formula}α∈EssΣ(a) iff α∈Σ and (i) α is strict,
       </list-item>
       <list-item>
        or (ii) {a mathematical formula}α=uv for some u⇌a,
       </list-item>
       <list-item>
        or (iii) {a mathematical formula}α=uv− for some u⇌a.
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph label="Definition 4.5">
       Where {a mathematical formula}〈V,Σ〉 is an inheritance network and {a mathematical formula}a∈V, let {a mathematical formula}d(a) be defined by
      </paragraph>
      <list>
       <list-item>
        {a mathematical formula}Y∈d(a) iff Y=X−EssΣ(a) for some minimal conflicting set X relative to a.
       </list-item>
      </list>
      <paragraph>
       The intuition is that the objects with property a are exempted from at least one rule in every set {a mathematical formula}Y∈d(a).
      </paragraph>
      <paragraph>
      ...
      <list>
       <list-item>
        {a mathematical formula}Y∈D(a) iff {a mathematical formula}Y∈d(b) for some b for which there is a positive path from a to b.
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph label="Definition 4.7">
       Let {a mathematical formula}〈V,Σ〉 an inheritance network and {a mathematical formula}a∈V.{a mathematical formula}X⊆Σ is an acceptable exception set for a iff {a mathematical formula}X∩Y≠∅ for all {a mathematical formula}Y∈D(a).Such an X is minimal if every proper subset of X is not an acceptable exception set for a.
      </paragraph>
      <paragraph label="Definition 4.8">
       Each minimal exception set represents a way to make as few exceptions as possible. A given conclusion b follows from a in a network if b can be reached from a under each of these ways. Let {a mathematical formula}〈V,Σ〉 be an inheritance network. Let {a mathematical formula}a,b∈V.
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}a⊢Σb iff for every minimal exception set X for a there is a positive path Y from a to b such that {a mathematical formula}X∩Y=∅.
       </list-item>
       <list-item label="•">
        {a mathematical formula}a⊢Σ¬b iff at least one of the following is true:
       </list-item>
      </list>
      <paragraph>
       We did not prepare the reader for the second clause of negative entailment. It is there for the special case in which there is no path from a to b. In such a case it may happen that objects with property b are so abnormal that one can safely assume that objects with property a{sup:18} do not have property b.
      </paragraph>
      <paragraph>
       To see how this works, notice first that if there is a path from a to b, {a mathematical formula}D(b)⊆D(a). So in that case every minimal exception set for a is an acceptable exception set for b.{sup:19} Now, if there is no path from a to b, and some minimal exception set for a is an acceptable exception set for b, this means that one has to reckon with the possibility that objects with property b are in all respects at least as normal as objects with property a. However, if no minimal exception set is an acceptable exception set for b, this means that the a's are normal in some respect in which the b's are exceptional. Assuming that the a's are as normal as possible, one may in such a case by default infer that the a's don't have the property b.{sup:20}
      </paragraph>
     </section>
     <section label="4.2">
      <section-title>
      ...
      <section label="4.2.1">
       ...
       <paragraph>
        {a mathematical formula} Therefore as desired we obtain {a mathematical formula}J⊢Σ¬E, {a mathematical formula}J⊢ΣA, {a mathematical formula}J⊢ΣB.
       </paragraph>
      </section>
      <section label="4.2.2">
       <section-title>
       ...
       <paragraph>
        The following network is a well-known extension of the Nixon Diamond, generally referred to as the Double Diamond.{a mathematical formula} The question is whether Nixon is Anti-military.
       </paragraph>
       <paragraph>
        In preemption based approaches (notably Horty et al. [3]), the positive path from N to A is disabled by the negative path from N to P, so that ¬A may be concluded. This outcome is considered counterintuitive since the negative path to A is itself disabled by its positive counterpart, which is why such paths are referred to as zombie paths. (See Makinson &amp; Schlechta [11].)
       </paragraph>
       <paragraph>
       ...
       <paragraph>
        We trivially obtain {a mathematical formula}N⊢ΣR, {a mathematical formula}N⊢ΣQ. But as to the other properties, nothing can be concluded, not even {a mathematical formula}N⊢ΣF.
       </paragraph>
      </section>
      <section label="4.2.3">
       <section-title>
       ...
       <paragraph>
        Also the next example is much discussed in the literature on inheritance networks.{sup:21}{a mathematical formula}
       </paragraph>
       <paragraph>
      ...
      <section label="4.2.4">
       ...
       <paragraph>
        The theory covers inheritance networks with cyclic paths. Here is an example.{a mathematical formula}
       </paragraph>
       <paragraph>
        At first glance, one might expect {a mathematical formula}d(A)={{DE−,BE}}. However, this is not the case.
       </paragraph>
       <paragraph>
        Since all points of the loop must be taken into account, we have {a mathematical formula}EssΣ(A)={AD,AB,BE,BC,CA}. Therefore the conflicting set {a mathematical formula}X={AB,AD,DE−, {a mathematical formula}BE} leads to the inclusion of {a mathematical formula}X−EssΣ(A)={DE−} in {a mathematical formula}d(A), rather than the inclusion of {a mathematical formula}{DE−,BE}. Thus, E may be validly concluded when starting at A, B or C.
       </paragraph>
      </section>
     </section>
     <section label="4.3">
      ...
      <paragraph label="Theorem 4.11">
       Let{a mathematical formula}Σ↑be the lift of the inheritance network{a mathematical formula}〈V,Σ〉. Then{a mathematical formula}Σ↑is coherent if and only if there is no{a mathematical formula}v∈Vwith{a mathematical formula}∅∈d(v).
      </paragraph>
      <paragraph label="Definition 4.12">
       The following definition and proposition describe what a network looks like when {a mathematical formula}∅∈d(v) for some {a mathematical formula}v∈V. The node x semi-strictly implies (semi-strictly refutes) y if there is a positive (negative) path from x to y where every arc after the first is strict.
      </paragraph>
      <paragraph label="Proposition 4.13">
       Let{a mathematical formula}〈V,Σ〉be an inheritance network. Then{a mathematical formula}∅∈d(v)iff there is some z and some{a mathematical formula}y⇌v,x⇌vsuch that y semi-strictly implies z and x semi-strictly refutes z.
      </paragraph>
     </section>
    </section>
   </content>
   <appendices>
    <section label="Appendix A">
     <section-title>
      Completeness of the network algorithm
     </section-title>
     <paragraph>
     ...
     <section label="A.1">
      <section-title>
       Reformulating the principles
      </section-title>
      <paragraph label="Definition A.1">
       The weak exemption extension {a mathematical formula}Σw of Σ is given by{a mathematical formula}
      </paragraph>
      <paragraph>
       Note that {a mathematical formula}Σw=Σ1∈. The weak exemption extension uses only one step of the definition of the regular exemption extension.
      </paragraph>
      <paragraph label="Definition A.2">
       <list>
        <list-item label="(i)">
         The clause {a mathematical formula}∀x(φ(x)→⋁δ∈ΔAbδx) is an inherited clause for Σ iff there is some ψ such that {a mathematical formula}∀x(φ(x)↝ψ(x))∈Σ and {a mathematical formula}Σ⊨∀x(ψ(x)→⋁δ∈ΔAbδx).
        </list-item>
        <list-item label="(ii)">
         The inheritance extension {a mathematical formula}ΣI of Σ is given by{a mathematical formula} where {a mathematical formula}Σ0I=Σ and {a mathematical formula}Σn+1I=ΣnI∪{φ|φ is an inherited clause for ΣnIa}.
        </list-item>
       </list>
      </paragraph>
      <paragraph>
      ...
      <list>
       <list-item label="(a)">
        {a mathematical formula}A⊨Σ↑∪Γ.
       </list-item>
       <list-item label="(b)">
        {a mathematical formula}A=A′.
       </list-item>
       <list-item label="(c)">
        For all individual constants c,{a mathematical formula}I(c)=I′(c).
       </list-item>
       <list-item label="(d)">
        For all predicates{a mathematical formula}Pi, the following holds:
       </list-item>
       <list-item label="(e)">
        For all{a mathematical formula}Piand{a mathematical formula}Pj,{a mathematical formula}I(AbPiPj)⊆I′(AbPiPj)and{a mathematical formula}I(AbPi¬Pj)⊆I′(AbPi¬Pj).
       </list-item>
      </list>
      <paragraph label="Proof">
       Left to the reader.On the way to the completeness theorem, we will often be looking for correspondences between notions that play a role in inheritance networks and notions in the circumscription framework. One such notion is the notion of a path.Clearly, if there is a positive path X from {a mathematical formula}vi to {a mathematical formula}vj in the network {a mathematical formula}〈V,Σ〉, then {a mathematical formula}Σ↑⊨∀x((Pix∧⋀α∈X¬Abαx)→Pjx).{sup:25} For coherent theories the converse is also true. This follows immediately from the following more general proposition.
      </paragraph>
      ...
      <paragraph>
       The next lemma shows that when Σ is the lift of a network, {a mathematical formula}ΣwI is ‘closed’ with regards to exemption clauses. That is: if {a mathematical formula}ΣwI entails some exemption clause, then this exemption clause or a stronger one is an element of {a mathematical formula}ΣwI.
      </paragraph>
      <paragraph label="Proof">
      ...
      <section label="A.1.1">
       ...
       <paragraph label="Definition A.8">
        Let Σ be a set of rules. The formulas φ and ψ are equivalent in Σ iff there are {a mathematical formula}φ1,…,φm, {a mathematical formula}ψ1,…,ψn such that {a mathematical formula}φm=ψ=ψ1, {a mathematical formula}ψn=φ=φ1 and for all {a mathematical formula}1≤i&lt;m, {a mathematical formula}1≤j&lt;n{a mathematical formula}Σ⊨∀x(φi(x)↝φi+1(x)), and {a mathematical formula}Σ⊨∀x(ψj(x)↝ψj+1(x)).If φ and ψ are equivalent in Σ, we write {a mathematical formula}φ≈Σψ, or simply {a mathematical formula}φ≈ψ if no confusion is possible.
       </paragraph>
       <paragraph>
        Instead of the weak exemption extension {a mathematical formula}Σw we will utilize the expanded weak exemption extension {a mathematical formula}ΣW.
       </paragraph>
       <paragraph label="Definition A.9">
        <list>
         <list-item label="(i)">
          The clause {a mathematical formula}∀x(φ(x)→⋁δ∈ΔAbδx) is an expanded exemption clause for Σ iff there are {a mathematical formula}ψ1≈ψ2≈…≈ψn≈φ such that{a mathematical formula}
         </list-item>
         <list-item label="(ii)">
          The expanded weak exemption extension {a mathematical formula}ΣW of Σ is given by{a mathematical formula}
         </list-item>
        </list>
       </paragraph>
       <paragraph>
        Of course, showing that {a mathematical formula}Σ∈ and {a mathematical formula}ΣWI are equally strong, will require some elaborations. Note that Lemma A.4, Lemma A.5 are already stated and proven in terms of general networks which may include cycles.
       </paragraph>
       <paragraph label="Proof">
     ...
     <section label="A.2">
      ...
      <paragraph>
       Below, when {a mathematical formula}X={α∈Σ|A⊨Abαc}, we will often say that X is the exception set represented by {a mathematical formula}A.
      </paragraph>
      <paragraph>
       Next proposition says that every minimal exception set is represented by at least one model in {a mathematical formula}F.
      </paragraph>
      <paragraph label="Proof">
      ...
      <paragraph label="Theorem A.18">
       Let{a mathematical formula}Σ↑be the lift of the inheritance network{a mathematical formula}〈V,Σ〉, with{a mathematical formula}V={v1,…,vn}. Let{a mathematical formula}I=〈Σ↑,{Pic}〉. Let{a mathematical formula}〈U,F〉be the information state generated by I.
      </paragraph>
      <list>
       <list-item label="(i)">
        If X is a minimal exception set for{a mathematical formula}vi, and there is a positive path from{a mathematical formula}vito{a mathematical formula}vjthat does not contain any element of X, then{a mathematical formula}A⊨Pjcfor every{a mathematical formula}A∈Frepresenting X in c.
       </list-item>
       <list-item label="(ii)">
        If f X is a minimal exception set for{a mathematical formula}vi, and there is a negative path from{a mathematical formula}vito{a mathematical formula}vjwhich does not contain any element of X, then{a mathematical formula}A⊨¬Pjcfor every{a mathematical formula}A∈Frepresenting X in c.
       </list-item>
       <list-item label="(iii)">
        If X is not an acceptable exception set for{a mathematical formula}vj, then{a mathematical formula}A⊨¬Pjcfor every{a mathematical formula}A∈Frepresenting X in c.
       </list-item>
       <list-item label="(iv)">
        If{a mathematical formula}A⊨Pjcfor every{a mathematical formula}A∈Frepresenting X in c, then there is a positive path from{a mathematical formula}vito{a mathematical formula}vjwhich does not contain any element of X.
       </list-item>
       <list-item label="(v)">
        If{a mathematical formula}A⊨¬Pjcfor every{a mathematical formula}A∈Frepresenting X in c, then either there is a negative path from{a mathematical formula}vito{a mathematical formula}vjwhich does not contain any element of X or X is not an acceptable exception set for{a mathematical formula}vj.
       </list-item>
      </list>
      <paragraph label="Proof">
       The proofs of (i) and (ii) are left to the reader.The proof of (iii) is also straightforward: If X is not an acceptable exception set for {a mathematical formula}vj, then there is some {a mathematical formula}Y∈D(vj) such that {a mathematical formula}X∩Y=∅. Since {a mathematical formula}Y∈D(vj), {a mathematical formula}(Σ↑)WI⊨∀x(Pjx→⋁α∈YAbαx) (Proposition A.14).Suppose {a mathematical formula}A∈F represents X in c. Since {a mathematical formula}X∩Y=∅, {a mathematical formula}A does not make {a mathematical formula}⋁α∈YAbαc true. Therefore, {a mathematical formula}A⊨¬Pjc.To prove (iv), suppose every {a mathematical formula}A∈F representing X in c makes {a mathematical formula}Pjc true. Construct {a mathematical formula}B=〈B,I〉 as follows:
       <list>
        For the domain {a mathematical formula}B, take the same domain as that of some other model in {a mathematical formula}F, and choose {a mathematical formula}I(c) arbitrarily.Stipulate that {a mathematical formula}I(c)∈I(Pi) and that {a mathematical formula}I(c)∈I(Abα) iff {a mathematical formula}α∈X.For all {a mathematical formula}Pj, stipulate that {a mathematical formula}I(c)∈I(Pj) if and only if there is a positive path from {a mathematical formula}vi to {a mathematical formula}vj that does not contain an element of X.For all {a mathematical formula}d≠I(c) and for all {a mathematical formula}Pj, stipulate that {a mathematical formula}d∉I(Pj).To prove (v), suppose every
       </list>
       <paragraph>
        {a mathematical formula}A∈F representing X in c makes {a mathematical formula}¬Pjc true. Construct {a mathematical formula}B′ like {a mathematical formula}B in (iv), but stipulate that {a mathematical formula}I(c)∈I(Pj). Then {a mathematical formula}B′∉F, and more specifically {a mathematical formula}B′⊭(Σ↑)WI. Pick {a mathematical formula}ϕ∈(Σ↑)WI such that {a mathematical formula}B′⊨¬ϕ. A number of cases arise, depending on ϕ.
       </paragraph>
       <list>
        <list-item label="(a)">
         {a mathematical formula}ϕ=∀x(Pkx∧¬Abϕx→¬Pjx) for some k, with {a mathematical formula}B′⊨Pkc∧¬Abϕc. In this case, there is a negative path from {a mathematical formula}vi to {a mathematical formula}vj (via {a mathematical formula}vk) that does not contain an element of X.
        </list-item>
        <list-item label="(b)">
         {a mathematical formula}ϕ=∀x(Pjx∧¬Abϕx→¬Pkx) for some k, with {a mathematical formula}B′⊨Pkc∧¬Abϕc. In this case too, there is a negative path from {a mathematical formula}vi to {a mathematical formula}vj (via {a mathematical formula}vk using modus tollens at the end) that does not contain an element of X.
        </list-item>
        <list-item label="(c)">
     ...
     <section label="A.3">
      ...
      <paragraph label="Theorem A.20">
       Let{a mathematical formula}〈V,Σ〉be an inheritance network with{a mathematical formula}V={v1,…,vn}. Then{a mathematical formula}Σ↑is incoherent if and only if{a mathematical formula}∅∈d(vi)for some i.
      </paragraph>
      <paragraph label="Proof">
