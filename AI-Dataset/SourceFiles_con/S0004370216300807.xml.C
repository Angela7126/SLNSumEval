 ...
 <body>
  <root>
   ...
   <content>
    <section label="1">
     ...
     <section label="1.1">
      ...
      <list>
       ...
       <list-item label="•">
        We showcase an implementation of the formal map framework and navigational language on the current Web of Linked Data via a tool called MaGe (Map Generator).{sup:6}
       </list-item>
       <list-item label="•">
    ...
    <section label="2">
     <section-title>
      Problem statement: why maps of the Web?
     </section-title>
     <paragraph>
     ...
     <paragraph>
      Fig. 2 shows two possible maps of the region in Fig. 1. Map 1 contains more nodes (e.g., M. Scorsese) and edges (e.g., the edge {a mathematical formula}e1) than Map 2. Map 2 adopts a specific abstraction strategy; it preserves reachability information only among pairs of distinguished nodes. The node M. Scorsese in Map 1 is not present in Map 2 since it is not a distinguished node. Note that in Map 2 the reachability between J. Ford and W. Herzog (both distinguished nodes) is encoded via the edge {a mathematical formula}e2. The edge {a mathematical formula}e1 in Map 1 is not present in Map 2; note that the only path between J. Ford and W. Allen in the region is J. Ford→E. Kazan→S. Kubrick→W. Allen, which is encoded in {a mathematical formula}e1. This path passes through S. Kubrick, which is a distinguished node and then must be in the map along with the edges J. Ford→S. Kubrick (representing the path J. Ford→E. Kazan→S. Kubrick) and S. Kubrick→W. Allen. Reachability information therefore encoded in {a mathematical formula}e1 in Map 1 is already represented in the path J. Ford → S. Kubrick → W. Allen in Map 2; hence {a mathematical formula}e1 can be discarded. We formalize this reasoning in the framework to build maps of graphs in the following sections.
     </paragraph>
     <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.1">
      <section-title>
       A formal definition of map
      </section-title>
      <paragraph>
       In order to introduce our map framework we model the Web space as a directed graph. As will be discussed in Section 4, we use a navigational language to automatically extract regions of the Web. The benefit of the framework we are going to present is that maps have formally defined properties and can be effectively treated as mathematical objects. We now introduce some notation and some basic definitions. Let {a mathematical formula}Γ=(VΓ,EΓ) be a Web Region, where {a mathematical formula}VΓ and {a mathematical formula}EΓ are the set of nodes and directed edges, respectively. In the process of map constructions, we ignore edge labels (if present) as we are interested in capturing reachability between nodes. Fig. 5 shows an example of Web Region. In the remainder of the paper, we use the following notation:
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}u→v denotes an edge {a mathematical formula}(u,v)∈EΓ. For instance, in Fig. 5 we have {a mathematical formula}S.Kubrick→W.Allen.
       </list-item>
       <list-item label="•">
        {a mathematical formula}u↠v denotes a path from u to v in Γ. In Fig. 5 we have {a mathematical formula}J.Ford↠S.Kubrick.
       </list-item>
       <list-item label="•">
        Let {a mathematical formula}N⊆VΓ, we write {a mathematical formula}u↠Nv if and only if there is a path from u to v in Γ not passing through intermediate nodes in N. As an example, if {a mathematical formula}N={M.Scorsese,W.Allen} by considering the region in Fig. 5 we have that {a mathematical formula}J.Ford↠NS.Kubrick but not {a mathematical formula}J.Ford↠NA.Baldwin.
       </list-item>
      </list>
      <paragraph label="Definition 3.1">
       MapA map{a mathematical formula}M=(VM,EM) of {a mathematical formula}Γ=(VΓ,EΓ) is a directed graph such that {a mathematical formula}VM⊆VΓ and each edge {a mathematical formula}(x,y)∈EM implies {a mathematical formula}x↠y in Γ.
      </paragraph>
      <paragraph>
       The idea of a map is essentially that of representing reachability information between pairs of distinguished nodes, in a concise way, that is, nodes in the set {a mathematical formula}VM⊆VΓ. By making a parallel with geography, Γ represents the “region” or “territory” being abstracted (see also Definition 2.1) via the map and nodes in {a mathematical formula}VM represent “points” that are relevant for the map; these are the distinctive traits used as the basis for the identity of the region (see Definition 2.2). For example, distinguished nodes can be our favorite directors in a graph of directors or relevant scientific papers in a graph of bibliographic data. Distinguished nodes alone capture a very basic and common form of map of the Web, that is, bookmarks. In this case, the region is the portion of the Web graph navigated to reach the distinguished nodes, that is, the pages bookmarked.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Definition 3.2">
       Complete mapA map{a mathematical formula}M=(VM,EM) of {a mathematical formula}Γ=(VΓ,EΓ) is complete if, and only if, {a mathematical formula}∀x,y∈VM, {a mathematical formula}x↠y in Γ implies {a mathematical formula}x↠y in M.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Definition 3.3">
       Route-complete mapA map{a mathematical formula}M=(VM,EM) of {a mathematical formula}Γ=(VΓ,EΓ) is route-complete if, and only if, {a mathematical formula}∀x,y∈VM{a mathematical formula}x↠VMy in Γ implies {a mathematical formula}x→y in M;
      </paragraph>
      <paragraph>
       The following Lemma shows that route-completeness is a more general notion than completeness.
      </paragraph>
      <paragraph label="Lemma 3.4">
       Every route-complete map M of Γ is also a complete map.
      </paragraph>
      <paragraph label="Proof">
       Let {a mathematical formula}x↠y in Γ, M be a route-complete map and {a mathematical formula}x,y∈VM. We show that {a mathematical formula}x↠y in M holds by induction on the number of nodes in {a mathematical formula}VM that appear in the path {a mathematical formula}x↠y in Γ (not considering x and y). Base case:if {a mathematical formula}x↠VMy in Γ, then by route-completeness {a mathematical formula}x→y in M.Inductive step:assume that {a mathematical formula}∀x,y∈VM such that {a mathematical formula}x↠y in Γ passes through at most n nodes in {a mathematical formula}VM then {a mathematical formula}x↠y in M. Let {a mathematical formula}x,y∈VM such that {a mathematical formula}x↠y in Γ passes via {a mathematical formula}n+1 nodes in {a mathematical formula}VM and let {a mathematical formula}z∈VM be a node such that {a mathematical formula}x↠z and {a mathematical formula}z↠y. We have that {a mathematical formula}x↠z and {a mathematical formula}z↠y both pass through at most n nodes in {a mathematical formula}VM and thus by induction we have that {a mathematical formula}x↠z and {a mathematical formula}z↠y in M, thus {a mathematical formula}x↠y in M. □
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Definition 3.5">
       Non-redundant mapA map{a mathematical formula}M=(VM,EM) of {a mathematical formula}Γ=(VΓ,EΓ) is non-redundant if, and only if, {a mathematical formula}∀x,y∈VM{a mathematical formula}x→y in M implies {a mathematical formula}x↠VMy in Γ.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Definition 3.6">
       Good mapA map M of Γ is good if, and only if, it is route-complete and non-redundant.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Theorem 3.7">
       Let{a mathematical formula}Γ=(VΓ,EΓ)be a Web Region. Given{a mathematical formula}N⊆VΓ, there is a unique good map M over Γ with{a mathematical formula}VM=N.
      </paragraph>
      <paragraph label="Proof">
       Given the set of distinguished nodes N, the existence of an edge {a mathematical formula}x→y in M is uniquely determined by the existence of a path in Γ connecting x to y. □
      </paragraph>
      <paragraph>
       Fig. 7 shows the unique good map of the region in Fig. 5. A flexible map framework should consider maps ranging from perfectly accurate (regions themselves) to minimal. Good maps are minimal maps where reachability information is also included. However, in some cases one would need to include more nodes besides the distinguished nodes included in good maps. To provide a flexible map framework, we introduce k-maps, a family of good maps that select nodes in the region by considering specific properties of the nodes. To this end, let {a mathematical formula}f:VΓ→R be a real-valued function defined over the nodes of the region. We say that f is selective if the property {a mathematical formula}{v|v∈VΓ s.t. f(v)≥a}⊆{v|v∈VΓ s.t. f(v)≥b} holds for every pair {a mathematical formula}a,b∈R s.t. {a mathematical formula}a≥b.
      </paragraph>
      <paragraph label="Definition 3.8">
       k-MapsLet {a mathematical formula}Γ=(VΓ,EΓ) be a Web Region. The k-map of Γ is the good map generated by the set of distinguished nodes {a mathematical formula}{v∈VΓ:f(v)≥k}, where {a mathematical formula}f:VΓ→R is a selective function measuring a specific property of the nodes in Γ.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Proposition 3.9">
       Let Γ be a Web Region and{a mathematical formula}n≥k≥0two integers. If{a mathematical formula}Mkis the k-map and{a mathematical formula}Mnis the n-map of Γ built using the same function f, then{a mathematical formula}Mnis a good map of{a mathematical formula}Mkover the set of distinguished nodes{a mathematical formula}VMn.
      </paragraph>
      <paragraph label="Proof">
       {a mathematical formula}Mn is a map of {a mathematical formula}Mk since {a mathematical formula}VMn⊆VMk. Now we have that {a mathematical formula}x→y in {a mathematical formula}Mn if, and only if, {a mathematical formula}x↠VMny in Γ if, and only if, ({a mathematical formula}x↠VMky in Γ or {a mathematical formula}x↠y in Γ passes through some nodes v for which {a mathematical formula}k≤f(v)&lt;n) if, and only if, ({a mathematical formula}x→y in {a mathematical formula}Mk or {a mathematical formula}x↠VMny in {a mathematical formula}Mk). □
      </paragraph>
     </section>
     <section label="3.2">
      <section-title>
      ...
      <paragraph>
       Maps capture information in a region given a set of nodes. When it comes to their computation, a question arises: what is the estimated size of a map? Fortunately this question has an easy answer: given a set of distinguished nodes {a mathematical formula}VM, the number of edges of M could be in the worst case {a mathematical formula}|VM|2. Consider a region Γ being a (directed) clique. If {a mathematical formula}VM=VΓ then {a mathematical formula}M=Γ; otherwise, since the map M is route-complete, for every pair of nodes {a mathematical formula}u,v∈M, there is a {a mathematical formula}z∈VΓ∖VM such that {a mathematical formula}u→z→v in Γ. Note that, depending on the set {a mathematical formula}VM⊆VΓ of node selected as distinguished, it may be the case that the number of edges in the map can be larger than the number of edges in the region, i.e., {a mathematical formula}|EM|≥|EΓ|. However, according to the above reasoning we have that {a mathematical formula}|EM|≤|VΓ|2 holds.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Theorem 3.10">
       Let{a mathematical formula}Γ=(VΓ,EΓ)be a Web Region. Given{a mathematical formula}VM⊆VΓ, the unique good map{a mathematical formula}M=(VM,EM)of Γ can be computed in time:
      </paragraph>
      <list>
       <list-item label="1.">
        {a mathematical formula}O(|VM|×(|VΓ∖VM|+|EΓ|))if Γ is a general graph by usingAlgorithm 1.
       </list-item>
       <list-item label="2.">
        {a mathematical formula}O((|VM|×|VΓ∖VM|)+|EΓ|)if Γ is a DAG by usingAlgorithm 2.
       </list-item>
      </list>
      <paragraph label="Proof">
      ...
      <list>
       <list-item label="1.">
        The bound can be obtained by adapting the BFS algorithm (see, Algorithm 1). The idea is to compute the BFS starting from each node {a mathematical formula}v∈VM. Each execution of the basic BFS costs {a mathematical formula}O(|VΓ|+|EΓ|), hence the total cost is {a mathematical formula}O(|VM|×(|VΓ|+|EΓ|)). However, since only paths that do not pass through any node in {a mathematical formula}VM are relevant in building the good map, the edges of Γ can be partitioned in two disjoint sets: {a mathematical formula}EΓ=EM∪EM‾, where {a mathematical formula}EM is the set of edges {a mathematical formula}(v,v′) s.t. {a mathematical formula}v∈VM and {a mathematical formula}EM‾ is the set of edges {a mathematical formula}(v,v′) s.t. {a mathematical formula}v∉VM. During the BFS, if a node {a mathematical formula}v∈VM is reached, the corresponding edge is added to the map (line 12) while all the outgoing edges are discarded since v is not added to the queue of the nodes to be visited. Thus, the cost of the BSF becomes {a mathematical formula}O(|VM|×(|VΓ∖VM|+|EM‾|)+|EM|). This is because edges in {a mathematical formula}EM‾ are visited when performing the BFS starting from each node in {a mathematical formula}VM and edges in {a mathematical formula}EM are visited only once when the BFS is executed from their originating (distinguished) node. A slight improvement of the running time can be obtained by stopping the BFS from a node as soon as all the nodes in {a mathematical formula}VM have been visited (lines 4, 7, 14). The space complexity is {a mathematical formula}O(|VΓ|), the same as BFS.
       </list-item>
       <list-item label="2.">
        The bound can be obtained by looking at Algorithm 2. Let {a mathematical formula}|VM|=N. The first step (line 1) is to topologically order the nodes in Γ and rename them (line 5) in order, say {a mathematical formula}v1,...,vn. The initialization of the data structures (lines 6–7) corresponds to the building of a list of N bits for each node {a mathematical formula}vi in {a mathematical formula}VΓ∖VM and initializing it with all cells equal to 0. The algorithm then visits nodes in order and proceeds as follows:
       </list-item>
      </list>
      <paragraph>
       When dealing with k-maps, the nodes {a mathematical formula}VMk⊆VΓ to be included in the map are obtained by checking the value of f for each node. For example, this can be done in time {a mathematical formula}O(|EΓ|) when using degree centrality. Algorithm 1 or Algorithm 2 is then executed over Γ considering the set of nodes {a mathematical formula}VMk as input; from Theorem 3.10, we can derive that k-maps can be constructed in time {a mathematical formula}O(|VM|×(|VΓ∖VM|+|EΓ|))+cf(VΓ) for general graphs and {a mathematical formula}O((|VM|×|VΓ∖VM|)+|EΓ|)+cf(VΓ) for DAGs, where {a mathematical formula}cf(VΓ) is the cost of computing the value of the function f for all the nodes in Γ.
      </paragraph>
     </section>
     <section label="3.3">
      <section-title>
       Computing maps: a running example
      </section-title>
      <paragraph>
       We now describe an example of how to compute good maps by using Algorithm 1. Consider the region Γ in Fig. 9 where the initial state of the data structures used in the algorithm is also shown. The array {a mathematical formula}VM contains the set of distinguished nodes, that is, nodes around which the map is built. The data structure reach (e.g., a hash table) is used to track the nodes reached at each iteration of the external for loop (see line 1).
      </paragraph>
      <paragraph>
       The algorithm starts (STEP 0) by picking one node {a mathematical formula}v∈VM (line 1) and by setting all the nodes in {a mathematical formula}VΓ as not reached from v (line 3). The algorithm then performs a variant of BFS starting from v (i.e., {a mathematical formula}V1 in Fig. 9). Fig. 9 also shows in STEP 1 and STEP 2 the first two iterations of the BFS (lines 7–16). The first node in the queue Q is extracted (i.e., {a mathematical formula}V1); then for all nodes reachable from this node, the algorithm checks if they have already been reached (lines 10–11) and changes their corresponding values in the reach data structure. Moreover, if these nodes belong to the set {a mathematical formula}VM (nodes in the map) then the corresponding edge is added to the output map and the number of reached nodes is incremented (STEP 2 in Fig. 9). The variable num_reached serves the purpose to stop the BFS from the current node if all the distinguished nodes have already been added to the map. If nodes encountered during the BFS are not distinguished (they do not belong to {a mathematical formula}VM) then they are inserted in the queue Q. Note that it is crucial to consider all the nodes in {a mathematical formula}VM (line 1) although they may have already been added to the map. Consider, for instance, the edge e between {a mathematical formula}V11 and {a mathematical formula}V5. If {a mathematical formula}V11 is not considered because it has been already added to the map in STEP 2, this edge would not be present in the map. Fig. 9 STEP n shows the final result of the algorithm.
      </paragraph>
     </section>
     <section label="3.4">
      <section-title>
      ...
      <paragraph label="Theorem 3.11">
       Let{a mathematical formula}Γ=(VΓ,EΓ)be a Web Region and{a mathematical formula}M(Γ)the set of all good maps over Γ. Let{a mathematical formula}Mi=(VMi,EMi)∈M(Γ)be the good map obtained by choosing the set of distinguished nodes{a mathematical formula}VMi⊆VΓ. Then:
      </paragraph>
      <list>
       <list-item>
        The binary relation ⊑ over{a mathematical formula}M(Γ), defined by{a mathematical formula}M1⊑M2iff{a mathematical formula}VM1⊆VM2, is a partial order on{a mathematical formula}M(Γ).
       </list-item>
       <list-item>
        The order ⊑ induces a Boolean algebra{a mathematical formula}(M(Γ),⊔,⊓,Γ,∅), where:{a mathematical formula}M1⊔M2is the unique good map of Γ over{a mathematical formula}VM1∪VM2;{a mathematical formula}M1⊓M2is the unique good map of Γ over{a mathematical formula}VM1∩VM2.
       </list-item>
       <list-item>
        There is an isomorphism of Boolean algebras from{a mathematical formula}(P(V),∪,∩,V,∅)to{a mathematical formula}(M(Γ),⊔,⊓,Γ,∅), given by{a mathematical formula}N↦MN(the unique good map of N over Γ).
       </list-item>
      </list>
      <paragraph label="Proof">
       1. The relation is clearly reflexive and transitive. For antisymmetry: if {a mathematical formula}M1⊑M2 and {a mathematical formula}M1⊑M2, then {a mathematical formula}VM1=VM2. Moreover it holds that {a mathematical formula}EM1⊆EM2⁎ and {a mathematical formula}EM2⊆EM1⁎, where {a mathematical formula}EMi⁎ is the set of edges of the transitive closure of {a mathematical formula}Mi. From Lemma 3.4 and the goodness of both maps, it follows: {a mathematical formula}x→y in {a mathematical formula}EM1 iff {a mathematical formula}x↠Ny in Γ iff {a mathematical formula}x→y in {a mathematical formula}EM2. Items 2. and 3. are long, but straightforward, and they check for definitions of Boolean algebra and isomorphism between algebras. □
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Proposition 3.12">
       Let{a mathematical formula}Mi=(VMi,EMi)i=1,2be good maps over Γ and let{a mathematical formula}MNΓdenote the good map of Γ over the set of nodes N. Then:
      </paragraph>
      <list>
       <list-item>
        {a mathematical formula}M1⊓M2=MVM1∩VM2M1∪MVM1∩VM2M2
       </list-item>
       <list-item>
        {a mathematical formula}EM1⊔M2⊆EM1∪EM2∪∪{(x,y)∈EΓ:x∈VM1,y∈VM2}∪{(y,x)∈EΓ:x∈VM1,y∈VM2}∪∪{x↠VM1∪VM2y,x∈VM1,y∈VM2}∪{y↠VM1∪VM2x,x∈VM1,y∈VM2}
       </list-item>
      </list>
      <paragraph label="Proof">
       [part 1]. Let {a mathematical formula}x,y∈VM1∩VM2 and {a mathematical formula}x→y in {a mathematical formula}M1⊓M2 we show that either {a mathematical formula}x→y in {a mathematical formula}MVM1∩VM2M1 or {a mathematical formula}x→y in {a mathematical formula}MVM1∩VM2M2 holds. For {a mathematical formula}x→y to be in {a mathematical formula}M1⊓M2 than {a mathematical formula}x↠VM1∩VM2y in Γ. We can follow in three cases: i) the edge {a mathematical formula}x→y is in {a mathematical formula}M1 or {a mathematical formula}M2, and thus belongs to the good map over {a mathematical formula}M1 or {a mathematical formula}M2; ii) the path {a mathematical formula}x↠VM1∩VM2y in Γ passes through at least one node z such that {a mathematical formula}z∈VM1 and {a mathematical formula}z∉VM2, and thus {a mathematical formula}x→y is in {a mathematical formula}MVM1∩VM2M1; iii) the path {a mathematical formula}x↠VM1∩VM2y in Γ passes through at least one node z such that {a mathematical formula}z∈VM2 and {a mathematical formula}z∉VM1, and thus {a mathematical formula}x→y is in {a mathematical formula}MVM1∩VM2M2. Now, we show that if {a mathematical formula}x,y∈VM1∩VM2 and {a mathematical formula}x→y in {a mathematical formula}MVM1∩VM2M1 or {a mathematical formula}x→y in {a mathematical formula}MVM1∩VM2M2 than {a mathematical formula}x→y in {a mathematical formula}M1⊓M2 holds. It is enough to note that each edge {a mathematical formula}x→y in {a mathematical formula}M1 implies that there exists a path {a mathematical formula}x↠VM1y in Γ and thus also {a mathematical formula}x↠VM1∩VM2y in Γ holds and we can conclude that if {a mathematical formula}x,y∈VM1∩VM2 than {a mathematical formula}x→y in {a mathematical formula}M1⊓M2 must hold. The same applies to edges in {a mathematical formula}M2.Then either: i) the edge {a mathematical formula}x→y is in {a mathematical formula}M1 or {a mathematical formula}M2, and thus belongs to the good map over {a mathematical formula}M1 or {a mathematical formula}M2 or ii) the path {a mathematical formula}x↠VM1∩VM2y in Γ can be represented as a sequence of 1's (nodes in {a mathematical formula}M1), 2's (nodes in {a mathematical formula}M2) and 0's (nodes neither in {a mathematical formula}M1 nor {a mathematical formula}M2). If the sequence contains a 1, then {a mathematical formula}x↠y in {a mathematical formula}M1: consider all 1's in the sequence; between each pair of them there is a path, either direct (thus in {a mathematical formula}M1) or indirect (using only nodes 2 and 0 and thus also in {a mathematical formula}M1). Thus, the edge {a mathematical formula}x→y is in {a mathematical formula}MVM1∩VM2M1. A similarly reasoning holds for terms involving {a mathematical formula}M2.[part 2]. If {a mathematical formula}x→y in {a mathematical formula}EM1⊔M2, then either there is a path in {a mathematical formula}EM1 or in {a mathematical formula}EM2 or if {a mathematical formula}x,y were in different {a mathematical formula}VMi's, then the path belongs to one of the four sets described. □
      </paragraph>
      <paragraph label="Corollary 3.13">
       Let{a mathematical formula}M1and{a mathematical formula}M2be two maps over Γ. Then:
      </paragraph>
      <list>
       <list-item label="1.">
        The map{a mathematical formula}M1⊓M2can be computed by using information available in the maps{a mathematical formula}M1,M2only, and in time{a mathematical formula}O(|VM1∩VM2|×(|VM1|+|EM1|+|VM2|+|EM2|).
       </list-item>
       <list-item label="2.">
        The superset of the edges of{a mathematical formula}M1⊔M2as discussed inProposition 3.12cannot be computed more efficiently than computing the good map over{a mathematical formula}VM1∪VM2from scratch.
       </list-item>
      </list>
      <paragraph label="Proof">
       [part 1]. It follows from Theorem 3.10. [part 2]. It is necessary to check for the existence of paths from nodes in {a mathematical formula}VM1 to nodes in {a mathematical formula}VM2 (and the other way around) in Γ. This check has a cost no lower than computing the good map of Γ on {a mathematical formula}VM1∪VM2. Note that edges {a mathematical formula}x→y in {a mathematical formula}M1 could come from a path {a mathematical formula}x→z→y with {a mathematical formula}z∈VM2 and after the union could become redundant; but this redundancy cannot be checked only with information in {a mathematical formula}M1 and {a mathematical formula}M2. □
      </paragraph>
      <paragraph>
    ...
    <section label="4">
     <section-title>
      Declarative specification of graph regions
     </section-title>
     <paragraph>
     ...
     <section label="4.1">
      <section-title>
       A general graph navigational language
      </section-title>
      <paragraph>
      ...
      <paragraph label="Problem 2">
       Given a directed graph {a mathematical formula}G=(VG,EG) and a set of nodes {a mathematical formula}N⊆VG obtained from the evaluation of a query Q in a navigational language, construct a subgraph (a region) {a mathematical formula}Γ=(VΓ,EΓ) of G such that {a mathematical formula}N⊆VΓ and Γ capture information relevant to Q.
      </paragraph>
      <paragraph>
       Abstractly defined, a navigational language {a mathematical formula}L over a graph {a mathematical formula}G=(VG,EG) is a set of functions (“queries”) of the form {a mathematical formula}VG→subgraphs(G)×P(VG) that assign to each node v a subgraph (the visited nodes and edges) plus a set of distinguished nodes (the resources selected). Many navigational languages (e.g., XPath [25], nSPARQL [14]) consider labeled graphs and are able to find pairs of nodes connected by a sequence of edge labels matching a pattern (or navigational expression) expressed via regular expressions over the alphabet of edge labels. Navigation is only referenced in the semantics as the means to get the resulting set of pairs of nodes. The relevant aspect of navigational languages is that their classical semantics is not enough for our goal; we need information about reachability in order to deal with regions and maps. To address this problem, we introduce the general navigational language {a mathematical formula}L, by taking inspiration from the NautiLOD language that was initially proposed with a semantics returning nodes [15] and then equipped with a semantics returning graph fragments in addition to sets of nodes [9], [26], [27], [28]. The idea is that information on the navigational paths, as well as the resources selected give us the piece of semantic information to build regions. Indeed, the paths traversed when finding nodes satisfying an expression are relevant to enrich information given by the nodes themselves. The syntax of {a mathematical formula}L is:
      </paragraph>
      <paragraph>
       {a mathematical formula}path::=label|path[test]|(path)⁎|(path|path)|path/path
      </paragraph>
      <paragraph>
       The core of {a mathematical formula}L is standard regular expressions built upon edge labels and boolean tests used to drive the navigation. In Section 5.1 we instantiate the language in the Web of Linked Data. Here, edge labels will concretely represent RDF predicates and tests will be of different types, for instance, ASK SPARQL queries (as in NautiLOD[9], [15], [27]) and/or nested regular expressions (e.g., as in nSPARQL [14]). The reader can refer to [27] for a comprehensive discussion about Web navigational languages.
      </paragraph>
     </section>
     <section label="4.2">
      <section-title>
       Graph regions obtained via navigational languages
      </section-title>
      <paragraph>
       A graph region Γ specified by a navigational expression is a tuple {a mathematical formula}〈(VΓ,EΓ),N〉 where {a mathematical formula}(VΓ,EΓ) is a standard directed graph and {a mathematical formula}N⊆VΓ is a set of distinguished nodes. To access the elements of Γ we use the notation {a mathematical formula}Γ.x where {a mathematical formula}x∈{VΓ,EΓ,N}. In particular, {a mathematical formula}Γ.N is the set containing the starting node from which the evaluation of an expression begins and the set of ending nodes, that is, the expression endpoints. A graph region Γ captures the specification given via a navigational expression e starting from a single node of the graph where {a mathematical formula}Γ.EΓ and {a mathematical formula}Γ.VΓ are edges and nodes navigated during the evaluation of e. The need to start from a node (or a set of nodes) is motivated by the fact that our framework is meant to be used on the Web, and thus, it is infeasible to start the evaluation of a navigational expression from all nodes of the Web graph (assuming one can enumerate all of them). Moreover, this also mimics the process of manual navigation on the Web where one starts from a single (or a small set of) Web page.
      </paragraph>
      <paragraph>
       The semantics of the general language {a mathematical formula}L is reported in Table 1, where {a mathematical formula}D(u) is the set of outgoing edges of u. The evaluation of an expression path over the Graph G is performed starting from a seed node u according to the rules R1–R16 (here the graph G is omitted for conciseness). The evaluation returns an ordered pair, that is, the directed graph and the set of distinguished nodes that correspond to the graph region Γ. The main module is the function {a mathematical formula}E{an inline-figure} (rule R1) that represents the starting point of the evaluation. The other modules are the following:
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}{an inline-figure}n〚path〛(u): This function takes as input a path (i.e., an expression defined according to the {a mathematical formula}L syntax) and a URI u. The function gives as output the set of all nodes visited during the evaluation of path;
       </list-item>
       <list-item label="•">
        {a mathematical formula}{an inline-figure}e〚path〛(u): This function takes as input a path, which is evaluated starting from the URI u, and gives as output the set of edges traversed during the evaluation of path.
       </list-item>
       <list-item label="•">
        {a mathematical formula}{an inline-figure}r〚path〛(u): This function takes as input a path, which is evaluated starting from the URI u, and gives as output the set of nodes reachable via sequences of edges satisfying path (i.e., result nodes).
       </list-item>
      </list>
      <paragraph>
       Note that the classic semantics of XPath, nSPARQL, NautiLOD and other navigational languages returning the endpoints can only be captured via this structure (i.e., {a mathematical formula}〈(N,∅),N〉). The {a mathematical formula}L language is parametric in the type of tests that can be handled by the function Evaluate (rule R13).
      </paragraph>
      <paragraph>
       An example of evaluation returning a graph region. We now provide an example of the evaluation of the expression {a mathematical formula}path=p/q in the language {a mathematical formula}L returning a graph region. Consider the graph in Fig. 11 (a) and {a mathematical formula}v1 as a starting point for the evaluation.
      </paragraph>
      <paragraph>
       Let Γ be the graph region that we want to build. The starting point for the evaluation of path is Rule R1 applied starting from node {a mathematical formula}v1. According to R1, {a mathematical formula}v1 is immediately added to the set {a mathematical formula}Γ.N. Then, the graph {a mathematical formula}(VΓ,EΓ) is built by calling the functions {a mathematical formula}{an inline-figure}n and {a mathematical formula}{an inline-figure}e, respectively. Similarly the other distinguished nodes are obtained via the function {a mathematical formula}{an inline-figure}r. Since these functions are very similar to one another we will discuss in detail only the application of {a mathematical formula}{an inline-figure}r. The first rule applied is R15 that makes usage of rule R12 on the label p. Rule R12 looks for edges labeled as p originating from {a mathematical formula}v1. This causes the traversing of the edges {a mathematical formula}〈v1,p,v2〉, {a mathematical formula}〈v1,p,v3〉 and {a mathematical formula}〈v1,p,v5〉 (that are added to {a mathematical formula}Γ.EΓ via rule R7; moreover, nodes {a mathematical formula}v2, {a mathematical formula}v3 and {a mathematical formula}v5 are also added to {a mathematical formula}Γ.VΓ via rule R2). Then, rule R15 calls again rule R12 starting from each of {a mathematical formula}v2, {a mathematical formula}v3 and {a mathematical formula}v5 looking for edges labeled as q. Rule R12 considers the edges {a mathematical formula}〈v2,q,v3〉, {a mathematical formula}〈v2,c,v4〉, {a mathematical formula}〈v3,q,v3〉 and {a mathematical formula}〈v3,q,v4〉 (again note that the corresponding nodes and edges are added to {a mathematical formula}Γ.VΓ via rule R2 and to {a mathematical formula}Γ.EΓ via rule R7). Nodes {a mathematical formula}v3 and {a mathematical formula}v4 are added to {a mathematical formula}Γ.N by R15. The resulting Web Region is shown in Fig. 11(b), where distinguished nodes (i.e., nodes in {a mathematical formula}Γ.N are shown in blue).
      </paragraph>
     </section>
     <section label="4.3">
      <section-title>
       Extracting regions from the web of linked data graph
      </section-title>
      <paragraph>
       This section deals with the extraction of regions from the Web of Linked Data graph. There is a huge amount of RDF data shared and interlinked on the Web. This is evolving the traditional Web of Documents into a Web of (Linked) Data (WoD) [12]. Here, Uniform Resource Identifiers (URIs) are used not only to identify Web documents and digital content, but also new kinds of resources such as real world objects (e.g., places) and abstract concepts (e.g., geography). Descriptions (or representations) for these resources can be obtained, in the same spirit of traditional documents, by dereferencing their associated URIs via the HTTP protocol. However, semantic links and descriptions are now expressed by using a common data format, that is, RDF. Fig. 12 offers a glimpse into the ideas underlying the WoD. The left part shows an excerpt of two HTML pages: one about R. Johnson and the other about the city where he passed away. The link between the two pages is a syntactic link. In the right part we have the same pieces of information represented in RDF. In this case the link (i.e., an RDF predicate) is given a meaningful label, and information about R. Johnson is given in the form of RDF triples.
      </paragraph>
      <paragraph label="Theorem 4.1">
       The Web Region Γ of the WoD graph corresponding to an expression e in the language{a mathematical formula}Lcan be extracted with no overhead, in term of the time required, with respect to the cost of evaluating e.
      </paragraph>
      <paragraph>
       To build Γ it is enough to store each RDF triple navigated when evaluating e. Note that in this context, the function {a mathematical formula}D(u) used in Table 1 simulate the URI dereferencing operation, which returns an RDF graph of triples about u.
      </paragraph>
     </section>
    </section>
    <section label="5">
     ...
     <paragraph>
      The framework to build maps of the Web of Linked Data has been implemented in Java in the MaGe tool. Expressions in the language {a mathematical formula}L to specify regions, regions and maps are represented in RDF and can be shared, exchanged and reused. Note that maps of general graphs (as discussed in Section 3) do not have edge labels, although an edge has the implicit semantics of “reachable”; an edge between nodes A and B means that B can be reached from A by not passing through distinguished nodes. In the RDF representation of a map the edge semantics is made explicit via a URI (i.e., “reachable”). The MaGe tool, the files used in the experiments and the results are available at the MaGe Web site.{sup:8}Fig. 13 shows the high-level architecture of the tool.
     </paragraph>
     <paragraph>
      The architecture comprises two main modules, the selection module and the abstraction module. The first module is responsible for the implementation of the language {a mathematical formula}L. In particular, given a seed URI and an expression in the language {a mathematical formula}L, this module retrieves a Web Region. The retrieval occurs as follows. The Interpreter receives the input, checks the syntax and initializes both the Execution Manager (with the seed URI) and the Automaton builder, which generates the automaton associated with the expression, which will be used to drive the evaluation of the expression on the Web. The Execution Manager controls the flow of the execution and passes the URIs to be dereferenced to the Network Manager, which performs the dereferencing of URIs via HTTP GET calls. The sets of RDF triples obtained are converted into Jena{sup:9} models by the RDF Manager. The Link Extractor takes in input the automaton and the model and selects a subset of outgoing links (to be possibly traversed at the next step of the navigation) according to the current state of the automaton. All the outgoing links (that are RDF triples) selected are added to the region Γ being built. Moreover, this set of links is given to the Execution Manager, which starts the cycle over.
     </paragraph>
     <paragraph>
     ...
     <paragraph>
      MaGe is also endowed wit a GUI (shown in Fig. 14). It includes four main tabs. The first one is used to specify the region via an expression in {a mathematical formula}L (shown in Fig. 14(b)). The second and fourth display the region retrieved in RDF and the expression endpoints. The third tab deals with the creation of maps and their visualization (shown in Fig. 14(a)). The bottom-right section enables one to compute the good map of the region and the k-maps. It is possible to save the map in RDF and use the algebra of maps. The bottom-left part enables controlling the visualization of the region/map.
     </paragraph>
     <section label="5.1">
      <section-title>
       Toward building web maps
      </section-title>
      <paragraph>
       Fig. 15 summarizes all the elements discussed so far to build maps of Web Regions. In particular, regions from the Web of Linked Data are obtained via formal specifications in the language {a mathematical formula}L (see the first arrow in Fig. 15). A region is the portion of the Web of Linked Data that is navigated during the evaluation of a navigational expression and it is represented as a directed graph (as discussed in Section 4). Regions are then abstracted by using the framework introduced in Section 3 to obtain a compact representation of reachability information among a set of nodes that have been marked as ‘distinguished’ in the region (see the second arrow in Fig. 15).
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Proposition 5.1">
       Given an expression e in the language{a mathematical formula}Lthat specifies a region{a mathematical formula}Γ=〈(VΓ,EΓ),N〉of the WoD graph:
      </paragraph>
      <list>
       <list-item>
        The good map{a mathematical formula}M=(N,EM)can be built in time{a mathematical formula}O(|N|×(|VΓ∖N|+|EΓ|))+eval(e)+Ctests, where{a mathematical formula}eval(e)is the cost of evaluating the expression e and{a mathematical formula}Cteststhe cost of evaluating the tests in e.
       </list-item>
      </list>
      <paragraph label="Proof">
       Given the expression e, the corresponding Web Region can be built in time {a mathematical formula}eval(e)+Ctests (see Theorem 4.1 and Theorem 3.10). Having the region, the good map can be computed in time {a mathematical formula}O(|N|×(|VΓ∖N|+|EΓ|)) where {a mathematical formula}|VΓ| and {a mathematical formula}|EΓ| are the number of nodes and edges of the region and N is the set of distinguished nodes selected by e. □
      </paragraph>
      <paragraph>
       Note that the space required to evaluate an {a mathematical formula}L expression and build the (k-)map of the corresponding region is proportional to the time, since only relevant nodes are visited. Indeed, the semantics of Γ is such that {a mathematical formula}x↠y in Γ if and only if there is a navigational path from x to y satisfying a subexpression of e. As a consequence, since edges in M correspond to paths, in Γ each {a mathematical formula}x→y in M means that there is a navigational path from x to y satisfying a subexpression of e.
      </paragraph>
     </section>
     <section label="5.2">
      <section-title>
      ...
      <paragraph label="Example 5.2">
       Specify two regions that contain people that have influenced or have been influenced up to distance 6 by TBL and SK, respectively. The ending nodes in the regions must be scientists. Compute maps and combine them using the algebra.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       {a mathematical formula} where test= ASK {?ctx rdf:type dbpedia:Scientist.}
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       This abstract representation tells us, for instance, that there exists an influence from TBL to G. Peano passing via P. Outlet. If one is interested in the whole path (also including non-scientists) it can be retrieved by looking to the region (see Fig. 16), obtaining TBL→P. Outlet→S. Briet→B. Latour→A.N. Whitehead→B. Russel→G. Peano.
      </paragraph>
      <paragraph>
       To include more information in the good map one can use k-maps. The notion of k-map is meant to provide a mechanism to construct multi-granular maps of Web Regions. By computing the 15-map of the region (23 nodes; 43 edges), it can be noted that some of the non-ending nodes in the path from TBL to G. Peano (i.e., A.N. Whitehead and B. Russel) are present in the 15-map meaning that their degree centrality in the region is greater or equal to 15.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       Algebra of maps. Let {a mathematical formula}M1 be the 60-map and {a mathematical formula}M2 be the 8-map obtained from the influence regions of TBL and SK respectively, Fig. 20 shows examples of the algebra of maps. In particular, Fig. 20 (a) shows the union and Fig. 20 (b) the intersection. The map of the union aids in putting together information from the two maps. Generally speaking, it discovers possible additional influence relations between pairs of nodes that are not present in the same map. In this specific example, there is no path between SK and TBL in either map or regions. However, the union discovers the connection between TBL and SK (i.e., TBL→P. Outlet→S. Briet→A.N. Withehead←SK). The intersection has been computed as the union of the good maps of {a mathematical formula}M1 and {a mathematical formula}M2 over {a mathematical formula}VM1∩VM2 (see Section 3.4).
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Example 5.3">
       We want to specify a region containing papers up to distance three from the paper “Linked Data on the Web”. The ending nodes must be only papers whose title contains the set of keywords “Linked”, “Data” and “Quer*” or the set “Web”, “Data” and “Quer*”. We want then to build different maps of this region.
      </paragraph>
      <paragraph>
       The region can be specified with the following expression ({a mathematical formula}e1) in {a mathematical formula}L considering the URI of the paper “Linked Data on the Web” in Google Scholar as starting node:
      </paragraph>
      <paragraph>
       {an inline-figure}
      </paragraph>
    ...
    <section label="6">
     ...
     <section label="6.1">
      <section-title>
       Summarization/compression of graphs
      </section-title>
      <paragraph>
