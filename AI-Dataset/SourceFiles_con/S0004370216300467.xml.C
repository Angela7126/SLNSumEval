  ...
  <root>
   ...
   <content>
    ...
    <section label="3">
     ...
     <paragraph>
      In the following we define formally various constraint languages that allow us to explore the scope of modeling possibilities {a mathematical formula}RDFi offers.
     </paragraph>
     <section label="3.1">
      <section-title>
       The language ECL
     ...
     <section label="3.2">
      ...
      <paragraph>
       The intended structure for PCL, denoted by {a mathematical formula}MPCL, has the set of non-empty regions as its domain. {a mathematical formula}MPCL interprets each constant symbol by the corresponding polygon in {a mathematical formula}Q2 and each of the predicate symbols by the corresponding topological relation of RCC-8.
      </paragraph>
      <paragraph>
    ...
    <section label="4">
     The {a mathematical formula}RDFi framework
     <paragraph>
    ...
    <section label="5">
     ...
     <paragraph label="Definition 5.2">
      In order to be able to go from {a mathematical formula}RDFi databases to the equivalent set of possible RDF graphs, the notion of valuation is needed. A valuation v is a function from U to C assigning to each e-literal from U a constant from C.
     </paragraph>
     <paragraph>
    ...
    <section label="6">
     ...
     <section label="6.1">
      ...
      <paragraph label="Definition 6.1">
       We first define the concept of e-mappings (“e” from the word “existential”) which extends the concept of mappings of Pérez et al. [75] with the ability to have literals from the set C and e-literals as values for a special query variable. An e-mapping ν is a partial function {a mathematical formula}ν:V→T such that {a mathematical formula}ν(x)∈I∪B∪L if {a mathematical formula}x∈Vn and {a mathematical formula}ν(x)∈C∪U if {a mathematical formula}x∈Vs.
      </paragraph>
      <paragraph label="Example 6.2">
       The notions of domain and restriction of an e-mapping as well as the notion of compatibility of two e-mappings are defined as for mappings in the obvious way [75] (we also use the same notation for them). The following are e-mappings:{a mathematical formula} where g2 is {a mathematical formula}x≥1∧x≤2∧y≥1∧y≤2. The domain of the above e-mappings consists of two query variables, namely, ?F and ?S. Variable ?F belongs to the set of normal query variables {a mathematical formula}Vn, while variable ?S belongs to the set of special query variables {a mathematical formula}Vs.
      </paragraph>
      <paragraph label="Definition 6.3">
       We now extend the concept of e-mappings and define conditional mappings. A conditional mapping μ is a pair {a mathematical formula}(ν,θ) where ν is an e-mapping and θ is an {a mathematical formula}L-constraint.
      </paragraph>
      <paragraph label="Example 6.4">
       The following are conditional mappings:{a mathematical formula} where g2 is as in Example 6.2, g3 is {a mathematical formula}x≥0∧x≤10∧y≥0∧y≤10, and g4 is {a mathematical formula}x≥0∧x≤1∧y≥0∧y≤1.
      </paragraph>
      <paragraph label="Definition 6.5">
       The domain of a conditional mapping {a mathematical formula}μ=(ν,θ), denoted by {a mathematical formula}dom(μ), is the domain of ν, i.e., the subset of V where the partial function ν is defined. If {a mathematical formula}W⊆dom(μ), then the restriction of the mapping μ to W, denoted by {a mathematical formula}μ|W, is the mapping {a mathematical formula}(ν|W,θ) where {a mathematical formula}ν|W is the restriction of mapping ν to W.
      </paragraph>
      <paragraph label="Definition 6.6">
       We now introduce the notion of compatible conditional mappings as it is done in [75] for standard mappings. To take into account e-literals, we also define the notion of possibly compatible conditional mappings, which is more relaxed than compatibility. Two conditional mappings {a mathematical formula}μ1=(ν1,θ1) and {a mathematical formula}μ2=(ν2,θ2) are compatible if the e-mappings {a mathematical formula}ν1 and {a mathematical formula}ν2 are compatible, i.e., for all {a mathematical formula}x∈dom(μ1)∩dom(μ2), we have {a mathematical formula}ν1(x)=ν2(x). The conditional mappings {a mathematical formula}μ1 and {a mathematical formula}μ2 are possibly compatible if for all {a mathematical formula}x∈dom(μ1)∩dom(μ2), we have {a mathematical formula}ν1(x)=ν2(x) or at least one of {a mathematical formula}ν1(x),ν2(x), where {a mathematical formula}x∈Vs, is an e-literal from U.
      </paragraph>
      <paragraph label="Example 6.7">
       Mappings {a mathematical formula}μ1 and {a mathematical formula}μ2 from Example 6.4 are not compatible, while mappings {a mathematical formula}μ2 and {a mathematical formula}μ3 are. Further, all conditional mappings of Example 6.4 are pairwise possibly compatible.
      </paragraph>
      <paragraph label="Definition 6.8">
       If two conditional mappings are possibly compatible, then we can define their join as follows. Let {a mathematical formula}μ1=(ν1,θ1) and {a mathematical formula}μ2=(ν2,θ2) be possibly compatible conditional mappings. The join{a mathematical formula}μ1⋈μ2 is a conditional mapping {a mathematical formula}(ν3,θ3) where:
      </paragraph>
      <list>
       <list-item label="i.">
        {a mathematical formula}ν3(x)=ν1(x) for {a mathematical formula}x∈dom(μ1)∖dom(μ2) and {a mathematical formula}ν3(x)=ν2(x) for {a mathematical formula}x∈dom(μ2)∖dom(μ1).
       </list-item>
       <list-item label="ii.">
        For each {a mathematical formula}x∈dom(μ1)∩dom(μ2),ν3(x)=ν1(x) if {a mathematical formula}ν1(x) is an e-literal, otherwise {a mathematical formula}ν3(x)=ν2(x).
       </list-item>
       <list-item label="iii.">
        {a mathematical formula}θ3 is {a mathematical formula}θ1∧θ2∧⋀x∈dom(μ1)∩dom(μ2)∩Vsξx where {a mathematical formula}ξx is {a mathematical formula}ν1(x)EQν2(x) if either {a mathematical formula}ν1(x) or {a mathematical formula}ν2(x) is an e-literal, otherwise {a mathematical formula}ξx is true.
       </list-item>
      </list>
      <paragraph label="Example 6.9">
       If {a mathematical formula}μ1 and {a mathematical formula}μ2 are the conditional mappings of Example 6.4, then{a mathematical formula}
      </paragraph>
      <paragraph>
       For two sets of conditional mappings {a mathematical formula}Ω1 and {a mathematical formula}Ω2, the operation of join is now defined as follows:{a mathematical formula}
      </paragraph>
      <paragraph>
       The reader is invited to compare this definition with the definition of join of mappings for SPARQL [75]. The new aspect with conditional mappings is that due to the presence of e-literals, we have to anticipate the possibility that two mappings from {a mathematical formula}Ω1 and {a mathematical formula}Ω2 become compatible when e-literals are replaced by constants from C. We anticipate this case by adding relevant constraints to the condition of a mapping.
      </paragraph>
      <paragraph>
       The operation of union is defined as in the standard case: {a mathematical formula}Ω1∪Ω2={μ|μ∈Ω1 or μ∈Ω2}.
      </paragraph>
      <paragraph>
       We now define the operator of difference:{a mathematical formula}
      </paragraph>
      <paragraph>
       In the above set, symbol ⊃ denotes logical implication.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       The operation of left-outer join is defined as in the standard case [75]: {a mathematical formula}Ω1{an inline-figure}Ω2=(Ω1⋈Ω2)∪(Ω1∖Ω2).
      </paragraph>
     </section>
     <section label="6.2">
      <section-title>
     ...
     <section label="6.3">
      ...
      <paragraph label="Definition 6.18">
       Having defined the evaluation of graph patterns, in this section we introduce the SELECT and CONSTRUCT query forms of SPARQL and study their evaluation in {a mathematical formula}RDFi. Pérez et al. [74]A SELECT query is a pair {a mathematical formula}(W,P) where W is a set of variables from the set V and P is a graph pattern.
      </paragraph>
      <paragraph label="Example 6.19">
      ...
      <paragraph label="Definition 6.22">
       Let us now introduce the notion of a template and define the CONSTRUCT query form. A template E is a finite subset of the set {a mathematical formula}(T∪V)×(I∪V)×(T∪V).
      </paragraph>
      <paragraph label="Definition 6.23">
       The elements of a template are like triple patterns but blank nodes and e-literals are also allowed in the subject and object positions. We denote by {a mathematical formula}var(E) and {a mathematical formula}blank(E) the set of variables and set of blank nodes appearing in the elements of E respectively. Templates are used to specify the graph that results from the evaluation of a CONSTRUCT query. A CONSTRUCT query is a pair {a mathematical formula}(E,P), where E is a template and P a graph pattern.
      </paragraph>
      <paragraph label="Example 6.24">
       Let us consider the query of Example 6.19. A new version of this query using the CONSTRUCT query form is:{a mathematical formula}
      </paragraph>
      <paragraph label="Example 6.25">
    ...
    <section label="8">
     ...
     <paragraph label="Definition 8.4">
     ...
     <paragraph label="Definition 8.5">
     ...
     <paragraph label="Definition 8.11">
      In a similar way, we can extend a valuation v to a set of mappings Ω as follows. Let Ω be a set of conditional mappings and {a mathematical formula}v:U→C a valuation. Then, {a mathematical formula}v(Ω)={v(μ)|μ=(ν,θ)∈Ω and ML⊨v(θ)}.
     </paragraph>
     <paragraph label="Definition 8.12">
      The next definition allows us to apply a valuation to an {a mathematical formula}RDFi database. Let {a mathematical formula}v:U→C be a valuation and {a mathematical formula}D=(G,ϕ) an {a mathematical formula}RDFi database such that {a mathematical formula}ML⊨v(ϕ). Then {a mathematical formula}v(D) denotes the RDF graph {a mathematical formula}v(G).
     </paragraph>
     <paragraph label="Proposition 8.13">
    ...
    <section label="9">
     ...
     <section label="9.1">
      ...
      <paragraph label="Example 9.7">
       The following example illustrates the algorithm of Theorem 9.6 for computing certain answers. Let us consider the {a mathematical formula}RDFi database {a mathematical formula}D=(G,ϕ) and the CONSTRUCT query q from Example 6.29. The certain answers to q over D can be computed according to the steps of Theorem 9.6 as follows:
      </paragraph>
      <list>
       <list-item label="i)">
        The {a mathematical formula}RDFi database {a mathematical formula}Dq=(Gq,ϕ) corresponding to the evaluation of q over D is the one from Example 6.29.
       </list-item>
       <list-item label="ii)">
        The EQ-completed form of {a mathematical formula}Dq, {a mathematical formula}DqEQ, is given in Example 9.2. The normalized form of {a mathematical formula}DqEQ, {a mathematical formula}DqEQ⁎, is given in Example 9.4.
       </list-item>
       <list-item label="iii)">
     ...
     <section label="9.2">
      ...
      <paragraph label="Definition 9.8">
   ...
   <appendices>
    <section label="Appendix A">
     ...
     <paragraph>
      This establishes the fact that {a mathematical formula}v(〚P〛D)⊆〚P〛v(D). The other direction of the proof is similar and goes as follows.
     </paragraph>
     <paragraph>
      Let {a mathematical formula}μ∈〚P〛v(D). Then, {a mathematical formula}μ(P)∈v(D). We now distinguish two cases corresponding to Definition 6.13 (1):
     </paragraph>
     <list>
      <list-item label="(i)">
     ...
     <paragraph>
      The above proves that the triple {a mathematical formula}〈D,Rep,QAUFC′〉 is a representation system.
     </paragraph>
     <paragraph>
      Proof for{a mathematical formula}QWDC′
     </paragraph>
     <paragraph>
      The proof that the triple {a mathematical formula}〈D,Rep,QWDC′〉 is also a representation system is the same as the previous one, while it differs only in the inductive step for the OPT operator. Thus, in this case, {a mathematical formula}P is P1OPTP2.
     </paragraph>
     <paragraph>
      We have {a mathematical formula}v(〚P1〛D)=〚P1〛v(D) and {a mathematical formula}v(〚P2〛D)=〚P2〛v(D) from the inductive hypothesis. We need to prove {a mathematical formula}v(〚P1OPTP2〛D)=〚P1OPTP2〛v(D) or equivalently{a mathematical formula}
     </paragraph>
     <paragraph>
     ...
     <list>
      ...
      <list-item label="2.">
       {a mathematical formula}μ′ is the conditional mapping {a mathematical formula}(ν′,θ′) and there exists a conditional mapping {a mathematical formula}μ″=(ν′,θ)∈〚P1〛D such that
       <list>
        {a mathematical formula}μ″ is not compatible to some mappings of {a mathematical formula}〚P2〛D andfor the rest of mappings {a mathematical formula}μi=(νi,θi)∈〚P2〛D, {a mathematical formula}μ″ and {a mathematical formula}μi are possibly compatible and{a mathematical formula}Since
       </list>
       <paragraph>
     ...
     <paragraph>
      This proves that {a mathematical formula}v(〚P1OPTP2〛D)⊆〚P1OPTP2〛v(D). The other direction of the proof is similar.
     </paragraph>
    </section>
    <section label="Appendix B">
     Proof for Proposition 8.14
     <paragraph>
      Let {a mathematical formula}〚q〛D be the {a mathematical formula}RDFi database {a mathematical formula}D′=(G′,ϕ) where{a mathematical formula}
     </paragraph>
     <paragraph>
      Then {a mathematical formula}v(〚q〛D) is the RDF graph {a mathematical formula}v(D′) where{a mathematical formula}
     </paragraph>
     <paragraph>
      Likewise, let {a mathematical formula}〚q〛v(D) be the RDF graph H. According to the definition of the evaluation of CONSTRUCT queries on RDF graphs [74], H is the set{a mathematical formula}
     </paragraph>
     <paragraph>
      To prove our proposition, we have to show that {a mathematical formula}H=v(D′). Let {a mathematical formula}t∈H be an RDF triple. Then, there exists a mapping {a mathematical formula}μ∈〚P〛v(D) such that{a mathematical formula}
     </paragraph>
     <paragraph>
     ...
     <paragraph>
      Working as previously, we set {a mathematical formula}fμ to behave as {a mathematical formula}fμ′ and hence the above relation becomes{a mathematical formula}
     </paragraph>
     <paragraph>
      From the above and because of (B.2) and {a mathematical formula}μ′∈〚P〛v(D), we get {a mathematical formula}t∈H.
     </paragraph>
    </section>
   </appendices>
  </root>
