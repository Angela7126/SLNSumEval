  ...
  <root>
   ...
   <content>
    <section label="1">
     ...
     <list>
      <list-item label="•">
       BK1: if more than half of the nodes expanded by Uni-HS have {a mathematical formula}g(n)≤{a mathematical formula}12C⁎, then Uni-HS will expand fewer nodes than Bi-HS.
      </list-item>
      <list-item label="•">
       BK2: If fewer than half of the nodes expanded by Uni-HS using heuristic h have {a mathematical formula}g(n)≤12C⁎, then adding h to Bi-BS will not decrease the number of nodes it expands.
      </list-item>
     </list>
     <paragraph>
      In other words, Barker and Korf claim that except for pathological cases (see their Section 3) there is no situation in which Bi-HS will be the method of choice; either Uni-HS will be best or Bi-BS will.{sup:2}
     </paragraph>
     <paragraph>
    ...
    <section label="3">
     ...
     <paragraph label="Definition 1">
      Meeting in the middleA bidirectional search algorithm meets in the middle if its forward search never expands a node n with {a mathematical formula}gF(n)&gt;{a mathematical formula}12C⁎ and its backward search never expands a node n with {a mathematical formula}gB(n)&gt;{a mathematical formula}12C⁎.
     </paragraph>
     <section label="3.1">
      <section-title>
       Previous Bi-HS algorithms fail to meet in the middle
      </section-title>
      <paragraph>
       We now show that previous Bi-HS algorithms fail to meet in the middle on the graph in Fig. 3. All edges cost 1 and the optimal path is {a mathematical formula}start,A,B,C,D,goal{a mathematical formula}(C⁎=5). Inside each node are its {a mathematical formula}hF-value (with a right-pointing arrow overhead) and its {a mathematical formula}hB-value (with a left-pointing arrow overhead). The heuristic values in each direction are consistent.
      </paragraph>
      <paragraph>
      ...
      <section label="3.1.1">
       <section-title>
        Cardinality criterion variants
       </section-title>
       <paragraph>
    ...
    <section label="4">
     MM: a novel Bi-HS family of algorithms
     <paragraph>
    ...
    <section label="5">
     ...
     <section label="5.1">
      ...
      <list>
       <list-item label="1.">
        PEMM separates the states in {a mathematical formula}Open and {a mathematical formula}Closed, which are stored on disk, from the information about those states, which can be summarized in smaller {a mathematical formula}Open and {a mathematical formula}Closed data structures in RAM.
       </list-item>
       <list-item label="2.">
        PEMM partitions {a mathematical formula}Open and {a mathematical formula}Closed on disk into buckets of states with similar properties for the efficiency of duplicate and solution detection.
       </list-item>
       <list-item label="3.">
        PEMM expands states with the same priority from low to high g-cost.
       </list-item>
       <list-item label="4.">
        PEMM uses delayed duplicate detection for finding duplicates.
       </list-item>
       <list-item label="5.">
        PEMM uses delayed solution detection for finding solutions.
       </list-item>
       <list-item label="6.">
        PEMM removes ϵ from the {a mathematical formula}gmin stopping condition and uses Basic MM's definition of {a mathematical formula}pr(n), because the ϵ-based variants do not work with delayed solution detection.
       </list-item>
       <list-item label="7.">
        PEMM performs many operations in parallel.
       </list-item>
       <list-item label="8.">
        PEMM assumes a consistent heuristic, an undirected search space, and unit-cost edges.
       </list-item>
      </list>
      <paragraph>
      ...
      <section label="5.1.1">
       States are not stored in {a mathematical formula}Open and {a mathematical formula}Closed in RAM
       <paragraph>
       ...
       <paragraph>
        Individual states are stored unsorted on {a mathematical formula}Open and {a mathematical formula}Closed on disk, so writes can be efficiently performed at any time by appending to a file on disk. Writes are buffered both in memory and by the filesystem, which improves efficiency and eliminates latency concerns when writing.
       </paragraph>
      </section>
      <section label="5.1.2">
       States in {a mathematical formula}Open and {a mathematical formula}Closed are broken into buckets on disk
       <paragraph>
        Since we cannot load all of {a mathematical formula}Open into RAM at once, it must be subdivided into smaller buckets that will fit into RAM so that we can load these files when doing duplicate detection.
       </paragraph>
       <paragraph>
      ...
      <section label="5.1.3">
       Tie-breaking from low to high g-cost
       <paragraph>
      ...
      <section label="5.1.4">
       <section-title>
        Delayed duplicate detection (DDD)
       </section-title>
       <paragraph>
    ...
    <section label="6">
     ...
     <section label="6.3">
      ...
      <paragraph>
       Uni-BS expands more nodes than MM0 iff (Eq. (5) &gt; Eq. (6)) as written in the following inequality:{a mathematical formula} To identify the core differences between the algorithms, i.e. regions explored by one algorithm but not the other, we ignore the difference between N and N′ and between F and F′, which simplifies Eq. (7) to:{a mathematical formula} We have identified two conditions that guarantee FD &gt; RN:
      </paragraph>
      <list>
       <list-item label="•">
        When {a mathematical formula}C⁎=D, the diameter of the space, there are no remote states, by definition, so RN is empty.
       </list-item>
       <list-item label="•">
        When the number of states far from {a mathematical formula}start is larger than the number of states near to {a mathematical formula}goal, i.e. if FD + FN &gt; FN + NN + RN, or equivalently,{sup:15} FD &gt; NN + RN. We say a problem ({a mathematical formula}start,goal) is bi-friendly if it has this property.
       </list-item>
      </list>
      <paragraph>
     ...
     <section label="6.7">
      ...
      <paragraph>
       Many factors come into play in determining which algorithm—A*, MM0, or MM—will expand the fewest nodes. Our general rules express what we think will be the most common outcomes. GR1 says that MM0 will expand more nodes than Uni-BS if region FD is smaller than region RN. Adding a heuristic to both brute-force searches in this situation is not expected to reverse this conclusion, so we expect A* to expand fewer nodes than MM0 and MM (and of course, Uni-BS) when FD is smaller than RN.
      </paragraph>
      <paragraph>
       When FD is larger than RN, the algorithm that expands the fewest nodes will depend on the accuracy of the heuristics. With sufficiently inaccurate heuristics, MM0 is expected to expand fewer nodes than A* (GR2) and MM (GR3). As the heuristics' accuracy increases, the advantage of MM0 over MM will diminish and eventually MM will expand fewer nodes than MM0. Because MMe is generally stronger than basic MM, MMe is expected to expand fewer nodes than MM0 with a less accurate heuristic than basic MM. Of course, A* will also be benefiting from the improved heuristic, but, contrary to Barker and Korf [4], we expect MM to expand fewer nodes than both MM0 and A* when the heuristics are moderately accurate. As the accuracy continues to increase, A* will eventually expand fewer nodes than either of the bidirectional searches (because FDU will become very small (GR2)).
      </paragraph>
     </section>
    </section>
    <section label="7">
     ...
     <section label="7.1">
      ...
      <paragraph>
       In the 10-pancake puzzle a state is a vector with 10 numbers and the task is to sort them. Operators only allow the reversal of prefixes of the vector. We ran MM0, MM, MMe, Uni-BS, and A* on 30 random instances for each possible value of {a mathematical formula}C⁎ ({a mathematical formula}1≤C⁎≤11). We used the GAP heuristic [27]{sup:18} and derived less accurate heuristics from it, referred to as GAP-X, by not counting the gaps involving any of the X smallest pancakes. For example, GAP-2 does not count the gaps involving pancakes 0 or 1.
      </paragraph>
      <section label="7.1.1">
       {a mathematical formula}C⁎=10
       <paragraph>
       ...
       <paragraph>
        We see that RN is small (929) and FD is very large (3.5M). As a consequence, MM0 expands many fewer nodes than Uni-BS (GR1), as shown in the first two rows. ND is identical in size to FN+RN (= DN) because of the symmetry in this space. The asymmetry of MM0's expansions in ND and FN+RN is because, for {a mathematical formula}C⁎=10, MM0 must expand all the nodes with {a mathematical formula}g(s)=4 in one direction but not the other. MM's expansions in these regions are much more balanced.
       </paragraph>
       <paragraph>
       ...
       <list>
        <list-item label="•">
         GR1: For every instance for every value of {a mathematical formula}C⁎, FD &gt; RN and MM0 expanded fewer nodes than Uni-BS.
        </list-item>
        <list-item label="•">
        ...
        <list-item label="•">
         GR3: With the best heuristic, GAP, MM expands many fewer nodes than MM0. As the heuristic becomes less accurate, the difference between MM and MM0 steadily diminishes and eventually (GAP-2) turns into a steadily growing advantage for MM0.
        </list-item>
       </list>
       <paragraph>
        In summary, as expected, with the very accurate heuristic (GAP), A* was the best. With moderately accurate heuristics (GAP-1 and GAP-2) MM was the best. Finally, with the weaker heuristics (GAP-3 and GAP-4) MM0 was the best. We have performed similar experiments with {a mathematical formula}C⁎=11. Similar trends were observed with some exceptions, which we explain below in Section 7.1.3.
       </paragraph>
      </section>
      <section label="7.1.2">
       Does MM outperform A* because of tie-breaking?
       <paragraph>
     ...
     <section label="7.2">
      ...
      <list>
       <list-item label="•">
        GR1: On average, in this map FD &gt; RN and, as predicted, MM0 expanded fewer nodes than Uni-BS.
       </list-item>
       <list-item label="•">
       ...
       <list-item label="•">
        GR3: With a heuristic weight of 0.1, MMe expands 0.1 more nodes than MM0 on average. This difference is minimal, suggesting that there are not a significant number of nodes that meet the conditions for GR3 in this state space.
       </list-item>
      </list>
      <paragraph>
     ...
     <section label="7.3">
      ...
      <paragraph>
       In general, these results confirm GR2 and show the phase transition that occurs from PEMM0 to PEMM to IDA* as the problems get easier or the heuristic gets more accurate. PEMM still has room for optimization, as we will see clearly when looking at timing results.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       To summarize the results here, we see that GR2 is descriptive of performance on Rubik's cube. While our implementation of PEMM0 set a new milestone by solving a depth-20 problem without heuristic guidance, there is still room for improving the performance of PEMM and PEMM0 by reducing the overhead from DSD in PEMM0 and the cost of heuristic lookups in PEMM.
      </paragraph>
     </section>
    </section>
   </content>
   <appendices>
    <section label="Appendix A">
     ...
     <list>
      <list-item>
       MMe's forward and backward searches meet in the middle; neither search expands a node whose distance from the search's origin ({a mathematical formula}gF(n) for forward search, {a mathematical formula}gB(n) for backward search) is larger than {a mathematical formula}12{a mathematical formula}(C⁎−ϵ) (Corollary 15).
      </list-item>
      <list-item>
       MMe never expands a node whose f-value exceeds {a mathematical formula}C⁎ (Corollary 15).
      </list-item>
      <list-item>
       MMe returns {a mathematical formula}C⁎ (Lemma 8 if there is no path from {a mathematical formula}start to {a mathematical formula}goal, Theorem 18 if there is).
      </list-item>
      <list-item>
       If there exists a path from {a mathematical formula}start to {a mathematical formula}goalMMe never expands a state in both search directions (Theorem 16).
      </list-item>
     </list>
     <section label="A.1">
      <section-title>
       Terminology – nodes vs. states
      </section-title>
      <paragraph>
       States and nodes are different kinds of entities. A state is an immutable element of a state space, with a fixed distance to {a mathematical formula}start and {a mathematical formula}goal. A node, by contrast, is an entity created and updated by a search algorithm representing a path (or set of paths) in the state space. At a minimum, node n stores the path's cost ({a mathematical formula}g(n)) and the last state on the path, which we call the state associated with n.
      </paragraph>
      <paragraph>
     ...
     <section label="A.2">
      ...
      <paragraph>
       We assume that all edge costs ({a mathematical formula}cost(u,v)) are non-negative (zero-cost edges are allowed), that {a mathematical formula}start≠goal, and that the heuristic used by MMe in each search direction is admissible.
      </paragraph>
      <paragraph>
       The following definition and Lemma 2, Lemma 3, Lemma 4 are closely based on Hart, Nilsson, and Raphael's Lemma 1 and its proof [23].
      </paragraph>
      <paragraph label="Definition 3">
       Node n is “permanently closed” in the forward search direction if {a mathematical formula}n∈ClosedF and {a mathematical formula}gF(n)=d(start,n). Likewise, n is permanently closed in the backward search direction if {a mathematical formula}n∈ClosedB and {a mathematical formula}gB(n)=d(n,goal).
      </paragraph>
      <paragraph>
       The name “permanently closed” is based on the following lemma.
      </paragraph>
      <paragraph label="Lemma 2">
       If node n is permanently closed in a particular search direction at the start of some iteration, it will be permanently closed in that direction at the start of all subsequent iterations.
      </paragraph>
      <paragraph label="Proof">
       This proof is for the forward search, the proof for the backward search is analogous. There is no code in Algorithm 1 to directly change {a mathematical formula}gF(n) while {a mathematical formula}n∈ClosedF, so n can only stop being permanently closed by being removed from {a mathematical formula}ClosedF. This is possible (line 17) but only if a strictly cheaper path to n is found (line 14). This is not possible since {a mathematical formula}gF(n)=d(start,n) for a node permanently closed in the forward direction. Therefore, once n is permanently closed in the forward direction it will remain so. □
      </paragraph>
      ...
      <paragraph label="Lemma 6">
       If{a mathematical formula}P=s0,s1,…snis an optimal path from{a mathematical formula}start(s0)to{a mathematical formula}goal(sn)that has not been found, let{a mathematical formula}nF=siand{a mathematical formula}nB=sjbe as defined inLemma 4. Then{a mathematical formula}gF(nF)+gB(nB)≤C⁎−ϵ.
      </paragraph>
      <paragraph label="Proof">
       Lemma 5 guarantees that {a mathematical formula}nF and {a mathematical formula}nB exist for P. Because {a mathematical formula}i&lt;j, {a mathematical formula}d(start,si)+d(si,sj)+d(sj,goal)=C⁎, the cost of the whole path P. Because edge costs are non-negative {a mathematical formula}d(si,sj)≥ϵ, and therefore {a mathematical formula}d(start,si)+d(sj,goal)≤C⁎−ϵ. The lemma follows because {a mathematical formula}gF(nF)=d(start,si) and {a mathematical formula}gB(nB)=d(sj,goal). □
      </paragraph>
      <paragraph label="Theorem 7">
       If, at the beginning of anMMeiteration, there exists an optimal path P from{a mathematical formula}startto{a mathematical formula}goalthat has not been found, then{a mathematical formula}C≤C⁎.
      </paragraph>
      <paragraph label="Proof">
     ...
     <section label="A.3">
      MMe with consistent heuristics
      <paragraph>
       In this section we consider additional properties of MMe if its heuristics are consistent.
      </paragraph>
      <paragraph>
       The following trivial lemma will be used in the proofs of Lemma 20, Lemma 24.
      </paragraph>
      <paragraph label="Lemma 19">
       If{a mathematical formula}a1&gt;a2and{a mathematical formula}b1&gt;b2then{a mathematical formula}max⁡(a1,b1)&gt;max⁡(a2,b2).
      </paragraph>
      <paragraph label="Proof">
