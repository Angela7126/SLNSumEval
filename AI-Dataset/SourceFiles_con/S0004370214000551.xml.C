  ...
  <root>
   ...
   <content>
    <section label="1">
     ...
     <paragraph>
      This paper extends our preliminary work [7], [8], [9] by (1) providing a substantially more rigorous theoretical analysis of the presented algorithms, (2) extending the experimental results, (3) adding a comprehensive discussion on the relation between bounded-cost search and anytime search, and (4) discussing the limitations of PTS and {a mathematical formula}APTS/ANA⁎.
     </paragraph>
     <paragraph>
      The structure of this paper is as follows. First, we provide background and related work. In Section 3, we introduce the bounded-cost search problem and present Potential Search (PTS). In Section 4, we present Anytime Potential Search (APTS) [7], [8] and Anytime Non-Parametric {a mathematical formula}A⁎ ({a mathematical formula}ANA⁎) [9], showing that they are equivalent and discussing their theoretical properties. Section 5 presents experimental results comparing PTS and {a mathematical formula}APTS/ANA⁎ with previous algorithms. Finally, we discuss a generalization of PTS (Section 6) and conclude with suggestions for future work (Section 7).
     </paragraph>
    </section>
    <section label="2">
     <section-title>
    ...
    <section label="3">
     ...
     <section label="3.1">
      <section-title>
       Applications of bounded-cost search
      </section-title>
      <paragraph>
      ...
      <paragraph>
       A bounded-cost search problem can be viewed as a constraint satisfaction problem (CSP), where the desired cost bound is simply a constraint on the solution cost. However, modeling a search problem as a CSP is non-trivial if one does not know the depth of the goal in the search tree.{sup:7} Furthermore, many search problems have powerful domain-specific heuristics, and it is not clear if general CSP solvers can use such heuristics. The bounded-cost search algorithm presented in Section 3.3 can use any given heuristic.
      </paragraph>
      <paragraph>
     ...
     <section label="3.2">
      <section-title>
       Naïve approaches to bounded-cost search
      </section-title>
      <paragraph>
     ...
     <section label="3.4">
      ...
      <paragraph>
       PTS is based on the novel {a mathematical formula}u(⋅) evaluation function. Next, we provide the first analytical derivation of {a mathematical formula}u(n), relating it to the probability that n leads to a goal of cost smaller than C. We call this probability the potential of a node.
      </paragraph>
      <paragraph>
       To motivate the concept of the potential of a node, consider the graph presented in Fig. 1. Assume that we are searching for a path from s to g. After expanding s, the search algorithm needs to decide which node to expand next: node a or node b. If the task is to find an optimal path from s to g, then clearly b should be expanded first, since there may be a path from s to g that passes through b whose cost is lower than the cost of the path that passes through a (as {a mathematical formula}f(b)=g(b)+h(b)=100&lt;f(a)=g(a)+h(a)=103).
      </paragraph>
      <paragraph>
       If, however, the task is to find a path of cost less than 120 ({a mathematical formula}C=120), then expanding b is not necessarily the best option. For example, it might be better to expand a, which is probably very close to a goal of cost less than 120 (as {a mathematical formula}h(a)=3). Informally, we may say that a has more potential to lead to a solution of cost lower than C than b. Next, we define the notion of potential, and show its relation to {a mathematical formula}u(⋅).
      </paragraph>
      <paragraph label="Definition 2">
       The potential of a node n is defined as the probability that a node with h value equal to {a mathematical formula}h(n) has a path to a goal with a cost smaller than {a mathematical formula}C−g(n). For clarity, we omit here the mathematical prerequisites for defining probabilities in this context and provide them later (see Definition 4). PotentialGiven a heuristic function h and a cost bound C, we define the potential of node n, denoted {a mathematical formula}PTh,C(n), as{a mathematical formula}
      </paragraph>
      <paragraph>
    ...
    <section label="4">
     ...
     <section label="4.1">
      <section-title>
       Rationale for a non-parametric anytime search algorithm
      </section-title>
      <paragraph>
       In general, the need for non-parametric algorithms is prevalent in many fields of computer science and AI [41], [42], [43]. The main reason is that algorithms with parameters require some method of selecting the values for this parameter, i.e., a method for parameter tuning. Parameter tuning are often:
      </paragraph>
      <list>
       <list-item label="•">
    ...
    <section label="5">
     ...
     <paragraph>
      In this section we empirically evaluate the performance of PTS as a bounded-cost search algorithm, and {a mathematical formula}APTS/ANA⁎ as an anytime search algorithm. This is done over a range of domains: 15-puzzle, KPP-COM, robot arm, grid world planning and multiple sequence alignment (MSA). Next, we describe the domains used in our experiments.
     </paragraph>
     <section label="5.1">
      <section-title>
       Domains
      ...
      <paragraph>
       For every domain, we provide the domain details (what is a state etc.), the heuristic function used, and how the problem instances were generated.
      </paragraph>
      <section label="5.1.1">
       <section-title>
        The 15-puzzle
