 ...
 <body>
  <root>
   ...
   <content>
    ...
    <section label="2">
     <section-title>
      Design of a robotics deliberation system
     </section-title>
     <section label="2.1">
      <section-title>
      ...
      <list>
       ...
       <list-item label="•">
        Observing: detects and recognizes features and relations characterizing the state of the world, as well as events, actions and plans relevant for the task. Observing combines bottom-up processes, from sensors to meaningful data, with top-down activities such as focus mechanisms, sensing actions and planning for information gathering.
       </list-item>
       <list-item label="•">
     ...
     <section label="2.2">
      <section-title>
       Characteristics and deliberation requirements of robotics applications
      </section-title>
      <paragraph>
     ...
     <section label="2.3">
      <section-title>
       Design options of a deliberation system
      </section-title>
      <paragraph>
      ...
      <section>
       <section>
        ...
        <paragraph>
         This option assumes full predictive, possibly nondeterministic models of the world, including its dynamics and possible effects of exogenous events. It needs reversible models of the sensing capabilities of the robot, i.e., how reality is mapped into percepts, as well as reversible models of its actuating capabilities, i.e., how commands are mapped into forces and motion. Inverse kinematic models map desired configurations into commands achieving them; inverse sensing models map sensed features into world objects. It requires a planner that takes as input these global predictive models together with the specifications of the mission, and provides as output a plan or a policy, mapping perceived states (or beliefs) into lower level commands, possibly with some guarantee of achieving the mission or meeting some optimality criteria. Under this view, acting is a simple triggering of commands given by the planned policy: at each step the robot senses, i.e., reads the current observation variables, applying the corresponding commands specified by the policy. Note that a discrete unit time approach has to be further extended for dealing with domains involving rich temporal and real-time constraints.
        </paragraph>
        <paragraph>
       ...
       <section>
        ...
        <paragraph>
         The choice of an appropriate architecture is another major design issue. A robot implements its deliberation online. Hence all approaches have to interleave deliberation functions through some form of nested closed loops (Fig. 1(b)): acting is performed at an inner loop level while planning is processed at an outer loop level. Various architectural paradigms have been proposed to address the integration issue (see survey [127]):
        </paragraph>
        <list>
         <list-item label="•">
     ...
     <section label="2.4">
      <section-title>
       Global view of surveyed systems
      </section-title>
      <paragraph>
      ...
      <paragraph>
       It is important to underline that no ranking is intended through this clustering, which is introduced solely for the commodity of the presentation, to help surveying a broad field into a readable picture. Table 2, Table 3 summarize the main characteristics of the analyzed and discussed systems: the deliberation functions integrated, their knowledge representations, their type of robotics applications, the architectures used and additional features when relevant.
      </paragraph>
      <paragraph>
       In what follows, we'll review contributions that have an emphasis on, successively, planning, acting, monitoring, observing and modeling (Sections 3 Planning, 4 Acting, 5 Monitoring and goal reasoning, 6 Observing, 7 Acquiring deliberation models). In each of these sections, an introduction gives an overview of the corresponding function within integrated deliberation systems; a summary outlines the main features of surveyed work. A synthesis (Section 8) reviews the analyzed deliberation systems with respect to their main properties and application requirements.
      </paragraph>
     </section>
    </section>
    <section label="3">
     ...
     <section label="3.3">
      ...
      <paragraph>
       These algorithms assume SSP problems with a proper policy closed for {a mathematical formula}s0 (i.e., one that reaches a goal with probability 1), and any improper policy has infinite cost. A generalization of SSPs to relax this last assumption allows to seek a policy that maximizes the probability of reaching a goal state, a very useful and desirable criteria [124]. Other issues, such as dead-ends (states from which it is not possible to reach a goal) have to be taken care of, in particular in critical domains [122]. Heuristic search algorithms in SSPs are more scalable than dynamic programming, but they still cannot address large domains, unless these domains are carefully engineered and decomposed.
      </paragraph>
      <paragraph>
     ...
     <section label="3.4">
      ...
      <paragraph>
       Most classical planners rely on the closed-world assumption: a fact not explicitly stated is false.{sup:3} This assumption allows for a concise description of states and efficient computation of state-transitions. But it is too restrictive and seldom acceptable in robotics. A few techniques relax this assumption, e.g., planning with state-variables and timelines allows for partially specified instances of a domain. But other issues remain open, among them the following:
      </paragraph>
      <list>
       <list-item label="•">
    ...
    <section label="4">
     ...
     <section label="4.1">
      <section-title>
       Direct mapping of plan steps into robot commands
      </section-title>
      <paragraph>
     ...
     <section label="4.2">
      ...
      <paragraph>
       Other similar procedure based approaches have been successfully deployed, such as IPEM [7], EXEC on DS1/RAX [157], PRS-Lite [158], which offers a richer goal semantics than the regular PRS, Cypress [223] and CPEF (Continuous Planning and Execution Framework) [159], which combines PRS and Sipe [222].
      </paragraph>
      <paragraph>
     ...
     <section label="4.3">
      <section-title>
       Acting with automata and Petri net approaches
      </section-title>
      <paragraph>
    ...
    <section label="5">
     ...
     <section label="5.2">
      ...
      <paragraph>
       Finally, let us mention the interesting approach of [28] that uses domain knowledge expressed in description logic to derive expectations of the effects of actions in a plan to be monitored during execution. A first order query language allows online matching of these expectations against observations. Basically, the action parameters refer to world objects which have derived properties. These properties are checked to be either consistent or inconsistent with observations. They may even be of an undetermined consistency, permitting the monitoring function to deal with an open world semantics. An interesting extension handles flexible monitoring with probabilistic models, akin to Bayesien belief update. It relies on probabilistic plans with nondeterministic actions as well as on probabilistic sensing models. The approach was tested on indoor robot navigation and on simulated manipulation tasks.
      </paragraph>
     </section>
     <section label="5.3">
      <section-title>
       Temporal action logic approach
      </section-title>
      <paragraph>
    ...
    <section label="6">
     ...
     <section label="6.2">
      <section-title>
       Object recognition and symbol anchoring
      </section-title>
      <paragraph>
     ...
     <section label="6.3">
      <section-title>
       Event and situation recognition
      </section-title>
      <paragraph>
     ...
     <section label="6.5">
      ...
      <paragraph>
       We reviewed here contributions to perception planning, anchoring symbols and signals, and recognizing situations. We also discussed a comprehensive stream-based approach for integrating an observing system with planning, acting and monitoring. More work on similar systems is required to better qualify their capabilities and limitations for various robotics environments (in particular those marked “h” in column (v) and (viii) of Table 1).
      </paragraph>
     </section>
    </section>
    <section label="7">
     <section-title>
      Acquiring deliberation models
     </section-title>
     <paragraph>
     ...
     <section label="7.2">
      <section-title>
       Acquiring planning and acting models
      </section-title>
      <paragraph>
