 ...
 <body>
  <root>
   ...
   <content>
    ...
    <section label="2">
     <section-title>
      The computational protein design approach
     </section-title>
     <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.1">
      ...
      <paragraph>
       Local consistency is enforced using Equivalence Preserving Transformations (EPTs) that move costs between different cost functions [79], [54], [57], [18], [55], [15], [17], [16], [14]. For example, a variable i violating the NC property because all its values {a mathematical formula}ir have a non-zero {a mathematical formula}E(ir) cost, can be made NC by subtracting the minimum cost from all {a mathematical formula}E(ir) and adding this cost to {a mathematical formula}E∅. The resulting network is equivalent to the original network, but it has an increased lower bound {a mathematical formula}E∅.
      </paragraph>
      <paragraph>
       Interestingly, in CPD, the admissible heuristic used in the DEE/{a mathematical formula}A⁎ algorithm at depth d of the search tree is [33]:{a mathematical formula}
      </paragraph>
      <paragraph>
    ...
    <section label="4">
     <section-title>
      Computational protein design instances
     </section-title>
     <paragraph>
    ...
    <section label="5">
     ...
     <section label="5.1">
      ...
      <paragraph label="Definition 2">
       A discrete Markov random field (MRF) is a pair {a mathematical formula}(X,Φ) where {a mathematical formula}X={1,…,n} is a set of n random variables and Φ is a set of potential functions. Each variable {a mathematical formula}i∈X has a finite domain {a mathematical formula}Di of values that can be assigned to it. A potential function {a mathematical formula}ϕS∈Φ, with scope {a mathematical formula}S⊆X, is a function {a mathematical formula}ϕS:DS↦R.
      </paragraph>
      <paragraph>
       A discrete Markov random field (MRF) implicitly defines a non-normalized probability distribution over X. For a given tuple t, the probability of t is defined as:{a mathematical formula} where Z is a normalizing constant.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       The CPD problem can therefore directly be modeled as the MAP problem in an MRF exactly as we have described for CFN before, additive using potentials to capture energies (see for example [86]). Combinations of values with cost k (forbidden) are mapped to an infinite additive potential or a 0 value if multiplicative (exponential) potentials are used.
      </paragraph>
      <paragraph>
       These models can be solved using MAP-MRF solvers such as daoopt[69] (winner of the Pascal Inference Challenge in 2011{sup:4}) or the recent version of the mplp[80] solver.
      </paragraph>
     </section>
     <section label="5.2">
      <section-title>
     ...
     <section label="5.4">
      <section-title>
       0/1 quadratic optimization model
      </section-title>
      <paragraph>
      ...
      <paragraph>
       The corresponding model can be written as:{a mathematical formula}
      </paragraph>
      <paragraph>
     ...
     <section label="5.5">
      ...
      <paragraph label="Definition 3">
       A weighted partial MaxSAT (WPMS) instance is a set of pair {a mathematical formula}〈C,w〉, where C is a clause and w is a number in {a mathematical formula}N∪{∞}, which is called the weight of that clause. A clause is a disjunction of literals. A literal is a Boolean variable or its negation.
      </paragraph>
      <paragraph>
       If the weight of a clause is ∞, it is called a hard clause, otherwise it is a soft clause. The objective is to find an assignment to the variables appearing in the clauses such that all hard clauses are satisfied and the weight of all falsified soft clauses is minimized.
      </paragraph>
      <paragraph>
     ...
     <section label="5.6">
      ...
      <paragraph>
       The resulting CSP model has been expressed in the minizinc[65] constraint programming (CP) language. It can be solved using any CP solvers such as gecode, mistral, or Opturion/CPX, the recent winner of the MiniZinc Challenge 2013.
      </paragraph>
     </section>
    </section>
    <section label="6">
     ...
     <paragraph>
      For computing the GMEC, all computations were performed on a single core of an AMD Operon 6176 at 2.3 GHz, 128 GB of RAM, and a 9000-second time-out. These computations were performed on the GenoToul cluster.
     </paragraph>
     <section label="6.1">
      <section-title>
       Solvers tested
      ...
      <section>
       ...
       <paragraph>
        We downloaded the sources for the recent version 2 of the mplp (Message Passing Linear Programming) implementation [80], [81] available at http://cs.nyu.edu/~dsontag/.
       </paragraph>
       <paragraph>
        This solver uses a Message Passing based bound and duality theory to identify optimal solutions of an MAP-MRF problem through successive tightening of subsets of variables. The message passing used in mplp defines reparametrizations of the underlying MRF. These reparametrizations are similar to the reformulations done by local consistencies in CFN [79], [18]. The solver is unique in all the solvers considered in that it does not use branching but only increasingly strong inference by applying reparametrizations to set of variables that initially contain only pairwise potentials, reasoning on stars [35], and are incrementally enlarged to include several potentials and strengthen the corresponding bound [81], [80].
       </paragraph>
       <paragraph>
        All costs were divided by 1000 and the optimality gap threshold kept to the default of {a mathematical formula}2⋅10−4. The solver does not have any parameter.
       </paragraph>
      </section>
      <section>
       <section-title>
        ILP and QP optimization
       </section-title>
       <paragraph>
        We used cplex version 12.2 with parameters EPAGAP, EPGAP, and EPINT set to zero to avoid premature stop. No other tuning was done.
       </paragraph>
      </section>
      <section>
       <section-title>
       ...
       <paragraph>
        We used the biqmac[78] solver (http://biqmac.uni-klu.ac.at/biqmaclib.html) from sources provided by Angelika Wiegele. biqmac is a branch-and-bound solver relying on a strong Semidefinite Programming (SDP) bound for Boolean quadratic optimization. The SDP framework is known to provide strong bounds for a variety of combinatorial optimization problems among which MaxCut and Max2SAT, with guaranteed approximation ratios [36]. Two solver settings (with branching rule set to 2 or 3, as advised by the author) were tried with no significant difference in the performances.
       </paragraph>
      </section>
      <section>
       <section-title>
        Weighted partial MaxSAT optimization
       </section-title>
       <paragraph>
        The same problems have been translated to WPMS using the two previously described encodings. There are two categories of complete WPMS solvers that we consider here: branch-and-bound (B&amp;B) solvers and sequence-of-SAT solvers.
       </paragraph>
       <list>
        <list-item label="•">
     ...
     <section label="6.3">
      ...
      <paragraph>
       The new variable ordering heuristics consistently offer improved results. The effect of additional DEE processing is mostly visible on the difficult instances, the most visible and persistent improvements being obtained when using DEE in pre-processing, and for some instances (e.g., 1BRS, 1RIS) also maintaining {a mathematical formula}DEE1 during search. They offer speedups up to 6 (on 1GVP). Further tests on a variety of CFN benchmarks (http://costfunction.org) are reported in [34]. They show that {a mathematical formula}DEE1 allows to solve more problems and {a mathematical formula}DEE1 is now a default option of toulbar2.
      </paragraph>
     </section>
     <section label="6.4">
      <section-title>
