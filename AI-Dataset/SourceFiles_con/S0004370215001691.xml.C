   ...
   <content>
    ...
    <section label="3">
     ...
     <paragraph label="Definition 2">
      Top-k modelLet Φ be a propositional formula, {a mathematical formula}M a model of Φ, ⪰ a preference relation over the models of Φ and X a set of propositional variables. {a mathematical formula}M is a Top-k model w.r.t. ⪰ and X iff {a mathematical formula}|[P(Φ,M,⪰)]X|≤k−1.
     </paragraph>
     <paragraph>
      Let us note that the number of the Top-k models of a formula is not necessarily equal to k. Indeed, it can be strictly greater or smaller than k. For instance, if a formula is unsatisfiable, then it does not have a Top-k model for any {a mathematical formula}k≥1. Nevertheless, if the considered preference relation is a complete order, then the number of Top-k models is always smaller than or equal to k.
     </paragraph>
     <paragraph>
      It is easy to see that we have the following monotonicity property: if {a mathematical formula}M is a Top-k model and {a mathematical formula}M′⪰M, then {a mathematical formula}M′ is also a Top-k model.
     </paragraph>
     <paragraph>
      Top-k SAT problem. Let Φ be propositional formula, ⪰ a preference relation over the models of Φ, X a set of propositional variables and k a strictly positive integer. We call the tuple {a mathematical formula}(Φ,⪰,X,k) a Top-k instance. The Top-k SAT problem consists in computing a set {a mathematical formula}L of Top-k models of Φ with respect to ⪰ and X satisfying the two following properties:
     </paragraph>
     <list>
      <list-item label="1.">
       for all Top-k model {a mathematical formula}M, there exists {a mathematical formula}M′∈L such that {a mathematical formula}M≈XM′; and
      </list-item>
      <list-item label="2.">
       for all {a mathematical formula}M and {a mathematical formula}M′ in {a mathematical formula}L, if {a mathematical formula}M≠M′ then {a mathematical formula}M≉XM′.
      </list-item>
     </list>
     <paragraph>
    ...
    <section label="4">
     An application of Top-k SAT in itemset mining
     <paragraph>
    ...
    <section label="5">
     ...
     <section label="5.1">
      ...
      <section>
       <section>
        ...
        <paragraph>
         Let Σ be a finite set of items, called alphabet. A sequence of items s over Σ is a simple sequence of symbols {a mathematical formula}s0⋯sn−1 belonging to Σ. We denote by {a mathematical formula}|s| its length and by {a mathematical formula}Ps the set {a mathematical formula}{0,…|s|−1} of all the locations of its symbols. A wildcard is a new symbol ∘ which is not in Σ. This symbol matches any symbol of the alphabet.
        </paragraph>
       </section>
       <section>
        <section-title>
     ...
     <section label="5.3">
      Frequent pattern mining in a sequence of itemsets ({a mathematical formula}FPSI)
      <paragraph>
    ...
    <section label="6">
     ...
     <paragraph>
      For the problem {a mathematical formula}FCIMmink, we considered a variety of datasets (18 data instances) taken from the FIMI repository{sup:2} and CP4IM.{sup:3} Regarding the problem {a mathematical formula}FCPSmink, we used two different datasets:
     </paragraph>
     <list>
      <list-item label="1.">
       Bioinformatics: proteinic data encoded as a sequence of items, where an item is an amino-acid{sup:4};
      </list-item>
      <list-item label="2.">
       Computer security: user data drawn from the command histories of UNIX computer users{sup:5}[47].
      </list-item>
     </list>
     <paragraph>
     ...
     <paragraph>
      We compared, our method with a LCM algorithm proposed by Takeaki Uno et al. in [48], one of the best specialized algorithm, using the Top-k option. In Table 3, we provide the CPU time in seconds needed by LCM(Top-k option) and {a mathematical formula}FCIM1k respectively (separated by the slash symbol) for different values of k varying from 100 to 100 000.
     </paragraph>
     <paragraph>
     ...
     <paragraph>
      Concerning sequence mining, we are not aware of any available tool that enumerates the Top-k frequent closed patterns with wildcards in a sequence ({a mathematical formula}FCPSmink) as described in this paper. Consequently, our last experimental evaluation (see Table 4) aims to simply show the flexibility of our proposed declarative approach and its feasibility. However, to provide an idea on the performance gap between our Top-k sequence mining approach and specialized algorithms, we compare with MaxMotif the state-of-the-art sequence mining tool [5].{sup:6} As MaxMotif does not provide a Top-k option, we proceed in two steps. In the first step, we generate all the frequent closed patterns and in a second step we generate the Top-k by sorting the patterns according to their frequencies.
     </paragraph>
     <paragraph>
      Similarly to {a mathematical formula}FCIMmink, we set the minimum length min of the patterns to 1, while the value of k is varied from 100 to 10 000.
     </paragraph>
     <paragraph>
      Let us first mention that on the considered data sets, the encoding of {a mathematical formula}FCPSmink leads to large CNF formula comparatively to those considered in {a mathematical formula}FCIMmink encoding. Indeed, for the sequence User-400 containing 400 solid characters, the set of clauses required for its encoding exceeds 4 millions of clauses. The same observation can be made on the number of variables.
     </paragraph>
     <paragraph>
