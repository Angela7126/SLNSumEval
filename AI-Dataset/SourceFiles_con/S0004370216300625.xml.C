  ...
  <root>
   ...
   <content>
    <section label="1">
     ...
     <paragraph>
      Our results demonstrate that parHydra transforms single solvers into parallel portfolios both well and robustly. Its performance on standard 8-core CPUs compared favorably with that of hand-crafted parallel SAT solvers. For the generation of parallel algorithm portfolios based on a set of both parallel and sequential solvers, we found that parHydrab was best among the alternatives we considered, notably outperforming pfolioUZK. More detailed experimental results and open-source code are available at http://www.cs.uni-potsdam.de/acpp.
     </paragraph>
    </section>
    <section label="2">
     <section-title>
     ...
     <section label="2.1">
      ...
      <paragraph>
       Katsirelos et al. [46] showed that an effective parallelization of a CDCL SAT solver does not merely hinge on picking a good clause sharing strategy, since it is not straightforward to obtain shorter resolution proofs by parallelization without essential changes of the underlying sequential reasoning mechanism. Our ACPP does not aim at parallelizing the resolution proof, but rather runs multiple algorithms and algorithm configurations in parallel, in order to maximize the probability that at least one of them finds a short proof quickly.
      </paragraph>
     </section>
     <section label="2.2">
      <section-title>
    ...
    <section label="3">
     <section-title>
      Parallel portfolio configuration from a single sequential solver
     </section-title>
     <paragraph>
     ...
     <section label="3.2">
      ...
      <section label="3.2.4">
       ...
       <paragraph>
        We chose a cutoff time of 180 seconds for algorithm configuration on the training set and 900 seconds for evaluating solvers on the test set (as in the 2012 SAT Challenge). Additionally, we performed three repetitions of each solver and test instance run and report the median of those three runs. We restricted all solver runs (on both training and test sets) to use at most 12 GB of memory (as in the 2012 SAT Challenge). If a solver terminated because of memory limitations, we recorded it as a timeout. We performed all solver and configurator runs on Dell PowerEdge R610 systems with 48 GB RAM and two Intel Xeon E5520 CPUs with four cores each (2.26 GHz and 8 MB Cache), running 64-bit Scientific Linux (2.6.18-348.6.1.el5).
       </paragraph>
      </section>
      <section label="3.2.5">
       <section-title>
    ...
    <section label="4">
     <section-title>
      Parallel portfolio configuration with multiple sequential solvers
     </section-title>
     <paragraph>
     ...
     <section label="4.2">
      ...
      <section label="4.2.1">
       ...
       <list>
        ...
        <list-item label="•">
         sparrow2011[67]: 0 parameters{sup:6}
        </list-item>
        <list-item label="•">
         TNM[50]: 0 parameters
        </list-item>
    ...
    <section label="5">
     <section-title>
      Parallel portfolio configuration with multiple sequential and parallel solvers
     </section-title>
     <paragraph>
     ...
     <section label="5.2">
      ...
      <paragraph>
       We used the set of solvers described in Section 4.2, with the addition of Plingeling. We added parHydrab to the set of ACPP methods considered and allowed {a mathematical formula}b∈{2,4}. We use the same setup as before, except that we allowed a 20-hour configuration budget per configured process, twice as much as before, to take into consideration the greater variation in solving behavior of Plingeling which induces a harder configuration task.
      </paragraph>
      <paragraph>
       We compared our results to a variety of state-of-the-art solvers from the 2012 SAT Challenge on this benchmark set. We considered two state-of-the-art sequential solvers: glucose (2.1) [6] (winner of the single-engine application track—like all other competition results cited below, in the 2012 SAT Challenge); and SATzilla-App[74], which is SATzilla trained on application instances (winner of the sequential portfolio application track). We also considered the following high-performance parallel solvers{sup:7}:
      </paragraph>
      <list>
       <list-item label="•">
        clasp ({a mathematical formula}2.1.3) [26];
       </list-item>
       <list-item label="•">
        Plingeling (ala) [14] and Plingeling (aqw) [15]{sup:8};
       </list-item>
       <list-item label="•">
        ppfolio[63] (bronze medal in the parallel track);
       </list-item>
       <list-item label="•">
        PeneLoPe[5] (silver medal in the parallel track);
       </list-item>
       <list-item label="•">
        and again pfolioUZK[70] (winner of the parallel track).
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph>
       parHydra4-MP(8) produced the best parallel portfolio solver overall, which turned out to be significantly faster than pfolioUZK. The portfolio solvers produced by parHydra-MP(8) and parHydra2-MP(8) exhibited no significant performance differences from pfolioUZK. Furthermore, parHydra4-MP(8) also solved more instances than Plingeling(aqw), although Plingeling(aqw) won the 2013 SAT competition and the solvers in parHydra4-MP(8) were mostly published in 2011, which gives Plingeling(aqw) an advantage of two additional years of development.
      </paragraph>
      <paragraph>
