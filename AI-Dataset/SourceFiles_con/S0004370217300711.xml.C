   ...
   <content>
    ...
    <section label="2">
     ...
     <section label="2.2">
      ...
      <paragraph>
       DL-Lite concepts are constructed and interpreted in a similar way to {a mathematical formula}EL concepts. We briefly introduce the syntax of the DL-Lite family following the notation in [10], restricting our attention to the members of DL-Lite that we use in this paper only. For a full overview on DL-Lite, we refer the reader to [10].
      </paragraph>
      <paragraph>
       Let A be a concept name, r a role name, and q a natural number. Then DL-Lite concepts and roles are constructed as follows:{a mathematical formula} Concepts of the form B are called basic, and those of form C are called general concepts. The semantics of this constructors are presented in the second column of Table 1.
      </paragraph>
      <paragraph>
     ...
     <section label="2.4">
      ...
      <paragraph label="Definition 1">
       Axiom pinpointing refers to the task of finding the specific axioms that are responsible for a given consequence to follow from a KB. When dealing with monotonic consequences, as in the case of DLs and propositional logic, this task equivalently corresponds to finding the minimal subsets (w.r.t. set inclusion) of the KB that entail the consequence. We call such a set a MinA.{sup:1}MinALet {a mathematical formula}K be a set of axioms and φ be a logical consequence of it, i.e., {a mathematical formula}K⊨φ. We call a set {a mathematical formula}M⊆K a minimal axiom set or MinA for φ in {a mathematical formula}K if {a mathematical formula}M⊨φ and for every {a mathematical formula}M′⊂M, it holds that {a mathematical formula}M′⊭φ.
      </paragraph>
      <paragraph>
       It has been shown that a single MinA for a consequence can be computed by calling a reasoner as many times as there are axioms in the KB [37], [38]. The idea is to systematically remove one axiom at a time from the KB, while the consequence is preserved. In particular this means that in every logic with polynomial-time reasoning, a MinA can be computed in polynomial time, with the degree of the polynomial increasing by one.
      </paragraph>
      <paragraph>
       However, a single consequence from a KB may have several, potentially exponentially many, MinAs, and in many cases computing only one of them does not suffice. For instance, if the consequence is unwanted, then correcting one of the ways in which it can be derived does not ensure that the consequence will not follow from some other combination of axioms in the KB. It is thus important to analyse the complexity of finding all the MinAs or deciding some of their properties.
      </paragraph>
      <paragraph>
       We notice that some previous work on axiom pinpointing has focused on the computation of a so-called pinpointing formula [39], [40], [41], that provides a compact encoding of all MinAs. However, extracting the specific MinAs and other information from this formula is already a hard problem. For that reason, in this paper we focus only on the computation of MinAs as subsets of the KB, and not in the construction of an alternative characterization.
      </paragraph>
      <paragraph>
       In order to analyze the complexity of enumerating and counting the (potentially exponentially many) solutions to a problem, it is necessary to consider some special complexity classes. Before presenting our results, we briefly recall some of the basic enumeration and counting complexity classes and their properties.
      </paragraph>
     </section>
     <section label="2.5">
      <section-title>
      ...
      <paragraph>
       In complexity theory, we are sometimes interested not only in deciding whether a problem has a solution or not, but also in enumerating all solutions of the problem. We call such problems enumeration problems. For analyzing the complexity of enumeration problems where the number of solutions can be exponential in the size of the input, one needs appropriate measures. One such measure is the notion of polynomial delay. We say that an algorithm runs with polynomial delay[42] if the time until the first solution is generated, and thereafter the time between any two consecutive solutions, is bounded by a polynomial in the size of the input. An example of such an algorithm is the one given in [43] that enumerates all maximal independent sets of a graph with polynomial delay.
      </paragraph>
      <paragraph>
       Another measure of performance is to take into account not only the size of the input, but also the size of the output. We say that an enumeration algorithm runs in output polynomial time (or polynomial total time) [42] if it outputs all solutions in time polynomial in the size of the input and the output. Clearly, every polynomial delay algorithm is also an output polynomial algorithm, i.e., the notion of polynomial delay is stronger than the notion of output polynomial. One advantage of an output polynomial algorithm is that it runs in polynomial time whenever the problem has polynomially many solutions. However, an output polynomial algorithm may for instance first compute all solutions and then output them all together. With polynomial delay algorithm on the other hand, a user needs to wait only polynomial time between each retrieved solution, regardless of how many solutions there are in total. Such an algorithm is especially good if one wants to enumerate the solutions one at a time and maybe stop the execution before all of them have been found; for instance after k solutions have been output.
      </paragraph>
      <paragraph>
       An intermediate notion between polynomial delay and output polynomial is that of incremental polynomial. An enumeration algorithm is incremental polynomial if the time required for generating each new solution is polynomial in the size of the input and the output generated so far. Clearly, every polynomial delay algorithm is incremental polynomial, and every incremental polynomial algorithm is also output polynomial.
      </paragraph>
      <paragraph label="Proposition 2">
       A more complicated situation is when the solutions are required to be output in some pre-specified order such as a lexicographic order (see Definition 11). In general, the complexity of enumerating solutions in a specified order is of interest only if polynomial delay or incremental polynomial algorithms exist. Indeed, if all solutions can be enumerated in output polynomial time, then they can be enumerated in output polynomial time in any desired order: one needs only to generate all solutions, sort them and output them in the new order. If an enumeration problem can be solved in output polynomial time, then it can be solved in any polynomially-computable order in output polynomial time.
      </paragraph>
      <paragraph>
       A good example of an algorithm that provides ordered solutions with polynomial delay is the one introduced in [42] that generates maximal independent sets of a graph in lexicographic order with polynomial delay.
      </paragraph>
     </section>
     <section label="2.6">
      <section-title>
    ...
    <section label="3">
     ...
     <paragraph>
      In this section, we analyze the complexity of the main decision problems that occur in axiom pinpointing, with a focus on the tractable DLs introduced before. Specifically, we study the problems of deciding whether a KB is a MinA (is-mina ), whether there exists a MinA that does not contain any of a given set of KBs (mina-irrelevance ), and whether there exists a MinA containing a given axiom (mina-relevance ). These results are summarized in Table 2. As it can be seen, the complexity of these problems is governed by that of standard reasoning in the logic. In Section 4 we analyze the complexity of enumerating MinAs, and in Section 5 we study the complexity of counting MinAs.
     </paragraph>
     <section label="3.1">
      Deciding is-mina
      <paragraph>
       We start by formalizing a result that was already hinted in Section 2.4 regarding the problem of deciding whether a given set of axioms is already a MinA.
      </paragraph>
      <list>
       <list-item>
        Problem:is-mina
     ...
     <section label="3.3">
      ...
      <list>
       ...
       <list-item>
        Input: A KB {a mathematical formula}K and an axiom φ of the same type as {a mathematical formula}K such that {a mathematical formula}K⊨φ, and an axiom {a mathematical formula}ψ∈K.
       </list-item>
       <list-item>
        Question: Is there a MinA {a mathematical formula}M for φ in {a mathematical formula}K such that {a mathematical formula}ψ∈M?
       </list-item>
      </list>
      <paragraph label="Proof">
       If we identify a specific axiom ψ as a possible culprit for an erroneous consequence φ from a KB, mina-relevance would allow us to decide whether ψ indeed appears in at least one MinA, and hence influences the deduction of the consequence φ from the KB. We now show that this problem is np-complete already for core KBs. mina-relevanceisnp-complete for core KBs.The problem is clearly in np: a nondeterministic algorithm for solving it first guesses a subset of {a mathematical formula}K, and then tests in polynomial time whether it is a MinA containing ψ. For showing hardness we provide a reduction from the following np-complete problem [52]:
       <list>
        Problem:path-via-nodeInput: A directed graph {a mathematical formula}G=(V,E) and three vertices {a mathematical formula}s,t,m∈V.Question: Is there a simple path from s to t in {a mathematical formula}G that passes through m?Let
       </list>
       <paragraph>
      ...
      <paragraph>
       As was the case for mina-irrelevance, hardness of the core case implies hardness for all the other types of KBs we are interested in.
      </paragraph>
      <paragraph label="Proof">
       For all kinds of KBs introduced in Section2,mina-relevanceisnp-hard. Moreover, for all of them, except for{a mathematical formula}DL-Litebool, the problem isnp-complete.np-hardness is a consequence of Theorem 7. For proving that the problem is in np, we use the same argument as in the proof of that theorem: a nondeterministic algorithm for solving it first guesses a set {a mathematical formula}M⊆K, then tests in polynomial time whether it is a MinA containing ψ.  □
      </paragraph>
      <paragraph>
       Once again, the algorithm proposed in the proof of this corollary yields a {a mathematical formula}Σ2p upper bound for the case of {a mathematical formula}DL-Litebool KBs. A matching lower bound for this logic can be obtained through a reduction from the {a mathematical formula}Σ2p-complete problem mus-membership[53].
      </paragraph>
      <list>
       <list-item>
        Problem:mus-membership
       </list-item>
       <list-item>
        Input: An inconsistent set of propositional clauses {a mathematical formula}F and a clause {a mathematical formula}ϕ∈F.
       </list-item>
       <list-item>
        Question: Is there a MUS {a mathematical formula}M for {a mathematical formula}F such that {a mathematical formula}ϕ∈M?
       </list-item>
      </list>
      <paragraph label="Proof">
       mina-relevanceis{a mathematical formula}Σ2p-complete for{a mathematical formula}DL-LiteboolNKBs.The upper bound was argued before. To show hardness, let {a mathematical formula}F,ϕ be an instance of mus-membership, and {a mathematical formula}TF,tϕ be constructed as in the proof of Theorem 4. Then, there is a MinA for {a mathematical formula}A⊑⊥ in {a mathematical formula}TF that contains {a mathematical formula}tϕ iff there is a MUS for {a mathematical formula}F that contains ϕ.  □
      </paragraph>
      <paragraph>
       We now change our attention to the problem of computing all the MinAs for a given consequence, and analyse the complexity of enumerating them with or without a specific ordering.
      </paragraph>
     </section>
    </section>
    <section label="4">
     ...
     <section label="4.1">
      ...
      <paragraph>
       Once that we have found an immediate MinA, Definition 14 introduces a class of sub-KBs {a mathematical formula}Ti,1≤i≤m of {a mathematical formula}GT. We show that this class induces a partition on the set of all other MinAs in {a mathematical formula}GT.
      </paragraph>
     ...
     <section label="4.2">
      ...
      <list>
       ...
       <list-item>
        Input: A KB {a mathematical formula}K and an axiom φ of the same type as {a mathematical formula}K such that {a mathematical formula}K⊨φ, a MinA {a mathematical formula}M for φ in {a mathematical formula}K, and a linear order on {a mathematical formula}K.
       </list-item>
       <list-item>
        Question: Is {a mathematical formula}M the first MinA w.r.t. the lexicographic order induced by the given linear order?
       </list-item>
      </list>
      <paragraph label="Proof">
       ...
       <list>
        {a mathematical formula}ψ&lt;χ for every {a mathematical formula}χ≠ψ, andfor every {a mathematical formula}χ∈M and every {a mathematical formula}χ′∈K∖(M∪{ψ}), {a mathematical formula}χ&lt;χ′.(⇒) Suppose that there is a MinA
       </list>
       <paragraph>
    ...
    <section label="5">
     <section-title>
      Complexity of counting MinAs
     </section-title>
     <paragraph>
      In applications where one is interested in computing all MinAs, it might also be useful to know in advance how many of them exist. For that reason, we consider the following counting problem.
     </paragraph>
     <list>
      <list-item>
       Problem:#mina
      ...
      <list-item>
       Input: A KB {a mathematical formula}K and an axiom ϕ of the same type as {a mathematical formula}K such that {a mathematical formula}K⊨ϕ.
      </list-item>
      <list-item>
       Output: The number of all MinAs for ϕ in {a mathematical formula}K.
      </list-item>
     </list>
     <paragraph>
      As already described in the previous sections, if {a mathematical formula}K is a core KB, the problem #mina boils down to the problem of counting simple paths between two vertices of a given directed graph. This problem called s-t connectedness was one of the first counting problems considered in [45].
     </paragraph>
     <list>
      <list-item>
       Problem:s-t paths
      ...
      <list-item>
       Input: A directed graph {a mathematical formula}G=(V,E), and two vertices {a mathematical formula}s,t∈V.
      </list-item>
      <list-item>
       Output: The number of simple paths from s to t in G.
      </list-item>
     </list>
     <paragraph>
      In [45] it was shown that this problem is #p-complete. Recall that #pis defined [44] as the class of functions counting the accepting paths of nondeterministic Turing machines. Typical members of this class are the problems of counting the number of solutions of np-complete problems. Among them, the most well-known one is #sat, which is the problem of counting the distinct truth assignments that satisfy a given Boolean formula in CNF. Intuitively, #pis the counting-complexity analogous of the class npfor decision problems.
     </paragraph>
     <paragraph>
      Since core KBs are the simplest type of KB, the hardness result applies to the other KB types we consider here. Moreover for most of the types of axioms considered in this paper, the problem of deciding whether a given set of axioms is a MinA is polynomial. This implies that for these fragments #mina is in #p, thus it is #p-complete.
     </paragraph>
     <paragraph label="Corollary 42">
      #minais #p-complete for core, Horn, dual-Horn, Horn-{a mathematical formula}EL,{a mathematical formula}EL,{a mathematical formula}DL-LitecoreNand{a mathematical formula}DL-LitehornNKBs.
     </paragraph>
     <paragraph>
      Next we consider a variant of this counting problem. Instead of the number of all MinAs, one can also be interested in the number of MinAs that contain a specific axiom. We call this problem #mina-relevance.
     </paragraph>
     <list>
      <list-item>
       Problem:#mina-relevance
