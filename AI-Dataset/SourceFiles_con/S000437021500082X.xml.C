  ...
  <root>
   ...
   <content>
    ...
    <section label="3">
     ...
     <paragraph>
      The use of heuristics in the SARSA algorithm was recently proposed by Bianchi et al. [38]. In the same paper, the authors also expand the number of HARL algorithms by proposing the Heuristically Accelerated Q(λ), HA-SARSA(λ) and HA-TD(λ), the first algorithms that used both heuristics and eligibility traces.
     </paragraph>
     <paragraph>
    ...
    <section label="4">
     <section-title>
      Transferring a case base of heuristics: the L3 meta-algorithm
     </section-title>
     <paragraph>
     ...
     <list>
      ...
      <list-item label="•">
       L3-SARSA(λ), which extends the SARSA(λ) algorithm to include the use of cases as heuristics in the action selection.
      </list-item>
     </list>
    </section>
    <section label="5">
     ...
     <paragraph>
      In this section we present experiments where two L3 algorithms, L3-Q and L3-SARSA(λ), are applied in two domains: one which is a traditional benchmark in Reinforcement Learning, the Mountain Car Problem; and a more recent control problem, the Stabilisation of a Humanoid Robot. The first experiment shows how a control strategy can be mapped and transferred between similar domains and the last experiment shows a more complex transfer procedure. Our claim here is that L3 outperforms its non-transfer learning version, and also state-of-the-art transfer learning algorithms.
     </paragraph>
     <paragraph>
     ...
     <list>
      <list-item label="1.">
       Jumpstart: the initial improvement of the performance of an agent in the target task, given by the transfer;
      </list-item>
      <list-item label="2.">
       Asymptotic performance: the final learned performance of the agent in the target task;
      </list-item>
      <list-item label="3.">
       Total reward: the total reward earned by the agent;
      </list-item>
      <list-item label="4.">
       Transfer ratio: the ratio between the total reward received by an agent that used the transfer and the one received by an agent that does not use transfer learning;
      </list-item>
      <list-item label="5.">
       Time to threshold: the time taken by an agent to achieve a pre-defined level of performance.
      </list-item>
     </list>
     <paragraph>
     ...
     <section label="5.1">
      <section-title>
       Experiment 1: mountain car problem
      </section-title>
      <paragraph>
      ...
      <list>
       <list-item label="•">
        Select N cases by random sampling the state set and finding the best action for that state. To do this, we define the position and speed using a random number generator, then we find the action that maximises this state ({a mathematical formula}argmax(Q)), and its Q value, saving this data as a case. In this way, the case base is built using the best actions for the 2D problem, for a uniformly distributed set of states.
       </list-item>
       <list-item label="•">
       ...
       <list-item label="•">
        Select N cases that have the best Q value. To do this, we sample {a mathematical formula}N⁎1000 times the state set, finding the best action and its Q value for a sampled state. Then, only the N cases with the highest values of Q are selected to be included in the case base. By doing this, we built a case-base with the N best state-action pairs.
       </list-item>
       <list-item label="•">
        For comparison purposes, we also built a set with the N worst cases, obtained analogously to the first possibility cited above, but using {a mathematical formula}argmin(Q) instead of {a mathematical formula}argmax(Q).
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph>
       Thirty training sessions (each of which contained 1000 episodes) were executed for four algorithms in the 3D domain: SARSA(λ), HA-SARSA(λ), L3-SARSA(λ) and the TiMRLA Value-Addition algorithm (described in Fachantidis et al. [31]). For comparison purposes we also include the results obtained by Taylor's MASTER Algorithm, published in [29, Chapter 7].
      </paragraph>
      <paragraph>
     ...
     <section label="5.2">
      ...
      <paragraph>
       At the last stage of the L3-Q algorithm, the case base is used in the CB-HAQL algorithm to learn Nao's equilibrium position. The features used to compute the distance between a case and the problem are the joint angles (assumed as states in both domains). In this experiment, the distance function is defined as the Gaussian distance between attributes of the source domain and target domain:{a mathematical formula} where {a mathematical formula}τx,τy are the radius of the scope around the object. The Gaussian distance is used because the larger the distance between two points, the lower is the similarity between them. Also, the {a mathematical formula}τx,τy parameters are used as thresholds that define a maximum distance allowed for two points to have some degree of similarity, if the distance is greater than a limit, for two object's a and b, then {a mathematical formula}Sim(a,b)=0.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       This is computed using the equation below:{a mathematical formula}
      </paragraph>
      <paragraph>
    ...
    <section label="6">
     ...
     <paragraph>
      In this paper we compared L3 with HAQL but not with its case-base counterpart, CB-HAQL. In order to compare with CB-HAQL a tailor-made case base would have to be built. Not only this is a tedious process, but the results obtained would be very similar to those obtained with HAQL, since a tailor-made case base would be constructed using some pre-defined heuristics.
     </paragraph>
     <paragraph>
