 ...
 <body>
  <root>
   ...
   <content>
    ...
    <section label="2">
     <section-title>
      Examples of multiple-instance classification problems
     </section-title>
     <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.2">
      <section-title>
       Completeness of the proposed categorization
      </section-title>
      <paragraph>
      ...
      <paragraph>
       In any categorization we will always find methods that fall close to the boundaries of two categories. For example, in our taxonomy this happens with the BARTMIP method (see Section 7.6). This is an ES method where each bag X is explicitly mapped into a vector {a mathematical formula}v→. However, in order to perform this mapping, the bag X is compared with other bags Y from the training set through the definition of a bag-level distance function {a mathematical formula}D(X,Y). In this sense, this method lies close to the boundary between the ES and BS categories.
      </paragraph>
     </section>
     <section label="3.3">
      <section-title>
    ...
    <section label="4">
     ...
     <section label="4.2">
      <section-title>
       IS methods following the collective assumption
      </section-title>
      <paragraph>
      ...
      <section label="4.2.1">
       ...
       <paragraph>
        As explained in Section 4.1.1, the IS methods do not deal well in situations where the discriminative classifier should consider information beyond the single instance. This type of situations require either BS or ES method, which we review in Sections 5 and 6 respectively.
       </paragraph>
      </section>
     </section>
    </section>
    <section label="5">
     ...
     <section label="5.1">
      <section-title>
       Distance-based and kernel-based classifiers
      </section-title>
      <paragraph>
      ...
      <paragraph>
       Conversely, the kernel functions provide some measure of similarity between bags, and they can be used in distance-based classifiers by using the following transformation: {a mathematical formula}D(X,Y)=K(X,X)−2K(X,Y)+K(Y,Y), as explained in [14].
      </paragraph>
      <paragraph>
    ...
    <section label="6">
     ...
     <section label="6.1">
      <section-title>
       ES methods without vocabularies
      </section-title>
      <paragraph>
       These methods simply aggregate statistics about the attributes of all the instances contained in the bag, without making any differentiation among these instances. Dong et al. [11], for example, propose the so-called Simple MI method that maps each bag X to the average of the instances inside {a mathematical formula}M(X)=1|X|∑x→∈Xx→.
      </paragraph>
      <paragraph>
    ...
    <section label="7">
     ...
     <section label="7.1">
      <section-title>
       Idea behind the vocabulary-based methods
      </section-title>
      <paragraph>
     ...
     <section label="7.2">
      <section-title>
       Characterization of the Vocabulary-based methods
      </section-title>
      <paragraph>
     ...
     <section label="7.3">
      ...
      <section label="7.3.2">
       <section-title>
        Bag-of-Words with Gaussian Mixture Models
       </section-title>
       <paragraph>
      ...
      <section label="7.3.4">
       ...
       <paragraph>
        A different thing would be that the whole vector {a mathematical formula}v→=(v1,v2) is passed into a learner. Based on this information, given for all the bags of the training set, the learner could obtain the optimal thresholds for each component {a mathematical formula}v1 and {a mathematical formula}v2 in order to obtain the bag-level classifier {a mathematical formula}F(X). This learning process would consider bag-level information represented by {a mathematical formula}v→=(v1,v2). Note that this happens in the ES methods, but not in the IS methods.
       </paragraph>
       <paragraph>
        As a conclusion, we can see that the SMI assumption can be considered a particular case of the assumption followed by part of the Vocabulary-based methods (the one based on histograms, which form just a small subset). However, according to the criteria followed in this work (i.e., at what level the information is learned in a discriminative manner) the IS methods in Section 4.1 cannot be considered part of the ES methods using vocabularies.
       </paragraph>
      </section>
     </section>
     <section label="7.4">
    ...
    <section label="9">
     ...
     <section label="9.2">
      ...
      <section label="9.2.1">
       <section-title>
        Implementation details for Vocabulary-based methods
       </section-title>
       <paragraph>
