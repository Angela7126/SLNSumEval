  ...
  <root>
   ...
   <content>
    ...
    <section label="3">
     ...
     <section label="3.1">
      ...
      <section label="3.1.1">
       ...
       <paragraph>
        A numerical constraint satisfaction problem (NCSP) is defined as a triple {a mathematical formula}〈v,[v],c〉 that consists of
       </paragraph>
       <list>
        <list-item label="•">
         a vector of variables{a mathematical formula}v=(v1,…,vn),
        </list-item>
        <list-item label="•">
         an initial domain, in the form of a box {a mathematical formula}[v]=([v1],…,[vn])∈IRn, and
        </list-item>
        <list-item label="•">
         a constraint{a mathematical formula}c(v):=(f(v)=0∧g(v)⩾0), {a mathematical formula}f:Rn→Re and {a mathematical formula}g:Rn→Ri, i.e., a conjunction of e equations and i inequalities.
        </list-item>
       </list>
       <paragraph>
        A solution of an NCSP is an assignment of its variables {a mathematical formula}v∈[v] that satisfies its constraints. The solution set Σ of an NCSP is the region within its initial domain that satisfies its constraints, i.e., {a mathematical formula}Σ([v]):={v∈[v]|c(v)}.
       </paragraph>
      </section>
      <section label="3.1.2">
       <section-title>
    ...
    <section label="4">
     ...
     <section label="4.1">
      <section-title>
       From the NCSP model to the generalized aspects computation
      </section-title>
      <paragraph>
       We aim at computing a (finite) set of boxes {a mathematical formula}S⊆IRn×IRn together with (undirected) links {a mathematical formula}N, i.e. 2-subsets of {a mathematical formula}S, satisfying the following three properties:
      </paragraph>
      <list>
       <list-item>
        {a mathematical formula}∀([x],[q])∈S,∀x∈[x],∃ a unique q∈[q],f(x,q)=0;
       </list-item>
       <list-item>
        {a mathematical formula}∀([x],[q])∈S,∀x∈[x],∀q∈[q],detFx(x,q)≠0∧detFq(x,q)≠0;
       </list-item>
       <list-item>
        {a mathematical formula}∀{([x],[q]),([x′],[q′])}∈N,∃(x,q)∈([x],[q])∩([x′],[q′]),f(x,q)=0.
       </list-item>
      </list>
      <paragraph>
     ...
     <section label="4.3">
      <section-title>
       Computing and certifying links
      </section-title>
      <paragraph>
       The computation of {a mathematical formula}N, i.e., the links that satisfy Property {a mathematical formula}(P3), is done in two steps:
      </paragraph>
      <list>
       <list-item label="1.">
        Maintain the neighborhood graph{a mathematical formula}(R,M), where {a mathematical formula}R=L∪S∪U is the set of all boxes produced by the algorithm, defined as the graph between the boxes in {a mathematical formula}R which share at least a common point;
       </list-item>
       <list-item label="2.">
        Compute the certified neighborhood graph{a mathematical formula}(S,N), where {a mathematical formula}N⊆M∩2S is the set of links between certified boxes that satisfy Property {a mathematical formula}(P3).
       </list-item>
      </list>
      <paragraph>
     ...
     <section label="4.4">
      ...
      <paragraph>
       We present two ways to compute connected sets of nonsingular configurations (CSNCs) from the graphs {a mathematical formula}(R,M) and {a mathematical formula}(S,N).
      </paragraph>
      <section label="4.4.1">
       <section-title>
        Simple CSNCs construction
       </section-title>
       <paragraph>
      ...
      <section label="4.4.2">
       <section-title>
        CSNCs construction and separation
       </section-title>
       ...
       <paragraph>
        Obviously, boxes in two different {a mathematical formula}(Ris,Mis) and {a mathematical formula}(Ri′s′,Mi′s′) cannot contain solutions that belong to the same aspect. Therefore,{a mathematical formula} is a lower bound of the number of aspect. On the other hand, as mentioned in the previous subsection, we expect {a mathematical formula}(Sijs,Nijs) to contain numerous spurious CSNCs due to the instability of the proving process close to singular regions. The same heuristic filtering can thus be used to isolate the largest CSNCs of practical interest.
       </paragraph>
      </section>
     </section>
    </section>
    <section label="5">
     ...
     <section label="5.4">
      ...
      <section label="5.4.2">
       ...
       <paragraph>
        As in the link collisions, this constraint will remove some regions from the computed CSNCs. Fig. 8(c) presents the joint-limited CSNCs of the RRRRR robot resulting from the original CSNC depicted in Fig. 8(a) when the angle at the end-effector P is limited to {a mathematical formula}[−π/2,π/2]. It also presents two poses: Pose {a mathematical formula}P1 is consistent with the joint limit and thus remains within the original joint-limited CSNC. Pose {a mathematical formula}P2 did belong to the original CSNC but it does not belong to the computed joint-limited CSNC since, at this pose, the joint limit is not respected. Blue boxes represent joint limit boundaries where the limited joint angle becomes {a mathematical formula}π/2 or {a mathematical formula}−π/2.
       </paragraph>
       <paragraph>
        The other joint-limited CSNCs are depicted in Fig. 10. We have twelve joint-limited CSNCs where the first and sixth original CSNCs in Fig. 7(b) have been split in two by the joint limit constraint.
       </paragraph>
      </section>
      <section label="5.4.3">
       <section-title>
