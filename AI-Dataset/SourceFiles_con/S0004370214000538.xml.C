   ...
   <content>
    <section label="1">
     ...
     <list>
      ...
      <list-item label="•">
       We propose bounded ε-first, the first algorithm that efficiently tackles the bounded MAB model.
      </list-item>
      <list-item label="•">
       We devise the first theoretically proven upper bound for the performance regret of the bounded ε-first algorithm.
      </list-item>
     </list>
     <paragraph>
    ...
    <section label="2">
     ...
     <section label="2.2">
      ...
      <paragraph>
       One area of work that is well suited to solving the expert crowdsourcing problem is the field of multi-armed bandits (MABs), a class of problems dealing with decision making under uncertainty. In these optimisation problems, actions (i.e., pulling a single arm) have initially unknown rewards that have to be learnt through noisy observations, and the goal is to maximise the total amount of rewards by sequentially choosing different actions over time [29], [1], [4]. In particular, a MAB model consists of a machine with K arms, each of which delivers rewards that are independently drawn from an unknown distribution when the machine's arm is pulled. Our goal is to choose which of these arms to play. At each time step, we pull one of the machine's arms and receive a reward (or payoff). The objective is to maximise the return; that is, to maximise the sum of the rewards received over a sequence of pulls. As the reward distributions differ from arm to arm, the goal is to find the arm with the highest expected payoff as early as possible, and then to keep gambling using that best arm [29], [4].
      </paragraph>
      <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.2">
      ...
      <paragraph>
       Given the bounded MAB model above, we now show how to map the expert crowdsourcing problem to bounded MABs. In particular, within an expert crowdsourcing system, an employer (agent) can assign tasks to a finite set of workers. This set of workers is usually determined through an open call for participation by the employer, to which qualified and available workers respond.{sup:7} Each worker i corresponds to an arm and assigning a single task to that worker can be regarded as pulling the arm. This incurs a cost {a mathematical formula}ci that is set by the worker, and the outcome of the assignment is of variable utility with unknown mean {a mathematical formula}μi (this corresponds to the rewards in the bounded MAB). As described in Section 1, each worker i has a different maximum number of tasks {a mathematical formula}Li that can be assigned to it. Finally, the employer has a total budget B to spend on crowdsourcing and it wishes to maximise the overall sum of the achieved utility.
      </paragraph>
      <paragraph>
       To illustrate this, an employer may wish to carry out a large software development project, where each task represents a single hour of work by one of the workers. The utility generated by such a task is the number of working features that meet certain quality requirements. However, workers charge different prices per hour, {a mathematical formula}ci, and have different skill levels, represented by their expected number of working features they can implement per hour, {a mathematical formula}μi. The employer has a set budget to spend on developers, e.g., {a mathematical formula}B=$5000, and wishes to maximise the total number of working features.{sup:8} In so doing, it wants to choose the best subset of workers who provide the optimal solution. However, the employer has to take into account the working hour preferences of each worker, which limits the total number of hours a worker can spend on the project.
      </paragraph>
      <paragraph>
       Given the mapping and the illustrative example above, the mapping between expert crowdsourcing and bounded MABs is trivial. With a slight abuse of notation, hereafter we will use both standard terms of MAB (i.e., arms, pulls, and agent) and expert crowdsourcing (i.e., workers, task assignment, and employer). In what follows, we propose an efficient algorithm to tackle the bounded MAB. We then continue with its theoretical and empirical performance analysis.
      </paragraph>
     </section>
    </section>
    <section label="4">
     The bounded ε-first algorithm
     <paragraph>
      Recall that within our setting, {a mathematical formula}μi are unknown a priori. Given this, the agent has to explore these values by repeatedly pulling a particular arm in order to estimate its expected reward value. However, if it solely focuses on exploration, the agent typically fails to maximise the total expected reward (i.e., exploit). In contrast, if it stops exploring too quickly, it may fail to determine the best arms to pull. Given this, the key challenge of bounded MABs (and of other bandit models in general) is to find an efficient trade-off between exploration and exploitation. Within this section, we propose a novel algorithm that efficiently trades off exploration with exploitation by splitting exploration from exploitation. The intuition behind this explicit distinction is that by doing so, we can control the degree of exploration by setting the value of ε, which becomes very useful for the theoretical analysis (see Section 5 for more details). Besides, this approach was shown to be efficient in many real-world applications, compared to other bandit based methods such as UCB or ε-greedy [30], [32], [33], [36]. In what follows, we first describe the exploration phase of the algorithm (Section 4.1), followed by its exploitation phase (Section 4.2).
     </paragraph>
     <section label="4.1">
      <section-title>
       Uniform exploration
     ...
     <section label="4.2">
      ...
      <paragraph>
       The pseudo code of the algorithm is depicted in Algorithm 1. In what follows, we formally examine the performance of this algorithm.
      </paragraph>
     </section>
    </section>
    <section label="5">
    ...
    <section label="6">
     ...
     <section label="6.1">
      ...
      <paragraph>
       To test our algorithm on realistic settings, we use real data from the expert crowdsourcing website oDesk.{sup:11} Specifically, we assume an employer wishes to crowdsource a large-scale software project and is looking to hire Java experts. Since only a small fraction of all registered Java experts will be available at any time, we determine the number of applicants by sampling from the real historical distribution of applicants per Java-related job. This distribution is shown in Fig. 1 (we consider only closed jobs and truncate the distribution to the interval {a mathematical formula}[2,100], as smaller jobs are trivial and as there was a small number of extremely large outliers).
      </paragraph>
      <paragraph>
     ...
     <section label="6.3">
      ...
      <section label="6.3.2">
       <section-title>
        Performance with variable numbers of candidates
       </section-title>
       <paragraph>
     ...
     <section label="6.4">
      <section-title>
       Comparison with other exploration policies
      </section-title>
      <paragraph>
      ...
      <paragraph>
       It is worth noting that we also achieve broadly similar results when we modify Hoeffding Races and SR to find the arm with the highest density, instead of the arm with the highest expected reward. A possible reason behind this is that it is not sufficient either to solely focus on arms with the highest density, as those might have low pulling limits and this will lead to a poor performance in the exploitation phase.
      </paragraph>
     </section>
    </section>
   </content>
