  ...
  <root>
   ...
   <content>
    ...
    <section label="2">
     ...
     <paragraph>
      We first review Dynamic Bayesian Networks and existing work on exact probabilistic inference with these representations. Upper-case letters (Y) denote random variables and lower case letters (y) denote their instantiations. Bold letters represent sets of variables (Y) and their instantiations (y).
     </paragraph>
     <section label="2.1">
      <section-title>
       Representation and tasks
      </section-title>
      <paragraph>
     ...
     <section label="2.2">
      ...
      <paragraph>
       The semantics of a DBN, for a finite number of time steps T, is defined by unrolling the transition model (2TBN) for all time slices (Equation (1)). Such an unrolled network (see Fig. 2) is equivalent to a static Bayesian network and allows one to perform inference with any standard algorithm for BNs [6], [5].
      </paragraph>
      <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.2">
      ...
      <section label="3.2.1">
       <section-title>
        Conversion to weighted model counting
       </section-title>
       <paragraph>
       ...
       <paragraph>
        We illustrate the conversion step on our running example by means of the encoding proposed by Fierens et al. [11]. The propositional formula for the CPT shown in Fig. 1d contains one parameter variable ({a mathematical formula}w(PFaulty|¬HA,t)=0.5) and six indicator variables ({a mathematical formula}w(⋅)=1):{a mathematical formula}
       </paragraph>
       <paragraph>
      ...
      <section label="3.2.2">
       ...
       <list>
        <list-item label="1.">
         Compile the knowledge base KB into a d-DNNF Δ [13].{a mathematical formula}
        </list-item>
        <list-item label="2.">
         Incorporate evidence e by setting to zero the weight of any indicator variable that is not compatible with the evidence.{a mathematical formula}
        </list-item>
        <list-item label="3.">
         Traverse the obtained d-DNNF to either:(a) compute the weighted model count, which corresponds to the probability of the evidence in the BN, with an upward pass only:{a mathematical formula} (b) compute the marginal probability {a mathematical formula}Pr⁡(Z|e), for all variables Z in parallel, with one upward and downward pass [5, Algorithm 34]:{a mathematical formula}
        </list-item>
       </list>
       <paragraph>
    ...
    <section label="4">
     <section-title>
      The structural interface algorithm
     </section-title>
     <paragraph>
    ...
    <section label="5">
     ...
     <list>
      <list-item>
       How do different algorithms scale with an increasing number of time steps?
      </list-item>
      <list-item>
       How do both of the interface algorithms scale in the presence of local structure in the transition model?
      </list-item>
      <list-item>
       How does the structural interface algorithm scale in case local structure is not fully exploited?
      </list-item>
      <list-item>
       How do the different interface encodings compare?
      </list-item>
     </list>
     <paragraph>
      We implemented our algorithm in ProbLog.{sup:4} For compilation, we use both the c2d{sup:5} and dsharp{sup:6} compilers, and retain the smallest circuit. Experiments are run with a working memory of 8 GB and a timeout of 1 hour.
     </paragraph>
     <section label="5.1">
      <section-title>
       Models
     ...
     <section label="5.2">
      ...
      <list>
       <list-item label="•">
        unrolled_JT: The junction tree algorithm on the unrolled network for which we used SMILE.{sup:7}
       </list-item>
       <list-item label="•">
        unrolled_COMP: Compiling the unrolled network, using the encoding introduced in section 3.2.1.
       </list-item>
       <list-item label="•">
        standard_IA: The standard interface algorithm{sup:8} where we experimented with the jtree_dbn_inf_engine as well as with the smoother_engine but did not observe any significant difference.
       </list-item>
       <list-item label="•">
