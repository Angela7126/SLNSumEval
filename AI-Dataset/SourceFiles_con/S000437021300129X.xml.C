   ...
   <content>
    <section label="1">
     ...
     <paragraph label="Example 2">
      Coming back to the present day, consider a similar café scenario with Carl, Ben and Alice. Carl and Ben are childhood friends and know each other like the back of their hands. Carl says to Ben: ‘On Valentineʼs day I went to the pub with Mike and Sara. It was a crazy night!’ This immediately catches the attention of Alice, who is in love with Mike. She asks: ‘What happened?’ Carl winks to Ben and says: ‘Nothing’. Knowing Carl very well, Ben immediately realizes that indeed nothing has happened, whereas Alice becomes unsure of that, as she saw the wink that Carl has given to Ben.
     </paragraph>
     <paragraph>
     ...
     <list>
      <list-item label="1.">
       epistemic models encoding state-dependent expected observations;
      </list-item>
      <list-item label="2.">
       an update mechanism for eliminating impossible worlds according to the observation of agents and their expectations;
      </list-item>
      <list-item label="3.">
       a formal language for specifying observations and protocols;
      </list-item>
      <list-item label="4.">
       protocol models that represent agentsʼ incomplete information about the ‘real’ protocols;
      </list-item>
      <list-item label="5.">
       an update mechanism for incorporating protocol information (as protocol models) in epistemic (observation) models;
      </list-item>
      <list-item label="6.">
       a notion of equivalence between protocol models;
      </list-item>
      <list-item label="7.">
       a logic for reasoning about knowledge based on protocols;
      </list-item>
      <list-item label="8.">
       fact-changing actions and factual change systems, in order to investigate how we modify our expectations in an evolving world.
      </list-item>
     </list>
     <paragraph>
      The paper is organized as follows. Section 2 introduces epistemic expectation models and a simple propositional dynamic logic (PDL)-style epistemic logic for reasoning about knowledge via matching agentsʼ expectations to their observations. Section 3 discusses how we obtain epistemic expectation models from protocol models (i.e., epistemic protocols). We characterize three classes of epistemic expectation models that can be generated from various epistemic models. Furthermore we give a characterization of the effective equivalence of epistemic protocols. A logic is then given to incorporate the updates of protocols and to model reasoning about knowledge and observations. In Section 4 we address incorporation of fact-changing actions. Section 5 discusses the application of the full framework, including factual changes, to a well-known logic puzzle. Finally, we point out relations to other research and future work in Sections 6 and 7.
     </paragraph>
     <paragraph>
    ...
    <section label="2">
     <section-title>
      Reasoning via expectation and observation
     </section-title>
     <paragraph>
    ...
    <section label="3">
     <section-title>
      Expectation comes from protocols
     </section-title>
     <paragraph>
      Epistemic expectation models describe the agentsʼ expected observations, which in turn influence their reasoning. We investigate how agents acquire and change their expectations, by looking at protocols and protocol models as sources for the expected observations.
     </paragraph>
     <section label="3.1">
      <section-title>
      ...
      <paragraph>
       Informally, a protocol is a rule telling us what we should do under what conditions. Protocols are ubiquitous in our daily life. A formal way of expressing such protocols or rules is to use a specification language. We specify protocols in the following language of protocol expressions{a mathematical formula}Lprot:
      </paragraph>
      <paragraph label="Definition 15">
       Protocol expressionThe language {a mathematical formula}Lprot of protocols is defined by the following BNF:{a mathematical formula} where δ stands for the empty language ∅, the constant ε represents the empty string, and {a mathematical formula}φ∈Bool(P).
      </paragraph>
      <paragraph>
       The above language of protocol expressions is obtained by adding Boolean tests to observation expressions. For example, {a mathematical formula}(?love⋅stay)⁎⋅(?¬love⋅separate) expresses ‘we should stay together as long as we are in love’. For a discussion on more complicated test scenarios (for example, considering agentsʼ knowledge), see Section 7. We use test conditions in protocol expressions to describe the conditions under which certain observations can happen. A protocol without tests corresponds to observations without any conditions. This is the difference between protocols and the observations that arise out of such protocols, and we maintain this difference by adding tests to the observation expressions in order to express protocols. In the latter part of this section we will talk about public and private protocols. To this end, we will use dynamic epistemic logic (DEL)-like models to discuss knowledge and ignorance about protocols.
      </paragraph>
      <paragraph>
       In the story of Example 7, there seems to be an underlying protocol: if you are Dutch-related, then you kiss three times and if you are non-Dutch-related, then you kiss two times. This is the reason for the agent to have the corresponding expectations of the observations. This protocol (call it {a mathematical formula}πK) can be expressed as {a mathematical formula}?pD⋅a⋅b⋅a+?¬pD⋅a⋅b. We would like to generate the epistemic expectation model in Example 7 (see p. 21) from the protocol {a mathematical formula}πK and the following epistemic model:{a mathematical formula}
      </paragraph>
      <paragraph>
       Intuitively, the information of the protocol {a mathematical formula}πK can be incorporated by adding to each state the possible observations allowed by the protocol. We now move on to the technical details.
      </paragraph>
      <paragraph>
       To compute the expected observations corresponding to a given protocol, we first define the semantics of protocol expressions. Intuitively, we associate to each protocol η a set {a mathematical formula}Lg(η) of guarded observations in the form of{a mathematical formula} where each {a mathematical formula}ρi⊆P denotes a state of affairs (the atomic propositions {a mathematical formula}p∈ρ are true while the others are false), encoding the conditions for the later observations to happen. For Boolean formulas φ, we write {a mathematical formula}ρ⊨φ if φ is true under ρ (viewed as a valuation: p is true iff {a mathematical formula}p∈ρ).
      </paragraph>
      <paragraph label="Definition 16">
       The set of guarded observations corresponding to a protocol expression is defined by induction, as follows:{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula} where ⋄ is the fusion product: {a mathematical formula}w⋄v=w′ρv′ when {a mathematical formula}w=w′ρ and {a mathematical formula}v=ρv′, and not defined otherwise.
      </paragraph>
      <paragraph>
       Note that the {a mathematical formula}ρiʼs in a guarded observation remain unchanged since no factual change is introduced by the execution of the actions (see Section 6 for a detailed discussion of fact-changing actions, such as toggling a light switch). We derive the set of observations to be expected under the same condition ρ according to η by a conversion function {a mathematical formula}fρ:Lprot→Lobs:{a mathematical formula}
      </paragraph>
      <paragraph label="Definition 17">
       Characteristic formulaLet {a mathematical formula}ρ⊆P. Then we denote by {a mathematical formula}φρ the characteristic formula for ρ: {a mathematical formula}⋀p∈ρp∧⋀p∉ρ¬p. For example, suppose that {a mathematical formula}P={p,q}, then {a mathematical formula}φ{p}=p∧¬q.
      </paragraph>
      <paragraph label="Proposition 18">
       <list>
        <list-item label="(a)">
         For any{a mathematical formula}η∈Lprot, it holds that{a mathematical formula}Therefore:
        </list-item>
        <list-item label="(b)">
         Every η has a normal form{a mathematical formula}η∘as follows:{a mathematical formula}such that{a mathematical formula}Lg(η)=Lg(η∘). Here{a mathematical formula}φρis the characteristic formula for ρ as defined inDefinition 17.
        </list-item>
       </list>
      </paragraph>
      <paragraph label="Proof">
       We first show (a) by induction on {a mathematical formula}η∈Lprot. The atomic cases are straightforward. Now we check the complex cases:{a mathematical formula}η=η1+η2:{a mathematical formula}{a mathematical formula}η=η1⋅η2:{a mathematical formula}{a mathematical formula}η=η1⁎:{a mathematical formula} This completes the proof for (a). From (a) and the definition of {a mathematical formula}Lg, it follows that:{a mathematical formula} Let {a mathematical formula}Gρη={ρa0ρa1…akρ|ρa0ρa1…akρ∈Lg(η)}, the set of all ρ-guarded expressions in {a mathematical formula}Lg(η). Then, by fusion product, it follows that {a mathematical formula}Lg(?φρ⋅fρ(η))=Gρη. Thus,{a mathematical formula} This proves (b).  □
      </paragraph>
      <paragraph>
       From Proposition 18, according to the protocol η, the expected observations on a state s in an epistemic model {a mathematical formula}M can be computed by {a mathematical formula}fVM(s)(η). For example, {a mathematical formula}f{p}(?p⋅a+?¬p⋅b)=a. However, not every epistemic expectation model can be generated by a single protocol. We will investigate this issue in the next subsection.
      </paragraph>
     </section>
     <section label="3.2">
      <section-title>
      ...
      <paragraph label="Definition 21">
       Protocol updateGiven an epistemic expectation model {a mathematical formula}Mexp=〈S,∼,V,Exp〉 and an epistemic protocol model {a mathematical formula}A=〈T,∼,Prot〉, we define the product {a mathematical formula}(Mexp⊗A)=(S′,∼′,V′,Exp′) as follows:
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}S′={(s,t)∈S×T:L(fVM(s)(Prot(t)))≠∅};
       </list-item>
       <list-item label="•">
        {a mathematical formula}(s,t)∼i′(s′,t′) iff {a mathematical formula}s∼is′ in {a mathematical formula}Mexp and {a mathematical formula}t∼it′ in {a mathematical formula}A;
       </list-item>
       <list-item label="•">
        {a mathematical formula}V′(s,t)=V(s);
       </list-item>
       <list-item label="•">
        {a mathematical formula}Exp′((s,t))=fVM(s)(Prot(t)).
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph label="Example 22">
       We now illustrate the definition of protocol update by the scenarios presented in Example 1, Example 2 of the introduction. In the pictures below, we assume reflexivity, symmetry, and transitivity of the accessibility relations. In the scenario of Example 1, at the beginning neither Jane nor Ann knows the basic proposition g (Kate is gay). However, one of them, Jane, is aware of the protocol that: if Kate is gay then she will make the statement ‘I am musical, I like Kathleen Ferrierʼs voice’ (action a); and if she is not gay, then she will talk about something else (action b). However, Ann has no idea whether a and b can carry such information. The scenario is modeled as follows, where the last model is the epistemic expectation model resulting from the update of the protocol on the first epistemic model:{a mathematical formula} Here, g denotes the fact that ‘Kate is gay’, a denotes the observation of Kate making the ‘musical statement’ and b stands for Kate saying something else.
      </paragraph>
      <paragraph label="Example 23">
       We now consider the scenario of Example 2. After Carlʼs first description of the night of Valentineʼs day, Ben and Alice still do not know what has happened. Now, the wink from Carl ‘installs’ the epistemic protocol which creates uncertainty in Alice about the meaning of Benʼs later statements. In contrast, because Ben knows Carl so well, he immediately gets the protocol Carl is using. The modeling is as follows:{a mathematical formula} Here, p denotes the fact that ‘Something has happened involving Mike and Sara on Valentineʼs night’, while ‘Y’ corresponds to Carl answering affirmatively to Aliceʼs question, and ‘N’ to Carl answering negatively.
      </paragraph>
      <paragraph>
       We assume that Aliceʼs confusion would lead her to consider the possibility of a protocol where Carl would say “Yes” if indeed nothing has actually happened. Because of Carlʼs wink, however, Alice becomes very distrustful towards him.
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Proposition 24">
       Given an epistemic expectation model{a mathematical formula}M=(N,Exp), there is an epistemic model{a mathematical formula}N′and an epistemic protocol model{a mathematical formula}Asuch that{a mathematical formula}M↔̲oN′⊗A.
      </paragraph>
      <paragraph label="Proof">
     ...
     <section label="3.4">
      ...
      <paragraph label="Definition 33">
       Language of EPLThe formulas φ of EPL are given by:{a mathematical formula} where {a mathematical formula}p∈P, {a mathematical formula}i∈I, {a mathematical formula}π∈Lobs, and {a mathematical formula}Ae is an epistemic protocol with the designated state e.
      </paragraph>
      <paragraph>
       In defining the language we restrict ourselves to finite protocol models. The models for the logic EPL are taken to be the epistemic expectation models {a mathematical formula}M=〈S,∼,V,Exp〉. The truth definition is given as follows:
      </paragraph>
      <paragraph label="Definition 34">
       Truth definition for EPLGiven an epistemic expectation model {a mathematical formula}M = {a mathematical formula}〈S,∼,V,Exp〉, a state {a mathematical formula}s∈S, and an EPL-formula φ, the truth conditions of φ at s coincide with POL for the formulas that they have in common. The truth condition for the new formula in EPL is defined as follows:{a mathematical formula}
      </paragraph>
      <paragraph>
       Recalling the meaning of the modal product operation, the expression ‘{a mathematical formula}[!Ae]φ’ therefore stands for ‘after installing the new epistemic protocol {a mathematical formula}Ae, the formula φ is true’. As an example, let us give the model of Example 1 from the introduction, the epistemic expectation model induced by the epistemic protocol (modelled on p. 27, call it {a mathematical formula}Ae), and the updated model according to observation a (in the picture, visualized by {a mathematical formula}|a):{a mathematical formula} Recall the original model {a mathematical formula}M:{a mathematical formula} Now we can verify for the actual state s:{a mathematical formula}{a mathematical formula}
      </paragraph>
      <paragraph>
       The picture corresponding to Example 2 from the introduction is as follows. Here, {a mathematical formula}Ae′′ is the corresponding epistemic protocol modelled on p. 27:{a mathematical formula}
      </paragraph>
      <paragraph>
       Recall the initial model {a mathematical formula}N:{a mathematical formula} Now we can verify for the actual state t:{a mathematical formula}{a mathematical formula}
      </paragraph>
     </section>
    </section>
    <section label="4">
     <section-title>
      Incorporating factual changes
     </section-title>
     ...
     <list>
      ...
      <list-item label="•">
       The outgoing action transitions are deterministic: for each action state q and each {a mathematical formula}a∈Σ, state q has one and only one a-successor.
      </list-item>
      <list-item label="•">
       The outgoing test transitions are deterministic: they are labelled by {a mathematical formula}{{ρ}|ρ⊆P} and for each test state q and each ρ, state q has one and only one {a mathematical formula}{ρ}-successor. Clearly these tests ρ at a test state are logically pairwise exclusive and altogether exhaustive (viewing ρ as the characteristic Boolean formula {a mathematical formula}φρ, see Definition 17).
      </list-item>
      <list-item label="•">
       The start state {a mathematical formula}q0 is a test state and all accept states are action states.
      </list-item>
      <list-item label="•">
     ...
     <paragraph>
      Based on Proposition 41, we can define the ‘installation’ of protocols with fact-changing actions on epistemic expectation models, similar to Definition 21. Before we proceed to the definition, note that given {a mathematical formula}ηF=∑ρ⊆P(?φρ⋅πρ), we have that {a mathematical formula}fρ(ηF)=πρ for any {a mathematical formula}ρ⊆P (cf. the definition of {a mathematical formula}fρ before Definition 17). Let us now see how the fact-changing actions affect our knowledge state in this evolving world. To this end we first introduce fact-changing epistemic expectation models and protocol models, {a mathematical formula}MexpF and {a mathematical formula}AF, given by {a mathematical formula}〈Mexp,F〉 and {a mathematical formula}〈A,F〉, where {a mathematical formula}Mexp is an epistemic expectation model, {a mathematical formula}A is a protocol model and {a mathematical formula}F is a factual change system.
     </paragraph>
     <paragraph label="Definition 42">
      Protocol update with factual changesGiven a fact-changing epistemic expectation model {a mathematical formula}MexpF=〈S,∼,V,Exp,F〉, and a fact-changing epistemic protocol model {a mathematical formula}AG=〈T,∼,Prot,G〉, we define the product {a mathematical formula}(MexpF⊗AG)=(S′,∼′,V′,Exp′,F′) as follows:
     </paragraph>
     <list>
      <list-item label="•">
       {a mathematical formula}S′={(s,t)∈S×T:L(fVM(s)(ProtG(t)))≠∅};
      </list-item>
      <list-item label="•">
       {a mathematical formula}(s,t)∼i′(s′,t′) iff {a mathematical formula}s∼is′ in {a mathematical formula}Mexp and {a mathematical formula}t∼it′ in {a mathematical formula}A;
      </list-item>
      ...
      <list-item label="•">
       {a mathematical formula}Exp′((s,t))=fVM(s)(ProtG(t));
      </list-item>
      <list-item label="•">
       {a mathematical formula}F′=G,
      </list-item>
     </list>
     <paragraph>
    ...
    <section label="5">
     ...
     <list>
      <list-item label="1.">
       Since initially the light is switched off, when a follower enters the room for the first time and observes that the light is on, it counts as an off-on change;
      </list-item>
      <list-item label="2.">
       When a follower is about to toggle the light from off to on according to the protocol, it also counts as an off-on change.
      </list-item>
     </list>
     <paragraph>
     ...
     <section label="5.1">
      ...
      <list>
       <list-item>
        {a mathematical formula}η1=(?¬fin⋅∑i=0n(ei⋅θi⋅xi))⁎, where:
       </list-item>
       <list-item>
        {a mathematical formula}η2=(?¬fin⋅∑i=0n(ei⋅θi′⋅xi))⁎, where:
       </list-item>
      </list>
      <paragraph>
       It is not hard to see that the formulas {a mathematical formula}θi are almost the literal translations of the specifications of Protocol 1. For {a mathematical formula}i&gt;0, {a mathematical formula}θi′ only adds the extra announcement action based on {a mathematical formula}θi. Note that {a mathematical formula}θi,θi′ are deterministic in the sense that there is always a unique way to proceed, due to the mutually exclusive preconditions of the actions.
      </paragraph>
     </section>
     <section label="5.2">
      <section-title>
       Some example runs of the protocols
      </section-title>
      <paragraph>
       The initial situation can be represented as a singleton expectation model {a mathematical formula}M,s with the universal protocol {a mathematical formula}ΣLB⁎ and the valuation assigning ⊤ only to {a mathematical formula}pi0 for all {a mathematical formula}i⩾0.
      </paragraph>
     ...
     <section label="5.3">
      <section-title>
       Correctness of the two protocols
      </section-title>
      <paragraph>
     ...
     <section label="5.4">
      ...
      <paragraph>
       We can verify that the guard will always know when the prisoners will make the announcements, given that the protocol is public (recall that g is the guard). Let {a mathematical formula}φi=(〈ai〉⊤→Kg〈ai〉⊤)∧(¬〈ai〉⊤→Kg¬〈ai〉⊤). Now the following is straightforward, since there is only one world in the model throughout the evaluation:{a mathematical formula}
      </paragraph>
      <paragraph>
       To confuse the guard, the prisoners may truthfully declare that they have agreed to use one of the two protocols, without telling the guard which one. Here we only model the uncertainty of the guard, not of the prisoners, by the following protocol model {a mathematical formula}AFLB:{a mathematical formula}
      </paragraph>
      <paragraph>
