   ...
   <content>
    <section label="1">
     ...
     <section label="1.2">
      ...
      <paragraph>
       In this paper, we present a combined task–motion planning system which combines search on the task planning level with search on the motion planning level. The key idea is to use two levels of backtracking: action backtracking to reconsider what actions to perform, and geometric backtracking to reconsider how to perform those actions on the geometric level. Through this combined search, we can address the problem in the opening scenario by finding at planning time a better pose for the first object when we detect that there is insufficient space to place the second one. The combined task–motion search space may grow extremely large, so we study several ways to manage this complexity.
      </paragraph>
      <paragraph>
       Compared to the existing work, this paper makes four important contributions:
      </paragraph>
      <list>
       <list-item label="•">
        we identify geometric backtracking as one key issue in the combination of task and motion planning;
       </list-item>
       <list-item label="•">
        we present a combined task–motion planning algorithm that implements geometric backtracking;
       </list-item>
       <list-item label="•">
        we leverage heuristics to explore the combined search space more effectively; and
       </list-item>
       <list-item label="•">
        we show the use of geometric constraints to reduce the search effort by pruning geometrically infeasible parts of the search space.
       </list-item>
      </list>
      <paragraph>
       The first two points were first introduced by Karlsson et al. in a workshop paper [2]. The last point was introduced by Lagriffoul et al. [3] and thoroughly evaluated on predefined action sequences [4]. The present paper extends the latter work by combining this constraint-based approach with a task planner. The four points mentioned above are elaborated to a much greater extent, and for the first time a formal description of a planning process that includes search at the symbolic level and search at the geometric level (geometric backtracking) is given. This paper also contributes an extensive experimental evaluation. None of the experiments presented here has been reported before.
      </paragraph>
      <paragraph>
       In order to evaluate our planner, the proposed heuristics, and our constraint propagation method, we report experiments performed both on the real Justin robot and on a simulator. We also report a series of experiments where our planner is used to plan the operation of an autonomous forklift truck in a warehouse, in the context of the Swedish project SAUNA.{sup:2} The purpose of the latter experiments is to show that geometric bactracking is also relevant in problems that are apparently simpler such as 2D navigation and object transportation. These experiments show that our proposed approach is effective also in those domains. Finally, we use the warehouse scenario to perform an empirical comparison of our approach to alternative approaches.
      </paragraph>
      <paragraph>
       The rest of the paper is organized as follows. Section 2 gives some background about task and motion planning, presents related work in the combination of the two, and explains how our approach fits into that context. It also defines the central notion of geometric backtracking. In Section 3, we give a more formal presentation of task and motion planning, and we present our algorithm for combined task and motion planning. Section 4 presents an algorithm for informed geometric backtracking, and also proposes different criteria for selecting what actions to backtrack over. Section 5 introduces a complementary constraint-based approach that aims at reducing the geometric sampling space for actions. Sections 6 and 7 present extensive empirical evaluations of our backtracking methods, and Section 8 demonstrates how the planner is used on the real Justin. In Section 9, we present a second domain involving an autonomous forklift transporting pallets, which is used to compare our approach to approaches with limited or no geometric backtracking. Finally, Sections 10 and 11 discuss the main results, outline directions for future work, and conclude.
      </paragraph>
     </section>
    </section>
    <section label="2">
    ...
    <section label="3">
     ...
     <section label="3.2">
      ...
      <paragraph>
       A hybrid state is a pair {a mathematical formula}s=〈syms,geos〉, where {a mathematical formula}syms is a symbolic state and {a mathematical formula}geos is a geometric state including the configuration of the robot(s). We assume that any hybrid state is fully observable.
      </paragraph>
      <paragraph>
       In order to refer to the geometric state in the preconditions and effects of actions, some of the symbolic terms and statements represent abstractions of the geometric representation used during motion planning. In particular:
      </paragraph>
      <list>
       <list-item label="•">
        Some constants are used to denote regions or bodies (i.e., solid regions) in the work space. For instance, the logical formula in(a,b) may include a term a that denotes a body representing a box, and a term b that denotes a small area representing a position.
       </list-item>
       <list-item label="•">
        Some atomic statements are used to denote subspaces of the configuration space. For instance, the above formula in(a,b) denotes the set of geometric states where the body representing the box denoted by a is inside the region denoted by b.
       </list-item>
      </list>
      <paragraph>
       In order to formalize these abstractions, we define a function ref such that {a mathematical formula}ref(t) gives the object or region o denoted by the constant term t; and a relation holds such that{a mathematical formula} is true if the relation pred holds true for the objects {a mathematical formula}〈o1,…,on〉 in geometric state {a mathematical formula}geos. The latter can be defined, e.g., in terms of constraints over {a mathematical formula}W, or through a query to a motion planner or to some other specialized reasoner. For convenience, we partition our predicates into two sets: {a mathematical formula}Predsym are those evaluated symbolically, and {a mathematical formula}Predgeo are those evaluated in the context of a geometric state. Then, an atomic statement {a mathematical formula}pred(t1,…,tn) holds in s iff either
      </paragraph>
      <list>
       <list-item label="•">
        {a mathematical formula}pred∈Predsym and {a mathematical formula}pred(t1,…,tn)∈syms, or
       </list-item>
       <list-item label="•">
        {a mathematical formula}pred∈Predgeo and {a mathematical formula}holds(pred,〈ref(t1),…,ref(tn)〉,geos).
       </list-item>
      </list>
      <paragraph>
       A compound statement is evaluated according to the standard definitions for connectives and quantifiers.
      </paragraph>
     </section>
     <section label="3.3">
      <section-title>
      ...
      <paragraph>
       An action schema o is a tuple {a mathematical formula}〈A(v1,⋯,vn),pre(v1,⋯,vn+m),eff(v1,⋯,vn+m)〉, where:
      </paragraph>
      <list>
       <list-item label="•">
     ...
     <section label="3.4">
      ...
      <paragraph>
       A hybrid planning domain is a tuple {a mathematical formula}D=〈O,C,Pred,N,holds,ref〉, where: O is a set of action schemas; C is a representation of the geometric state space according to Section 3.1; Pred is a set of predicates; N is a set of constants (names); and holds and ref are as above.
      </paragraph>
      <paragraph>
       A hybrid planning problem is a tuple {a mathematical formula}Pb=〈s0,D,g〉, where: {a mathematical formula}s0=(syms0,geos0) is the initial state consisting of a symbolic state {a mathematical formula}syms0 and a geometric state {a mathematical formula}geos0; D is a hybrid planning domain; and g is a goal formula. A solution to a hybrid planning problem is a hybrid plan P that is valid when starting from {a mathematical formula}s0, and for which g holds in the final state {a mathematical formula}sn.
      </paragraph>
      <paragraph>
       Alternatively, in an HTN planner, we may have the following definitions. A hybrid planning domain is a tuple {a mathematical formula}D=〈O,M,C,Pred,N,holds,ref〉, where:
      </paragraph>
      <list>
       <list-item label="•">
        M is a set of methods for decomposing tasks that are composite or atomic partially instantiated actions. The methods concisely specify a (possibly infinite) set of hybrid plans {a mathematical formula}PM(T) for each task list T.
       </list-item>
       <list-item label="•">
     ...
     <section label="3.5">
      ...
      <paragraph>
       In line 7, the function {a mathematical formula}actions(…) produces a set of available actions. In our HTN-implementation of the algorithm, the task-list (in g) is also updated. We have used the task decomposition algorithm of Shop2, and we refer to the literature about that planner [41] for further details. (The other algorithms we present (in particular for backtracking) are independent of whether the top-level algorithm is goal- or task-driven.) The importance of this line is that it provides the available actions in some manner, which depends on the particular task planner employed. Line 7 would be a backtrack point in a deterministic implementation: this is how the algorithm searches over alternative plans on the symbolic level.
      </paragraph>
      <paragraph>
       In line 8, the function {a mathematical formula}evalPreconditions(…) (see Algorithm 2) is called with the present state sequence S (where the last element is the current state), the present (partial) plan P, and the selected action a. It returns an updated state sequence, and a value γ binding (among other things) a path τ in the geometric space if this is generated by the preconditions. γ may also be None in case of failure (lines 9–10).
      </paragraph>
      <paragraph>
       If the preconditions are successfully satisfied, then the algorithm computes the effects of {a mathematical formula}a/γ, adds the resulting state to the state sequence S, and adds the action {a mathematical formula}a/γ to the plan P (lines 11–13).
      </paragraph>
      <paragraph>
    ...
    <section label="4">
     ...
     <section label="4.1">
      ...
      <section label="4.1.2">
       ...
       <paragraph>
        The second major factor behind failed preconditions is the kinematic constraints of the robot. The KIN criterion takes these constraints into account. If failed attempts are owing to the violations of kinematic constraints while placing an object which has been picked by action {a mathematical formula}ai, then we select {a mathematical formula}ai as follows.
       </paragraph>
       <paragraph>
        First, we check whether kinematic constraints have been violated when trying to find a geometric instance of a that manipulates object o. This check is performed by calling function {a mathematical formula}nbViolatedKinCst(…) which takes a as parameter and returns the number of times kinematic constraints have been violated (including joint limits). More precisely, nbViolatedKinCst considers constraints when the planner attempts to find an inverse kinematics (IK) solution for the target pose of the hand when picking up or placing an object. The violations are most often owing to the target orientation: the position is a less frequent problem.
       </paragraph>
       <paragraph>
        Second, we select the latest hybrid action {a mathematical formula}ai of P that manipulates object o in P and that is not already selected.
       </paragraph>
      </section>
      <section label="4.1.3">
       <section-title>
    ...
    <section label="5">
     ...
     <section label="5.2">
      ...
      <paragraph>
       A placement constraint represents the set of all possible relative stable positions of a manipulable object at/in a fixed location loc. This constraint applies to the translation part of the object pose p. A flat surface is assumed, but the formulation can be easily generalized to a volume (Fig. 11).
      </paragraph>
      <paragraph>
       Assuming that the dimensions of all the objects that can be used as a location are known, we can formulate the following constraint in the frame of the location:{a mathematical formula}{a mathematical formula} In order to get this constraint expressed in the world frame, it has to be transformed according to the translation {a mathematical formula}tloc and the rotation matrix {a mathematical formula}Rloc of the location:{a mathematical formula}
      </paragraph>
     </section>
     <section label="5.3">
      <section-title>
    ...
    <section label="8">
     ...
     <section label="8.1">
      ...
      <paragraph>
       The perception module is based on analysis of point clouds obtained from a Kinect sensor. Given known geometric models of possible objects in the actual scene, an interpretation of that scene in terms of object classes and poses is computed [48], [49].
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       The planner queries the world model in order to get geometric information about the planning problem addressed. From the world model, it constructs a geometric model which represents the initial geometric state (see the screenshot on the right in Fig. 26). Purely symbolic information is given in a problem file. Next, the planner searches for a plan.
      </paragraph>
      <paragraph>
       If the planner is successful, the plan is used to generate a robot program in the form of a sequence of Python scripts: one for each action. The scripts contain, among other things, arm motion paths computed during geometric reasoning, finger motions for grasping operations, and specific commands to detect contacts (when placing an object) or reduce the stiffness of the robot (during re-grasping operations with both hands).
      </paragraph>
      <paragraph>
       The main limitation of the system is that the execution happens in open-loop: it relies on accurate estimation of object poses before planning, and precise execution of movements in the robot. Grasping an object for the first time is generally not a problem, but errors accumulate when objects are moved and grasped several times. If the object changes position within the hand during movement, after releasing it, it has a pose slightly different from the one assumed in the geometric model, which may compromise the next action in which it is grasped again.
      </paragraph>
     </section>
     <section label="8.2">
      <section-title>
      ...
      <paragraph>
       Fig. 27 depicts various scenarios that we used in our experiments with the real platform. The scenarios presented below (mainly A and C) emphasize the task planning part of our system, i.e., the capacity to reason about symbolic preconditions and effects of actions in order to reach a given goal.
      </paragraph>
      <paragraph>
