   ...
   <content>
    ...
    <section label="3">
     ...
     <section label="3.1">
      ...
      <paragraph>
       In the AGM paradigm, belief revision is modelled as a function ⁎ mapping a theory K and a sentence ϕ to the theory {a mathematical formula}K⁎ϕ. The celebrated AGM postulates for revision, which are widely accepted as necessary properties of any rational revision function, are listed below (see [8], [15] for detailed discussion on the postulates){sup:3}:
      </paragraph>
      <list>
       <list-item label="(K*1)">
        {a mathematical formula}K⁎ϕ is a theory of L.
       </list-item>
       <list-item label="(K*2)">
        {a mathematical formula}ϕ∈K⁎ϕ.
       </list-item>
       <list-item label="(K*3)">
        {a mathematical formula}K⁎ϕ⊆K+ϕ.
       </list-item>
       <list-item label="(K*4)">
        If {a mathematical formula}¬ϕ∉K then {a mathematical formula}K+ϕ⊆K⁎ϕ.
       </list-item>
       <list-item label="(K*5)">
        {a mathematical formula}K⁎ϕ⊨⊥ iff {a mathematical formula}⊨¬ϕ.
       </list-item>
       <list-item label="(K*6)">
        If {a mathematical formula}ϕ≡ψ then {a mathematical formula}K⁎ϕ=K⁎ψ.
       </list-item>
       <list-item label="(K*7)">
        {a mathematical formula}K⁎(ϕ∧ψ)⊆(K⁎ϕ)+ψ.
       </list-item>
       <list-item label="(K*8)">
        If {a mathematical formula}¬ψ∉K⁎ϕ then {a mathematical formula}(K⁎ϕ)+ψ⊆K⁎(ϕ∧ψ).
       </list-item>
      </list>
     </section>
     <section label="3.2">
      ...
      <paragraph>
       Apart from the above axiomatic approach to belief revision, a number of explicit constructions for this process have been proposed. One popular construction is that proposed by Katsuno and Mendelzon, [9], based on total preorders over possible worlds.
      </paragraph>
      <paragraph>
       For a theory K, a preorder over possible worlds ⪯ is said to be faithful to K iff it is total and such that the minimal worlds (with respect to ⪯) are those satisfying K; i.e. {a mathematical formula}min(M,⪯)=[K].{sup:4} Given a preorder ⪯ that is faithful to K, the revision of K by any sentence ϕ can be defined as follows:
      </paragraph>
      <list>
       <list-item label="(⪯⁎)">
        {a mathematical formula}K⁎ϕ=t(min([ϕ],⪯)).
       </list-item>
      </list>
      <paragraph>
       Intuitively ⪯ represents a plausibility ranking over possible worlds: the more plausible a world r is, the lower it appears in the ranking. Hence (⪯⁎) essentially defines {a mathematical formula}K⁎ϕ as the theory corresponding to the most plausible worlds satisfying the new information ϕ.
      </paragraph>
      <paragraph>
       Katsuno and Mendelzon proved that the functions induced from faithful preorders are precisely those satisfying the AGM postulates for revision. One of the main aims of this article is to characterise the subclass of faithful preorders that induce revision functions satisfying axiom (P) (in addition to (K*1)–(K*8)).
      </paragraph>
      <paragraph>
       For ease of presentation, in this paper we shall not consider the limiting cases of an inconsistent belief set and/or epistemic input, nor a tautological epistemic input. Hence from now, unless explicitly stated otherwise, we assume that the initial belief set K is a consistent theory, and that the epistemic input ϕ is a contingent sentence (i.e. neither a tautology, nor a contradiction).
      </paragraph>
     </section>
    </section>
    <section label="4">
     ...
     <paragraph>
      When revising a theory K by a sentence ϕ it seems plausible to assume that only the beliefs that are relevant to ϕ should be affected, while the rest of the belief corpus is unchanged. For example, an agent that is revising her beliefs about planetary motion is unlikely to revise her beliefs about Malaysian politics. This simple intuition is not fully captured in the AGM paradigm. To see this consider the trivial revision function {a mathematical formula}⁎a defined below:{a mathematical formula}
     </paragraph>
     <paragraph>
    ...
    <section label="5">
     ...
     <paragraph>
      Depending on the context one could argue in favour of the weak or the strong version of axiom (P). To avoid ambiguity between the two versions, we recast this axiom in terms of the following two conditions that make no reference to a local revision operator:
     </paragraph>
     <list>
      <list-item label="(R1)">
       If {a mathematical formula}K=Cn(χ,ψ), {a mathematical formula}Lχ∩Lψ=⌀, and {a mathematical formula}ϕ∈Lχ, then {a mathematical formula}(K⁎ϕ)∩Lχ‾=K∩Lχ‾.
      </list-item>
      <list-item label="(R2)">
       If {a mathematical formula}K=Cn(χ,ψ), {a mathematical formula}Lχ∩Lψ=⌀, and {a mathematical formula}ϕ∈Lχ, then {a mathematical formula}(K⁎ϕ)∩Lχ=(Cn(χ)⁎ϕ)∩Lχ.
      </list-item>
     </list>
     <paragraph>
      Condition (R1) is straightforward: when revising a theory K by a sentence ϕ, the part of K that is not related to ϕ is not affected by the revision. Condition (R1) alone, essentially corresponds to weak (P). To get strong (P) we need to add condition (R2). To see this, consider a revision function ⁎ (which defines a revision policy for all theories of L), and let {a mathematical formula}K=Cn(χ,ψ) and {a mathematical formula}K′=Cn(χ,ψ′) be two theories such that {a mathematical formula}Lχ∩Lψ=Lχ∩Lψ′=⌀. Consider now any sentence {a mathematical formula}ϕ∈Lχ. The relevant part to ϕ of K and {a mathematical formula}K′ is in both cases the same. Then, according to (R2), the way that this relevant part is modified in both K and {a mathematical formula}K′ is also the same; namely, as dictated by the revision function ⁎ itself when applied to {a mathematical formula}Cn(χ) (once again, recall that ⁎ is defined for all theories, including {a mathematical formula}Cn(χ)).
     </paragraph>
     <paragraph>
      Theorem 1 below proves that strong (P) is equivalent to (R1) and (R2). First however let us review an auxiliary result reported in [11] as Lemma A, that will be used extensively in our own proofs.
     </paragraph>
     <paragraph>
    ...
    <section label="7">
     ...
     <section label="7.1">
      ...
      <paragraph>
       It is not hard to verify that the revision function ⁎ induced by the above preorder ⪯, satisfies (R1). At the same time however, ⪯ violates (SP). In particular consider the worlds {a mathematical formula}r={a,b,¬c,d} and {a mathematical formula}r′={¬a,b,c,¬d}}. Although {a mathematical formula}Diff(K,r)={c,d}⊂Diff(K,r′)={a,b,c,d}, the two worlds r and {a mathematical formula}r′ are equidistant from the beginning of ⪯.
      </paragraph>
      <paragraph>
       Despite its failure to generalise, (SP) should not be disregarded altogether. It can still serve as a guide in formulating the appropriate counterpart(s) of (R1) for the general case. In particular, consider the conditions (Q1) and (Q2) below:
      </paragraph>
      <list>
       <list-item label="(Q1)">
        If {a mathematical formula}Diff(K,r)⊂Diff(K,r′) and {a mathematical formula}Diff(r,r′)∩Diff(K,r)=∅, then {a mathematical formula}r≺r′.
       </list-item>
       <list-item label="(Q2)">
        If {a mathematical formula}Diff(K,r)=Diff(K,r′) and {a mathematical formula}Diff(r,r′)∩Diff(K,r)=∅, then {a mathematical formula}r≈r′.
       </list-item>
