  ...
  <root>
   ...
   <content>
    ...
    <section label="4">
     ...
     <section label="4.1">
      ...
      <paragraph>
       We will use “+” to denote a general expansion operator, and we will propose a set of postulates that will characterize a class of expansion operators whose behavior is such that they will preserve the property of soundness of the credibility base. In what follows, we will assume that {a mathematical formula}C∈C is a sound credibility base, {a mathematical formula}A,B,D∈A are three agents, and {a mathematical formula}T∈A×A is a credibility tuple.
      </paragraph>
      <paragraph>
       E1 – Relative success: {a mathematical formula}C+[T,A]=C or {a mathematical formula}[T,A]∈C+[T,A].
      </paragraph>
      <paragraph>
       This postulate is characterizing the behavior of the operator as a particular expansion where the attempt to augment the credibility base might fail or be successful. It is interesting to observe that in the case of failing in the expansion the credibility base is not altered.
      </paragraph>
      <paragraph>
       E2 – Weak success: if {a mathematical formula}(B,A)∉Cl(C) then {a mathematical formula}[(A,B),D]∈C+[(A,B),D].
      </paragraph>
      <paragraph>
       This postulate establishes that {a mathematical formula}[(A,B),D] is accepted in the expanded credibility base if there is no pair {a mathematical formula}(B,A) in {a mathematical formula}Cl(C).
      </paragraph>
      <paragraph>
       E3 – Inclusion: {a mathematical formula}C⊆C+[T,A].
      </paragraph>
      <paragraph>
       This postulate reflects the fact that the agent will not loose information during the expansion.
      </paragraph>
      <paragraph>
       E4 – Vacuity: if {a mathematical formula}[T,A]∈C then {a mathematical formula}C+[T,A]=C.
      </paragraph>
      <paragraph>
       A particular case of expansion occurs when a credibility base {a mathematical formula}C is expanded by a credibility object {a mathematical formula}[T,A] which is already in {a mathematical formula}C. In this case, expanding {a mathematical formula}C by {a mathematical formula}[T,A] does not generate any change in {a mathematical formula}C. The name vacuity follows the tradition of belief revision literature, showing a case in which the expansion operator does not change anything.
      </paragraph>
      <paragraph>
       E5 – Soundness: if {a mathematical formula}C is a sound credibility base then {a mathematical formula}C+[T,A] is a sound credibility base.
      </paragraph>
      <paragraph>
       An operator satisfying this postulate will guarantee the preservation of soundness over the resulting expanded credibility base.
      </paragraph>
      <paragraph>
       E6 – Minimality: {a mathematical formula}C+[T,A] is the smallest set satisfying E1 to E5.
      </paragraph>
      <paragraph>
       Similarly to expansion in AGM [23, p. 51], we require that {a mathematical formula}C+[T,A] does not contain more credibility objects than the ones required by other postulates.
      </paragraph>
     </section>
     <section label="4.2">
      <section-title>
      ...
      <paragraph>
       We will give now the realization of a credibility based expansion operator for credibility bases called C-expansion using reliability (or CR-expansion for short) that is denoted “⊕”. A CR-expansion operator will add a new credibility object to a credibility base when soundness is preserved or will leave the credibility base unaffected rejecting the addition in case soundness is violated by it.
      </paragraph>
      <paragraph label="Definition 9">
       {a mathematical formula}CR-expansionLet {a mathematical formula}C∈C be a sound credibility base, and {a mathematical formula}[(A,B),S] a credibility object. The operator “⊕”, or CR-expansion, is defined as follows:{a mathematical formula}
      </paragraph>
      <paragraph label="Proposition 1">
       An expansion operator ⊕ satisfies E1 to E6 if and only if ⊕ is defined according toDefinition 9.
      </paragraph>
    ...
    <section label="5">
     ...
     <section label="5.1">
      ...
      <paragraph>
       C1 – Success: {a mathematical formula}(D,E)∉Cl(C−(D,E)).
      </paragraph>
      <paragraph>
       A tuple cannot be entailed by the credibility base resulting from its contraction.
      </paragraph>
      <paragraph>
       C2 – Inclusion: {a mathematical formula}C−(D,E)⊆C.
      </paragraph>
      <paragraph>
       Since {a mathematical formula}C−(D,E) follows from withdrawing some credibility objects from {a mathematical formula}C without adding anything, it is natural to think that {a mathematical formula}C−(D,E) does not contain elements that do not belong to {a mathematical formula}C.
      </paragraph>
      <paragraph>
       C3 – Safe retainment: {a mathematical formula}[T1,D]∈C−T2 if and only if {a mathematical formula}[T1,D] is a safe element with respect to {a mathematical formula}T2 in {a mathematical formula}C.
      </paragraph>
      <paragraph>
       The credibility objects which prevail after contraction will be the objects that are safe objects before the contraction, similarly to [2].
      </paragraph>
      <paragraph>
       C4 – Soundness: if {a mathematical formula}C is sound then {a mathematical formula}C−(D,E) is sound.
      </paragraph>
      <paragraph>
       Since contraction is basically a process of elimination, this operation should not introduce cycles. This postulate is related with C2 as it is shown in the following proposition.
      </paragraph>
      <paragraph label="Proposition 2">
       For sound credibility bases, if a contraction operator satisfies C2 – Inclusion then it satisfies C4 – Soundness.
      </paragraph>
      <paragraph label="Proof">
       Straightforward. □
      </paragraph>
      <paragraph>
       We have not included a postulate similar to uniformity as was introduced by Hansson [24]. The reason is that an adaptation to our approach would be as the following: if for all {a mathematical formula}C′⊆C, {a mathematical formula}(D,E)∈Cl(C′) if and only if {a mathematical formula}(F,G)∈Cl(C′) then {a mathematical formula}C−(D,E)=C−(F,G); but, since contraction receives a tuple and not a credibility object (see Remark 3) this statement will collapse to triviality as it is reflected in Proposition 3.
      </paragraph>
      <paragraph label="Proposition 3">
       Let{a mathematical formula}D,E,F,G∈A,{a mathematical formula}C∈C. If for all subsets{a mathematical formula}C′of{a mathematical formula}C,{a mathematical formula}(D,E)∈Cl(C′)if and only if{a mathematical formula}(F,G)∈Cl(C′)then{a mathematical formula}(D,E)={a mathematical formula}(F,G).
      </paragraph>
      <paragraph label="Proof">
       See Appendix A. □
      </paragraph>
     ...
     <section label="5.2">
      ...
      <paragraph>
       In this section, we introduce the construction of the contraction operator for credibility bases, called C-contraction using reliability (or CR-contraction for short). Consider again Example 3 where all the paths from {a mathematical formula}A1 to {a mathematical formula}A4 are shown. It is clear from Remark 2 that for the contraction of {a mathematical formula}CB by {a mathematical formula}(A1,A4) we need to eliminate at least one credibility object in every path of {a mathematical formula}CB(A1−A4). In other words, we need to eliminate a set of credibility objects from {a mathematical formula}CB so that no path is left from {a mathematical formula}A1 to {a mathematical formula}A4 in the new credibility base.
      </paragraph>
      <paragraph>
       The CR-contraction operator that we will define below is based on the contraction of a credibility order by a credibility tuple {a mathematical formula}(A,B) proposed in [47]. That proposal uses a mechanism, based on [25], to decide which tuples are erased from each path from A to B. Next, this mechanism is adapted for a credibility base.
      </paragraph>
      <paragraph label="Definition 10">
       Cut functionLet {a mathematical formula}C be a credibility base, a cut function σ for {a mathematical formula}C is a function such that for all {a mathematical formula}(A,B)∈Cl(C):
      </paragraph>
      <list>
       <list-item label="1.">
        {a mathematical formula}σ(C(A−B))⊆⋃(C(A−B)).
       </list-item>
       <list-item label="2.">
        For each {a mathematical formula}P∈C(A−B),P∩σ(C(A−B))≠∅.
       </list-item>
      </list>
      <paragraph>
       Definition 10 does not specify how the cut function selects the credibility objects that are being discarded from each path; this matter will be addressed using the reliability of the credibility tuples. Thus, the cut function will select the least reliable credibility objects of each path.
      </paragraph>
      <paragraph>
       Given a set P of credibility objects, the following function returns all the credibility objects that are associated with identifiers which are not more credible than other identifier associated with a credibility object in P.
      </paragraph>
      <paragraph label="Definition 11">
       Minimal sources function{a mathematical formula}minC:C→C, is a function such that for a given credibility base {a mathematical formula}C∈C and {a mathematical formula}P⊆C,{a mathematical formula}
      </paragraph>
      <paragraph label="Definition 12">
       Bottom cut functionGiven a paths set {a mathematical formula}C(A−B), {a mathematical formula}σ↓ is a bottom cut function if it is a cut function for {a mathematical formula}C such that{a mathematical formula}
      </paragraph>
      <paragraph label="Example 6">
       Consider the credibility base of agent B of Example 2 and the paths set {a mathematical formula}CB(A1−A4)={P1,P2,P3} obtained in Example 3. As stated in Definition 12, {a mathematical formula}σ↓(CB(A1−A4)) will contain those elements from each path whose associated identifier is not greater than any other in the path. Hence, in path {a mathematical formula}P1 the selected credibility object is {a mathematical formula}[(A1,A2),F1] because {a mathematical formula}F1 is less credible than {a mathematical formula}F3; in path {a mathematical formula}P2 the selected credibility object is {a mathematical formula}[(A2,A4),B]; and in path {a mathematical formula}P3 is {a mathematical formula}[(A1,A3),F1]. Therefore, {a mathematical formula}σ↓(CB(A1−A4))={[(A1,A2),F1],[(A2,A4),B],[(A1,A3),F1]}.
      </paragraph>
      <paragraph>
       Note that, the cut function can select more than one object from a single path when the reliability of the selected objects is incomparable. In particular, if all the agents associated with the credibility objects in a path are incomparable, then, the bottom cut function selects all of them. Observe that we use the credibility base itself to decide which information prevails. Then, we avoid to have a separate data structure maintaining the measure of reliability. Next, the CR-contraction operator, denoted {a mathematical formula}⊖σ↓, is introduced.
      </paragraph>
      ...
      <paragraph label="Example 7">
       Consider the credibility base in Example 2, {a mathematical formula}CB={[(B,F1),F2],[(F1,F2),F3],[(F2,F3),B],[(A1,A2),F1],[(A1,A3),F1],[(A2,A4),B],[(A2,A4),F3],[(A3,A4),F2]}. Then, suppose B wants to reflect that the advisor {a mathematical formula}A1 is no longer less credible than the advisor {a mathematical formula}A4. That is, B wants to effect a contraction by the credibility tuple {a mathematical formula}(A1,A4) using “{a mathematical formula}⊖σ↓”. As showed in Example 6, {a mathematical formula}σ↓(CB(A1−A4))={[(A1,A2),F1],[(A2,A4),B],[(A1,A3),F1]}. Hence, {a mathematical formula}CB⊖σ↓(A1,A4)=CB∖σ↓(CB(A1−A4))={[(B,F1),F2],[(F1,F2),F3],[(F2,F3),B],[(A2,A4),F3],[(A3,A4),F2]}. Fig. 4 shows (left) the graph for the original credibility base {a mathematical formula}CB and (right) the graph for the resulting credibility base after the contraction by {a mathematical formula}(A1,A4) where three arcs have been deleted.
      </paragraph>
      <paragraph>
       Next, we introduce the Representation Theorem for this new contraction operator (C-contraction using reliability “{a mathematical formula}⊖σ↓”). This theorem proves the correspondence between the set of postulates and the construction.
      </paragraph>
      <paragraph label="Theorem 1">
       Given{a mathematical formula}C∈C, “{a mathematical formula}⊖σ↓” is a C-contraction using reliability for{a mathematical formula}Cif and only if it satisfies success (C1), inclusion (C2), and safe retainment (C3).
      </paragraph>
      <paragraph label="Proof">
       See Appendix A. □
      </paragraph>
      <paragraph>
       The principle of minimal change is an accepted policy in the belief change research community; that is, beliefs should be given up only when one is forced to do so, and even in that situation, as few of them as possible should be given up [27]. In the present formalism, if some particular information (tuple) must be abandoned then all the credibility objects containing that tuple should be erased to achieve that, and that is the minimal possible characterization of the effect. Although our contraction operator can produce a change that is not minimal with respect to the number of credibility objects in the credibility base {a mathematical formula}C, the change is minimal as discussed in [27].
      </paragraph>
     </section>
    </section>
    <section label="6">
     <section-title>
      Prioritized revision operator using reliability
     </section-title>
     <paragraph>
      In the existing literature, several prioritized methods for sentences can be found, e.g., partial meet revision[1] and kernel revision from kernel contraction[25]. In these methods, the new information has priority over the beliefs in the base of the receiver agent.
     </paragraph>
     <paragraph>
      In this section, following [25], we will define a new prioritized revision operator for credibility bases. This operator makes use of the safe element idea introduced for contraction at the beginning of the previous section, considered now in the context of revision. We will show first the postulates of this operator, then its construction, and finally the representation theorem.
     </paragraph>
     <section label="6.1">
      <section-title>
       Postulates for the prioritized revision operator
      </section-title>
      <paragraph>
       Let {a mathematical formula}D,E,F,G,H,I∈A, let {a mathematical formula}C∈C be a sound credibility bases, and let {a mathematical formula}T∈A×A be a credibility tuple. We will use “⁎” to denote a general prioritized revision operator, and we will propose the following postulates for prioritized revision.
      </paragraph>
      <paragraph>
       PR1 – Success: {a mathematical formula}[T,D]∈C⁎[T,D].
      </paragraph>
      <paragraph>
       Since the revision operator defined here is considered prioritized (the new information has priority), the first postulate establishes that the revision should be successful. That is, the result of revising a credibility base {a mathematical formula}C by a credibility object {a mathematical formula}[T,D] should be a new credibility base that contains {a mathematical formula}[T,D].
      </paragraph>
      <paragraph>
       PR2 – Inclusion: {a mathematical formula}C⁎[T,D]⊆C∪{[T,D]}.
      </paragraph>
      <paragraph>
       This postulate states that besides {a mathematical formula}[T,D] no other element will be added upon revision of {a mathematical formula}C by {a mathematical formula}[T,D].
      </paragraph>
      <paragraph>
       PR3 – Soundness: if {a mathematical formula}C is sound then {a mathematical formula}C⁎[T,D] is sound.
      </paragraph>
      <paragraph>
       This postulate guarantees that soundness is preserved in the resulting revised credibility base.
      </paragraph>
      <paragraph>
       PR4 – Uniformity: {a mathematical formula}C∩(C⁎[T,D])=C∩(C⁎[T,E]).
      </paragraph>
      <paragraph>
       This postulate establishes that {a mathematical formula}C⁎[T,D] preserves from {a mathematical formula}C the same credibility objects as {a mathematical formula}C⁎[T,E].
      </paragraph>
      <paragraph>
       PR5 – Safe retainment: {a mathematical formula}[(D,E),F]∈C⁎[(G,H),I] if and only if {a mathematical formula}[(D,E),F] is a safe element with respect to {a mathematical formula}(H,G) in {a mathematical formula}C.
      </paragraph>
      <paragraph>
     ...
     <section label="6.2">
      ...
      <list>
       <list-item label="1.">
        to maintain the soundness of {a mathematical formula}C. If {a mathematical formula}(B,A)∈Cl(C) (i.e., the addition of {a mathematical formula}(A,B) generates a cycle in the new credibility base), then is necessary to erase some credibility objects from {a mathematical formula}C to avoid cycles.
       </list-item>
       <list-item label="2.">
        to add {a mathematical formula}[(A,B),S] to {a mathematical formula}C. As shown above, if {a mathematical formula}(A,B)∈Cl(C) then this operation might also increase the reliability of {a mathematical formula}(A,B).
       </list-item>
      </list>
      <paragraph>
       The first task can be accomplished contracting by {a mathematical formula}(B,A). The second task can be accomplished expanding by {a mathematical formula}[(A,B),S]. This composition is based on the Levi identity[34], [22], [1], which proposes that a revision can be constructed out of two operations: a contraction and an expansion.
      </paragraph>
      ...
      <paragraph>
       The representation theorem for this new proposed prioritized revision operator {a mathematical formula}⊛σ↓ is introduced below. This theorem proves the correspondence between postulates and construction.
      </paragraph>
      <paragraph label="Theorem 2">
       Given{a mathematical formula}C∈C, “{a mathematical formula}⊛σ↓” is a prioritized revision using reliability for{a mathematical formula}Cif and only if it satisfies success (PR1), inclusion (PR2), soundness (PR3), uniformity (PR4), and safe retainment (PR5).
      </paragraph>
      <paragraph label="Proof">
       See Appendix A. □
      </paragraph>
      ...
      <paragraph label="Proposition 4">
       If “⊕” satisfies E1, E2, E3, E4, E5 and E6, and “{a mathematical formula}⊖σ↓” satisfies C1, C2 and C3, then “{a mathematical formula}⊛σ↓” satisfies PR1, PR2, PR3, PR4, and PR5.
      </paragraph>
      <paragraph label="Proof">
       See Appendix A. □
      </paragraph>
      <paragraph label="Example 8">
       Consider the credibility base of Example 2, {a mathematical formula}CB={[(B,F1),F2],[(F1,F2),F3],[(F2,F3),B],[(A1,A2),F1],[(A1,A3),F1],[(A2,A4),B],[(A2,A4),F3],[(A3,A4),F2]}. Then, suppose that the fellow worker {a mathematical formula}F3 tells B that, in his opinion, the advisor {a mathematical formula}A4 is less credible than the advisor {a mathematical formula}A1. That is to say, B has to revise by {a mathematical formula}[(A4,A1),F3] using “{a mathematical formula}⊛σ↓”. Since {a mathematical formula}(A1,A4)∈Cl(CB) then it is first necessary to contract {a mathematical formula}CB by {a mathematical formula}(A1,A4) as shown in Example 7, and then to expand {a mathematical formula}CB⊖σ↓(A1,A4) by {a mathematical formula}[(A4,A1),F3]. Thus, {a mathematical formula}CB⊛σ↓[(A4,A1),F3]={[(B,F1),F2],[(F1,F2),F3],[(F2,F3),B],[(A2,A4),F3],[(A3,A4),F2],[(A4,A1),F3]}. Fig. 5 shows (left) the graph for the original credibility base {a mathematical formula}CB of Example 2, and (right) the graph for the resulting credibility base after the revision by {a mathematical formula}[(A4,A1),F3] where three arcs have been deleted, and a new arc from {a mathematical formula}A4 to {a mathematical formula}A1 was added.
      </paragraph>
     </section>
    </section>
    <section label="7">
     <section-title>
      Non-prioritized revision operator using reliability
     </section-title>
     <paragraph>
      A prioritized revision operator is characterized by the satisfaction of the success postulate by the operator; that is, the incoming information is always accepted, becoming a part of the beliefs of the agent. However, as is mentioned in [17], oftentimes this is an unrealistic feature since actual epistemic agents, when confronted with information that contradicts previous beliefs, choose to reject the recent arrival. For instance, in a multi-agent domain if the information comes from different sources, and these sources are not equally credible, a non-prioritized method could be more adequate. Several models of belief revision have been developed where either the new information is completely accepted or it is completely rejected [26], [39], [28], [12]. In the literature of uncertain evidence revision there exist other proposals that not just simply accept or reject the new information, for instance [17], [9], [38], [32]. In this section, we define a non-prioritized operator which completely accepts or rejects the new information.
     </paragraph>
     <section label="7.1">
      <section-title>
       Postulates for the non-prioritized revision operator
      </section-title>
      <paragraph>
       Let {a mathematical formula}A,B,S∈A, {a mathematical formula}C∈C be a credibility base, {a mathematical formula}T∈A×A be a credibility tuple. We will use “⊚⊚” to denote a general non-prioritized revision operator, and we will propose the following postulates for non-prioritized revision.
      </paragraph>
      <paragraph>
       NPR1 – Relative success: {a mathematical formula}C⊚⊚[T,S]=C or {a mathematical formula}[T,S]∈C⊚⊚[T,S].
      </paragraph>
      <paragraph>
       This postulate, inspired by [28], says that all or nothing is accepted.
      </paragraph>
      <paragraph>
       NPR2 – Weak success: if {a mathematical formula}(B,A)∉Cl(C) then {a mathematical formula}[(A,B),S]∈C⊚⊚[(A,B),S].
      </paragraph>
      <paragraph>
       This postulate establishes that {a mathematical formula}[(A,B),S] is accepted in the revised credibility base if there is no path in {a mathematical formula}C from B to A.
      </paragraph>
      <paragraph>
       NPR3 – Conditional success: {a mathematical formula}[(A,B),S]∈C⊚⊚[(A,B),S] when for all objects {a mathematical formula}[(D,E),F] that are not safe with respect to {a mathematical formula}(B,A) in {a mathematical formula}C it holds that {a mathematical formula}(F,S)∈Cl(C).
      </paragraph>
      <paragraph>
       Observe that both NPR1 and NPR2 do not consider the reliability of credibility objects. Nevertheless, NPR3 establishes that a credibility object is accepted when its informant is sufficiently credible. That is, the input will be accepted when the informants of those elements that are not safe with respect to {a mathematical formula}(B,A) in {a mathematical formula}C are less credible than the informant of the new incoming tuple {a mathematical formula}(A,B).
      </paragraph>
      <paragraph>
       NPR4 – Inclusion: {a mathematical formula}C⊚⊚[T,D]⊆C∪{[T,D]}.
      </paragraph>
      <paragraph>
       This postulate states that besides {a mathematical formula}[T,D] no element will be added upon revision of {a mathematical formula}C by {a mathematical formula}[T,D].
      </paragraph>
      <paragraph>
       NPR5 – Soundness: if {a mathematical formula}C is sound then {a mathematical formula}C⊚⊚[T,D] is sound.
      </paragraph>
      <paragraph>
       This postulate guarantees that soundness is preserved in the revised credibility base.
      </paragraph>
      <paragraph>
       NPR6 – Uniformity: If it holds that {a mathematical formula}[T,D]∈C⊚⊚[T,D] if and only if {a mathematical formula}[T,E]∈C⊚⊚[T,E], then {a mathematical formula}C∩(C⊚⊚[T,D])=C∩(C⊚⊚[T,E]).
      </paragraph>
      <paragraph>
       Given a tuple T and two informant agents D and E, if {a mathematical formula}[T,D] is accepted in the revised base whenever {a mathematical formula}[T,E] is accepted in the revised base, then {a mathematical formula}C⊚⊚[T,D] and {a mathematical formula}C⊚⊚[T,E] preserve the same credibility objects.
      </paragraph>
      <paragraph>
       NPR7 – Safe retainment: If {a mathematical formula}[(G,H),I]∈C⊚⊚[(G,H),I] then {a mathematical formula}[(D,E),F]∈C⊚⊚[(G,H),I] if and only if {a mathematical formula}[(D,E),F] is a safe element with respect to {a mathematical formula}(H,G) in {a mathematical formula}C.
      </paragraph>
      <paragraph>
       The credibility objects which prevail after a revision by a credibility object accepted in the revised credibility base, will be the objects that are safe objects before the revision, similarly to [2].
      </paragraph>
     </section>
     <section label="7.2">
      <section-title>
      ...
      <paragraph>
       Given a set P of credibility objects, the following function returns all the credibility objects that are associated with identifiers which are not less credible than other identifier associated with the credibility objects in P.
      </paragraph>
      <paragraph label="Definition 15">
       Maximal sources function{a mathematical formula}maxC:C→C, is a function such that for a given credibility base {a mathematical formula}C∈C and {a mathematical formula}P⊆C, {a mathematical formula}maxC(P)={[T,A]:[T,A]∈P and for all [T′,B]∈P,(A,B)∉Cl(C)}.
      </paragraph>
      <paragraph>
       Another auxiliary function denoted Sources is introduced next. This function takes a credibility base {a mathematical formula}C and returns the set of agent identifiers which are sources of credibility objects that belong to {a mathematical formula}C.
      </paragraph>
      <paragraph label="Example 9">
       Sources functionThe function {a mathematical formula}Sources:C→2A is such that {a mathematical formula}Sources(C)={A: there is T∈A×A and[T,A]∈C}, for a given credibility base {a mathematical formula}C∈C.For the {a mathematical formula}CB in Example 2{a mathematical formula}Sources(CB)={B,F1,F2,F3}.
      </paragraph>
      <paragraph>
       Based on a credibility base {a mathematical formula}C, we will define a function {a mathematical formula}Rl((A,B),C) that given a credibility tuple {a mathematical formula}(A,B)∈Cl(C), returns a set of agent identifiers that represent the reliability of {a mathematical formula}(A,B) with respect to {a mathematical formula}C. Recall that the function Sources returns a set of agent identifiers (Definition 16) and that {a mathematical formula}C(A−B) represents the set of all paths from A to B.
      </paragraph>
      <paragraph label="Definition 17">
       Reliability functionThe reliability function, {a mathematical formula}Rl:(A×A)×C→2A, is a function such that for a given credibility base {a mathematical formula}C∈C and a credibility tuple {a mathematical formula}(A,B)∈Cl(C):{a mathematical formula}
      </paragraph>
      <paragraph>
       Note that the function Rl requires that {a mathematical formula}(A,B) be in {a mathematical formula}Cl(C), and therefore {a mathematical formula}C(A−B)≠∅ (see Remark 2). Observe also that the function {a mathematical formula}maxC can return more than one agent identifier, therefore Rl can return a set of pairwise incomparable agents (see Example 12, Example 13). Below, we show how Rl is used in our non-prioritized operator to analyze the input and decide if the input is rejected or accepted. First, we show how Rl works with our running example.
      </paragraph>
      <paragraph label="Example 10">
       Consider the credibility base of agent B given in Example 2, {a mathematical formula}CB={[(B,F1),F2],[(F1,F2),F3],[(F2,F3),B],[(A1,A2),F1],[(A1,A3),F1],[(A2,A4),B],[(A2,A4),F3],[(A3,A4),F2]}. Suppose that agent B needs to calculate the reliability of the credibility tuple {a mathematical formula}(A1,A4). Next, we show the paths set from {a mathematical formula}A1 to {a mathematical formula}A4 in {a mathematical formula}CB (according to Example 3) and the application of the function {a mathematical formula}minCB to each path in the paths set. {a mathematical formula}CB(A1−A4)={P1,P2,P3}, where:{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula} Then, {a mathematical formula}maxCB({[(A1,A2),F1],[(A2,A4),B],[(A1,A3),F1]})={[(A1,A2),F1],[(A1,A3),F1]}. From this set, the set containing the agent identifiers is obtained through the function Sources, {a mathematical formula}Sources({[(A1,A2),F1],[(A1,A3),F1]})={F1}. Then, {a mathematical formula}Rl((A1,A4),CB)={F1}.
      </paragraph>
      <paragraph label="Definition 18">
       Non-prioritized C-revision using reliabilityLet {a mathematical formula}C be a credibility base in {a mathematical formula}C, and {a mathematical formula}[(A,B),S] a credibility object. Let {a mathematical formula}⊛σ↓ be a prioritized CR-revision operator and ⊕ the CR-expansion operator. The operator “{a mathematical formula}⊚⊚σ↓”, called non-prioritized CR-revision, is defined as follows:{a mathematical formula}
      </paragraph>
      <paragraph>
       The first case in Definition 18 states that when no contradiction arises the input is accepted and added (expansion). The second case states that if a contradiction arises ({a mathematical formula}(B,A)∈Cl(C)) then the input will be accepted and added if every agent returned by {a mathematical formula}Rl((B,A),C) is less credible than S. The third case states that the input is rejected when there exists some agent identifier returned by Rl that is incomparable with S or more credible than S.
      </paragraph>
      <paragraph>
       The following proposition establishes a relation between this non-prioritized revision operator {a mathematical formula}⊚⊚σ↓ and the postulates presented.
      </paragraph>
   ...
   <appendices>
    <section label="Appendix A">
     <paragraph label="Proposition 3">
      Let{a mathematical formula}D,E,F,G∈A,{a mathematical formula}C∈C. If for all subsets{a mathematical formula}C′of{a mathematical formula}C,{a mathematical formula}(D,E)∈Cl(C′)if and only if{a mathematical formula}(F,G)∈Cl(C′)then{a mathematical formula}(D,E)=(F,G).
     </paragraph>
     <paragraph label="Proof">
      By reductio ad absurdum. Suppose that {a mathematical formula}(D,E)≠(F,G). Consider {a mathematical formula}C′={[(D,E),X]} for some {a mathematical formula}X∈A. Therefore, {a mathematical formula}(D,E)∈Cl(C′) and {a mathematical formula}(F,G)∉Cl(C′) showing that the supposition is untenable.Before giving the proofs of the theorems, we introduce an auxiliary result that will be used in the proof of Theorem 1 and Theorem 2. This proposition is adapted from a property proposed in [27]. □
     </paragraph>
     ...
     <paragraph label="Proof">
      • Postulates to construction. We need to show that if an operator (−) satisfies the enumerated postulates, then it is possible to build an operator in the way specified in the theorem ({a mathematical formula}⊖σ↓).
      <list>
       Let “{a mathematical formula}σ↓” be a function such that, for every credibility base {a mathematical formula}C∈C and for every tuple {a mathematical formula}(A,B) holds that {a mathematical formula}σ↓(C(A−B))=C∖C−(A,B).1. “
      </list>
      <paragraph>
     ...
     <paragraph label="Proof">
      • Postulates to construction. We need to show that if an operator (⁎) satisfies the enumerated postulates, then it is possible to build an operator in the way specified in the theorem ({a mathematical formula}⊛σ↓).
      <list>
       Let “{a mathematical formula}σ↓” be a function such that for every credibility base {a mathematical formula}C∈C and for every tuple {a mathematical formula}(D,E) holds {a mathematical formula}σ↓(C(E−D))=C∖C⁎[(D,E),F].1. “
      </list>
      <paragraph>
