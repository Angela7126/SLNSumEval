  ...
  <root>
   ...
   <content>
    ...
    <section label="2">
     ...
     <section label="2.4">
      <section-title>
       Symmetry breaking during search and its variants
      </section-title>
      <paragraph>
    ...
    <section label="4">
     ...
     <section label="4.1">
      ...
      <list>
       <list-item label="1.">
        Given the input symmetries G. Upon each backtrack, ReSBDS adds symmetry breaking constraints added by ParSBDS.
       </list-item>
       <list-item label="2.">
        ReSBDS maintains a backtrackable set T of assignments, which is initially empty at the root node. Whenever ReSBDS adds a symmetry breaking constraint, {a mathematical formula}Ag⇒(x≠v)g (where {a mathematical formula}g∈G), we add the following into T:
       </list-item>
       <list-item label="3.">
        After constraint propagation at every node {a mathematical formula}P0 with partial assignment E during search, ReSBDS performs:
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph>
       Given a CSP {a mathematical formula}P=(X,D,C). We present the ReSBDS algorithm with given symmetries G in the following.{sup:1}
      </paragraph>
      <paragraph>
       We show how to implement ReSBDS in a depth first search solver in Algorithm 1. To add the symmetry breaking constraint, we need to know the current partial assignment, which is recorded in E. T is the backtrackable set that is maintained by ReSBDS while VT records the set of violated assignments in T after constraint propagation at a search node.
      </paragraph>
      <paragraph>
     ...
     <section label="4.2">
      ...
      <paragraph label="Lemma 2">
       Given the same set of symmetries to ReSBDS and LReSBDS. Suppose both use the same variable and value orderings. If the prunings of all violated assignments in T are effected by the symmetry breaking constraints when using ReSBDS, ReSBDS{a mathematical formula}=nLReSBDS and ReSBDS{a mathematical formula}=sLReSBDS.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       Given a CSP {a mathematical formula}P=(X,D,C). We present the LReSBDS algorithm with given symmetries G in the following.{sup:2}
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       Algorithm 5 shows how to add symmetry breaking constraints according to the given symmetries G, current partial assignment E and the branching assignment {a mathematical formula}x=v where backtrack happens or violated assignment {a mathematical formula}x=v. For each of the non-broken symmetry in G, one symmetry breaking constraint is added (line 2).
      </paragraph>
      <paragraph>
       Once a constraint is added to the constraint store or some form of consistency is enforced at a search node, Algorithm 6, an AC3-like constraint propagation algorithm, would be called to trigger propagators of all constraints in the constraint store. In this algorithm, a constraint c would be chosen from the constraint store (line 3) and deleted from Q (line 4). After that, we call its propagator (line 5). If its propagator can prune some values (line 5), all other constraints that have been deleted from Q but have the same variable in their scope with that in c would be added back to Q (line 6). The while-loop would trigger all constraints in Q until Q is empty (line 2).
      </paragraph>
      <paragraph>
       Algorithm 7 shows the constraint propagation algorithm of all symmetry breaking constraints added by LReSBDS which is in the form {a mathematical formula}A⇒(x≠v). Values are pruned when all assignments except {a mathematical formula}xi=vi in the assignment set {a mathematical formula}A∧(x=v) are true (line 1). Now {a mathematical formula}xi=vi is enforced to be false (line 2). Additional symmetry breaking constraints are added according to this pruning (line 3). And these symmetry breaking constraints' propagation is also done by Algorithm 7. Hence the recursive addition of constraints is done by the propagation mechanism which stops propagation only when every variable's domain does not change and no extra constraints are added then.
      </paragraph>
      <paragraph>
       Similar to the implementation of SBDS and ReSBDS, LReSBDS does not add symmetry breaking constraints for broken symmetries and also a boolean variable is constructed for each symmetry {a mathematical formula}h∈G at each node P representing whether {a mathematical formula}Eh is satisfied or not where E is the partial assignment of P. Along branching, {a mathematical formula}bhP is incrementally computed in the similar way of that in ReSBDS.
      </paragraph>
      <paragraph>
       We give theorems on the termination of Step 2.
      </paragraph>
      <paragraph label="Theorem 5">
       Step 2 of LReSBDS always terminates.
      </paragraph>
      <paragraph label="Proof">
       The number of all possible assignments is limited. Once a value is pruned from its variable's domain, this value never be pruned from its variable's domain again in subsequent constraint propagation. Thus the recursive addition of constraints will always terminate. □
      </paragraph>
     </section>
     <section label="4.3">
      <section-title>
      ...
      <paragraph label="Proof">
       ...
       <paragraph>
        further shows ReSBDS {a mathematical formula}=s LReSBDS. Thus LReSBDS {a mathematical formula}≻s LexLeader follows directly. □
       </paragraph>
      </paragraph>
      <paragraph>
      ...
      <paragraph label="Proof">
       ReSBDS{a mathematical formula}≻sDoubleLex and LReSBDS{a mathematical formula}≻sDoubleLex.ReSBDS {a mathematical formula}⪰s DoubleLex and LReSBDS {a mathematical formula}⪰s DoubleLex following directly from Theorem 14. To show strictness, consider the ECCLD results in Table 6. ReSBDS and LReSBDS are given the same subset of symmetries as that to DoubleLex, but ReSBDS and LReSBDS leave less solutions than DoubleLex does for all cases. □
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       Even though LexLeader leaves 57 nodes while ReSBDS leaves 55 nodes for the {a mathematical formula}n×n unconstrained matrix problem with {a mathematical formula}n=2 and {a mathematical formula}d=3, ReSBDS is not stronger in nodes pruning than LexLeader in general.
      </paragraph>
      <paragraph label="Theorem 17">
       Given the same set of symmetries. ReSBDS/LReSBDS{a mathematical formula}∦nLexLeader when both search with the same static variable ordering used by LexLeader and min (max) value ordering.
      </paragraph>
      <paragraph label="Proof">
      ...
      <paragraph label="Theorem 21">
       Suppose G is the set of symmetries such that adjacent rows (columns) are interchangeable. Applying ReSBDS/LReSBDS on G and searching with the input variable variable ordering and static value ordering eliminates all symmetries in Σ, where Σ is the symmetry group of G.
      </paragraph>
      <paragraph label="Proof">
       For interchangeable rows (columns), LexLeader can eliminate Σ by being only given G according to the input variable variable ordering and static value ordering. Thus, results follow directly from Theorem 14. □
      </paragraph>
      <paragraph>
    ...
    <section label="5">
     ...
     <section label="5.1">
      ...
      <section label="5.1.3">
       The {a mathematical formula}n×n queen problem
       <paragraph>
      ...
      <section label="5.1.4">
       ...
       <paragraph>
        The ECCLD problem is prob036 in CSPLib [40]. The task is to find the maximum number b of codes of length n drawn from 4 symbols {a mathematical formula}{1,2,3,4} such that the Lee distance between any pair of codes is exactly c, where the Lee distance between two symbols a and b is min {a mathematical formula}{|a−b|,4−|a−b|}. We model it into a {a mathematical formula}b×n matrix with domain {a mathematical formula}{1…4}. Similar to Lee and Li [12], in order to illustrate the effect on solution set size, we transform the optimization problem to a satisfaction one by setting b in advance. This model has matrix symmetries which is exponential to the problem size. We only try to break a subset of the available matrix symmetries in the problem.
       </paragraph>
       <paragraph>
     ...
     <section label="5.2">
      ...
      <section label="5.2.3">
       <section-title>
        Optimization versions of ECCLD
       </section-title>
       <paragraph>
    ...
    <section label="6">
     ...
     <section label="6.2">
      ...
      <paragraph>
       Another dynamic symmetry breaking method is by constructing GE-trees [8] which is a search tree containing a unique representative of each class of full assignments. Such a search tree also has the property that no node is isomorphic to any other node. Puget [61] proposes dynamic lexicographic constraints (DLC) to use a dynamic variable order (the one used during search) in the lexicographic constraints to avoid incompatibility with search heuristics.
      </paragraph>
      <paragraph>
