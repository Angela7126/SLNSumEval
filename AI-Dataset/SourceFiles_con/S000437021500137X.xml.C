  ...
  <root>
   ...
   <content>
    ...
    <section label="4">
     ...
     <paragraph>
      Here, we briefly describe the two main ideas behind SSTF and develop these in more detail in successive sections.
     </paragraph>
     <section label="4.1">
      <section-title>
       Overview of SSTF
    ...
    <section label="5">
     ...
     <section label="5.2">
      ...
      <list>
       <list-item label="1.">
        BPMF, Bayesian probabilistic matrix factorization [53], analyzes ratings by users of items without tags; it cannot predict tags, although the predicted personalized tags can assist users in understanding the predicted items [21].
       </list-item>
       <list-item label="2.">
        BPMFI applies an item vocabulary to BPMF as per our approach.
       </list-item>
       <list-item label="3.">
        NTF, Non-negative tensor factorization [65], is a generalization of Non-negative matrix factorization (NMF) [66] and imposes non-negative constraints on tensor and factor matrices. We selected this method since it is a well-known tensor factorization approach and the baseline of the other tensor factorization methods (VNTF [67] and NMTF [23]) selected for comparison.
       </list-item>
       <list-item label="4.">
        BPTF proposed by [3].
       </list-item>
       <list-item label="5.">
        VNTF, Variational Non-negative Tensor Factorization, is another Bayesian-based tensor factorization method and is a generalization of Variational Non-negative Matrix Factorization (VNMF) [67]. Since we used the BPTF, which is the Bayesian-based method, we selected the VNTF for comparison.
       </list-item>
       <list-item label="6.">
        BPTFT, which utilizes only tag taxonomy.
       </list-item>
       <list-item label="7.">
        BPTFI, which utilizes only item vocabulary.
       </list-item>
       <list-item label="8.">
        SSTFR, Semantic Sensitive Tensor Factorization with Random disambiguation, f links sparse objects (items and tags) to one of their classes randomly (we call this process as random disambiguation). Then it augments the tensors and factorizes the original tensor and augmented tensors simultaneously by using the process explained in Section 4.3.
       </list-item>
       <list-item label="9.">
        SRTF[16] is our previous method that incorporates semantic biases into tensor factorization without sensitively considering the degree of sparsity of objects (here, we set parameter δ used in SRTF to 0.2 because it achieved the highest accuracy if we changed δ from 0.0 to 0.2).
       </list-item>
       <list-item label="10.">
        GCTF is the most popular method that factorizes several tensors/matrices simultaneously [8], [9].
       </list-item>
       <list-item label="11.">
        NMTF[23] is an another state-of-the-art method that utilizes the auxiliary information like GCTF. It factorizes the target and auxiliary tensors simultaneously. The auxiliary data tensors compensate for the sparseness of the target data tensor. Thus, this method is related to our approach and should be compared with SSTF.
       </list-item>
      </list>
     </section>
     <section label="5.3">
      <section-title>
       Methodology and parameter setup
      </section-title>
      <paragraph>
       Following the methodology used in the BPTF paper [3], we used root mean square error (RMSE) as the performance metric. It is computed as {a mathematical formula}(∑i=1n(Pi−Ri)2)/n, where n is the number of entries in the test dataset, and {a mathematical formula}Pi and {a mathematical formula}Ri are the predicted and actual ratings of the i-th entry, respectively. Smaller RMSE values indicate higher accuracy. We divided each dataset into three and performed three-fold cross validation. The results below are the average values of the three evaluations. Following [3], the parameters are {a mathematical formula}μ0=O, {a mathematical formula}ν0=D, {a mathematical formula}β0=1, {a mathematical formula}W0=I, {a mathematical formula}ν˜0=1, {a mathematical formula}Λ˜=1, and {a mathematical formula}W˜0=1. L is 500. D was set from 25 to 100. We used the Itakura–Saito (IS) divergence [68] for the optimization function of GCTF rather than the IS divergence, Kullback–Leibler (KL) divergence, or Euclidean distance [56] because it achieved the highest accuracy when using ID distance for our datasets. For VNTF, we used 200 iterations for learning the target distribution in variational Bayes, since it converges with this number for all datasets. To run NMTF, we combined the user–item–tag tensor with item–class and tag–class matrices. We set the weights on the auxiliary matrices used by NMTF as 0.01 for MovieLens, 0.1 for Last.fm, and 0.1 for the Yelp dataset since NMTF outputs the best RMSE values with the above parameter settings. This weighting parameter determines how strongly the biases from the auxiliary matrices are incorporated in the user–item–tag tensor.
      </paragraph>
     </section>
     <section label="5.4">
      <section-title>
      ...
      <section>
       ...
       <section>
        ...
        <paragraph>
         Table 13 presents the computation times of BPTF and SSTF when {a mathematical formula}L=500. All experiments were conducted on a Linux 3.33 GHz Intel Xeon (24 cores) server with 192 GB of main memory. All methods were implemented with Matlab and GCC. We can see that the computation time of SSTF is shorter than the case that increases linearly in proportion to 2X (the number of different kinds of sparse item sets and sparse tag sets.). Furthermore, we can set L smaller than 500 (see Fig. 5) and computation time is linear in L. Thus, we can conclude that SSTF can compute more accurate predictions quickly; it works better than BPTF on real applications.
        </paragraph>
       </section>
      </section>
     </section>
