  ...
  <root>
   ...
   <content>
    <section label="1">
     ...
     <paragraph>
      Most nonmonotonic logics deal with inconsistent prototypes by maximizing the set of satisfied nonmonotonic axioms. Intuitively, it is like identifying all optimal repairs of the inconsistent prototype and computing the inferences that hold for all repairs. Of course, in writing a knowledge base that models a concrete scenario, computing the invariants across all repairs is not necessarily the right approach. There may be a single meaningful (application dependent) way of removing the inconsistency, and the knowledge engineer should be involved in deciding how to repair the prototype.
     </paragraph>
     <paragraph label="Example 1">
      Consider the famous Nixon's diamond:
     </paragraph>
     <list>
      <list-item label="1.">
       Quakers are normally pacifist;
      </list-item>
      <list-item label="2.">
       Republicans are normally not pacifist;
      </list-item>
      <list-item label="3.">
    ...
    <section label="2">
     ...
     <paragraph>
      In this paper, we will sometimes mention some important DLs that have been extensively studied in the literature and constitute the foundation of semantic web standards. The logic {a mathematical formula}ALC is defined by the following grammar, where R ranges over role names:{a mathematical formula}
     </paragraph>
     <paragraph>
    ...
    <section label="3">
     ...
     <section label="3.2">
      ...
      <paragraph label="Example 4">
       ...
       <list>
        ...
        <list-item label="2.">
         {a mathematical formula}NMamRedBldCel does not satisfy (8) in {a mathematical formula}I. Then, take a {a mathematical formula}DLN-interpretation {a mathematical formula}J of {a mathematical formula}KB such that:
        </list-item>
       </list>
       <paragraph>
        Summarizing, (6) may or may not be overridden in {a mathematical formula}NMamRedBldCel/I depending on whether {a mathematical formula}NMamRedBldCel satisfies (8) in {a mathematical formula}I.  □
       </paragraph>
      </paragraph>
      <paragraph>
       We will not directly apply the definition of overriding anymore in the rest of the paper, because we can equivalently use a simple, handy test based on classical reasoning, that will be introduced in Section 3.4.
      </paragraph>
      <paragraph label="Remark 1">
       Note that, according to this definition of overriding, a DI δ can be blocked only by higher priority DIs. In particular, it is possible that none of the “acceptable improvements” {a mathematical formula}J in the above definition provides a global solution by satisfying also all {a mathematical formula}δ′∈KB incomparable with δ (i.e. {a mathematical formula}δ′⊀δ and {a mathematical formula}δ⊀δ′). As a consequence, {a mathematical formula}DLN solves only the conflicts that can be settled by the priority relation ≺. This is a characterizing feature of {a mathematical formula}DLN. In the other nonmonotonic description logics (based on default logic, MKNF, circumscription, etc.) a nonmonotonic axiom/rule ν can contribute to blocking an incomparable axiom/rule {a mathematical formula}ν′, which leads to complex conflict resolution procedures. This may be not appropriate to inheritance with overriding, as argued in Section 1.  □
      </paragraph>
      <paragraph>
       The requirement that each normality concept {a mathematical formula}NC should satisfy all the DIs that are not overridden in {a mathematical formula}NC (while overridden DIs can be ignored) naturally leads to the following notion of DI satisfaction:
      </paragraph>
      ...
      <paragraph>
       Now the notion of {a mathematical formula}DLN model can be extended from strong axioms to a full {a mathematical formula}KB simply by stating that all the members of {a mathematical formula}KB must be satisfied:
      </paragraph>
     ...
     <section label="3.6">
      ...
      <paragraph>
       Some nonmonotonic description logics adopt two different approaches for reasoning about TBoxes and ABoxes, e.g. [19]. On the contrary, {a mathematical formula}DLN is one of the logics that treat TBox and ABox reasoning in a uniform way. Recall that the default properties of a concept C can be found by proving inclusions like {a mathematical formula}NC⊑D, as shown in the examples discussed so far; similarly, the default properties of an individual a can be found by proving inclusions {a mathematical formula}N{a}⊑D.
      </paragraph>
      <paragraph label="Example 16">
       Extend the situs inversus example (Example 5) with an ABox containing{a mathematical formula}{a mathematical formula} Let {a mathematical formula}δ1 be the unique DI in {a mathematical formula}KB, that is, (2). Using (27) it is easy to check that {a mathematical formula}δ1N{Ann} belongs to {a mathematical formula}KBΣ, while {a mathematical formula}δ1N{Bob} does not, because the properties of SitusInversus are inconsistent with {a mathematical formula}δ1 (i.e. {a mathematical formula}δ1 is overridden in the concept {a mathematical formula}N{Bob}).Accordingly, both {a mathematical formula}N{Ann} and {a mathematical formula}N{Bob} are satisfiable w.r.t. {a mathematical formula}KBΣ, and the hearts of Ann and Bob are located where it should be expected:{a mathematical formula}{a mathematical formula}  □
      </paragraph>
      <paragraph>
       It is worth noting that DIs only state what standard individuals look like; DIs do not force any individual to be standard.{sup:21} Accordingly, a concept {a mathematical formula}N{a} may happen to be empty, when a contingently violates its default profile. One of the advantages of this approach is that if the prototype associated to a nominal {a mathematical formula}{a} is inconsistent, then only {a mathematical formula}N{a} is inconsistent; the knowledge base remains globally consistent, and it is easier to locate inconsistent prototypes.
      </paragraph>
      <paragraph>
       Of course, if {a mathematical formula}N{a} is satisfiable w.r.t. {a mathematical formula}KB (as it happens with {a mathematical formula}N{Ann} and {a mathematical formula}N{Bob} in the above example), then a could be safely asserted to be normal by adding the assertion {a mathematical formula}N{a}(a) to {a mathematical formula}KB. This assertion restricts the {a mathematical formula}DLN-models of {a mathematical formula}KB to those where a conforms to its default properties.{sup:22} Note, however, that the queries {a mathematical formula}N{a}⊑D simply constitute an alternative way of inspecting the same models: these queries are consequences of {a mathematical formula}KB iff a belongs to D in all the {a mathematical formula}DLN-models where a satisfies its default properties, that is, {a mathematical formula}N{a}(a) holds.{sup:23} So, an alternative phrasing of the query {a mathematical formula}N{a}⊑D is: “if a is normal, then it satisfies D”.
      </paragraph>
      <paragraph>
       The eukaryotic cell example is similar to the above example in many respects, so here we only provide some details about the policy example, that illustrates a more general combination of concept and role assertions.
      </paragraph>
      <paragraph label="Example 17">
       Extend the policy {a mathematical formula}KB of Example 8 with the following ABox, encoding three access requests submitted by Ann, Bob, and Cal:{a mathematical formula} The default decision for each read request {a mathematical formula}reqi can be inspected with a corresponding query {a mathematical formula}N{reqi}⊑∃privilege.Read, so here we choose{a mathematical formula} By constructing {a mathematical formula}KBΣ, it can be verified that:{a mathematical formula}{a mathematical formula}{a mathematical formula} therefore (as expected) Ann and Cal's requests are rejected while Bob's is accepted.  □
      </paragraph>
      <paragraph>
       If your favorite logic does not support nominals, please refer to Section 5.2, that shows how to carry out the same kind of reasoning through a suitable translation.
      </paragraph>
     </section>
     <section label="3.7">
      The logic of {a mathematical formula}DLN
      <paragraph>
       The main logical properties of {a mathematical formula}DLN are illustrated in this section. Obviously, {a mathematical formula}DLN extends classical logic, by definition:
      </paragraph>
      <paragraph label="Proposition 5">
       If{a mathematical formula}S⊨αthen{a mathematical formula}S∪D{an inline-figure}α.
      </paragraph>
      <paragraph>
       The next two results show that concepts and axioms can be replaced by (classical) equivalents without affecting any inference. The first result holds for all priority relations.
      </paragraph>
      <paragraph label="Theorem 6">
       Let{a mathematical formula}S and{a mathematical formula}S′be classically equivalent sets of inclusions and assertions. Then, for all sets of DIs{a mathematical formula}D,
      </paragraph>
      <list>
       <list-item label="1.">
        {a mathematical formula}I is a{a mathematical formula}DLN model of{a mathematical formula}S′∪Diff{a mathematical formula}I is a{a mathematical formula}DLN model of{a mathematical formula}S∪D;
       </list-item>
       <list-item label="2.">
        for all subsumptions/assertions/DIs ϵ,{a mathematical formula}S′∪D{an inline-figure}ϵiff{a mathematical formula}S∪D{an inline-figure}ϵ.
       </list-item>
      </list>
      <paragraph>
       The next theorem, instead, requires a mild assumption on priority relations. Roughly speaking, ≺ should be insensitive to substitutions with logical equivalents:
      </paragraph>
      <paragraph label="Definition 4">
       A priority relation ≺ for {a mathematical formula}KB is semantic iff {a mathematical formula}δ1≺δ2 and {a mathematical formula}C≡KBD imply {a mathematical formula}δ1′≺δ2′, where each {a mathematical formula}δi′ is obtained from {a mathematical formula}δi by replacing some occurrences of C with D.
      </paragraph>
      <paragraph>
       Note that the specificity-based relation is a semantic priority relation.
      </paragraph>
      <paragraph label="Theorem 7">
       Suppose that{a mathematical formula}C≡Dis a (classically) valid equivalence and let{a mathematical formula}KB¯be a knowledge base obtained from{a mathematical formula}KB by replacing some occurrences of C with D. If ≺ is semantic, then
      </paragraph>
      <list>
       <list-item label="1.">
        {a mathematical formula}I is a{a mathematical formula}DLN model of{a mathematical formula}KB¯iff{a mathematical formula}I is a{a mathematical formula}DLN model of{a mathematical formula}KB;
       </list-item>
       <list-item label="2.">
        for all subsumptions/assertions/DIs ϵ,{a mathematical formula}KB¯{an inline-figure}ϵ¯iff{a mathematical formula}KB{an inline-figure}ϵ,
       </list-item>
      </list>
      <paragraph>
    ...
    <section label="4">
     ...
     <paragraph>
      In this section we deal with the complexity of nonmonotonic inference in {a mathematical formula}DLN. The decision problems of interest are the following:
     </paragraph>
     <list>
      <list-item label="•">
       Subsumption checking in{a mathematical formula}DLN: given a {a mathematical formula}DLN knowledge base {a mathematical formula}KB and two {a mathematical formula}DLN concepts C and D, check whether {a mathematical formula}KB{an inline-figure}C⊑D;
      </list-item>
      <list-item label="•">
       Instance checking in{a mathematical formula}DLN: given a {a mathematical formula}DLN knowledge base {a mathematical formula}KB and a {a mathematical formula}DLN assertion {a mathematical formula}α=C(a) or {a mathematical formula}α=R(a,b), check whether {a mathematical formula}KB{an inline-figure}α;
      </list-item>
      <list-item label="•">
       N-free DI inference in{a mathematical formula}DLN: given an N-free {a mathematical formula}DLN  knowledge base {a mathematical formula}KB and two (N-free) {a mathematical formula}DL concepts C and D, check whether {a mathematical formula}KB{an inline-figure}C⊑nD.{sup:25}
      </list-item>
     </list>
     <paragraph>
     ...
     <list>
      <list-item label="•">
       {a mathematical formula}DL supports ⊓ in the left-hand side of inclusions, and
      </list-item>
      <list-item label="•">
       subsumption (resp. instance) checking in{a mathematical formula}DL belongs to a complexity class{a mathematical formula}C, and
      </list-item>
      <list-item label="•">
       deciding the preference relation ≺ belongs to{a mathematical formula}PC,{sup:26}
      </list-item>
     </list>
     ...
     <list>
      ...
      <list-item label="•">
       subsumption checking in{a mathematical formula}DL belongs to a complexity class{a mathematical formula}C, and
      </list-item>
      <list-item label="•">
       the preference relation ≺ belongs to{a mathematical formula}PC,
      </list-item>
     </list>
     <paragraph>
      If {a mathematical formula}C=P, then {a mathematical formula}coNPC equals coNP, and we immediately obtain:
     </paragraph>
     <paragraph label="Corollary 18">
      If subsumption is tractable in a logic{a mathematical formula}DL that supports ⊓ in the left-hand side of inclusion, and ≺ can be decided in polynomial time, then N-free DI inference in{a mathematical formula}DLN is in coNP. In particular, if ≺ is specificity, then this complexity bound holds for:
     </paragraph>
     <list>
      <list-item label="•">
       {a mathematical formula}(EL++)N,
      ...
      <list-item label="•">
       {a mathematical formula}(DL-litehorn(HN))N.
      </list-item>
     </list>
     <paragraph>
      This result and Theorem 16 prove that N-free DI inference is coNP complete for {a mathematical formula}(EL++)N and {a mathematical formula}(DL-litehorn(HN))N.
     </paragraph>
     <paragraph>
      If {a mathematical formula}C=ExpTime, then the nondeterministic search performed by the polynomial time Turing machine with ExpTime oracle can be turned into an ExpTime deterministic search, that preserves the asymptotic cost of the oracle's tests; so the {a mathematical formula}coNPC bound collapses to ExpTime, and we immediately obtain:
     </paragraph>
    ...
    <section label="5">
     ...
     <section label="5.1">
      ...
      <paragraph label="Theorem 22">
       Let{a mathematical formula}KB=S∪Dbe a canonical{a mathematical formula}DLN knowledge base. Then{a mathematical formula}KB is satisfiable iff{a mathematical formula}S is (classically) satisfiable.
      </paragraph>
      <paragraph>
       Again, the hypothesis that {a mathematical formula}KB is canonical is necessary: Example 19 provides a counterexample to satisfiability preservation in non-canonical knowledge bases.
      </paragraph>
     </section>
     <section label="5.2">
      <section-title>
       Reasoning on individuals without nominals
      </section-title>
      <paragraph>
       The default properties of individuals are queried using nominals, by solving inference problems like {a mathematical formula}KB{an inline-figure}N{a}⊑C. If {a mathematical formula}DL and its inference engine do not support nominals, then it may be convenient to reduce the above problem to an inference problem that does not involve nominals.
      </paragraph>
      <paragraph>
      ...
      <list>
       <list-item label="1.">
        {a mathematical formula}KB contains no nominals;
       </list-item>
       <list-item label="2.">
        {a mathematical formula}{a}is the only nominal contained in Σ, that is, for all{a mathematical formula}N{b}∈Σ,{a mathematical formula}b=a.
       </list-item>
      </list>
      <paragraph label="Definition 6">
       Let {a mathematical formula}tr(KBΣ) be the set of axioms obtained from {a mathematical formula}KBΣ by:
      </paragraph>
      <list>
       <list-item label="1.">
        removing {a mathematical formula}N{a}⊑{a};
       </list-item>
       <list-item label="2.">
        adding the assertion {a mathematical formula}F(a), where F is a fresh concept name;
       </list-item>
       <list-item label="3.">
        replacing each {a mathematical formula}δiN{a} with {a mathematical formula}δiF=F⊓pre(δi)⊑con(δi).
       </list-item>
      </list>
      <paragraph>
       The resulting KB is correct, and hence reasoning about nominals can be reduced to an instance problem in a {a mathematical formula}DL theory without nominals.
      </paragraph>
      <paragraph label="Theorem 23">
       {a mathematical formula}KB{an inline-figure}N{a}⊑Ciff{a mathematical formula}tr(KBΣ)⊨C(a).
      </paragraph>
     </section>
     <section label="5.3">
      <section-title>
       Relaxing default properties in subclasses
      </section-title>
      <paragraph>
       We expect that in some applications it may be necessary to override some of the default properties of a class by relaxing them, rather than asserting their negation. For example, consider the definition of a subclass of humans whose heart is located somewhere in the chest, rather than specifically in its left- or right-hand side. Then the DI of Example 3, stating that human hearts are normally located in the left-hand side of the body, should be overridden without asserting that the heart is not placed there. This behavior can be simulated in {a mathematical formula}DLN in a uniform way by transforming each DI {a mathematical formula}δi=(C⊑nD) into {a mathematical formula}C⊑nD⊓¬Abi, where {a mathematical formula}Abi is a fresh symbol representing abnormality with respect to {a mathematical formula}δi. Now, for any given concept {a mathematical formula}C′, {a mathematical formula}δi can be blocked without asserting ¬D, by means of a DI {a mathematical formula}C′⊑nAbi. The approach is robust: the transformation of {a mathematical formula}δi alone does not affect inferences unless some blocking axioms {a mathematical formula}C′⊑nAbi are introduced. A practical version of {a mathematical formula}DLN might introduce syntactic sugar to handle the additional abnormality predicates {a mathematical formula}Abi implicitly.
      </paragraph>
     </section>
    </section>
    <section label="6">
     <section-title>
      Comparison with other nonmonotonic DLs
     </section-title>
     <paragraph>
      In this section we assume the reader to be familiar with the nonmonotonic DLs compared with {a mathematical formula}DLN. Their syntax and semantics can be found in [3], [4], [21], [10], [18], [29], [19], [40]. All the logics dealt with in this section have two features in common, that make {a mathematical formula}DLN an appealing alternative:
     </paragraph>
     <list>
      <list-item label="1.">
       they depart from desired behavior in two or more of the motivating examples;
      </list-item>
      <list-item label="2.">
       they do not enjoy any tractable fragment that covers the set of motivating examples (unlike {a mathematical formula}DLN); actually, in most cases, no tractable fragment is known.
      </list-item>
     </list>
     <paragraph>
      A detailed comparison with the design patterns illustrated in [44], [47] can be found in Appendix C.
     </paragraph>
     <section label="6.1">
      <section-title>
      ...
      <list>
       <list-item label="1.">
        All dentists have an assistant;
       </list-item>
       <list-item label="2.">
        Normally, a dentist's assistant is not a dentist;
       </list-item>
       <list-item label="3.">
        Ann is a dentist.
       </list-item>
     ...
     <section label="6.2">
      ...
      <paragraph>
       Some of the earliest nonmonotonic DLs are based on Default logic and prioritized extensions thereof [3], [4]. Undecidability problems led the authors of these papers to restrict the application of default rules to the individuals that are explicitly named in the knowledge base; this means that the implicit individuals introduced by existential quantification are not subject to nonmonotonic axioms.
      </paragraph>
      <paragraph>
       As a consequence, Default DLs cannot encode Example 12, because the range of role {a mathematical formula}has_organ consists of implicit individuals to which default rules do not apply.
      </paragraph>
      <paragraph>
     ...
     <section label="6.3">
      ...
      <paragraph>
       Autoepistemic description logics [21] have many properties in common with the description logics based on Default logic, so we provide only a brief summary of their features highlighting the relationships between the two approaches.
      </paragraph>
      <paragraph>
       The autoepistemic approach is based on a version of MKNF (the logic of minimal knowledge and negation as failure) where all interpretations have the same domain: a denumerable set of constants called standard names. The examples of Section 6.2 can be transformed into analogous examples for MKNF by replacing each default rule{a mathematical formula} with the MKNF inclusion {a mathematical formula}Kα⊓¬A¬β⊑Kβ, where K is an epistemic operator and the modality {a mathematical formula}¬A is essentially negation as failure.
      </paragraph>
      <paragraph>
       The above nonmonotonic inclusions define the defeasible properties β of individuals. Apparently, since the above inclusions need not be grounded on syntactic domains, they apply to all individuals (including those that are not denoted by any constant occurring in the knowledge base). However, the precondition {a mathematical formula}Kα is very strong. Only individuals that are denoted by a constant occurring in the knowledge base can satisfy it, so in practice nonmonotonic rules apply only to such individuals, as it happens with Default logic. It follows that Autoepistemic description logics are not able to encode Example 12.
      </paragraph>
      <paragraph>
       Conflict handling is based on repairs and may yield multiple models that correspond to the alternative default extensions of default theories. It is not hard to see that the computational complexity of skeptical subsumption is at least coNP-hard, even if the underlying DL is tractable.
      </paragraph>
      <paragraph>
     ...
     <section label="6.5">
      ...
      <paragraph>
       In general, {a mathematical formula}ALC+Tmin resolves conflicts like Circumscription does: In Example 9 (reformulated in {a mathematical formula}ALC+Tmin) it would be possible to conclude neither {a mathematical formula}T(RepQuaker)⊑Pacifist nor the alternative inclusion {a mathematical formula}T(RepQuaker)⊑¬Pacifist, and {a mathematical formula}T(RepQuaker) would be satisfiable. Only direct conflicts such as {a mathematical formula}T(A)⊑C and {a mathematical formula}T(A)⊑¬C would make {a mathematical formula}T(A) inconsistent (so, in Example 10, {a mathematical formula}T(Body)⊑⊥ holds). Another similarity with Circumscription is the CWA effect on exceptional concepts:
      </paragraph>
      <paragraph label="Example 28">
       The {a mathematical formula}ALC+Tmin knowledge base{a mathematical formula}{a mathematical formula}{a mathematical formula} entails that there are no whales, that is: {a mathematical formula}Whale⊑⊥.  □
      </paragraph>
      <paragraph>
       Similar effects may force role ranges to be normal.
      </paragraph>
      <paragraph label="Example 29">
       The above knowledge base entails {a mathematical formula}T(Mammal)≡Mammal, and in this case it is not possible to distinguish {a mathematical formula}∃R.T(Mammal) from {a mathematical formula}∃R.Mammal. In other words, R's ranges necessarily over typical instances.  □
      </paragraph>
      <paragraph>
       Similarly to the other preferential semantics, {a mathematical formula}ALC+Tmin is affected by inheritance blocking. Here is an example.
      </paragraph>
      <paragraph label="Example 30">
       Consider the following variant of the penguins-and-birds example:
      </paragraph>
      <list>
       <list-item label="1.">
        Penguins are birds;
      ...
      <paragraph>
       Analogously, {a mathematical formula}ALC+Tmin does not deal correctly with Example 6.
      </paragraph>
      <paragraph>
       Similarly to conditional entailment, in {a mathematical formula}ALC+Tmin priorities are implicit and grounded in specificity. In the analogue of Example 26, namely,{a mathematical formula}{a mathematical formula}ALC+Tmin is able to resolve the conflict: {a mathematical formula}T(A) is satisfiable and {a mathematical formula}T(A)⊑C is entailed.
      </paragraph>
      <paragraph>
       The computational complexity of reasoning in {a mathematical formula}ALC+Tmin has been analyzed in some selected cases. So far, tractable fragments have not been identified. Some restricted fragments (called left local) fall within the second level of the polynomial hierarchy [27], [28].
      </paragraph>
      <paragraph>
       More nonmonotonic DLs based on rational closure can be found in [18], [17]. Differently from {a mathematical formula}ALC+Tmin, they have a special inclusion operator for DIs but no equivalent of N. They satisfy the Rational closure axioms and are subject to inheritance blocking, like {a mathematical formula}ALC+Tmin (Example 6, Example 30 are not dealt with correctly). Moreover, they are not able to infer any standard property about role values (Example 12 cannot be encoded). No tractable fragments are known; the available results show that complexity is preserved only for ExpTime-hard logics.
      </paragraph>
      <paragraph>
       These logics have been refined in [19] to remove inheritance blocking. The new approach has a syntactic nature (no model-theoretic semantics) and is articulated in two stages:
      </paragraph>
      <list>
       <list-item label="•">
        in the first stage, the knowledge base is converted into a boolean inheritance network (introduced in [19] itself); a set of nonmonotonic inclusions is derived using the network; in general, some of these inclusions cannot be derived by rational closure due to inheritance blocking;
       </list-item>
       <list-item label="•">
     ...
     <section label="6.6">
      ...
      <paragraph>
       Priorities over conditional constraints are automatically derived based on a notion of specificity, similar to those adopted by conditional entailment and rational closure. They all behave in the same way on Example 26, Example 27.
      </paragraph>
      <paragraph>
       Probabilistic DLs do not preserve tractability [40, Theorem 6.4(c)]. On the positive side, probabilistic DLs do not suffer from inheritance blocking (cf. [40, Example 4.17]), and satisfy suitable probabilistic variants of the KLM axioms [40, Theorems 4.19, 4.20].
      </paragraph>
     </section>
     <section label="6.7">
      <section-title>
    ...
    <section label="7">
     ...
     <section label="7.3">
      <section-title>
       Experimental results: test case structure
      </section-title>
      <paragraph>
      ...
      <paragraph>
       For all figures, every single reported value is obtained as the average over ten different non monotonic ontologies and fifty different queries on each ontology, each of which involved the construction of a different translation {a mathematical formula}KBΣ (as Σ depends on the query).
      </paragraph>
      <paragraph>
       Fig. 4 is devoted to generation approach (i). As the DA rate grows, the figure reports the percentage of overridden applicable DIs, plus the average and maximum height of the priority hierarchy of applicable DIs. Fig. 5 reports the same values for generations approach (ii).
      </paragraph>
      <paragraph>
       These figures are reasonable, given GO's structure. The length of the longest path in GO's classification (i.e. the maximum possible hierarchy depth) is 15, and the average length 3.66. In the experiments concerning DIs obtained from strong CIs (see Fig. 5) the average (resp. maximal) depth of the applicable DIs hierarchy range between 41% and 55% of the average (resp. 26.6% and 40% of the maximal) path length in the original ontology, coherently with the random placement of normality concepts in the hierarchy.
      </paragraph>
      <paragraph>
       For fully synthetic DIs (Fig. 4) these values are lower and vary between 34% and 42% of the average length, and between 20% and 33.3% of the maximum length.
      </paragraph>
      <paragraph>
       As expected, in Fig. 5, each increment of the DA-rate causes an increment of the percentage of overridden DIs. This relation is less evident in Fig. 4, probably due to the further randomness introduced by synthetic DI generation.
      </paragraph>
     </section>
     <section label="7.4">
      <section-title>
   ...
   <appendices>
    <section label="Appendix A">
     ...
     <section label="A.1">
      ...
      <list>
       <list-item label="1.">
        {a mathematical formula}δk is not overridden in {a mathematical formula}NC/I;
       </list-item>
       <list-item label="2.">
        {a mathematical formula}NC∉satI(δk) or, equivalently, {a mathematical formula}I⊭δkNC.
       </list-item>
      </list>
      <paragraph>
     ...
     <section label="A.4">
      ...
      <paragraph label="Proof">
       The proof is analogous to the proof of Theorem 12. Let {a mathematical formula}KB=S∪D. The complement of the N-free DI entailment can be decided by guessing a subset {a mathematical formula}D′⊑D and checking whether the condition of Theorem 4 is satisfied. Given the estimates (A.2) and (A.4), this algorithm runs in polynomial time, using an oracle for {a mathematical formula}DL subsumption and ≺. It follows that N-free DI entailment is in {a mathematical formula}coNPC.  □
      </paragraph>
      <paragraph label="Remark 7">
       Corollaries of Section 4The lower complexity bounds (hardness) in the corollaries of Section 4 follow immediately from two observations: if {a mathematical formula}D=∅ (i.e. the knowledge base contains no DIs) then
      </paragraph>
      <list>
       <list-item label="•">
        for all N-free subsumptions and assertions α, {a mathematical formula}KB{an inline-figure}α iff {a mathematical formula}KB⊨α;
       </list-item>
       <list-item label="•">
        for all N-free concepts C and D, {a mathematical formula}KB{an inline-figure}C⊑nD iff {a mathematical formula}KB⊨C⊑D.
       </list-item>
      </list>
     </section>
     <section label="A.5">
      ...
      <paragraph label="Proof">
       {a mathematical formula}KB is satisfiable (i.e. it has a {a mathematical formula}DLN model) iff {a mathematical formula}KB{an inline-figure}⊤⊑⊥ iff {a mathematical formula}S⊭⊤⊑⊥ (by Theorem 21) iff {a mathematical formula}S is classically satisfiable.  □
      </paragraph>
      <paragraph>
       For technical convenience, we identify an intermediate step within the transformation tr of Definition 6. We let {a mathematical formula}tr1(KBΣ) be the set of axioms obtained from {a mathematical formula}KBΣ by removing {a mathematical formula}N{a}⊑{a}, and replacing each {a mathematical formula}δiN{a} with{a mathematical formula}
      </paragraph>
      <paragraph label="Lemma 31">
       {a mathematical formula}KBΣ⊨N{a}⊑Ciff{a mathematical formula}tr1(KBΣ)⊨{a}⊑C.
      </paragraph>
      <paragraph label="Proof">
       (If.) We prove the contrapositive. Assume that {a mathematical formula}KBΣ⊭N{a}⊑C and let {a mathematical formula}I be any model of {a mathematical formula}KBΣ such that {a mathematical formula}N{a}I⊈CI, which implies {a mathematical formula}N{a}I≠∅ and hence (by {a mathematical formula}N{a}⊑{a}∈KBΣ) {a mathematical formula}N{a}I={a}I. This equality implies that for all {a mathematical formula}δ∈KB:
       <list>
        {a mathematical formula}I⊨δN{a} iff {a mathematical formula}I⊨δ{a}, and hence {a mathematical formula}I⊨tr1(KBΣ).{a mathematical formula}{a}I≠∅.(Only if.)
       </list>
       <paragraph>
      ...
      <paragraph label="Lemma 32">
       {a mathematical formula}tr1(KBΣ)⊨{a}⊑Ciff{a mathematical formula}tr(KBΣ)⊨C(a).
      </paragraph>
      <paragraph label="Proof">
       Again, we prove the contrapositives.(If.) Assume that {a mathematical formula}tr1(KBΣ)⊭{a}⊑C. Then there exists {a mathematical formula}I such that {a mathematical formula}I⊨tr1(KBΣ) and {a mathematical formula}aI∉CI. Extend {a mathematical formula}I  to a model of {a mathematical formula}tr(KBΣ) by setting {a mathematical formula}FI={a}I. The extended {a mathematical formula}I clearly satisfies {a mathematical formula}δiF for all {a mathematical formula}δi{a}∈tr1(KBΣ), so {a mathematical formula}I witnesses that {a mathematical formula}tr(KBΣ)⊭C(a).(Only if.) Suppose that {a mathematical formula}tr(KBΣ)⊭C(a). Then there exists {a mathematical formula}I such that {a mathematical formula}I⊨tr(KBΣ) and {a mathematical formula}aI∉CI. Since {a mathematical formula}{a}I⊆FI (by {a mathematical formula}F(a)∈tr(KBΣ)), {a mathematical formula}I satisfies also all inclusions {a mathematical formula}δi{a}∈tr1(KBΣ). Then {a mathematical formula}I⊨tr1(KBΣ), and hence {a mathematical formula}tr1(KBΣ)⊭{a}⊑C.  □
      </paragraph>
      <paragraph label="Theorem 23">
       {a mathematical formula}KB{an inline-figure}N{a}⊑Ciff{a mathematical formula}tr(KBΣ)⊨C(a).
      </paragraph>
      <paragraph label="Proof">
       A straightforward consequence of Theorem 1, Lemma 31, and Lemma 32.  □
      </paragraph>
     </section>
    </section>
    <section label="Appendix B">
     <section-title>
      Classical translations of the examples
     </section-title>
     <paragraph>
      In this section we report the translations {a mathematical formula}KBΣ of the examples illustrated in this paper, with the exception of those fully illustrated in Section 3.5. We do not report the entire{a mathematical formula}KBΣ; the obvious parts are not included, that is: the strong axioms and the inclusions {a mathematical formula}NC⊑C that belong to {a mathematical formula}S0Σ (i.e. we report only the translations of the DIs that are not overridden). In all examples, Σ is the set of all normality concepts that occur in {a mathematical formula}KB or in the queries.
     </paragraph>
     <paragraph>
      Example 6(Nose, inheritance blocking)
     </paragraph>
     <paragraph>
      {a mathematical formula}Σ={NHuman,NSitusInversus}.
     </paragraph>
     <paragraph>
      {a mathematical formula}δ1=(2), {a mathematical formula}δ2=(13). They are incomparable.
     </paragraph>
     <paragraph>
      DI translations in {a mathematical formula}KBΣ:{a mathematical formula}{a mathematical formula}{a mathematical formula}
     </paragraph>
     <paragraph>
      {a mathematical formula}δ1NSitusInversus is discarded because{a mathematical formula}
     </paragraph>
     <paragraph>
      Example 8(Policy, multiple exception levels)
     </paragraph>
     <paragraph>
      Abbreviations:{a mathematical formula}{a mathematical formula}{a mathematical formula}
     </paragraph>
     <paragraph>
      {a mathematical formula}Σ={NU,NS,NB}.
     </paragraph>
     <paragraph>
      {a mathematical formula}δ1=(17), {a mathematical formula}δ2=(16).  {a mathematical formula}(17)≺(16).
     </paragraph>
     <paragraph>
      DI translations in {a mathematical formula}KBΣ:{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula}
     </paragraph>
     <paragraph>
      {a mathematical formula}δ1NB is discarded because{a mathematical formula}
     </paragraph>
     <paragraph>
      {a mathematical formula}δ2NS is discarded because{a mathematical formula}
     </paragraph>
     <paragraph>
      Example 15(Nixon's diamond, inconsistent prototype)
     </paragraph>
     <paragraph>
      {a mathematical formula}Σ={NRepQuaker}.
     </paragraph>
     <paragraph>
      {a mathematical formula}δ1=(19), {a mathematical formula}δ2=(20). They are incomparable.
     </paragraph>
     <paragraph>
      DI translations in {a mathematical formula}KBΣ:{a mathematical formula}{a mathematical formula}
     </paragraph>
     <paragraph>
      Example 10(Prostate) is similar to Nixon's diamond, details are omitted.
     </paragraph>
     <paragraph>
      Example 11(Lawyer's customers, reasoning about role ranges)
     </paragraph>
     <paragraph>
      {a mathematical formula}Σ={NLawyer}.
     </paragraph>
     <paragraph>
      {a mathematical formula}δ1=Person⊑nHonest.
     </paragraph>
     <paragraph>
      DI translations in {a mathematical formula}KBΣ:{a mathematical formula}
     </paragraph>
     <paragraph>
      Example 12(Normal organs, Default role range)
     </paragraph>
     <paragraph>
      {a mathematical formula}Σ={NHuman,NHumanOrgan}.
     </paragraph>
     <paragraph>
      {a mathematical formula}δ1=(24), {a mathematical formula}δ2=(25). They are not comparable.
     </paragraph>
     <paragraph>
      DI translations in {a mathematical formula}KBΣ:{a mathematical formula}{a mathematical formula}{a mathematical formula}{a mathematical formula}
     </paragraph>
     <paragraph>
      Example 16(Situs inversus, reasoning with individuals)
     </paragraph>
     <paragraph>
      {a mathematical formula}Σ={N{Ann},N{Bob}}.
     </paragraph>
     <paragraph>
      {a mathematical formula}δ1=(2).
     </paragraph>
     <paragraph>
      DI translations in {a mathematical formula}KBΣ:{a mathematical formula}
     </paragraph>
     <paragraph>
      {a mathematical formula}δ1N{Bob} is discarded because{a mathematical formula}
     </paragraph>
    </section>
