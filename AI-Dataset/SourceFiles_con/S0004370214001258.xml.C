  ...
  <root>
   ...
   <content>
    <section label="1">
     ...
     <list>
      <list-item label="1.">
       The knowledge structures are isotropic, i.e., they are such that, potentially, everything can be relevant to the outcome of an inference process. They are also Quinean: candidate explanations are sensitive to the entire belief system [17], [18].
      </list-item>
      <list-item label="2.">
       The inferences are provably computationally tractable (either to compute exactly or to approximate) under realistic assumptions with respect to situational constraints [43], [53].
      </list-item>
      <list-item label="3.">
       The resulting explanations have an optimal or close-to-optimal posterior probability in many cases, i.e., MFE actually ‘tracks truth’ in the terms of Glass [28].
      </list-item>
     </list>
     <paragraph>
    ...
    <section label="2">
     ...
     <section label="2.1">
      ...
      <list>
       ...
       <list-item>
        Instance: A Bayesian network {a mathematical formula}B=(GB,Pr), where V is partitioned into evidence variables E with joint value assignment e, explanation variables H, and intermediate variables I.
       </list-item>
       <list-item>
    ...
    <section label="3">
     ...
     <paragraph label="Example 1">
      Consider in the ALARM network the observations that PCWP and BP are high and the other observable variables take their non-alarming states. The actual value of ACO2 does not influence the most probable value of the observable variables in the network, i.e., {a mathematical formula}argmaxhPr(h,e,i,ACO2=high)=argmaxhPr(h,e,i,ACO2=mid)=argmaxhPr(h,e,i,ACO2=low) for every joint value assignment i to the intermediate variables other than ACO2. However, ACO2 is not conditionally independent of (e.g.) KINK given the observed evidence variables.
     </paragraph>
     <paragraph>
     ...
     <section label="3.1">
      ...
      <paragraph>
       As computing the relevance of a variable I is {a mathematical formula}NP-hard, i.e., intractable in general (see Appendix A for a formal proof), we introduce the notion estimated relevance of I as a subjective assessment of {a mathematical formula}R(I) which may or may not correspond to the actual value. Such a subjective assessment might be based on heuristics, previous knowledge, or by approximating the relevance, e.g., by sampling some instances of {a mathematical formula}Ω(I∖{I}). Where confusion may arise, we will use the term intrinsic relevance to refer to the actual statistical property ‘relevance’ of I, in contrast to the subjective assessment thereof. Note that both intrinsic and estimated relevance of a variable are relative to a particular set of candidate explanations H, and conditional on a particular observation, i.e., a value assignment e to the evidence nodes E.
      </paragraph>
      <paragraph label="Example 5">
       Let, in the ALARM network, pulmonary capillary wedge pressure and blood pressure be high, and let all other observable variables take their non-alarming default values. The intrinsic relevance of the intermediate variables for the diagnosis is given in Fig. 2.
      </paragraph>
      <paragraph>
     ...
     <section label="3.2">
      ...
      <list>
       ...
       <list-item>
        Question: Is there a joint value assignment h to the nodes in H such that for more than k disjoint joint value assignments i to {a mathematical formula}I−, {a mathematical formula}Pr(h,i,e)&gt;q?
       </list-item>
      </list>
      <paragraph>
       It will be immediately clear that MFE is intractable, as it has the {a mathematical formula}NPPP-complete MAP [51] and MSE [35] problems as special cases for {a mathematical formula}I−=∅, respectively {a mathematical formula}I+=∅. In this section we show that MFE happens to be even harder, viz., that it is {a mathematical formula}NPPPPP-complete, making it one of few real world-problems that are complete for that class.{sup:6} The canonical Satisfiability-variant that is complete for this class is E-MajMajsat, defined as follows [61].
      </paragraph>
      <list>
       EMajMajsat
       <list-item>
        Instance: A Boolean formula ϕ whose n variables {a mathematical formula}x1…xn are partitioned into three sets {a mathematical formula}E=x1…xk, {a mathematical formula}M1=xk+1…xl, and {a mathematical formula}M2=xl+1…xn for some numbers k, l with {a mathematical formula}1≤k≤l≤n.
       </list-item>
       <list-item>
        Question: Is there a truth assignment to the variables in E such that for the majority of truth assignments to the variables in {a mathematical formula}M1 it holds, that the majority of truth assignments to the variables in {a mathematical formula}M2 yield a satisfying truth instantiation to {a mathematical formula}E∪M1∪M2?
       </list-item>
      </list>
      <paragraph>
      ...
      <paragraph label="Theorem 6">
       MFEis{a mathematical formula}NPPPPP-complete.
      </paragraph>
   ...
   <appendices>
    <section label="Appendix A">
     <section-title>
      Computing relevance is NP-hard
     </section-title>
     <paragraph>
     ...
     <list>
      Intrinsic Relevance
      <list-item>
       Instance: A Bayesian network {a mathematical formula}B=(GB,Pr), where V is partitioned into evidence variables E with joint value assignment e, explanation variables H, and intermediate variables I, and a designated variable {a mathematical formula}I∈I.
      </list-item>
      <list-item>
       Question: Is the intrinsic relevance{a mathematical formula}R(I)&gt;0?
      </list-item>
     </list>
     <paragraph>
      We reduce from the following {a mathematical formula}NP-complete decision problem [37]:
     </paragraph>
     <list>
      IsA-Relevant Variable
      <list-item>
       Instance: A Boolean formula ϕ with n variables, describing the characteristic function {a mathematical formula}1ϕ:{false,true}n→{1,0}, and a designated variable {a mathematical formula}xr∈ϕ.
      </list-item>
      <list-item>
       Question: Is {a mathematical formula}xr a relevant variable in ϕ, that is, is {a mathematical formula}1ϕ(xr=true)≠1ϕ(xr=false)?
      </list-item>
     </list>
     <paragraph>
      Here, the characteristic function {a mathematical formula}1ϕ of a Boolean formula ϕ maps truth assignments to ϕ to {a mathematical formula}{0,1}, such that {a mathematical formula}1ϕ(x)=1 if and only if x denotes a satisfying truth assignment to ϕ, and {a mathematical formula}1ϕ(x)=0 otherwise. We will use the formula {a mathematical formula}ϕex=¬(x1∨x2)∧x3 as a running example, where {a mathematical formula}x3 is the variable of interest. Note that {a mathematical formula}x3 is relevant, since for {a mathematical formula}x1=x2=false, {a mathematical formula}1ϕ(x3=true)≠1ϕ(x3=false).
     </paragraph>
     <paragraph>
