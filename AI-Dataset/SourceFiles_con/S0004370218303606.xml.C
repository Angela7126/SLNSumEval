  ...
  <root>
   ...
   <content>
    ...
    <section label="4">
     ...
     <list>
      <list-item label="1.">
       The joint state space may become very large making the solution of the joint MDP intractable. This is especially problematic as the approximation given in Eq. 9 requires repeatedly solving the MDP optimally for each new set of candidate reward weights.
      </list-item>
      <list-item label="2.">
       Due to occlusion, one of the robots may be observed at a time step while the other is not, resulting in the state of the joint MDP being partially observed. The resulting problem may not be solved by traditional IRL algorithms. Choi and Kim [14] present a method for performing IRL with partial observability by modeling the expert as a POMDP. But, the technique requires a complete finite-state controller (policy) or the expert's belief trajectory as input, both of which are unrealistic to obtain.
      </list-item>
     </list>
     <paragraph>
      In light of the above limitations, we pursue an alternate approach which continues to model each robot using a separate, individual MDP. The behavior of the robots during an interaction is modeled separately and overrides the behavior prescribed by these MDPs for the duration of the interaction. A nonlinear program that combines the one in (2) for each mobile robot and is used for learning the rewards and policies of all robots is given by:{a mathematical formula}{a mathematical formula} We label this simple multi-agent extension of IRL{sup:⁎} as mIRL{sup:⁎}. The above program may also be decomposed into N similar programs for the N robots if there is no interaction between the robots.
     </paragraph>
     <section label="4.1">
      <section-title>
     ...
     <section label="4.2">
      ...
      <paragraph>
       Note that while the interaction game modeled by L may admit multiple equilibria, the robots pick one to resolve their interaction. In the case where L is unable to determine the specific equilibrium used by the robots through observation or side information, how should L model the interactions?
      </paragraph>
      <paragraph>
    ...
    <section label="5">
     <section-title>
      mIRL with Transition Function Estimation
     </section-title>
     <paragraph>
     ...
     <section label="5.6">
      ...
      <paragraph>
       We limit our consideration to a finite number of error models that can be iterated. An extension to parametric models is straightforward as each model's parameters could be estimated from the available data first before selecting the maximum likelihood parameterized model.
      </paragraph>
      <paragraph>
      ...
      <paragraph>
       Other alternatives to the maximum likelihood error model include fitting a mixture of models, and learning a dynamic Bayesian network from the unintended transitions. However, given that errant transitions are expected to occur sparsely, there may not be sufficient data to learn these more complex error models.
      </paragraph>
     </section>
    </section>
    <section label="6">
     ...
     <section label="6.1">
      ...
      <section label="6.1.2">
       ...
       <paragraph>
        In the smaller environment, both {a mathematical formula}RI(s,a) and {a mathematical formula}RJ(s,a) are composed of the following two types of binary feature functions because their trajectories are simple cycles:
       </paragraph>
       <list>
        <list-item label="1.">
         Has moved, which returns 1 if the action causes the patroller to leave its current location, otherwise 0; and
        </list-item>
        <list-item label="2.">
         Turn around at state s, which returns 1 if the robot turns around 180{sup:∘} at the location given by s, otherwise 0.
        </list-item>
       </list>
       <paragraph>
        The true vector of weights θ, not known to L, emphasizes moving in hallways and turning around after I or J has turned the corner. It penalizes turning around at other locations in hallways, thereby creating a cyclic patrolling pattern.
       </paragraph>
       <paragraph>
        Reward functions of the patrollers in the larger environment additionally include the following binary feature functions as their trajectories may also go through rooms in the large hallways.
       </paragraph>
       <list>
        <list-item label="1.">
         Enter room, which returns 1 if the patroller enters a room from one of the hallways, otherwise 0;
        </list-item>
        <list-item label="2.">
         In room, which returns 1 if the patroller is in any of the rooms in the map, otherwise 0; and
        </list-item>
        <list-item label="3.">
         Leave room, which returns 1 if the patroller leaves a room to enter a hallway.
        </list-item>
       </list>
       <paragraph>
        Similar to the smaller environment, the true vector of weights penalizes entering any side rooms from the hallways. Consequently, the MDP-based policy of each patroller generates trajectories that move through the hallways turning around in proximity to the corners at the end of each hallway.
       </paragraph>
      </section>
     </section>
     <section label="6.2">
      ...
      <paragraph>
       We promote a deeper understanding and evaluate the contributions of Sections 4 and 5 in the context of the robotic patrolling domain described previously both in simulation and on physical robots. While simulations were performed in both indoor environments, one larger than the other, physical robot experiments were performed in the smaller environment only.
      </paragraph>
      <paragraph>
       Evaluation metrics.  Rewards learned through IRL are generally not directly comparable to the true preferences of the expert. This is because a demonstrated policy admits infinitely-many reward functions (for example affine transformations of the true reward function) that explain it. Consequently, we utilize two measures to evaluate the performance of our approaches. First, is the learned behavior accuracy that is the proportion of all states at which the actions prescribed by the inversely learned policies of both patrollers and the interaction model (if it applies) coincide with their actual actions. The latter are found by solving the patrollers' MDPs with the true rewards in conjunction with their true interaction behavior. This measure is a strong indicator of a successful learning algorithm, but does not incorporate the robotic aspects of the experiment. The measure also strongly correlates with ILE because if the learned behavior accuracy is high, then ILE is low, and analogously if the accuracy is low.
      </paragraph>
      <paragraph>
       Success rate comprehensively measures all aspects of the experiment by reporting the proportion of runs L reached the designated goal state without being detected. This measure has practical implications – it assesses whether any approach and its implementation are actually feasible in the patrolling domain. Not only does it reflect the ability of L to accurately learn useful policies for the patrollers, but also that L can predict their future positions, and traverse a route around them following its own model of the world within an allotted time.
      </paragraph>
      <paragraph>
       Robotic platform and software.  All robots are TurtleBots equipped with a Microsoft XBox 360 Kinect, both in simulation and in physical experiments. Thus, robots are equipped with a stereo camera and infrared laser for depth perception. TurtleBots are differential drive robots carrying a laptop running ROS Fuerte on Ubuntu 12.04. The robots identify each other by way of unique color tags using CMVision, which is ROS's blob finder. We use ROS's default localization and motion planning for Turtlebots, and the centers of the cells are translated into goal positions for the path planning algorithm. As such, the robots are self-localized and move autonomously. Trajectories of state-action pairs are generated by discretizing the observed feed of movement readings from the blob-finder into the most likely state and action taken during a time step. Specifically, we extracted 4 variables: blob color and centroid, distance to the blob, and the position on the current map. Each time step was calibrated to range for 2 seconds, which resulted in a feed of about 60 frames per time step. We utilized a simple voting protocol to resolve any conflict and arrive at a consensus state-action pair for each time step. We vary the starting locations of the patrollers across runs.
      </paragraph>
      <paragraph>
       We begin with Section 6.2.1 that reports on the experiments involving two and more patrolling robots with varying levels of occlusion, followed by Section 6.2.2 that reports on results of additionally learning the transition function as well. While experiments in these sections are simulations, Section 6.3 presents the outcomes of evaluating the methods on actual physical TurtleBots conducting patrols in a building hallway.
      </paragraph>
      <section label="6.2.1">
       <section-title>
        Multiple Experts Under Occlusion
       </section-title>
       <section>
        <section-title>
        ...
        <paragraph>
         In addition to the method mIRL{sup:⁎}+Int, which generalizes IRL to settings involving multiple robots under occlusion, we make use of three other approaches to provide baselines and benchmarks for evaluation.
        </paragraph>
        <list>
         <list-item label="•">
          mIRL{sup:⁎} extends IRL{sup:⁎} to multiple agents and accounts for occlusion but assumes no interaction between the robots. Consequently, mIRL{sup:⁎} involves independent use of the maximum entropy IRL method for each patroller, and provides a key benchmark by a previously known technique.
         </list-item>
         <list-item label="•">
          KnownPolicy provides an upper bound on performance by revealing each patroller's policy and interaction behavior to L, removing the need to perform any learning.
         </list-item>
         <list-item label="•">
          Finally, Random ignores all observations and simply waits a random amount of time before moving directly to the goal state.
         </list-item>
        </list>
       </section>
       <section>
    ...
    <section label="7">
     ...
     <paragraph>
      Few other approaches investigate relaxing the prior knowledge of the expert's transition function requirement of IRL. Boularias et al. [7] propose model-free IRL by minimizing the relative entropy between distributions over trajectories generated by a baseline and target policies and utilizing importance sampling. In contrast, we explicitly learn the transition function first under occlusion making this a semi-model based method. Furthermore, to the best of our knowledge, this article is the first to consider IRL in the presence of occluded trajectories. Often a factor when the observed robot is mobile, occlusion significantly complicates methods that advance IRL.
     </paragraph>
     <paragraph>
      Bard [5] used a maximum entropy approach to estimate state probabilities when the probabilities associated with aggregates of states are known. Our technique, mIRL{a mathematical formula}/T⁎+Int, is similar to Bard's approach but has several key differences: rather than discrete states we use random variables whose values are unknown during each transition, the presence of occlusion, and our novel extension to learning transition probabilities.
     </paragraph>
     <paragraph>
      In general, IRL differs from plan and activity recognition [35], [4] by focusing on learning the underlying reward function of the observed expert, which is often a highly underconstrained problem leading to many solutions. In comparison, plan and activity recognition seek to recognize the plan or activity that is being executed based on a known library of action plans and activities. IRL is closer to intent recognition in its aim with intent being closer in semantics to preferences or rewards. Indeed, learning the preferences of the expert can allow us to recognize its intent and current plan. Therefore, IRL may be seen as an enabling method for plan, activity or intent recognition, and indeed Kitani et al. has utilized it for activity forecasting [21].
     </paragraph>
     <paragraph>
      Finally, in the context of our application domain, related research has predominantly focused on generating robot patrolling trajectories that are theoretically difficult to learn from observations by relying on randomized behavior [2]. This direction is motivated by the aim of building robots that could be deployed for perimeter patrolling, which is complementary to our focus of exploring state-of-the-art techniques for building robots that may learn the simple patrolling behavior of multiple other robots or human agents.
     </paragraph>
    </section>
   </content>
   <appendices>
    <section label="Appendix A">
     <paragraph>
      To conveniently recall the various methods that were introduced in this article and their corresponding labels, we compile them in the table below. The first three methods are used as baselines while the last two serve as upper bounds in the performance comparison.
     </paragraph>
     <paragraph>
      {a mathematical formula}
     </paragraph>
